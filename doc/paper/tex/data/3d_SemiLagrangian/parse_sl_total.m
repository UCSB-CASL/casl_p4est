...
events = {
    'Semilagrangian::update_p4est_second_order_last_grid_CF2'
...
flag = 0;
t = zeros(length(events) + 1, length(files));
...
legend_titles = {'Total', 'Interpolation', 'p4est\_nodes\_new', 'p4est\_partition', 'p4est\_refine\_coarsen'};
...
axis square
filename = strcat(type, '_SemiLagrangian_', run, '_CFL_', num2str(cfl));
...
% print('-depsc2', '-r300', '-f1', strcat(type, '_SemiLagrangian_', run, '_CFL_', num2str(cfl)));
