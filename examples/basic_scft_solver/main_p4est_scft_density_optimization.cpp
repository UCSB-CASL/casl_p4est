...
// example of two wells
double xc0 = -1.6, yc0 = 0;
double xc1 =  1.6, yc1 = 0;
double xc2 =  0, yc2 = 1.13;
double r0 = 1.0;

int case_num =2;

class mu_t_cf_t : public CF_2
{
public:
  double operator()(double x, double y) const
  {
    switch (case_num)
    {
      case 0: // 1 cylinder
        {
          ...
          double phi = sqrt(SQR(x-xc0)+SQR(y-yc0)) - r0;
          ...
          return 0.5*XN*(sign*(exp(-fabs(phi)*5.0)-1.0));
        }
      case 1: // 2 cylinders
        {
          double xc0 = -1.6, yc0 = 0;
          ...
          double phi0 = sqrt(SQR(x-xc0)+SQR(y-yc0)) - r0;
          double phi1 = sqrt(SQR(x-xc1)+SQR(y-yc1)) - r0;

          double phi = MIN(phi0, phi1);
          double sign = -1;
      ...
      case 2: // 3 cylinders
        {
          double xc0 = -1.6, yc0 = -1.2;
          double xc1 =  1.6, yc1 = -1.2;
          double xc2 =  0.0, yc2 = 1.2;

          double phi0 = sqrt(SQR(x-xc0)+SQR(y-yc0)) - r0;
          double phi1 = sqrt(SQR(x-xc1)+SQR(y-yc1)) - r0;
          double phi2 = sqrt(SQR(x-xc2)+SQR(y-yc2)) - r0;

          double phi = MIN(phi0, phi1, phi2);
          double sign = -1;
...
public:
  double operator()(double x, double y) const
  {
    double phi0 = sqrt(SQR(x-0.5*xc0)/1.6+SQR(y-yc0+0.2)) - r0;
    double phi1 = sqrt(SQR(x-0.5*xc1)/1.3+SQR(y-yc1-0.2)) - r0;
...
class phi_cf_0_t : public CF_2
{
public:
  double operator()(double x, double y) const
  {
    double phi0 = sqrt(SQR(x-0.5*xc0)/1.6+SQR(y-yc0+0.2)) - 2.*r0;
    double phi1 = sqrt(SQR(x-0.5*xc1)/1.3+SQR(y-yc1-0.2)) - 2.*r0;

    double epsilon = 5.0;

    return 0.5*(phi0+phi1-(sqrt(SQR(phi0-phi1)+epsilon)-epsilon/sqrt(SQR(phi0-phi1)+epsilon)));
  }
} phi_cf_0;
...
inline double gamma_a_0(double x, double y) { return 0.00;} double_to_cf_t gamma_a_0_cf(&gamma_a_0);
inline double gamma_b_0(double x, double y) { return -0.00;} double_to_cf_t gamma_b_0_cf(&gamma_b_0);

inline double gamma_a_1(double x, double y) { return 0.0;} double_to_cf_t gamma_a_1_cf(&gamma_a_1);
inline double gamma_b_1(double x, double y) { return 0.0;} double_to_cf_t gamma_b_1_cf(&gamma_b_1);

//inline double mu_target(double x, double y) { return 0.5*XN*cos(x)*cos(y);} double_to_cf_t mu_t_cf(&mu_target);
...
int main (int argc, char* argv[])
{
  ...
  cmd.add_option("case_num", "case_num");
  ...
  case_num = cmd.get("case_num", case_num);
...
//  ierr = VecSetRandom(mu_m, NULL); CHKERRXX(ierr);
  ...
  command2 << "mkdir -p " << out_dir << "/density_opt";
  ret_sys = system(command2.str().c_str());
  if(ret_sys<0)
    throw std::invalid_argument("my_p4est_bialloy_t::save_vtk could not create directory");
...
//  scft.initialize_bc_smart();
  ...
  scft.DO_initialize(mu_t_cf);
  scft.DO_initialize_fields();
  ...
  for (int move_iter = 0; move_iter < num_of_moves; move_iter++)
  {
    ...
    scft.DO_save_VTK_before_moving(move_iter);
    ...
    do {
      ...
      {
        scft.DO_save_VTK(iteration/save_every_n_iteration);
...
//    iteration = iteration/save_every_n_iteration;
//    save_every_n_iteration = 1;
    if (first_time)
    {
      scft.DO_initialize_fields();
    }
...
//    ierr = PetscPrintf(mpi.comm(), "Solving SCFT\n", scft.lam_0, scft.DO_get_pressure_force(), scft.DO_get_exchange_force()); CHKERRXX(ierr);
    do {
      scft.DO_solve_for_propogators();
      scft.DO_compute_densities();
      scft.DO_update_potentials();
...
//        scft.DO_save_VTK(iteration/save_every_n_iteration);
      ...
      iteration++;
//    } while (scft.DO_get_pressure_force() > tol || scft.DO_get_exchange_force() > tol);
    } while (scft.DO_get_exchange_force() > tol);

    // advance interface

    if (first_time)
    {
      initial_energy = scft.DO_get_cost_func();
      energy_nm1 = scft.DO_get_cost_func();
      first_time = false;
    }

    total_change_exp = scft.DO_get_cost_func()-initial_energy;
    immed_change_exp = scft.DO_get_cost_func()-energy_nm1;

    ierr = PetscPrintf(mpi.comm(), "Func: %e;\n", scft.DO_get_cost_func()); CHKERRXX(ierr);
    ...
    if(save_change)
    {
      ...
      PetscFPrintf(mpi.comm(), fich, "%d %e %e %e %e %e\n", move_iter, scft.DO_get_cost_func(), immed_change_exp, immed_change_thr, total_change_exp, total_change_thr);
      ierr = PetscFClose(mpi.comm(), fich); CHKERRXX(ierr);
    ...
    scft.DO_compute_shape_derivative(0);

    // imposed velocity
    Vec velocity;
    ierr = VecCreateGhostNodes(scft.p4est, scft.nodes, &velocity); CHKERRXX(ierr);

    double *shape_grad_ptr, *velo_ptr;
    ierr = VecGetArray(velocity, &velo_ptr); CHKERRXX(ierr);
    ierr = VecGetArray(scft.density_shape_grad, &shape_grad_ptr); CHKERRXX(ierr);
    ...
    ierr = VecRestoreArray(scft.density_shape_grad, &shape_grad_ptr); CHKERRXX(ierr);

//    energy_nm1 = scft.DO_compute_change_in_functional(0, velocity_int, small_interval);
    immed_change_thr = scft.DO_compute_change_in_functional(0, velocity, interface_speed*scft.dt_density);
    total_change_thr += immed_change_thr;

//    scft.update_grid(velocity_int, 0, small_interval);
    scft.DO_save_VTK_before_moving(move_iter);
    scft.update_grid(velocity, 0, interface_speed*scft.dt_density);
    scft.DO_initialize(mu_t_cf);
