...
//flower_phi_t phi_cf_0(3.0, -0.0, -0.0, 0.4, 1);
//flower_phi_t phi_cf_1(6.5, -2, -7.5, 0.5, -1);
...
public:
  ...
  {
    double phi = sqrt(SQR(x/3.0)+SQR(y/1.0)) - 1.;

    return phi;
  }
} phi_cf_0;

//flower_phi_t phi_cf_0(1.4, -0.0, -0.0, 0.2, 1);
flower_phi_t phi_cf_1(3.8, -2, -4, 0.0, -1);
...
inline double gamma_a_0(double x, double y) { return 1.0+0.3;} double_to_cf_t gamma_a_0_cf(&gamma_a_0);
inline double gamma_b_0(double x, double y) { return 1.0-0.4;} double_to_cf_t gamma_b_0_cf(&gamma_b_0);
...
inline double gamma_a_1(double x, double y) { return 0.5-0.31*cos(0.5*PI*(x-y));} double_to_cf_t gamma_a_1_cf(&gamma_a_1);
inline double gamma_b_1(double x, double y) { return 0.5+0.299*cos(0.5*PI*(x-y));} double_to_cf_t gamma_b_1_cf(&gamma_b_1);
...
//    scft.compute_energy_shape_derivative_contact_term(1,0);
    ...
    immed_change_thr = scft.compute_change_in_energy(0, velocity, interface_speed*scft.dt_energy);
//    contact_term = scft.compute_change_in_energy_contact_term(1, 0, velocity, interface_speed*scft.dt_energy);
//    immed_change_thr += contact_term;

    total_change_thr += immed_change_thr;

    scft.save_VTK(move_iter);
    scft.update_grid(velocity, 0, interface_speed*scft.dt_energy);
