...
const static string main_description =
    string("Bubbles chasing under free surface + gravity! \n")
...
const double mu_plus                  = 1.0; // those are constant, not freely defined
// problem setup: 2) free quantities
const double default_domain[P4EST_DIM] = {DIM(3.0, 6.0, 3.0)};
const bool default_periodic[P4EST_DIM] = {DIM(true, false, false)};
const double default_ratio_rho  = 100.0;  // rho_plus/rho_minus
const double default_ratio_mu   = 2.0;    // mu_plus/mu_minus
const double default_Re         = 10;     // sqrt(g*D)case*rho_plus*d/mu_plus
const double default_We         = 10;     // (rho_plus - rho_minus)*g*D*D/gamma
// grid-related
...
const int default_ntree[P4EST_DIM] = {DIM(1, 2, 1)};
// simulation-related:
...
const double default_t_end = 24.0;
...
#if defined(STAMPEDE)
const string default_work_folder = "/scratch/04965/tg842642/two_phase_flow/bubble_chase_under_surface/" + to_string(P4EST_DIM) + "D";
#elif defined(POD_CLUSTER)
const string default_work_folder = "/scratch/regan/two_phase_flow/bubble_chase_under_surface/" + to_string(P4EST_DIM) + "D";
#else
const string default_work_folder = "/home/regan/workspace/projects/two_phase_flow/bubble_chase_under_surface/" + to_string(P4EST_DIM) + "D";
#endif
...
public:
  ...
  {
    double f1 = 2.9 - y;
    double f2 = ABSD(x, y - 1, z) - 0.4;
    double f3 = ABSD(x, y - 2, z) - 0.5;
    return MIN(f1, f2, f3);
  }
} initial_level_set;
                                ...
                                my_p4est_two_phase_flows_t* &solver, my_p4est_brick_t* &brick, p4est_connectivity_t* &connectivity)
{
  ...
  const double xyz_min[P4EST_DIM]       = { DIM(-0.5*domain_size[0],             0.0, -0.5*domain_size[2]) };
  ...
  const double uniform_band_in_dxmin    = cmd.get<double> ("uniform_band",      default_uniform_band_to_radius*0.5/dxmin);
  ...
  const double Reynolds                 = cmd.get<double> ("Reynolds", default_Re);
  const double Weber                    = cmd.get<double> ("Weber", default_We);
  const double surface_tension          = (SQR(mu_plus)*(rho_plus - rho_minus)/(rho_plus*rho_plus))*(SQR(Reynolds)/Weber);
  ...
  if(1 > MIN(DIM(domain_size[0], domain_size[1], domain_size[2])))
    throw std::invalid_argument("main for bubble chase under surface: create_solver_from_scratch: invalid bubble radius (the bubble is larger than the computational domain in some direction).");
...
int main (int argc, char* argv[])
{
  ...
  streamObj.str(""); streamObj << default_Re;
  cmd.add_option("Reynolds", "The desired Reynolds number Re = sqrt(g*D)*rho_plus*D/mu_plus. Default value is " + streamObj.str());
  streamObj.str(""); streamObj << default_We;
  cmd.add_option("Weber", "The desired Weber number We = (rho_plus - rho_minus)*g*D*D/surface_tension. Default value is " + streamObj.str());
  ...
  {
    try {
      cmd.get<double>("Morton");
    } catch (runtime_error& e) {
      ierr = PetscFPrintf(mpi.comm(), stderr, "main for buoyant bubble: I can't restart properly and safely without the Morton number, you must give me the Morton number (i.e., '-Morton xxx') to determine the body force."); CHKERRXX(ierr);
      return EXIT_FAILURE;
    }
  ...
  cf_const_t body_force_y(-cmd.get<double>("Weber", default_We)*two_phase_flow_solver->get_surface_tension()/((two_phase_flow_solver->get_rho_plus() - two_phase_flow_solver->get_rho_minus())*1.0*1.0));
#ifdef P4_TO_P8
...
//  const double time_unit  = two_phase_flow_solver->get_mu_plus()*1.0/two_phase_flow_solver->get_surface_tension();
  ...
  streamObj << "mu_ratio_" << two_phase_flow_solver->get_mu_plus()/two_phase_flow_solver->get_mu_minus()
            ...
            << "_Re_" << sqrt(fabs(body_force_y(DIM(0.0, 0.0, 0.0)))*1.0)*two_phase_flow_solver->get_rho_plus()*1.0/two_phase_flow_solver->get_mu_plus()
            << "_We_" << (two_phase_flow_solver->get_rho_plus() - two_phase_flow_solver->get_rho_minus())*fabs(body_force_y(DIM(0.0, 0.0, 0.0)))*1.0*1.0/two_phase_flow_solver->get_surface_tension();
...
//  const double dt_visc = 1.0/(MAX(nu_minus, nu_plus)*(SUMD(2.0/SQR((brick->xyz_max[0] - brick->xyz_min[0])/(brick->nxyztrees[0]*(1 << sp->max_lvl))), 2.0/SQR((brick->xyz_max[1] - brick->xyz_min[1])/(brick->nxyztrees[1]*(1 << sp->max_lvl))), 2.0/SQR((brick->xyz_max[2] - brick->xyz_min[2])/(brick->nxyztrees[2]*(1 << sp->max_lvl))))));
  ...
  two_phase_flow_solver->save_vtk("/home/regan/workspace/projects/two_phase_flow/bubble_chase_under_surface/2D", 0);
