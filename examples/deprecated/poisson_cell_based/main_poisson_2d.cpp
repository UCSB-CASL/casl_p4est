...
int main (int argc, char* argv[]){
  ...
  std::ostringstream grid; grid << P4EST_DIM << "d_grid_" << p4est->mpisize;
  ...
  ierr = VecDuplicate(rhs, &uex); CHKERRXX(ierr);
...
//  VecSet(phi , -1);
  ...
  PoissonSolverCellBase solver(&cell_neighbors, &node_neighbors);
  solver.set_phi(phi);
  solver.set_rhs(rhs);
  solver.set_bc(bc);
  ...
  my_p4est_vtk_write_all(p4est, nodes, ghost,
                         ...
                         VTK_CELL_DATA , "sol", sol_p,
                         VTK_CELL_DATA , "uex", uex_p);
