...
const static string main_description =
    string("Droplet evaporation test (as presented in Frederic's paper, though the final time seems odd in that paper...) \n")
    + string("Developer: Raphael Egan (raphaelegan@ucsb.edu), 2019-2020-2021-...-2523\n");

// problem setup: 1) set parameters
const double default_rho_plus                     = 5;
const double default_rho_minus                    = 200;
const double default_surface_tension              = 0.1;
const double default_mu_plus                      = 0.005;
const double default_mu_minus                     = 0.1;
const double default_thermal_conductivity_plus    = 1;
const double default_thermal_conductivity_minus   = 40;
const double default_specific_heat_capacity_plus  = 200;
const double default_specific_heat_capacity_minus = 400;
const double default_latent_heat                  = 1000; // positive since plus == vapor here
const double default_T_sat                        = 0;
const double default_T_inf                        = 10;
// problem setup: 2) free quantities
//const double initial_radius = 0.001;
//const double default_domain[P4EST_DIM] = {DIM(0.02, 0.02, 0.02)};
const double initial_radius = 0.02;
const double default_domain[P4EST_DIM] = {DIM(0.07, 0.07, 0.07)};
...
const double default_t_end = 1.0;
...
#if defined(STAMPEDE)
const string default_work_folder = "/scratch/04965/tg842642/two_phase_flow/drop_evaporation/" + to_string(P4EST_DIM) + "D";
#elif defined(POD_CLUSTER)
const string default_work_folder = "/scratch/regan/two_phase_flow/drop_evaporation/" + to_string(P4EST_DIM) + "D";
#else
const string default_work_folder = "/home/regan/workspace/projects/two_phase_flow/drop_evaporation/" + to_string(P4EST_DIM) + "D";
#endif
...
public:
  ...
  {
    return ABSD(x, y, z) - initial_radius;
  }
};

class pressure_wall_bc_type_t : public WallBCDIM {
public:
  pressure_wall_bc_type_t() {}
...
class temperature_wall_bc_value_t : public CF_DIM {
  const double T_inf;
public:
  temperature_wall_bc_value_t(const double T_inf_) : T_inf(T_inf_) {}
...
int main (int argc, char* argv[])
{
  ...
  streamObj.str(""); streamObj << default_T_inf;
  cmd.add_option("T_inf", "The desired far-field temperature. Default value is " + streamObj.str());
  ...
  {
    try {
      cmd.get<double>("T_inf");
    } catch (runtime_error& e) {
      ierr = PetscFPrintf(mpi.comm(), stderr, "main for drop evaporation: I can't restart properly and safely without T_inf: you must give me that one."); CHKERRXX(ierr);
      return EXIT_FAILURE;
    }
  ...
  pressure_wall_bc_type_t pressure_wall_type;
  velocity_wall_bc_type_t velocity_wall_type;
  ...
  temperature_wall_bc_type_t  temp_wall_type;
  temperature_wall_bc_value_t temp_wall_value(cmd.get<double>("T_inf", default_T_inf));
