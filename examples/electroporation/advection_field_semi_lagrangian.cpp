...
using namespace std;
int test_number = 0;  //0 - rotation, 1 - vortex, 2- vertical flow past a sphere
double xmin = 0;
double xmax = 1;
double ymin = 0;
double ymax = 1;
...
struct u_t : CF_3
{
  double operator()(double x, double y, double z) const {
	...
	double u_th = 5 + 5*sin(theta)*(1-0.75*R1/r-0.25*R1*R1*R1/r/r/r);
	double u_ph = 5;
...
void advect_field_semi_lagrangian(p4est_t *p4est, p4est_nodes_t *nodes, my_p4est_node_neighbors_t *ngbd_n, Vec phi, Vec *phi_xx, double dt, Vec v[P4EST_DIM], Vec **vxx, Vec M_n, Vec *M_xx_n, double *M_np1_p, bool periodic_adv)
{
    PetscErrorCode ierr;
...
void advect(p4est_t *p4est, p4est_nodes_t *nodes, my_p4est_node_neighbors_t *ngbd_n, Vec phi, Vec v[3], double dt, Vec &M, bool periodic_adv)
{
    PetscErrorCode ierr;
    Vec *M_xx, *phi_xx;
    /* compute vx_xx, vx_yy */
    ...
    advect_field_semi_lagrangian(p4est, nodes, ngbd_n, phi, phi_xx, dt, v, vxx, M, M_xx, M_np1_p, periodic_adv);
...
int main (int argc, char* argv[])
{
  ...
  PetscPrintf(mpi.comm(), "test= %d, time-step= %g, dt=%g, cfl=%g\n", test_number, dt, dxyz_min[0], cfl);
  ...
  for(p4est_locidx_t n=0; n<nodes->num_owned_indeps; ++n)
  {
    ...
    err = max(err, (M_p[n]-exact_M(x,y,z)));
  }
