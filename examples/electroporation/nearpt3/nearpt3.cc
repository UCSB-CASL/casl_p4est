...
namespace nearpt3 {   // start of namespace nearpt3
  ...
  double Distance2(const std::array<Coord_T, 3> a, const std::array<Coord_T, 3> b) {
...
#include "cellsearchorder"
  ...
  public:                  // start of Grid_T
    ...
    const std::array<Coord_T,3>  * pts;   // pointer to user-supplied points.
    ...
    const Cell3 Compute_Cell_Containing_Point(const std::array<Coord_T,3> p);
    ...
    void querythiscell(const Cell3 thiscell, const std::array<Coord_T, 3> querypt,
    ...
    int Query_Fast_Case(const std::array<Coord_T, 3> q,   const Cell3 querycell);
  ...
  Compute_Cell_Containing_Point(const std::array<Coord_T,3> p) {
  ...
  template<typename Coord_T> void nearpt3::Grid_T<Coord_T> :: 
  querythiscell(const Cell3 thiscell, const std::array<Coord_T, 3> querypt,
		int &closestpt, double &dist2) {
    closestpt = -1;
  ...
  template<typename Coord_T> int nearpt3::Grid_T<Coord_T> :: 
  Query_Fast_Case(const std::array<Coord_T, 3> q,   const Cell3 querycell)
  {               //  Query_Fast_Case
  ...
  template<typename Coord_T> Grid_T<Coord_T> * 
    Preprocess(const int nfixpts, const std::array<Coord_T, 3> pts[]) { // preprocess
    Grid_T<Coord_T> *g;
  ...
  template <typename Coord_T>
  int Query( Grid_T<Coord_T> *g, const std::array<Coord_T, 3> q) { // Query
...
template<typename Coord_T> 
ostream &operator<<(ostream &o, const std::array<Coord_T,3> &c) {
  o << '(' << c[0] << ',' << c[1] << ',' << c[2] << ')';
  return o;
}
