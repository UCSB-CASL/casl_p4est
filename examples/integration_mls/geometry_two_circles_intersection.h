#ifndef GEOMETRY_TWO_CIRCLES_INTERSECTION_H
#define GEOMETRY_TWO_CIRCLES_INTERSECTION_H
#include <vector>
...
class geometry_two_circles_intersection_t
{
public:
...
//#endif
    ...
    exact0.ID = mu_sph_0 -mu_sec_0 +mu_tri_0 -mu_sec_1+mu_tri_1;
    exact1.ID = mi_sph_0 -mi_sec_0 +mi_tri_0 -mi_sec_1+mi_tri_1;

    exact0.ISB.push_back(mu_bnd_0-mu_seg_0);
    exact1.ISB.push_back(mi_bnd_0-mi_seg_0);

    exact0.ISB.push_back(mu_seg_1);
    exact1.ISB.push_back(mi_seg_1);

#ifdef P4_TO_P8
    exact0.IX.push_back(2.0*PI*sqrt(r0*r0-d0*d0));
    exact1.IX.push_back(2.0*PI*sqrt(r0*r0-d0*d0)*(r0*r0+R0*R0+2.*r0d0));
#else
    exact0.IX.push_back(2.0);
    exact1.IX.push_back(2.0*(r0*r0+R0*R0+2.0*r0d0));
#endif
...
#endif // GEOMETRY_TWO_CIRCLES_INTERSECTION_H
