...
const static string main_description =
    string("Marangoni-driven bubble motion, inspired from \"Marangoni bubble motion phenomenon in zero gravity\"! \n")
    + string("We consider a nitrogen bubble of 6mm in diameter in a 12cm x 12cm (x 12cm) box of ethanol \n")
    + string("in a gravity-free environment. The surface tension coefficient depends linearly on y \n")
    + string("with a gradient that is set to match the desired Marangoni number: \n")
    + string("\t Ma = (rho^{+}*SQR(a)*(|gradient of gamma|))/(SQR(mu^{+}))\n")
    + string("wherein a is the bubble RADIUS (***not*** diameter). \n")
    + string("The ascenscion velocity U_asc is monitored during the simulation and reported in its \n")
    + string("nondimensional form:\n")
    + string("\t Re = rho^{+}*U_asc*a/mu^{+}.\n\n")
    + string("Note that we do not take thermal effects into account (i.e., we do not solve for the energy equation):\n")
    + string("we do set and enforce a linear temperature profile throughout (testing the ability to generate and \n")
    + string("resolve a Marangoni-driven motion). \n")
    + string("Developer: Raphael Egan (raphaelegan@ucsb.edu), 2019-2020-2021-...-2523\n");

// problem setup: 1) set parameters
const double initial_bubble_diameter  = 0.006;
const double rho_plus                 = 780.0;
const double rho_minus                = 1.251;
const double mu_plus                  = 0.0011;
...
const double default_t_end = 5.0;
const double default_vmax_abort = 10.0;
...
#if defined(STAMPEDE)
const string default_work_folder = "/scratch/04965/tg842642/two_phase_flow/marangoni_driven_bubble/" + to_string(P4EST_DIM) + "D";
#elif defined(POD_CLUSTER)
const string default_work_folder = "/scratch/regan/two_phase_flow/marangoni_driven_bubble/" + to_string(P4EST_DIM) + "D";
#else
const string default_work_folder = "/home/regan/workspace/projects/two_phase_flow/marangoni_driven_bubble/" + to_string(P4EST_DIM) + "D";
#endif
...
class variable_surface_tension_t: public CF_DIM {
  const double grad_gamma;
public:
  variable_surface_tension_t(const double& grad_gamma_) :
    grad_gamma(grad_gamma_)
  {
    lip = 1.2;
  }
  double operator()(DIM(double, double y, double)) const
  {
    return gamma_0 + grad_gamma*y;
  }
};
...
void initialize_exportations(const double &tstart, const mpi_environment_t& mpi, const string& results_dir, string& datafile)
{
  ...
  if(mpi.rank() == 0)
  {
    if(!file_exists(datafile))
    {
      ...
      fprintf(fp_results, "%% tn | Re_v_bubble_y | circularity | volume | dt/dt_visc | dt/dt_capillar \n");
      fclose(fp_results);
    }
    else
    ...
    if(!file_exists(liveplot))
    {
      ...
      fprintf(fp_liveplot, "set ylabel \"Rising-velocity Re \" font \"Arial,14\"\n");
      ...
      fprintf(fp_liveplot, "set term wxt 2 position 1650,0 noraise\n");
      fprintf(fp_liveplot, "set key top right Left font \"Arial,14\"\n");
      fprintf(fp_liveplot, "set xlabel \"Time\" font \"Arial,14\"\n");
      ...
      fprintf(fp_liveplot, "plot  \"monitoring_results.dat\" using 1:(100 * (original_volume - $4)/original_volume) notitle with lines lw 3\n");
...
void export_results(const double& tn, const double& Re_v_bubble_y,
                    ...
                    const double& dt_to_dt_visc, const double& dt_to_dt_capillary, const string& datafile)
{
  ...
  if(fp_data == NULL)
    throw std::invalid_argument("main for Marangoni-driven bubble: could not open file for output of raw results.");
  fprintf(fp_data, "%g %g %g %g %g %g \n", tn, Re_v_bubble_y, circularity, bubble_volume, dt_to_dt_visc, dt_to_dt_capillary);
  fclose(fp_data);
}
...
void create_solver_from_scratch(const mpi_environment_t &mpi, const cmdParser &cmd,
                                ...
                                const variable_surface_tension_t& gamma_functor, Vec& gamma)
{
  ...
  PetscErrorCode ierr = delete_and_nullify_vector(gamma); CHKERRXX(ierr);
  my_p4est_interface_manager_t* itfc_manager = solver->get_interface_manager();
  ierr = itfc_manager->create_vector_on_interface_capturing_nodes(gamma); CHKERRXX(ierr);
  sample_cf_on_nodes(itfc_manager->get_interface_capturing_ngbd_n().get_p4est(), itfc_manager->get_interface_capturing_ngbd_n().get_nodes(), gamma_functor, gamma);
  solver->set_surface_tension(gamma);
...
int main (int argc, char* argv[])
{
  ...
  streamObj.str(""); streamObj << default_Marangoni;
  cmd.add_option("Ma", "The desired Marangoni number. Default Ma is " + streamObj.str());
  ...
  if(cmd.contains("restart"))
  {
    try {
      ...
      if(mpi.rank() == 0)
        std::cerr << "I need you to give me Ma if restarting (critical data to restart, can't use defaul value)" << std::endl;
      return EXIT_FAILURE;
    }
  }

  if(cmd.contains("restart") && !cmd.contains("Ma"))
  {
    if(mpi.rank() == 0)
      std::cerr << "I need you to give me Ma if restarting (critical data to restart)" << std::endl;
    return EXIT_FAILURE;
  }


  Vec gamma = NULL;
  const double Ma = cmd.get<double>("Ma", default_Marangoni);
  variable_surface_tension_t surface_tension_functor(-SQR(mu_plus)*Ma/(rho_plus*SQR(0.5*initial_bubble_diameter)));
  if(cmd.contains("restart"))
    load_solver_from_state(mpi, cmd, two_phase_flow_solver, brick, connectivity, surface_tension_functor, gamma);
  else
    create_solver_from_scratch(mpi, cmd, two_phase_flow_solver, brick, connectivity, surface_tension_functor, gamma);
  ...
  if(vtk_dt <= 0.0)
    throw std::invalid_argument("main for marangoni-driven bubble: the value of vtk_dt must be strictly positive.");
  ...
  if(create_directory(export_dir.c_str(), mpi.rank(), mpi.comm()))
    throw std::runtime_error("main for marangoni-driven bubble: could not create exportation directory " + export_dir);
  if(create_directory(results_dir.c_str(), mpi.rank(), mpi.comm()))
    throw std::runtime_error("main for marangoni-driven bubble: could not create directory for exportation of results, i.e., " + results_dir);
  if(save_vtk && create_directory(vtk_dir, mpi.rank(), mpi.comm()))
    throw std::runtime_error("main for marangoni-driven bubble: could not create directory for visualization files, i.e., " + vtk_dir);
  ...
  while(two_phase_flow_solver->get_tn() < t_end)
  {
    ...
    my_p4est_interface_manager_t* itfc_manager = two_phase_flow_solver->get_interface_manager();
    ierr = delete_and_nullify_vector(gamma);
    ierr = itfc_manager->create_vector_on_interface_capturing_nodes(gamma); CHKERRXX(ierr);
    sample_cf_on_nodes(itfc_manager->get_interface_capturing_ngbd_n().get_p4est(), itfc_manager->get_interface_capturing_ngbd_n().get_nodes(), surface_tension_functor, gamma);
    two_phase_flow_solver->set_surface_tension(gamma);
    ...
    double avg_velocity_bubble[P4EST_DIM], bubble_volume, interface_area;
    two_phase_flow_solver->get_average_velocity_in_domain(-1, avg_velocity_bubble, &bubble_volume, &interface_area);
    if(mpi.rank() == 0)
      export_results(two_phase_flow_solver->get_tnp1(),
                     avg_velocity_bubble[1]*rho_plus*0.5*initial_bubble_diameter/mu_plus,
    #ifdef P4_TO_P8
          (M_PI*SQR(initial_bubble_diameter))/interface_area,
    #else
          (M_PI*initial_bubble_diameter)/interface_area,
    #endif
          bubble_volume,
  ...
  delete data;
  delete subrefined_data;
  ierr = delete_and_nullify_vector(gamma); CHKERRXX(ierr);
