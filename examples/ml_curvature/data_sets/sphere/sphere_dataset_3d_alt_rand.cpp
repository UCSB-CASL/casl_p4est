/**
 * Generate data set for training a neural network on spherical interfaces using samples from a reinitialized non-signed level-set function.
 *
 * Unlike sphere_dataset_3d.cpp, here we use an approach similar to
 ...
 * where we use a uniform distribution to draw a target mean curvature.  Given that hk*, we collect M samples, and proceed to draw another
 * hk*.  Note that this process produces a uniform distribution for hk* but not for radii (unlike Patel et al., who chose random radii ins-
 * tead of random hk*).
 *
 ...
 * non-saddle regions.  For this reason, we can use the same techniques from 2d and normalize the samples to the negative-mean-curvature
 * spectrum followed by gradient reorientation and augmentaion by reflecting about the plane y - x = 0.
 *
