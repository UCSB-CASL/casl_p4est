...
public:
  ...
  Exact()
  {
    ...
    ISB.push_back(2.*PI*r0*(r0-d));
    ISBr2.push_back(2.*PI*(1-d/r0)*r0*r0*(r0*r0+d*d-r0*d*(1+d/r0)));
    /* intersections */
...
public:
  ...
  Geometry()
  {
    ...
    LSF.push_back(&ls_circle_1); action.push_back(coloration); color.push_back(1);
  }
} geometry;
...
//    vector<Vec> phi_vec_aux;
//    phi_vec_aux.push_back(phi);
    ...
    construct_domain(p4est, nodes, phi_vec, geometry.action, geometry.color, cubes, 0);
//    construct_domain(p4est, nodes, phi_vec, geometry.action, geometry.color, cubes, 1);
//    vector<Action> ac(1,intersection);
//    vector<int> co(1,0);
//    construct_domain(p4est, nodes, phi_vec_aux, ac, co, cubes, 0);
    ...
    {
      exact.ID = (integrate_over_negative_domain(p4est, nodes, mlt, phi_vec, geometry.action, geometry.color, f));
      exact.IDr2 = (integrate_over_negative_domain(p4est, nodes, mlt, phi_vec, geometry.action, geometry.color, f_r2));
      exact.IB = (integrate_over_interface(p4est, nodes, mlt, phi_vec, geometry.action, geometry.color, f, -1));
      exact.IBr2 = (integrate_over_interface(p4est, nodes, mlt, phi_vec, geometry.action, geometry.color, f_r2, -1));

      for (int i = 0; i < exact.n_subs; i++)
      {
        exact.ISB.push_back(integrate_over_interface(p4est, nodes, mlt, phi_vec, geometry.action, geometry.color, f, geometry.color[i]));
        exact.ISBr2.push_back(integrate_over_interface(p4est, nodes, mlt, phi_vec, geometry.action, geometry.color, f_r2, geometry.color[i]));
      }

      for (int i = 0; i < exact.n_Xs; i++)
      {
        exact.IXr2.push_back(integrate_over_intersection(p4est, nodes, mlt, phi_vec, geometry.action, geometry.color, f_r2, exact.IXc0[i], exact.IXc1[i]));
      }
    }
