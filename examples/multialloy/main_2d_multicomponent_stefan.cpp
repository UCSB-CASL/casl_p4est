...
param_t<BoundaryConditionType> bc_wall_type_pressure(pl, NEUMANN, "bc_wall_type_pressure", "DIRICHLET/NEUMANN");
...
double liquidus_value(double *c)
{
  ...
  if (linearized_liquidus.val)
  {
    ...
    for (int i = 0; i < num_comps.val; ++i)
      conc_term += (*liquidus_slope_all[i]) * c[i];
  ...
  else
  {
    ...
    {
    case 0:
      throw std::invalid_argument("Real liquidus surfaces is not available for this alloy\n");
    case 1:
      throw std::invalid_argument("Real liquidus surfaces is not available for this alloy\n");
    case 2: // Co-W-Al
    {
      ...
      return p00 + p10 * pow(c0, 1) * pow(c1, 0) + p01 * pow(c0, 0) * pow(c1, 1) + p20 * pow(c0, 2) * pow(c1, 0) + p11 * pow(c0, 1) * pow(c1, 1) + p02 * pow(c0, 0) * pow(c1, 2) + p30 * pow(c0, 3) * pow(c1, 0) + p21 * pow(c0, 2) * pow(c1, 1) + p12 * pow(c0, 1) * pow(c1, 2) + p03 * pow(c0, 0) * pow(c1, 3) + p40 * pow(c0, 4) * pow(c1, 0) + p31 * pow(c0, 3) * pow(c1, 1) + p22 * pow(c0, 2) * pow(c1, 2) + p13 * pow(c0, 1) * pow(c1, 3) + p04 * pow(c0, 0) * pow(c1, 4);
    }
    case 3: // Co-Al-W
    {
      double c0 = 100. * c[1];
...
double liquidus_slope(int which_comp, double *c)
{
  if (linearized_liquidus.val)
  {
    ...
    {
    case 0:
      return liquidus_slope_0.val;
    case 1:
      return liquidus_slope_1.val;
    case 2:
      return liquidus_slope_2.val;
    case 3:
      return liquidus_slope_3.val;
    }
  }
  else
  {
    ...
    case 2: // Co-W-Al
    {
      ...
      {
      case 0:
        return 100. * (0. + p10 * 1. * pow(c0, 0) * pow(c1, 0) + p01 * 0. * pow(c0, 0) * pow(c1, 1) + p20 * 2. * pow(c0, 1) * pow(c1, 0) + p11 * 1. * pow(c0, 0) * pow(c1, 1) + p02 * 0. * pow(c0, 0) * pow(c1, 2) + p30 * 3. * pow(c0, 2) * pow(c1, 0) + p21 * 2. * pow(c0, 1) * pow(c1, 1) + p12 * 1. * pow(c0, 0) * pow(c1, 2) + p03 * 0. * pow(c0, 0) * pow(c1, 3) + p40 * 4. * pow(c0, 3) * pow(c1, 0) + p31 * 3. * pow(c0, 2) * pow(c1, 1) + p22 * 2. * pow(c0, 1) * pow(c1, 2) + p13 * 1. * pow(c0, 0) * pow(c1, 3) + p04 * 0. * pow(c0, 0) * pow(c1, 4));
      case 1:
        return 100. * (0. + p10 * 0. * pow(c0, 1) * pow(c1, 0) + p01 * 1. * pow(c0, 0) * pow(c1, 0) + p20 * 0. * pow(c0, 2) * pow(c1, 0) + p11 * 1. * pow(c0, 1) * pow(c1, 0) + p02 * 2. * pow(c0, 0) * pow(c1, 1) + p30 * 0. * pow(c0, 3) * pow(c1, 0) + p21 * 1. * pow(c0, 2) * pow(c1, 0) + p12 * 2. * pow(c0, 1) * pow(c1, 1) + p03 * 3. * pow(c0, 0) * pow(c1, 2) + p40 * 0. * pow(c0, 4) * pow(c1, 0) + p31 * 1. * pow(c0, 3) * pow(c1, 0) + p22 * 2. * pow(c0, 2) * pow(c1, 1) + p13 * 3. * pow(c0, 1) * pow(c1, 2) + p04 * 4. * pow(c0, 0) * pow(c1, 3));
      default:
...
double part_coeff(int which_comp, double *c)
{
  if (const_part_coeff.val)
  {
    ...
    {
    case 0:
      return part_coeff_0.val;
    case 1:
      return part_coeff_1.val;
    case 2:
      return part_coeff_2.val;
    case 3:
      return part_coeff_3.val;
    }
  }
  else
  {
...
namespace analytic
{
  ...
  double F(double x)
  {
    ...
    {
    case -3:
      return .5 * (exp(-x * x) / x - sqrt(PI) * boost::math::erfc(x));
    case -2:
      return boost::math::expint(1, x * x);
    case -1:
...
#ifdef P4_TO_P8
    case -3:
      return analytic::rf_exact(t) - ABS3(x - xc(),
                                          y - yc(),
                                          z - zc());
#endif
    case -2:
      return analytic::rf_exact(t) - ABS2(x - xc(),
                                          y - yc());
    case -1:
      return analytic::rf_exact(t) - ABS1(y - ymin());
    case 0:
      return -(y - front_location()) + 0.000 / (1. + 100. * fabs(x / (xmin.val + xmax.val) - .5)) * double(rand()) / double(RAND_MAX) + 0.000 / (1. + 1000. * fabs(x / (xmin.val + xmax.val) - .75));
    case 1:
      return -(ABS2(x - xc(), y - yc()) - seed_radius());
    case 2:
      return (ABS2(x - xc(), y - yc()) - (container_radius_outer() - front_location()));
    case 3:
      return (ABS2(x - xc(), y - yc()) - (container_radius_outer() - front_location()));
    case 4:
      return -(ABS2(x - xc(), y - yc()) - (container_radius_inner() + front_location()));
    case 5:
    {
...
public:
  double operator()(DIM(double x, double y, double z)) const
  {
...
#ifdef P4_TO_P8
    case -3:
      return MAX(-container_radius_outer() + ABS3(x - xc(), y - yc(), z - zc()),
                 +container_radius_inner() - ABS3(x - xc(), y - yc(), z - zc()));
#endif
    case -2:
      return MAX(-container_radius_outer() + ABS2(x - xc(), y - yc()),
    ...
    case 0:
      return -1;
    case 1:
      return -container_radius_outer() + ABS2(x - xc(), y - yc());
    case 2:
      return -container_radius_outer() + ABS2(x - xc(), y - yc());
    case 3:
      return MAX(-container_radius_outer() + ABS2(x - xc(), y - yc()),
...
public:
  double operator()(DIM(double x, double y, double z)) const
  {
    ...
    case 5:
    {
      ...
      if (dist0 <= MIN(dist1, dist2))
        return 0;
      if (dist1 <= MIN(dist0, dist2))
        return 1;
      return 2;
    }
    case 6:
    {
      ...
      if (dist0 <= MIN(dist1, dist2, front))
        return 0;
      if (dist1 <= MIN(dist2, dist0, front))
        return 1;
      if (dist2 <= MIN(dist0, dist1, front))
        return 2;
      return 3;
    }
    case 7:
    {
      ...
      if (dist0 <= MIN(dist1, front))
        return 0;
      if (dist1 <= MIN(dist0, front))
        return 1;
      return 2;
    }
    default:
...
{
  if (seed_type.val == 0)
    return crystal_orientation.val;
  else
  {
    switch (geometry.val)
    {
    ...
    case 0:
      ...
      {
      case 0:
        return crystal_orientation.val - PI / 6.;
      case 1:
        return crystal_orientation.val + PI / 6.;
      default:
        throw;
    ...
    case 5:
      ...
      {
      case 0:
        return crystal_orientation.val - PI / 7.;
      case 1:
        return crystal_orientation.val + PI / 6.;
      case 2:
        return crystal_orientation.val - PI / 5.;
      default:
        throw;
      }
    case 6:
...
#ifdef P4_TO_P8
    case -3:
      return analytic::cl_exact(idx, t, ABS3(x - xc(), y - yc(), z - zc()));
#endif
    case -2:
      return analytic::cl_exact(idx, t, ABS2(x - xc(), y - yc()));
    case -1:
      return analytic::cl_exact(idx, t, ABS1(y - ymin()));
    case 0:
...
#ifdef P4_TO_P8
    case -3:
      return analytic::tl_exact(t, ABS3(x - xc.val, y - yc.val, z - zc.val));
#endif
    case -2:
      return analytic::tl_exact(t, ABS2(x - xc.val, y - yc.val));
    case -1:
      return analytic::tl_exact(t, ABS1(y - ymin.val));
    case 0:
      return analytic::Tstar + (y - (front_location.val + cooling_velocity.val * t)) * temp_gradient();
    case 1:
      return analytic::Tstar;
    case 2:
      return analytic::Tstar;
    case 3:
      return analytic::Tstar - container_radius_outer() * temp_gradient() * log(MAX(0.001, 1. + front_phi_cf(DIM(x, y, z)) / (container_radius_outer() - front_location())));
    case 4:
      return analytic::Tstar + container_radius_outer() * temp_gradient() * log(MAX(0.001, 1. - front_phi_cf(DIM(x, y, z)) / (container_radius_inner() + front_location())));
    case 5:
      return analytic::Tstar;
    case 6:
      return analytic::Tstar + (y - (front_location.val + cooling_velocity.val * t)) * temp_gradient();
...
#ifdef P4_TO_P8
    case -3:
      return analytic::ts_exact(t, ABS3(x - xc(), y - yc(), z - zc()));
#endif
    case -2:
      return analytic::ts_exact(t, ABS2(x - xc(), y - yc()));
    case -1:
      return analytic::ts_exact(t, ABS1(y - ymin()));
    case 0:
      return analytic::Tstar + (y - (front_location.val + cooling_velocity.val * t)) * temp_gradient() * thermal_cond_l.val / thermal_cond_s.val;
    case 1:
      return analytic::Tstar;
    case 2:
      return analytic::Tstar;
    case 3:
      return analytic::Tstar - container_radius_outer() * temp_gradient() * log(MAX(0.001, 1. + front_phi_cf(DIM(x, y, z)) / (container_radius_outer() - front_location()))) * thermal_cond_l() / thermal_cond_s();
    case 4:
      return analytic::Tstar + container_radius_outer() * temp_gradient() * log(MAX(0.001, 1. - front_phi_cf(DIM(x, y, z)) / (container_radius_inner() + front_location()))) * thermal_cond_l() / thermal_cond_s();
    case 5:
      return analytic::Tstar;
    case 6:
      return analytic::Tstar + (y - (front_location.val + cooling_velocity.val * t)) * temp_gradient() * thermal_cond_l.val / thermal_cond_s.val;
...
public:
  ...
  double operator()(DIM(double x, double y, double z)) const
  {
    ...
    case -3:
      ...
      {
      case VAL:
        return analytic::vn_exact(t);
      case DDX:
        return analytic::vn_exact(t) * (x - xc.val) / (EPS + ABS3(x - xc.val, y - yc.val, z - zc.val));
      case DDY:
        return analytic::vn_exact(t) * (y - yc.val) / (EPS + ABS3(x - xc.val, y - yc.val, z - zc.val));
      case DDZ:
        return analytic::vn_exact(t) * (z - zc.val) / (EPS + ABS3(x - xc.val, y - yc.val, z - zc.val));
      default:
        return 0;
      }
#endif
    case -2:
      ...
      case VAL:
        return analytic::vn_exact(t);
      case DDX:
        return analytic::vn_exact(t) * (x - xc.val) / (EPS + ABS2(x - xc.val, y - yc.val));
      case DDY:
        return analytic::vn_exact(t) * (y - yc.val) / (EPS + ABS2(x - xc.val, y - yc.val));
      default:
        return 0;
      }
    case -1:
...
#ifdef P4_TO_P8
    case -3:
      return -analytic::vf_exact(ABS3(x - xc.val, y - yc.val, z - zc.val));
#endif
    case -2:
      return -analytic::vf_exact(ABS2(x - xc.val, y - yc.val));
    case -1:
      return -analytic::vf_exact(ABS1(y - ymin.val));
    case 0:
      if (start_from_moving_front.val)
      {
        return -cooling_velocity.val;
...
#ifdef P4_TO_P8
    case -3:
      return analytic::ft_exact(ABS3(x - xc.val, y - yc.val, z - zc.val));
#endif
    case -2:
      return analytic::ft_exact(ABS2(x - xc.val, y - yc.val));
    case -1:
      return analytic::ft_exact(ABS1(y - ymin.val));
    case 0:
      if (start_from_moving_front.val)
      {
  ...
  double operator()(DIM(double x, double y, double z)) const
  {
...
#ifdef P4_TO_P8
      case -3:
        return ABS3(x - xc.val, y - yc.val, z - zc.val) < .5 * (container_radius_inner.val + container_radius_outer.val) ? analytic::ts_exact(t, container_radius_inner.val) : analytic::tl_exact(t, container_radius_outer.val);
#endif
      case -2:
        return ABS2(x - xc.val, y - yc.val) < .5 * (container_radius_inner.val + container_radius_outer.val) ? analytic::ts_exact(t, container_radius_inner.val) : analytic::tl_exact(t, container_radius_outer.val);
      case -1:
        return y < .5 * (ymin.val + ymax.val) ? analytic::ts_exact(t, 0) : analytic::tl_exact(t, ymax.val - ymin.val);
      case 0:
        return y < .5 * (ymin.val + ymax.val) ? analytic::Tstar + (y - (front_location.val + cooling_velocity.val * t)) * temp_gradient()
    ...
    case NEUMANN:
...
#ifdef P4_TO_P8
      case -3:
        return ABS3(x - xc.val, y - yc.val, z - zc.val) < .5 * (container_radius_inner.val + container_radius_outer.val) ? -thermal_cond_s.val * analytic::dts_exact(t, container_radius_inner.val) : +thermal_cond_l.val * analytic::dtl_exact(t, container_radius_outer.val);
#endif
      case -2:
        return ABS2(x - xc.val, y - yc.val) < .5 * (container_radius_inner.val + container_radius_outer.val) ? -thermal_cond_s.val * analytic::dts_exact(t, container_radius_inner.val) : +thermal_cond_l.val * analytic::dtl_exact(t, container_radius_outer.val);
      case -1:
        return y < .5 * (ymin.val + ymax.val) ? -analytic::dts_exact(t, 0) : analytic::dtl_exact(t, ymax.val - ymin.val);
      case 0:
        return y > .5 * (ymin.val + ymax.val) ? temp_gradient.val : -(temp_gradient.val * thermal_cond_l.val / thermal_cond_s.val + cooling_velocity.val * (latent_heat.val + density_l.val * temp_gradient.val * heat_capacity_l.val * (ymax.val - ymin.val)) / thermal_cond_s.val * smooth_start(t));
      case 1:
        return -seed_radius.val / container_radius_outer.val * cooling_velocity.val * latent_heat.val * smooth_start(t);
      case 2:
        return -(container_radius_outer.val - front_location.val) / container_radius_outer.val * cooling_velocity.val * latent_heat.val * smooth_start(t);
      case 3:
        if (ABS2(x - xc.val, y - yc.val) > .5 * (container_radius_inner.val + container_radius_outer.val))
        {
          return -thermal_cond_s.val * temp_gradient.val - (container_radius_outer.val - front_location.val) / container_radius_outer.val * cooling_velocity.val * latent_heat.val * smooth_start(t);
        }
        else
        {
      ...
      case 4:
        if (ABS2(x - xc.val, y - yc.val) > .5 * (container_radius_inner.val + container_radius_outer.val))
        {
          return thermal_cond_l.val * temp_gradient.val - (container_radius_inner.val + front_location.val) / container_radius_outer.val * cooling_velocity.val * latent_heat.val * smooth_start(t);
        }
        else
        {
...
#ifdef P4_TO_P8
      case -3:
        return analytic::cl_exact(idx, t, container_radius_outer.val);
#endif
      case -2:
        return analytic::cl_exact(idx, t, container_radius_outer.val);
      case -1:
        return analytic::cl_exact(idx, t, ABS1(ymax.val - ymin.val));
      case 0:
      case 1:
      case 2:
      case 3:
...
#ifdef P4_TO_P8
      case -3:
        return (*solute_diff_all[idx]) * analytic::dcl_exact(idx, t, container_radius_outer.val);
#endif
      case -2:
        return (*solute_diff_all[idx]) * analytic::dcl_exact(idx, t, container_radius_outer.val);
      case -1:
        return (*solute_diff_all[idx]) * analytic::dcl_exact(idx, t, ymax.val - ymin.val);
      case 0:
      case 1:
      case 2:
      case 3:
...
public:
  ...
  {
    if (geometry.val == 5)
      return -cooling_velocity.val * latent_heat.val * 2. * PI * seed_radius.val / (xmax.val - xmin.val) / (ymax.val - ymin.val) / box_size.val * smooth_start(t);
    else
      return 0;
  }
} volumetric_heat_cf;
...
class bc_wall_value_pressure : public CF_DIM{
  ...
  { 
    switch(example_){
    case BASIC_MULTI_ALLOY:
        switch (bc_wall_type_pressure.val)
        {
...
int main(int argc, char *argv[])
  {
    ...
    {
      ierr = PetscFOpen(mpi.comm(), filename_error_max, "w", &fich);
      CHKERRXX(ierr);
      ierr = PetscFClose(mpi.comm(), fich);
      CHKERRXX(ierr);
      ierr = PetscFOpen(mpi.comm(), filename_error_avg, "w", &fich);
      CHKERRXX(ierr);
      ierr = PetscFClose(mpi.comm(), fich);
      CHKERRXX(ierr);
    }
    ...
    {
      ierr = PetscFOpen(mpi.comm(), filename_characteristics, "w", &fich);
      CHKERRXX(ierr);
    ...
    {
      ierr = PetscFOpen(mpi.comm(), filename_analytic, "w", &fich);
      CHKERRXX(ierr);
      ierr = PetscFPrintf(mpi.comm(), fich, "step "
                                            "time "
                                            "phi "
      ...
      for (int i = 0; i < num_comps.val; ++i)
      {
        ierr = PetscFPrintf(mpi.comm(), fich, "cl%d ", i);
        CHKERRXX(ierr);
        ierr = PetscFPrintf(mpi.comm(), fich, "cs%d ", i);
        CHKERRXX(ierr);
      }
    ...
    // rochi comment - non dim to be done here
    ...
    if (start_from_moving_front.val && geometry.val == 0)
    {
      if (const_part_coeff.val)
      {
        for (int i = 0; i < num_comps.val; ++i)
        {
          analytic::Cstar[i] = (*initial_conc_all[i]) / analytic::kp[i];
        }
      }
      else
      {
    ...
    {
    case -2:
      analytic::set_analytical_solution(gradient_ratio.val, cooling_velocity.val, container_radius_inner.val + front_location.val);
      break;
    case -1:
      analytic::set_analytical_solution(gradient_ratio.val, cooling_velocity.val, front_location.val);
      break;
    default:
      break;
    }
    ...
    if (mpi.rank() == 0)
    {
      ...
      CHKERRXX(ierr);
      ierr = PetscPrintf(mpi.comm(), "thermal_diff_l: %g\n", thermal_cond_l.val / density_l.val / heat_capacity_l.val);
      CHKERRXX(ierr);
      ierr = PetscPrintf(mpi.comm(), "thermal_diff_s: %g\n", thermal_cond_s.val / density_s.val / heat_capacity_s.val);
      CHKERRXX(ierr);
    ...
    ierr = VecGhostUpdateBegin(front_phi.vec, INSERT_VALUES, SCATTER_FORWARD);
    CHKERRXX(ierr);
    ierr = VecGhostUpdateEnd(front_phi.vec, INSERT_VALUES, SCATTER_FORWARD);
    CHKERRXX(ierr);
    ...
    while (1)
    {
      ...
      if (save_characteristics.val)
      {
        ...
        mpiret = MPI_Allreduce(MPI_IN_PLACE, &buffer, 2, MPI_INT, MPI_SUM, p4est->mpicomm);
        SC_CHECK_MPI(mpiret);
        ...
        // write into file
        ierr = PetscFOpen(mpi.comm(), filename_characteristics, "a", &fich);
        CHKERRXX(ierr);
      ...
      if (save_now)
      {
        ...
        if (save_vtk_solid.val)
          mas.save_VTK_solid(vtk_idx);
        if (save_p4est.val)
          mas.save_p4est(vtk_idx);
        if (save_p4est_solid.val)
          mas.save_p4est_solid(vtk_idx);
        ...
        if (save_vtk_analytical.val || save_accuracy.val)
        {
          ...
          vec_and_ptr_t front;
          front.vec = mas.get_front_phi();
          vec_and_ptr_t contr;
          contr.vec = mas.get_contr_phi();
          vec_and_ptr_t ts;
          ts.vec = mas.get_ts();
          vec_and_ptr_t tl;
          tl.vec = mas.get_tl();
          ...
          vec_and_ptr_t ft(front.vec);
          interp.set_input(mas.get_ft(), linear);
          interp.interpolate(ft.vec);
          vec_and_ptr_t tf(front.vec);
          interp.set_input(mas.get_tf(), linear);
          interp.interpolate(tf.vec);
          vec_and_ptr_t vf(front.vec);
          interp.set_input(mas.get_vf(), linear);
          interp.interpolate(vf.vec);
          ...
          vec_and_ptr_t front_exact(front.vec);
          sample_cf_on_nodes(p4est, nodes, front_phi_cf, front_exact.vec);
          vec_and_ptr_t ts_exact(front.vec);
          sample_cf_on_nodes(p4est, nodes, ts_cf, ts_exact.vec);
          vec_and_ptr_t tl_exact(front.vec);
          sample_cf_on_nodes(p4est, nodes, tl_cf, tl_exact.vec);
          vec_and_ptr_t vn_exact(front.vec);
          sample_cf_on_nodes(p4est, nodes, vn_cf, vn_exact.vec);
          vec_and_ptr_t ft_exact(front.vec);
          sample_cf_on_nodes(p4est, nodes, ft_cf, ft_exact.vec);
          vec_and_ptr_t tf_exact(front.vec);
          sample_cf_on_nodes(p4est, nodes, tf_cf, tf_exact.vec);
          vec_and_ptr_t vf_exact(front.vec);
          sample_cf_on_nodes(p4est, nodes, vf_cf, vf_exact.vec);
          ...
          contr.get_array();
          front.get_array();
          front_exact.get_array();
          front_error.get_array();
          ts.get_array();
          ts_exact.get_array();
          ts_error.get_array();
          tl.get_array();
...
//                                         cell_data, cell_data_names);
          ...
          contr.restore_array();
          front.restore_array();
          front_exact.restore_array();
          front_error.restore_array();
          ts.restore_array();
          ts_exact.restore_array();
          ts_error.restore_array();
          tl.restore_array();
          tl_exact.restore_array();
          tl_error.restore_array();
          vn.restore_array();
          vn_exact.restore_array();
          vn_error.restore_array();
          ft.restore_array();
          ft_exact.restore_array();
          ft_error.restore_array();
          tf.restore_array();
          tf_exact.restore_array();
          tf_error.restore_array();
          vf.restore_array();
          vf_exact.restore_array();
          vf_error.restore_array();
          cl.restore_array();
          cl_exact.restore_array();
          cl_error.restore_array();
          cs.restore_array();
          cs_exact.restore_array();
          cs_error.restore_array();
          kps.restore_array();
          ...
          if (save_accuracy.val)
          {
            ...
            ierr = VecMax(front_error.vec, NULL, &errors_max[0]);
            CHKERRXX(ierr);
            ierr = VecMax(ts_error.vec, NULL, &errors_max[1]);
            CHKERRXX(ierr);
            ierr = VecMax(tl_error.vec, NULL, &errors_max[2]);
            CHKERRXX(ierr);
            ierr = VecMax(vn_error.vec, NULL, &errors_max[3]);
            CHKERRXX(ierr);
            ierr = VecMax(ft_error.vec, NULL, &errors_max[4]);
            CHKERRXX(ierr);
            ierr = VecMax(tf_error.vec, NULL, &errors_max[5]);
            CHKERRXX(ierr);
            ierr = VecMax(vf_error.vec, NULL, &errors_max[6]);
            CHKERRXX(ierr);
            for (int i = 0; i < num_comps.val; ++i)
            {
              ierr = VecMax(cl_error.vec[i], NULL, &errors_max[7 + i]);
              CHKERRXX(ierr);
              ierr = VecMax(cs_error.vec[i], NULL, &errors_max[7 + num_comps.val + i]);
              CHKERRXX(ierr);
            }

            mpiret = MPI_Allreduce(MPI_IN_PLACE, errors_max.data(), errors_max.size(), MPI_DOUBLE, MPI_MAX, p4est->mpicomm);
            SC_CHECK_MPI(mpiret);
            ...
            ierr = PetscFOpen(mpi.comm(), filename_analytic, "a", &fich);
            CHKERRXX(ierr);
            ierr = PetscFPrintf(mpi.comm(), fich, "%d ", iteration);
            CHKERRXX(ierr);
            ierr = PetscFPrintf(mpi.comm(), fich, "%e ", tn);
            CHKERRXX(ierr);
            ierr = PetscFPrintf(mpi.comm(), fich, "%e ", errors_max[0]);
            CHKERRXX(ierr);
            ierr = PetscFPrintf(mpi.comm(), fich, "%e ", errors_max[1]);
            CHKERRXX(ierr);
            ierr = PetscFPrintf(mpi.comm(), fich, "%e ", errors_max[2]);
            CHKERRXX(ierr);
            ierr = PetscFPrintf(mpi.comm(), fich, "%e ", errors_max[3]);
            CHKERRXX(ierr);
            ierr = PetscFPrintf(mpi.comm(), fich, "%e ", errors_max[4]);
            CHKERRXX(ierr);
            ierr = PetscFPrintf(mpi.comm(), fich, "%e ", errors_max[5]);
            CHKERRXX(ierr);
            ierr = PetscFPrintf(mpi.comm(), fich, "%e ", errors_max[6]);
            CHKERRXX(ierr);
            for (int i = 0; i < num_comps.val; ++i)
            {
              ierr = PetscFPrintf(mpi.comm(), fich, "%e ", errors_max[7 + i]);
              CHKERRXX(ierr);
              ierr = PetscFPrintf(mpi.comm(), fich, "%e ", errors_max[7 + num_comps.val + i]);
              CHKERRXX(ierr);
            }
            ierr = PetscFPrintf(mpi.comm(), fich, "\n");
            CHKERRXX(ierr);
            ...
            if (!keep_going)
            {
              ...
              ierr = PetscFOpen(mpi.comm(), name_last, "w", &fich);
              CHKERRXX(ierr);
              ierr = PetscFPrintf(mpi.comm(), fich, "%e ", dxyz[0]);
              CHKERRXX(ierr);
              ierr = PetscFPrintf(mpi.comm(), fich, "%d ", iteration);
              CHKERRXX(ierr);
              ierr = PetscFPrintf(mpi.comm(), fich, "%e ", tn);
              ...
              ierr = PetscFOpen(mpi.comm(), name_max, "w", &fich);
              CHKERRXX(ierr);
              ierr = PetscFPrintf(mpi.comm(), fich, "%e ", dxyz[0]);
              CHKERRXX(ierr);
              ierr = PetscFPrintf(mpi.comm(), fich, "%d ", iteration);
              ...
              ierr = PetscFPrintf(mpi.comm(), fich, "%e ", tn);
              CHKERRXX(ierr);
              ierr = PetscFPrintf(mpi.comm(), fich, "%e ", errors_max_over_time[0]);
              CHKERRXX(ierr);
              ierr = PetscFPrintf(mpi.comm(), fich, "%e ", errors_max_over_time[1]);
              CHKERRXX(ierr);
              ierr = PetscFPrintf(mpi.comm(), fich, "%e ", errors_max_over_time[2]);
              CHKERRXX(ierr);
              ierr = PetscFPrintf(mpi.comm(), fich, "%e ", errors_max_over_time[3]);
              CHKERRXX(ierr);
              ierr = PetscFPrintf(mpi.comm(), fich, "%e ", errors_max_over_time[4]);
              CHKERRXX(ierr);
              ierr = PetscFPrintf(mpi.comm(), fich, "%e ", errors_max_over_time[5]);
              CHKERRXX(ierr);
              ierr = PetscFPrintf(mpi.comm(), fich, "%e ", errors_max_over_time[6]);
              CHKERRXX(ierr);
              for (int i = 0; i < num_comps.val; ++i)
              {
                ierr = PetscFPrintf(mpi.comm(), fich, "%e ", errors_max_over_time[7 + i]);
                CHKERRXX(ierr);
                ierr = PetscFPrintf(mpi.comm(), fich, "%e ", errors_max_over_time[7 + num_comps.val + i]);
                CHKERRXX(ierr);
              }
              ierr = PetscFPrintf(mpi.comm(), fich, "\n");
              CHKERRXX(ierr);
      ...
      {
        // max bc error
        ierr = PetscFOpen(mpi.comm(), filename_error_max, "a", &fich);
        CHKERRXX(ierr);
        for (size_t i = 0; i < bc_error_max_all.size(); ++i)
        {
          ierr = PetscFPrintf(mpi.comm(), fich, "%e ", bc_error_max_all[i]);
          CHKERRXX(ierr);
        }
        for (int i = bc_error_max_all.size(); i < max_iterations(); ++i)
        {
          ierr = PetscFPrintf(mpi.comm(), fich, "%e ", 0.0);
          CHKERRXX(ierr);
        }
        ierr = PetscFPrintf(mpi.comm(), fich, "\n");
        CHKERRXX(ierr);
        ...
        // avg bc error
        ierr = PetscFOpen(mpi.comm(), filename_error_avg, "a", &fich);
        CHKERRXX(ierr);
        for (size_t i = 0; i < bc_error_avg_all.size(); ++i)
        {
          ierr = PetscFPrintf(mpi.comm(), fich, "%e ", bc_error_avg_all[i]);
          CHKERRXX(ierr);
        }
      ...
      mpiret = MPI_Allreduce(MPI_IN_PLACE, &total_growth, 1, MPI_DOUBLE, MPI_MAX, p4est->mpicomm);
      SC_CHECK_MPI(mpiret);
      ...
      ierr = PetscPrintf(mpi.comm(), "------------------------------------------------------------------------------------------------------\n"
                                     ...
                                     "------------------------------------------------------------------------------------------------------\n",
                         iteration, total_growth, tn, w1.get_duration_current());
      CHKERRXX(ierr);
      ...
      if (save_timings.val && save_now)
      {
        ...
        if (vtk_idx == 0)
        {
          ierr = PetscFOpen(mpi.comm(), filename_timings, "w", &fich);
          CHKERRXX(ierr);
        ...
        ierr = PetscFOpen(mpi.comm(), filename_timings, "a", &fich);
        CHKERRXX(ierr);
