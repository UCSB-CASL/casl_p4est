...
DEFINE_PARAMETER(pl,int,timing_every_n,1000,"Print timing info every n iterations (default 100)");
...
//      T_cyl = 260.;   // Physical cylinder temp [K]
    ...
    case NS_GIBOU_EXAMPLE:
      ...
      break;
    case DENDRITE_TEST:
      cp_s = 1913.; // Specific heat of solid, [J/kg]
...
//  my_p4est_node_neighbors_t* ngbd_nm1;
  ...
  for(int grid_res_iter=0;grid_res_iter<=num_splits;grid_res_iter++){
    ...
    set_geometry();
    PetscPrintf(mpi.comm(),"B example %d \n",example_);
    ...
    if(!loading_from_previous_state){
      ...
      if(solve_navier_stokes)NS_norm = max(u0,v0);
