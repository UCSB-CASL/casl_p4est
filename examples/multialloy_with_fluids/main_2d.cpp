...
int main(int argc, char** argv) {
  ...
  splitting_criteria_cf_t sp(3, 7, &level_set); // same as Daniil, minus lipschitz
  ...
  for (double t = 0; t<tf; t+=dt, tstep++){
      ...
      // --------------------------------------------------------------------------------------------------------------
      ierr = PetscPrintf(mpi.comm(),"Beginning setup for the Poisson nodes problem ... \n \n");
      ...
      ierr = PetscPrintf(mpi.comm(),"Beginning setup for the Poisson nodes solver ... \n \n");

      solver_T = new my_p4est_poisson_nodes_mls_t(ngbd);
      solver_T->add_interface(MLS_INTERSECTION,phi.vec,phi_dd.vec,interface_jump_value_temp,interface_jumpflux_value_temp);
      ...
      ierr = PetscPrintf(mpi.comm(),"Finishes setting up the Temperature solver.. \n \n");


      // Create vector to hold the solution:
      vec_and_ptr_t soln;
      soln.create(tl.vec);
      ierr = PetscPrintf(mpi.comm(),"Beginning solution for the Poisson nodes problem ... \n \n");

      // Solve the system:
      solver_T->solve(soln.vec);

      ierr = PetscPrintf(mpi.comm(),"Finishes solution for the Poisson nodes problem ... \n \n");

      // Copy the solution over to tl and ts:
      VecCopyGhost(soln.vec,tl.vec);
      VecCopyGhost(soln.vec,ts.vec);
      ...
      rhs_tl.destroy();
      rhs_ts.destroy();
      ierr = PetscPrintf(mpi.comm(),"Beginning field extension ... \n \n");

      // Extend the fields over the interface:
      my_p4est_level_set_t ls(ngbd);
      ls.extend_Over_Interface_TVD_full(phi.vec,phi.vec,tl.vec,50,2);
      ...
      ls.extend_Over_Interface_TVD_full(phi.vec,phi.vec,ts.vec,50,2);
      VecScale(phi.vec,-1.0);


      // --------------------------------------------------------------------------------------------------------------
      // PART II: ADVECTING THE LSF UNDER A VELOCITY FIELD -- Advect LSF and update grid
      // --------------------------------------------------------------------------------------------------------------
      ierr = PetscPrintf(mpi.comm(),"Beginning grid advection ... \n \n");
      ...
      ierr = PetscPrintf(mpi.comm(),"Finish copying the grid ... \n \n");
      ...
      ierr = PetscPrintf(mpi.comm(),"Finishes creating semi-lagrangian object ... \n \n");

//      ierr = PetscPrintf(mpi.comm(),"vx: \n");

//      ierr = VecView(vel_n.vec[0],PETSC_VIEWER_STDOUT_WORLD);
//      ierr = PetscPrintf(mpi.comm(),"vy: \n");

//      ierr = VecView(vel_n.vec[1],PETSC_VIEWER_STDOUT_WORLD);
//      ierr = PetscPrintf(mpi.comm(),"phi: \n");

//      ierr = VecView(phi.vec,PETSC_VIEWER_STDOUT_WORLD);

      // Advect the grid under the velocity field:
      sl.update_p4est(vel_n.vec,dt,phi.vec);
      ierr = PetscPrintf(mpi.comm(),"Finishes grid advection ... \n \n");
      ...
      ierr = PetscPrintf(mpi.comm(),"Begin updating fields onto new grid... \n \n");

      // Update the velocity field onto the new grid:
      ierr = PetscPrintf(mpi.comm(),"Sampling velocity on new grid... \n \n");
      vel_n.destroy();
      ...
      ierr = PetscPrintf(mpi.comm(),"Grabbing interpolation locations from new grid... \n \n");
      ...
      ierr = PetscPrintf(mpi.comm(),"Interpolating temperature to the new grid... \n \n");
      ...
      tl.destroy(); ts.destroy();
      tl.create(phi.vec); ts.create(phi.vec); // note that the phi object was automatically transferred to the new grid
