...
DEFINE_PARAMETER(pl, double, ns_soln_order, 1, "Solution order used for the navier stokes solver");
...
int main (int argc, char* argv[])
{
  ...
  // Elyce: Add fluids ------------------------------------------
  // Build the boundary conditions object
  ...
  while (keep_going)
  {
    ...
    if (iteration>0){
        ...
        // Set the LSF:
        vec_and_ptr_t phi_NS(front_phi.vec);
        ierr = VecCopy(front_phi.vec,phi_NS.vec);
        ns->set_phi(phi_NS.vec);
        ...
        ierr = PetscPrintf(mpi.comm(),"Sets the boundary conditions \n");
        ...
        PetscPrintf(mpi.comm(),"old hodge destroyed \n");
        // Interpolate the velocity values to the nodes:
        ns->compute_velocity_at_nodes();
        PetscPrintf(mpi.comm(),"Velocity computed at nodes \n");
        // Compute the pressure:
        ns->compute_pressure();
        PetscPrintf(mpi.comm(),"Pressure has been computed \n");

        // Check the velocity norm to make sure it isn't unreasonable:
        P4EST_ASSERT(ns->get_max_L2_norm_u()<200.0);

        delete ns;
        PetscPrintf(mpi.comm(),"NS solver deleted \n");

        p4est_destroy(p4est_NS);
        PetscPrintf(mpi.comm(),"NS forest deleted \n");

        p4est_nodes_destroy(nodes_NS);
        PetscPrintf(mpi.comm(),"NS nodes deleted \n");

        p4est_ghost_destroy(ghost_NS);
        PetscPrintf(mpi.comm(),"NS ghost deleted \n");
    ...
    PetscPrintf(mpi.comm(),"\n Computing the new timestep \n");
    mas.compute_dt();
    PetscPrintf(mpi.comm(),"Advancing the front  \n");
    mas.update_grid();
    PetscPrintf(mpi.comm(),"Proceeding to the next step \n");
