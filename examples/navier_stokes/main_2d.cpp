...
void check_error_analytic_vortex(mpi_environment_t& mpi, my_p4est_navier_stokes_t *ns)
{
  ...
  for(int dir=0; dir<P4EST_DIM; ++dir)
  {
    ...
    ierr = PetscPrintf(mpi.comm(), "Error on velocity in direction %d : %.5e\n", dir, err_v[dir]); CHKERRXX(ierr);
  }
...
//        ns.set_dt(dxmin*n_times_dt/u0);
    ...
    if(save_vtk && iter%save_every_n==0)
    {
      ...
      {
#ifdef P4_TO_P8
      case 0: sprintf(name, "%s/3d/vtu/analytic_vortex/without_time_%d", out_dir, iter/save_every_n); break;
      case 1: sprintf(name, "%s/3d/vtu/analytic_vortex/with_time_%d", out_dir,iter/save_every_n); break;
      case 2: sprintf(name, "%s/3d/vtu/karman/karman_%d", out_dir, iter/save_every_n); break;
      case 3: sprintf(name, "%s/3d/vtu/smoke_drop/smoke_drop_%d", out_dir, iter/save_every_n); break;
      case 4: sprintf(name, "%s/3d/vtu/smoke_hill/smoke_hill_%d", out_dir, iter/save_every_n); break;
      case 5: sprintf(name, "%s/3d/vtu/oscillating_sphere/oscillating_sphere_%d", out_dir, iter/save_every_n); break;
      case 6: sprintf(name, "%s/3d/vtu/smoke_city/smoke_city_%d", out_dir, iter/save_every_n); break;
#else
      case 0: sprintf(name, "%s/2d/vtu/analytic_vortex/without_time_%d", out_dir, iter/save_every_n); break;
      case 1: sprintf(name, "%s/2d/vtu/analytic_vortex/with_time_%d", out_dir, iter/save_every_n); break;
      case 2: sprintf(name, "%s/2d/vtu/driven_cavity/cavity_%d", out_dir, iter/save_every_n); break;
      case 3: sprintf(name, "%s/2d/vtu/driven_cavity_with_hole/hole_%d", out_dir, iter/save_every_n); break;
      case 4: sprintf(name, "%s/2d/vtu/karman/karman_%d", out_dir, iter/save_every_n); break;
      case 5: sprintf(name, "%s/2d/vtu/oscillating_cylinder/oscillating_%d", out_dir, iter/save_every_n); break;
      case 6: sprintf(name, "%s/2d/vtu/naca/naca_%d", out_dir, iter/save_every_n); break;
      case 7: sprintf(name, "%s/2d/vtu/smoke_drop/smoke_drop_%d", out_dir, iter/save_every_n); break;
      case 8: sprintf(name, "%s/2d/vtu/smoke_hill/smoke_hill_%d", out_dir, iter/save_every_n); break;
      case 9: sprintf(name, "%s/2d/vtu/rotating_cylinder/rotating_cylinder_%d", out_dir, iter/save_every_n); break;
#endif
      default: throw std::invalid_argument("choose a valid test.");
      }
#endif
