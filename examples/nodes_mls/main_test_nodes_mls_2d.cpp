...
int lmin = 5; // uniform grid for time being
int lmax = 5;
int nb_splits = 1;
...
//    bc.back().setWallValues(bc_wall_val);
    ...
    bc.back().setInterfaceValue(bc_interface_coeff_0);

    bc.push_back(BoundaryConditions2D());
    bc.back().setInterfaceType(bc_itype_1);
    bc.back().setInterfaceValue(bc_interface_val_1);
    bc.back().setInterfaceValue(bc_interface_coeff_1);
