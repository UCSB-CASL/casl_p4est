...
//                                                      &quad);
    ...
    if (rank_current == rank_found){
      local_dep_points_info.push(xy_departure, tree_idx, quad_idx/*quad->p.piggy3.local_num//*Comment here, quad, ni-nodes_->offset_owned_indeps);   //Push_back values for interpolation
...
*/
  ...
  //Set old phi to equal new phi
  phi_ptr = phi_np1_ptr;

//  ierr = VecRestoreArray(phi_np1, &phi_np1_ptr);
...
*/
  ...
  std::cout << " Loop through nodes " << endl;
  ...
  std::cout << " Update Vector phi " << endl;
  //Set the vector we want to interpolate from. Our input vector
  bif.update_vector(phi);
  std::cout << " Interpolate Vector phi " << endl;
  //Interpolate into our output vector
  bif.interpolate(phi_np1);
  std::cout << " Done! " << endl;
