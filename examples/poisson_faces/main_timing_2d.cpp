    ...
    for(int l=0; l<lmax; ++l)
    {
      ...
      if (!out_dir) { 
        ...
        mkdir(out_dir, 0755);
			...
			char name[1000];
#ifdef P4_TO_P8
			sprintf(name, "%s/scaling/vtu/scaling_faces_3d_%d", out_dir, mpi.size());
#else
			sprintf(name, "%s/scaling/vtu/scaling_faces_2d_%d", out_dir, mpi.size());
#endif
      my_p4est_vtk_write_all(p4est, nodes, NULL, P4EST_FALSE, P4EST_FALSE, 0, 1, name, VTK_CELL_DATA, "leaf_level", p);
			ierr = PetscPrintf(mpi.comm(), "saved visuals in %s\n", name);
