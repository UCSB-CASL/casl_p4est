...
static const double default_mu_m    = 1.0;
static const double default_mu_p    = 20.0;
...
static const shape_t default_shape  = FLOWER;
...
const string default_work_folder = "/scratch/regan/poisson_jump_faces";
#else
const string default_work_folder = "/home/regan/workspace/projects/poisson_jump_faces";
#endif

class LEVEL_SET : public CF_DIM {
  shape_t shape;
  const double xyz_c[P4EST_DIM];
  const double LHW[P4EST_DIM];
public:
  LEVEL_SET(const double *xyz_min, const double *xyz_max)
    : xyz_c{DIM(0.5*(xyz_min[0] + xyz_max[0]), 0.5*(xyz_min[1] + xyz_max[1]), 0.5*(xyz_min[2] + xyz_max[2]))},
      LHW{DIM(xyz_max[0] - xyz_min[0], xyz_max[1] - xyz_min[1], xyz_max[2] - xyz_min[2])}
  {
    lip = 1.2; shape = CIRCLE;
  }
...
#ifdef P4_TO_P8
      double phi, theta;
      if(fabs(sqrt(SQR(x - xyz_c[0]) + SQR(y - xyz_c[1])) < EPS*MAX(LHW[0], LHW[1])))
        phi = M_PI_2;
      else
        phi = acos((x - xyz_c[0])/sqrt(SQR(x - xyz_c[0]) + SQR(y - xyz_c[1]))) + (y - xyz_c[1] > 0.0 ? 0.0 : M_PI);
      theta = (sqrt(SQR(x - xyz_c[0]) + SQR(y - xyz_c[1]) + SQR(z - xyz_c[2])) > EPS*MAX(LHW[0], LHW[1], LHW[2]) ? acos((z - xyz_c[2])/sqrt(SQR(x - xyz_c[0]) + SQR(y - xyz_c[1]) + SQR(z - xyz_c[2]))) : 0.0);
      return SQR(x - xyz_c[0]) + SQR(y - xyz_c[1]) + SQR(z - xyz_c[2]) - SQR(0.7 + 0.2*(1.0 - 0.2*cos(6.0*phi))*(1.0 - cos(6.0*theta)));
#else
      double alpha = 0.02*sqrt(5.0);
      double tt;
      if(fabs(x - xyz_c[0] - alpha) < EPS*LHW[0])
        tt = (y > xyz_c[1] + alpha + EPS*LHW[1] ? 0.5*M_PI : (y < xyz_c[1] + alpha - EPS*LHW[1] ? -0.5*M_PI : 0.0));
      else
        tt = (x > xyz_c[0] + alpha + EPS*LHW[0] ? atan((y - xyz_c[1] - alpha)/(x - xyz_c[0] - alpha)) : ( y >= xyz_c[1] + alpha ? M_PI + atan((y - xyz_c[1] - alpha)/(x - xyz_c[0] - alpha)) : -M_PI + atan((y - xyz_c[1] - alpha)/(x - xyz_c[0] - alpha))));
      return SQR(x - xyz_c[0] - alpha) + SQR(y - xyz_c[1] - alpha) - SQR(0.5 + 0.2*sin(5*tt));
#endif
      break;
    }
    default:
      throw std::invalid_argument("main_poisson_jump_faces: choose a valid level set.");
      break;
    }
  }
...
public:
...
#endif
    default:
      throw std::invalid_argument("EXACT_SOLUTION::du_minus_d : unknown differentiation direction");
      break;
    }
  }
...
#endif
    default:
      throw std::invalid_argument("EXACT_SOLUTION::dv_minus_d : unknown differentiation direction");
      break;
    }
  }
  inline double laplace_v_minus(const double *xyz) const
  {
...
#endif
    default:
      throw std::invalid_argument("EXACT_SOLUTION::du_plus_d : unknown differentiation direction");
      break;
    }
  }
...
#endif
    default:
      throw std::invalid_argument("EXACT_SOLUTION::dv_plus_d : unknown differentiation direction");
      break;
    }
  }
  inline double laplace_v_plus(const double *xyz) const
  {
  ...
  inline double laplace_w_plus(const double *xyz) const
  {
  ...
  inline double solution(const char& sign, const u_char& comp, const double* xyz) const
  {
    switch (comp) {
    case dir::x:
      return (sign < 0 ? u_minus(xyz) : u_plus(xyz));
      break;
    case dir::y:
      return (sign < 0 ? v_minus(xyz) : v_plus(xyz));
      break;
#ifdef P4_TO_P8
    case dir::z:
      return (sign < 0 ? w_minus(xyz) : w_plus(xyz));
      break;
#endif
    default:
      std::invalid_argument("EXACT_SOLUTION::solution_minus: unkonwn component");
      break;
    }
    return NAN;
  }

  inline double gradient_solution(const char& sign, const u_char& comp, const u_char& der, const double* xyz) const
  {
    switch (comp) {
    case dir::x:
      return (sign < 0 ? du_minus_d(der, xyz) : du_plus_d(der, xyz));
      break;
    case dir::y:
      return (sign < 0 ? dv_minus_d(der, xyz) : dv_plus_d(der, xyz));
      break;
#ifdef P4_TO_P8
    case dir::z:
      return (sign < 0 ? dw_minus_d(der, xyz) : dw_plus_d(der, xyz));
      break;
#endif
    default:
      std::invalid_argument("EXACT_SOLUTION::gradient_solution: unkonwn component");
      break;
    }
    return NAN;
  }
  ...
  inline double laplace_solution(const char& sign, const u_char& comp, const double* xyz) const
  {
    switch (comp) {
    case dir::x:
      return (sign < 0 ? laplace_u_minus(xyz) : laplace_u_plus(xyz));
      break;
    case dir::y:
      return (sign < 0 ? laplace_v_minus(xyz) : laplace_v_plus(xyz));
      break;
#ifdef P4_TO_P8
    case dir::z:
      return (sign < 0 ? laplace_w_minus(xyz) : laplace_w_plus(xyz));
      break;
#endif
    default:
      std::invalid_argument("EXACT_SOLUTION::gradient_solution: unkonwn component");
...
void sample_and_flatten_validation_jumps(my_p4est_interface_manager_t* interface_manager, Vec& jump_solution, Vec& jump_flux_components, const double& mu_minus, const double& mu_plus)
{
  PetscErrorCode ierr;
  double *jump_solution_p, *jump_flux_components_p;
  ierr = VecGetArray(jump_solution,         &jump_solution_p);        CHKERRXX(ierr);
  ierr = VecGetArray(jump_flux_components,  &jump_flux_components_p); CHKERRXX(ierr);
  const my_p4est_node_neighbors_t& interface_capturing_ngbd = interface_manager->get_interface_capturing_ngbd_n();
  double xyz_node[P4EST_DIM];
  for (size_t k = 0; k < interface_capturing_ngbd.get_layer_size(); ++k) {
    const p4est_locidx_t node_idx = interface_capturing_ngbd.get_layer_node(k);
    node_xyz_fr_n(node_idx, interface_capturing_ngbd.get_p4est(), interface_capturing_ngbd.get_nodes(), xyz_node);
    for (u_char comp = 0; comp < P4EST_DIM; ++comp) {
      jump_solution_p[P4EST_DIM*node_idx + comp] = exact_solution.solution(1, comp, xyz_node) - exact_solution.solution(-1, comp, xyz_node);
      for (u_char der = 0; der < P4EST_DIM; ++der)
        jump_flux_components_p[SQR_P4EST_DIM*node_idx + P4EST_DIM*comp + der] = mu_plus*exact_solution.gradient_solution(1, comp, der, xyz_node) - mu_minus*exact_solution.gradient_solution(-1, comp, der, xyz_node);
    }
  }
  ierr = VecGhostUpdateBegin(jump_solution,         INSERT_VALUES, SCATTER_FORWARD); CHKERRXX(ierr);
  ierr = VecGhostUpdateBegin(jump_flux_components,  INSERT_VALUES, SCATTER_FORWARD); CHKERRXX(ierr);
  for (size_t k = 0; k < interface_capturing_ngbd.get_local_size(); ++k) {
    const p4est_locidx_t node_idx = interface_capturing_ngbd.get_local_node(k);
    node_xyz_fr_n(node_idx, interface_capturing_ngbd.get_p4est(), interface_capturing_ngbd.get_nodes(), xyz_node);
    for (u_char comp = 0; comp < P4EST_DIM; ++comp) {
      jump_solution_p[P4EST_DIM*node_idx + comp] = exact_solution.solution(1, comp, xyz_node) - exact_solution.solution(-1, comp, xyz_node);
      for (u_char der = 0; der < P4EST_DIM; ++der)
  ...
  ierr = VecGhostUpdateEnd(jump_solution,         INSERT_VALUES, SCATTER_FORWARD); CHKERRXX(ierr);
  ierr = VecGhostUpdateEnd(jump_flux_components,  INSERT_VALUES, SCATTER_FORWARD); CHKERRXX(ierr);
  ierr = VecRestoreArray(jump_solution,         &jump_solution_p);        CHKERRXX(ierr);
  ierr = VecRestoreArray(jump_flux_components,  &jump_flux_components_p); CHKERRXX(ierr);

  my_p4est_level_set_t ls(&interface_capturing_ngbd);
  Vec jump_solution_flat, jump_flux_components_flat;
  ierr = VecCreateGhostNodesBlock(interface_capturing_ngbd.get_p4est(), interface_capturing_ngbd.get_nodes(), P4EST_DIM, &jump_solution_flat); CHKERRXX(ierr);
  ierr = VecCreateGhostNodesBlock(interface_capturing_ngbd.get_p4est(), interface_capturing_ngbd.get_nodes(), SQR_P4EST_DIM, &jump_flux_components_flat); CHKERRXX(ierr);
  ls.extend_from_interface_to_whole_domain_TVD(interface_manager->get_phi(), jump_solution,         jump_solution_flat,         20, NULL, 2, 10, NULL, interface_manager->get_grad_phi(), P4EST_DIM);
  ls.extend_from_interface_to_whole_domain_TVD(interface_manager->get_phi(), jump_flux_components,  jump_flux_components_flat,  20, NULL, 2, 10, NULL, interface_manager->get_grad_phi(), SQR_P4EST_DIM);
  // delete non-flat data
  ierr = delete_and_nullify_vector(jump_solution); CHKERRXX(ierr);
  ierr = delete_and_nullify_vector(jump_flux_components); CHKERRXX(ierr);
  // return the flat ones
  jump_solution = jump_solution_flat;
  jump_flux_components = jump_flux_components_flat;
  return;
}

void sample_rhs(const double& mu_minus, const double& mu_plus, const my_p4est_faces_t* faces, Vec rhs_minus[P4EST_DIM], Vec rhs_plus[P4EST_DIM])
{
  PetscErrorCode ierr;
  for (u_char dir = 0; dir < P4EST_DIM; ++dir) {
    ...
    for (p4est_locidx_t f_idx = 0; f_idx < faces->num_local[dir]; ++f_idx) {
      ...
      rhs_minus_dir_p[f_idx]  = -mu_minus*exact_solution.laplace_solution(-1, dir, xyz_face);
      rhs_plus_dir_p[f_idx]   = -mu_plus*exact_solution.laplace_solution(+1, dir, xyz_face);
    }
...
int main (int argc, char* argv[])
{
  ...
  cmd.add_option("ntree", "number of trees in the macromesh along every dimension of the computational domain. Default value is " + to_string(default_ntree) + " (side length of domain is 2.5)");
  cmd.add_option("uniform_band", "number of grid cells of uniform grid layering the interface on either side (default is such that the finest level covers the region where the absolue value of phi is less than "  + to_string(uniform_band_factor*r0) + ")");
  ...
  cmd.add_option("shape",     "the shape of the interface (0: circle, 1: flower), default shape is " + string(default_shape == CIRCLE ? "circle)" : "flower.)" ));
...
\tThis is required if saving vtk or summary files: work_dir/vtu for vtk files work_dir/summaries for summary files. Default is " + default_work_folder);
  ...
  const shape_t ls_shape = (cmd.contains("shape") ? (cmd.get<int>("shape") == 0 ? CIRCLE : FLOWER) : default_shape);
  const string root_export_dir = cmd.get<std::string>("work_dir", (getenv("OUT_DIR") == NULL ? default_work_folder : getenv("OUT_DIR"))) + "/" + (ls_shape == CIRCLE ? "circle" : "flower") + "_shape_" + to_string(P4EST_DIM) + "D";
  ...
  for (int k_grid = 0; k_grid < ngrids; ++k_grid) {
    ...
    const double uniform_band = cmd.get<double>("uniform_band", uniform_band_factor*r0/dxyzmin);
    ...
    Vec jump_solution, jump_flux_components;
    ierr = VecCreateGhostNodesBlock(interface_capturing_ngbd_n->get_p4est(), interface_capturing_ngbd_n->get_nodes(), P4EST_DIM,      &jump_solution);        CHKERRXX(ierr);
    ierr = VecCreateGhostNodesBlock(interface_capturing_ngbd_n->get_p4est(), interface_capturing_ngbd_n->get_nodes(), SQR_P4EST_DIM,  &jump_flux_components); CHKERRXX(ierr);
    sample_and_flatten_validation_jumps(interface_manager, jump_solution, jump_flux_components, mu_minus, mu_plus);
    ...
    sample_rhs(mu_minus, mu_plus, faces, rhs_minus, rhs_plus);
