...
double phi_x(double x, double y, double z)
{
  ...
  {
  case 0:
    return -(x-(double) nx/2)/sqrt(SQR(x-(double) nx/2)+SQR(y-(double) ny/2)+SQR(z-(double) nz/2));
  default:
    throw std::invalid_argument("Choose a valid level set.");
  }
}
...
double phi_y(double x, double y, double z)
{
  ...
  {
  case 0:
    return -(y-(double) ny/2)/sqrt(SQR(x-(double) nx/2)+SQR(y-(double) ny/2)+SQR(z-(double) nz/2));
  default:
    throw std::invalid_argument("Choose a valid level set.");
  }
}
...
double phi_z(double x, double y, double z)
{
  ...
  {
  case 0:
    return -(z-(double) nz/2)/sqrt(SQR(x-(double) nx/2)+SQR(y-(double) ny/2)+SQR(z-(double) nz/2));
  default:
    throw std::invalid_argument("Choose a valid level set.");
  }
}
...
double phi_x(double x, double y)
{
  ...
  {
  case 0:
    return -(x-(double) nx/2)/sqrt(SQR(x-(double) nx/2)+SQR(y-(double) ny/2));
  default:
    throw std::invalid_argument("Choose a valid level set.");
  }
}
...
double phi_y(double x, double y)
{
  ...
  {
  case 0:
    return -(y-(double) ny/2)/sqrt(SQR(x-(double) nx/2)+SQR(y-(double) ny/2));
  default:
    throw std::invalid_argument("Choose a valid level set.");
  }
}
...
{
  double r = sqrt(SQR(x-(double) nx/2) + SQR(y-(double) ny/2));
  switch(test_number)
  {
  case 0:
    return 1+log(r/.5);
...
double grad_u_m(double x, double y)
{
  ...
  {
  case 0:
    ux = (x-(double) nx/2)/(SQR(x-(double) nx/2)+SQR(y-(double) ny/2));
    uy = (y-(double) ny/2)/(SQR(x-(double) nx/2)+SQR(y-(double) ny/2));
    break;
  case 1:
    ux = -sin(x)*sin(y);
    uy =  cos(x)*cos(y);
...
public:
  double operator()(double x, double y) const
  {
    ...
    else
    {
      ...
      if(ABS(x-nx)<EPS) return  cos(x)*sin(y);
...
void shift_Neumann_Solution(p4est_t *p4est, p4est_nodes_t *nodes, Vec sol)
{
  ...
  for(size_t n=0; n<nodes->indep_nodes.elem_count; ++n)
    sol_p[n] += (ex_int - sol_int)/((double) nx * (double) ny);
...
//  if(p4est->mpirank==0)
//    solver.print_voronoi_VTK("/home/guittet/code/Output/p4est_jump/vtu/voronoi");
//  solver.compute_voronoi_mesh();
//  solver.setup_negative_laplace_matrix();
//  solver.setup_negative_laplace_rhsvec();

  char out_stats[10000];
#ifdef STAMPEDE
  char *out_dir;
  out_dir = getenv("OUT_DIR");
  sprintf(out_stats, "%s/stats.dat", out_dir);
#else
  sprintf(out_stats, "%d.dat", p4est->mpisize);
#endif
  solver.write_stats(out_stats);

//  solver.print_voronoi_VTK("voronoi");
...
int main (int argc, char* argv[])
{
  ...
  double xyz_max [] = {(double)nx, (double)ny, (double)nz};
#else
  int n_xyz [] = {nx, ny, 1};
  double xyz_min [] = {0, 0, 0};
  double xyz_max [] = {(double)nx, (double)ny, 1};
#endif
...
#ifdef P4_TO_P8
      double z = node_z_fr_n(n, p4est, nodes);
      err_p[n] = ABS(u_exact(x,y,z) - sol_p[n]);
#else
      err_p[n] = ABS(u_exact(x,y) - sol_p[n]);
#endif
      if(err_p[n]>err_n)
      {
        x_err = x; y_err = y;
