...
void set_example(int example_)
{
   switch (example_) {
      ...
      case 34: // To test projections step - div ( grad(hodge) ) = rhs in standard rectangular domain with Neumann bc
         ...
         which_geometry_for_bdry_00.val = 0;
...
class u_exact_choices_cf_t : public CF_DIM {
public:
    int *n;
    double *mag;
    cf_value_type_t what;

    u_exact_choices_cf_t(cf_value_type_t what) : what(what){}
    ...
    double operator()(DIM(double x, double y, double z)) const override {
        switch (*n) {
            case 0:
                switch (what) {
#ifdef P4_TO_P8
                    throw std::invalid_argument("Invalid case of class u_exact_choices_cf_t in 3D");
#else
                    case VAL:
                        return sin(x) * cos(y);
                    case DDX:
...
#endif
                    default:
                        throw std::invalid_argument("Invalid \"what\" in case 0 of class u_exact_choices_cf_t");
                }
            default:
                throw std::invalid_argument("Invalid case of class u_exact_choices_cf_t");
        }
    }
};

u_exact_choices_cf_t u_exact_cf(VAL);

// EXACT VNP1 VALUE

class v_exact_choices_cf_t : public CF_DIM {
public:
    int *n;
    double *mag;
    cf_value_type_t what;

    v_exact_choices_cf_t(cf_value_type_t what) : what(what){}
    ...
    double operator()(DIM(double x, double y, double z)) const override {
        switch (*n) {
            case 0:
                switch (what) {
#ifdef P4_TO_P8
                    throw std::invalid_argument("Invalid case of class v_exact_choices_cf_t in 3D");
#else
                    case VAL:
                        return -sin(y) * cos(x);
                    case DDX:
...
#endif
                    default:
                        throw std::invalid_argument("Invalid \"what\" in case 0 of class v_exact_choices_cf_t");
                }
            default:
                throw std::invalid_argument("Invalid case of class v_exact_choices_cf_t");
        }
    }
};

v_exact_choices_cf_t v_exact_cf(VAL);
...
int main(int argc, char *argv[]) {
   ...
   for (int iter = 0; iter < num_splits(); ++iter) {
      ...
      Vec hodge_vec; double *hodge_ptr;    // hodge is the solution in the minus and plus domains.
      ierr = VecCreateGhostNodes(p4est, nodes, &hodge_vec); CHKERRXX(ierr);
      ...
      solver.set_wc(bc_wall_type, hodge_cf);
       ...
       *
       ***********************************************************************************/
      Vec ustar_vec; double *ustar_ptr;
      Vec vstar_vec; double *vstar_ptr;
      ierr = VecCreateGhostNodes(p4est, nodes, &ustar_vec); CHKERRXX(ierr);
      ierr = VecCreateGhostNodes(p4est, nodes, &vstar_vec); CHKERRXX(ierr);
      ierr = VecGetArray(ustar_vec, &ustar_ptr); CHKERRXX(ierr);
      ierr = VecGetArray(vstar_vec, &vstar_ptr); CHKERRXX(ierr);

      Vec unp1_vec; double *unp1_ptr;
      Vec vnp1_vec; double *vnp1_ptr;
      ierr = VecCreateGhostNodes(p4est, nodes, &unp1_vec); CHKERRXX(ierr);
      ierr = VecCreateGhostNodes(p4est, nodes, &vnp1_vec); CHKERRXX(ierr);
      ierr = VecGetArray(unp1_vec, &unp1_ptr); CHKERRXX(ierr);
      ierr = VecGetArray(vnp1_vec, &vnp1_ptr); CHKERRXX(ierr);

      Vec minus_div_Ustar_vec; double *minus_div_Ustar_ptr;
      ierr = VecCreateGhostNodes(p4est, nodes, &minus_div_Ustar_vec); CHKERRXX(ierr);
      ierr = VecGetArray(minus_div_Ustar_vec, &minus_div_Ustar_ptr); CHKERRXX(ierr);

      foreach_local_node(n, nodes) {
         double xyz[P4EST_DIM];
         node_xyz_fr_n(n, p4est, nodes, xyz);
         double x = xyz[0], y = xyz[1], z = xyz[2];
         ustar_ptr[n] =  sin(x)*cos(y) + x*(PI-x)*y*y*(y/3.-PI/2.);
         vstar_ptr[n] = -sin(y)*cos(x) + y*(PI-y)*x*x*(x/3.-PI/2.);
      }

      ierr = VecGhostUpdateBegin(ustar_vec, INSERT_VALUES, SCATTER_FORWARD); CHKERRXX(ierr);
      ierr = VecGhostUpdateBegin(vstar_vec, INSERT_VALUES, SCATTER_FORWARD); CHKERRXX(ierr);
      ierr = VecGhostUpdateEnd(ustar_vec, INSERT_VALUES, SCATTER_FORWARD); CHKERRXX(ierr);
      ierr = VecGhostUpdateEnd(vstar_vec, INSERT_VALUES, SCATTER_FORWARD); CHKERRXX(ierr);
      ...
      foreach_local_node(n, nodes) {
         ...
         minus_div_Ustar_ptr[n] = -(qnnn.dx_central(ustar_ptr) + qnnn.dy_central(vstar_ptr));
      }

      // solve for the hodge variable with the right-hand side set to minus_div_Ustar_vec
      // TODO: we will have to compute minus_div_Ustar_vec in the minus and plus regions when considering irregular domains.
//      solver.set_rhs(rhs_m, rhs_p);
      solver.set_rhs(minus_div_Ustar_vec, minus_div_Ustar_vec);
      ...
      if (use_nonzero_guess()) sample_cf_on_nodes(p4est, nodes, hodge_cf, hodge_vec);

      // actual solve for Hodge:
      solver.solve(hodge_vec, use_nonzero_guess());
       ...
       *
       ***********************************************************************************/
       // Reset the pointer hodge_ptr to the beginning of hodge_vec
      ierr = VecGetArray(hodge_vec, &hodge_ptr); CHKERRXX(ierr);
      ierr = VecGhostUpdateBegin(hodge_vec, INSERT_VALUES, SCATTER_FORWARD); CHKERRXX(ierr);
      ierr = VecGhostUpdateEnd(hodge_vec, INSERT_VALUES, SCATTER_FORWARD); CHKERRXX(ierr);
      ...
      foreach_local_node(n, nodes) {
         ...
         if (!is_node_Wall(p4est, ni) && qnnn.is_stencil_in_negative_domain(mask_m_ptr)) {
            ngbd_n.get_neighbors(n, qnnn);
            unp1_ptr[n] = ustar_ptr[n] - qnnn.dx_central(hodge_ptr);
            vnp1_ptr[n] = vstar_ptr[n] - qnnn.dy_central(hodge_ptr);
         } else { // set Unp1 to be the exact solution, since in Navier-Stokes we will impose a Dirichlet boundary condition (no-slip):
            double xyz[P4EST_DIM];
            node_xyz_fr_n(n, p4est, nodes, xyz);
            double x = xyz[0], y = xyz[1], z = xyz[2];
            unp1_ptr[n] =  sin(x)*cos(y);
            vnp1_ptr[n] = -sin(y)*cos(x);
         }
      }

      // Restoring vectors:
      ierr = VecRestoreArray(ustar_vec, &ustar_ptr); CHKERRXX(ierr);
      ierr = VecRestoreArray(vstar_vec, &vstar_ptr); CHKERRXX(ierr);
      ierr = VecDestroy(ustar_vec); CHKERRXX(ierr);
      ierr = VecDestroy(vstar_vec); CHKERRXX(ierr);

      ierr = VecGhostUpdateBegin(minus_div_Ustar_vec, INSERT_VALUES, SCATTER_FORWARD); CHKERRXX(ierr);
      ierr = VecGhostUpdateEnd(minus_div_Ustar_vec, INSERT_VALUES, SCATTER_FORWARD); CHKERRXX(ierr);
      ierr = VecRestoreArray(minus_div_Ustar_vec, &minus_div_Ustar_ptr); CHKERRXX(ierr);
      ierr = VecDestroy(minus_div_Ustar_vec); CHKERRXX(ierr);

      ierr = VecGhostUpdateBegin(unp1_vec, INSERT_VALUES, SCATTER_FORWARD); CHKERRXX(ierr);
      ierr = VecGhostUpdateBegin(vnp1_vec, INSERT_VALUES, SCATTER_FORWARD); CHKERRXX(ierr);
      ierr = VecGhostUpdateEnd(unp1_vec, INSERT_VALUES, SCATTER_FORWARD); CHKERRXX(ierr);
      ierr = VecGhostUpdateEnd(vnp1_vec, INSERT_VALUES, SCATTER_FORWARD); CHKERRXX(ierr);
      ierr = VecRestoreArray(unp1_vec, &unp1_ptr); CHKERRXX(ierr);
      ierr = VecRestoreArray(vnp1_vec, &vnp1_ptr); CHKERRXX(ierr);
      ierr = VecDestroy(unp1_vec); CHKERRXX(ierr);
      ierr = VecDestroy(vnp1_vec); CHKERRXX(ierr);
      ...
      Vec hodge_m = hodge_vec; double *hodge_m_ptr;
      Vec hodge_p = hodge_vec; double *hodge_p_ptr;




       //----------------------------------------------------------------------------------------------
       // calculate error of unp1,vnp1
       //----------------------------------------------------------------------------------------------
       Vec vec_error_unp1; double *vec_error_unp1_ptr;
       Vec vec_error_vnp1; double *vec_error_vnp1_ptr;

       ierr = VecCreateGhostNodes(p4est, nodes, &vec_error_unp1); CHKERRXX(ierr);
       ierr = VecCreateGhostNodes(p4est, nodes, &vec_error_vnp1); CHKERRXX(ierr);

       ierr = VecGetArray(unp1_vec, &unp1_ptr); CHKERRXX(ierr);
       ierr = VecGetArray(vnp1_vec, &vnp1_ptr); CHKERRXX(ierr);

       ierr = VecGetArray(vec_error_unp1, &vec_error_unp1_ptr); CHKERRXX(ierr);
       ierr = VecGetArray(vec_error_vnp1, &vec_error_vnp1_ptr); CHKERRXX(ierr);

       foreach_local_node(n, nodes) {
           double xyz[P4EST_DIM];
           node_xyz_fr_n(n, p4est, nodes, xyz);

           vec_error_unp1_ptr[n] = mask_m_ptr[n] < 0 ? ABS(unp1_ptr[n] - u_exact_cf.value(xyz)) : 0;
           vec_error_vnp1_ptr[n] = mask_p_ptr[n] < 0 ? ABS(vnp1_ptr[n] - v_exact_cf.value(xyz)) : 0;
       ...
       ierr = VecRestoreArray(vec_error_unp1, &vec_error_unp1_ptr); CHKERRXX(ierr);
       ierr = VecRestoreArray(vec_error_vnp1, &vec_error_vnp1_ptr); CHKERRXX(ierr);

       ierr = VecGhostUpdateBegin(vec_error_unp1, INSERT_VALUES, SCATTER_FORWARD); CHKERRXX(ierr);
       ierr = VecGhostUpdateBegin(vec_error_vnp1, INSERT_VALUES, SCATTER_FORWARD); CHKERRXX(ierr);
       ierr = VecGhostUpdateEnd(vec_error_unp1, INSERT_VALUES, SCATTER_FORWARD); CHKERRXX(ierr);
       ierr = VecGhostUpdateEnd(vec_error_vnp1, INSERT_VALUES, SCATTER_FORWARD); CHKERRXX(ierr);
...
//        quad_neighbor_nodes_of_node_t qnnn{};
      ...
      if (iter > -1) {
         ...
         ierr = PetscPrintf(p4est->mpicomm, "gradient hodge = %3.2e (%+3.2f), ", err_grad_hodge_m_max, log(error_gradient_hodge_m_arr[iter - 1] / error_gradient_hodge_m_arr[iter]) / log(2)); CHKERRXX(ierr);
         ...
         ierr = PetscPrintf(p4est->mpicomm, "gradient hodge = %3.2e (%+3.2f), ", err_grad_hodge_p_max, log(error_gradient_hodge_p_arr[iter - 1] / error_gradient_hodge_p_arr[iter]) / log(2)); CHKERRXX(ierr);
         ierr = PetscPrintf(p4est->mpicomm, "\n"); CHKERRXX(ierr);
      }
      ...
      if (save_vtk()) {
         ...
         ierr = VecDuplicate(hodge_vec, &hodge_exact); CHKERRXX(ierr);
         sample_cf_on_nodes(p4est, nodes, hodge_cf, hodge_exact);

         ierr = VecGetArray(bdry_phi_eff, &bdry_phi_eff_ptr); CHKERRXX(ierr);

         ierr = VecGetArray(hodge_vec, &hodge_ptr); CHKERRXX(ierr);
         ierr = VecGetArray(hodge_exact, &hodge_exact_ptr); CHKERRXX(ierr);
         ...
         my_p4est_vtk_write_all(p4est, nodes, ghost,
                                ...
                                VTK_POINT_DATA, "hodge", hodge_ptr,
         ...
         ierr = VecRestoreArray(hodge_vec, &hodge_ptr); CHKERRXX(ierr);
         ierr = VecRestoreArray(hodge_exact, &hodge_exact_ptr); CHKERRXX(ierr);
         ...
         ierr = VecDestroy(hodge_exact); CHKERRXX(ierr);
      ...
      // destroy Vec's with errors
       ...
       ierr = VecDestroy(vec_error_unp1); CHKERRXX(ierr);
       ierr = VecDestroy(vec_error_vnp1); CHKERRXX(ierr);
      ...
      ierr = VecDestroy(hodge_vec); CHKERRXX(ierr);
