/*
 *
 * THERE ARE SOME U'S LEFT IN THE DD'S
...
int main(int argc, char *argv[]) {
    ...
    for (int iter = 0; iter < num_splits(); ++iter) {
        ...
        Vec hodge; double *hodge_ptr;    // hodge is the solution in the minus and plus domains.
        ierr = VecCreateGhostNodes(p4est, nodes, &hodge); CHKERRXX(ierr);
        ...
        if (use_nonzero_guess()) sample_cf_on_nodes(p4est, nodes, hodge_cf, hodge);

        // Solve for the Hodge variable:
        solver.solve(hodge, use_nonzero_guess());
        ...
        foreach_local_node(n, nodes) {   // no need for hiding of communication in this case.
            // will be needed when iterating the projection.
            double xyz[P4EST_DIM];
...
//        double *mask_p_ptr;
        ...
        Vec hodge_m = hodge; double *hodge_m_ptr;
        Vec hodge_p = hodge; double *hodge_p_ptr;
...
//        quad_neighbor_nodes_of_node_t qnnn{};
        ...
        if (save_vtk()) {
            ...
            ierr = VecRestoreArray(hodge, &hodge_ptr); CHKERRXX(ierr);
