...
int main(int argc, char *argv[]) {
   ...
   vector<double> error_unp1_arr, error_vnp1_arr;    // we'll have to have such an array in the positive and negative region.
   ...
   for (int iter = 0; iter < num_splits(); ++iter) {
      ...
      foreach_local_node(n, nodes) {
         ...
         double x = xyz[0], y = xyz[1], z = xyz[2];
          ...
          // TODO: needs to separate minus and plus solutions.
      ...
      // TODO: we will have to compute vec_minus_div_Ustar_m in the minus and plus regions when considering irregular domains.
//      solver.set_rhs(vec_rhs_m, vec_rhs_p);
      ...
      foreach_local_node(n, nodes) {
         ...
         if (!is_node_Wall(p4est, ni) && qnnn.is_stencil_in_negative_domain(mask_m_ptr)) {
            ngbd_n.get_neighbors(n, qnnn);
            ptr_unp1_m[n] = ptr_ustar_m[n] - qnnn.dx_central(hodge_ptr);
            ptr_vnp1_m[n] = ptr_vstar_m[n] - qnnn.dy_central(hodge_ptr);
      ...
      double err_unp1_max = 0, err_vnp1_max = 0;

      foreach_local_node(n, nodes) {
         double xyz[P4EST_DIM], x, y, z;
         node_xyz_fr_n(n, p4est, nodes, xyz); x = xyz[0], y = xyz[1], z = xyz[2];

         err_unp1_max = MAX(err_unp1_max, ABS(ptr_unp1_m[n] - sin(x) * cos(y) ));
         err_vnp1_max = MAX(err_vnp1_max, ABS(ptr_vnp1_m[n] + sin(y) * cos(x) ));
      }

      // Take the max of the local unp1 we just computed:
      mpiret = MPI_Allreduce(MPI_IN_PLACE, &err_unp1_max, 1, MPI_DOUBLE, MPI_MAX, p4est->mpicomm); SC_CHECK_MPI(mpiret);
      mpiret = MPI_Allreduce(MPI_IN_PLACE, &err_vnp1_max, 1, MPI_DOUBLE, MPI_MAX, p4est->mpicomm); SC_CHECK_MPI(mpiret);
      ...
      error_unp1_arr.push_back(err_unp1_max);
      error_vnp1_arr.push_back(err_vnp1_max);
      ...
      if (iter > -1) {
         ...
         ierr = PetscPrintf(p4est->mpicomm, "unp1 = %3.2e (%+3.2f), ", err_unp1_max, log(error_unp1_arr[iter - 1] / error_unp1_arr[iter]) / log(2)); CHKERRXX(ierr);
         ierr = PetscPrintf(p4est->mpicomm, "vnp1 = %3.2e (%+3.2f).", err_vnp1_max, log(error_vnp1_arr[iter - 1] / error_vnp1_arr[iter]) / log(2)); CHKERRXX(ierr);
         ...
         ierr = PetscPrintf(p4est->mpicomm, "unp1 = to do (todo), ");
         ierr = PetscPrintf(p4est->mpicomm, "vnp1 = to do (todo).");
         ierr = PetscPrintf(p4est->mpicomm, "\n"); CHKERRXX(ierr);
      }
