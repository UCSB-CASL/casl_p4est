...
#else
#include "../../../external_libraries/p4est_debug/include/p4est_bits.h"
#include "../../../external_libraries/p4est_debug/include/p4est_extended.h"
...
param_t<int>    example (pl, 36, "example", "Predefined example:\n"
                                            ...
                                            "32 - Union of two spheres - protein example (Added by Faranak)\n"
                                            "33 - Union of three spheres - protein example (Added by Faranak)\n"
                                            "34 - Union of Mutiple spheres - protein example (Added by Faranak)");
...
param_t<bool>   fv_scheme         (pl, 1, "fv_scheme",         "Scheme for finite volume discretization on boundaries: 0 - symmetric, 1 - superconvergent");
...
// Heat equation parameters - Faranak's Edition
param_t<bool>   solve_heat_equation (pl, false, "solve_heat_equation", "Solve time-dependent heat equation");
param_t<double> dt                  (pl, 0.01, "dt", "Time step size");
param_t<double> t_final             (pl, 5.0 , "t_final", "Final simulation time");
param_t<double> current_time        (pl, 0.0 , "current_time", "Current simulation time");
param_t<double> theta_value         (pl, 0.5 , "theta_value", "Theta for theta method");
param_t<int>    save_timestep_every (pl, 100  , "save_timestep_every", "Save VTK every N timesteps");
param_t<int>    time_method         (pl, 0   , "time_method", "0=BE, 1=CN, 2=BDF2, 3=Theta");
...
void set_example(int example)
{
  switch (example)
  {
    case 6: // POISSON: union of two spheres - UNCHANGED
      solve_heat_equation.val = false;

      // Solution and coefficients
      sol_m.val = 3; sol_m_mult.val = 1;   // Solution type 3 outside
      sol_p.val = 0; sol_p_mult.val = 0;   // No solution inside

      diff_coeff_m.val = 0; diff_coeff_m_mult.val = 1;  // μ = 1 outside
      diff_coeff_p.val = 0; diff_coeff_p_mult.val = 1;  // μ = 1 inside

      linear_term_m_coeff.val = 0; linear_term_m_mult.val = 0;  // No reaction
      linear_term_p_coeff.val = 0; linear_term_p_mult.val = 0;

      nonlinear_term_m.val = 0; nonlinear_term_m_mult.val = 0;
      nonlinear_term_p.val = 0; nonlinear_term_p_mult.val = 0;
      nonlinear_term_m_coeff.val = 0; nonlinear_term_m_mult.val = 0;
      nonlinear_term_p_coeff.val = 0; nonlinear_term_p_mult.val = 0;

      // Geometry: two spheres
      num_infc.val = 0;
      num_bdry.val = 2;

      bdry_00_present.val = 1; bdry_00_geometry.val = 6; bdry_00_opn.val = MLS_INT;
      bdry_00_coeff.val = 0; bdry_00_coeff_mult.val = 1; bdry_00_type.val = ROBIN;

      bdry_01_present.val = 1; bdry_01_geometry.val = 7; bdry_01_opn.val = MLS_INT;
      bdry_01_coeff.val = 3; bdry_01_coeff_mult.val = 1; bdry_01_type.val = ROBIN;

      bdry_02_present.val = 0;
      bdry_03_present.val = 0;

      // RHS from MMS
      rhs_m_value.val = 0;  // 0 = automatic MMS
      rhs_p_value.val = 0;

      // Wall BC type
      wc_type.val = DIRICHLET;

      break;

    case 36: // HEAT: Basic heat equation with constant coefficients
      solve_heat_equation.val = true;

      // Solution: sin(x)cos(y)e^{-2μt} inside domain
      sol_m.val = 0; sol_m_mult.val = 1.0;   // Inside domain (φ<0)
      sol_p.val = 0; sol_p_mult.val = 0.0;   // Outside (zero)

      // Constant diffusion coefficient
      diff_coeff_m.val = 0; diff_coeff_m_mult.val = 1.0;  // μ = 1 inside
      diff_coeff_p.val = 0; diff_coeff_p_mult.val = 1.0;  // μ = 1 outside

      // No reaction term (pure diffusion)
      linear_term_m_coeff.val = 0; linear_term_m_mult.val = 0.0;
      linear_term_p_coeff.val = 0; linear_term_p_mult.val = 0.0;

      // No nonlinear terms
      nonlinear_term_m.val = 0; nonlinear_term_m_mult.val = 0.0;
      nonlinear_term_p.val = 0; nonlinear_term_p_mult.val = 0.0;
      nonlinear_term_m_coeff.val = 0; nonlinear_term_m_mult.val = 0.0;
      nonlinear_term_p_coeff.val = 0; nonlinear_term_p_mult.val = 0.0;

      // RHS computed from MMS
      rhs_m_value.val = 0;  // 0 = automatic MMS
      rhs_p_value.val = 0;

      // Geometry: same two spheres as case 6
      num_infc.val = 0;
      num_bdry.val = 2;

      bdry_00_present.val = 1;
      bdry_00_geometry.val = 6;  // First sphere
      bdry_00_opn.val = MLS_INT;
      bdry_00_type.val = DIRICHLET;  // Use exact solution on boundary
      bdry_00_coeff.val = 0;         // Not used for Dirichlet
      bdry_00_coeff_mult.val = 0;

      bdry_01_present.val = 1;
      bdry_01_geometry.val = 7;  // Second sphere
      bdry_01_opn.val = MLS_INT;
      bdry_01_type.val = DIRICHLET;  // Use exact solution on boundary
      bdry_01_coeff.val = 0;         // Not used for Dirichlet
      bdry_01_coeff_mult.val = 0;
      ...
      dt.val = 0.01;                // Time step size
      t_final.val = 2.5;             // Final time
      time_method.val = 0;           // 0=BE, 1=CN, 2=BDF2, 3=Theta
      theta_value.val = 0.5;         // For theta method (ignored for BE/CN/BDF2)
      save_timestep_every.val = 10;  // Save VTK every 10 steps

      // Initial time (set in main)
      current_time.val = 0.0;

      break;

    case 37: // HEAT: Time-varying diffusion coefficient
      solve_heat_equation.val = true;

      // Solution: same as case 36
      sol_m.val = 0; sol_m_mult.val = 1.0;   // Inside domain
      sol_p.val = 0; sol_p_mult.val = 0.0;   // Outside

      // Time-varying diffusion: μ(t) = 1 + 0.5*sin(2πt)
      diff_coeff_m.val = 11;  // Case 11 is time-varying
      diff_coeff_m_mult.val = 1.0;
      diff_coeff_p.val = 11;
      diff_coeff_p_mult.val = 1.0;

      // Add small reaction term for testing
      linear_term_m_coeff.val = 1;  // Constant k
      linear_term_m_mult.val = 0.1; // k = 0.1
      linear_term_p_coeff.val = 1;
      linear_term_p_mult.val = 0.1;
      ...
      // RHS computed from MMS (automatically handles time-varying μ)
      rhs_m_value.val = 0;  // 0 = automatic MMS
      rhs_p_value.val = 0;

      // Geometry: same as case 36
      num_infc.val = 0;
      num_bdry.val = 2;

      bdry_00_present.val = 1;
      bdry_00_geometry.val = 6;
      bdry_00_opn.val = MLS_INT;
      bdry_00_type.val = DIRICHLET;
      bdry_00_coeff.val = 0;
      bdry_00_coeff_mult.val = 0;

      bdry_01_present.val = 1;
      bdry_01_geometry.val = 7;
      bdry_01_opn.val = MLS_INT;
      bdry_01_type.val = DIRICHLET;
      bdry_01_coeff.val = 0;
      bdry_01_coeff_mult.val = 0;
      ...
      // Time stepping parameters (can use different settings)
      dt.val = 0.005;               // Smaller timestep for time-varying coeffs
      t_final.val = 2.0;             // Longer simulation
      time_method.val = 1;           // Crank-Nicolson for better accuracy
      theta_value.val = 0.5;
      save_timestep_every.val = 20;

      current_time.val = 0.0;

      break;

    case 38: // HEAT: Space-time varying coefficients (optional)
      solve_heat_equation.val = true;

      sol_m.val = 0; sol_m_mult.val = 1.0;
      sol_p.val = 0; sol_p_mult.val = 0.0;

      // Space-time varying: μ(x,y,t) = 1 + 0.3*sin(x)*cos(y)*cos(t)
      diff_coeff_m.val = 12;  // Case 12 is space-time varying
      diff_coeff_m_mult.val = 1.0;
      diff_coeff_p.val = 12;
      diff_coeff_p_mult.val = 1.0;

      // Time-dependent reaction: k(t) = e^{-t}
      linear_term_m_coeff.val = 3;  // Time-decaying
      linear_term_m_mult.val = 0.2;
      linear_term_p_coeff.val = 3;
      linear_term_p_mult.val = 0.2;
    ...
    default:
      throw std::invalid_argument("Invalid example number. Use 6 (Poisson) or 36-38 (Heat)");
  }
}
...
public:
  diff_coeff_cf_t(cf_value_type_t what, int &n, double &mag, double *t_ptr = nullptr)
    : what(what), n(&n), mag(&mag), t_ptr(t_ptr) {}

  double operator()(DIM(double x, double y, double z)) const {
    double t = (t_ptr && solve_heat_equation.val) ? *t_ptr : 0.0;
    switch (*n) {
      case 0: switch (what) {
          case VAL: return (*mag);
          case DDX: return 0.;
      ...
      // NEW TIME-DEPENDENT CASES for heat equation
      case 11: // Time-varying constant: μ(t) = 1 + 0.5*sin(ωt)
      {
        double omega = 2.0 * PI;
        double mu_t = (*mag) * (1.0 + 0.5*sin(omega*t));
        switch (what) {
          case VAL: return mu_t;
          case DDX: return 0.;
          case DDY: return 0.;
#ifdef P4_TO_P8
          case DDZ: return 0.;
      ...
      case 12: // Space-time varying: μ(x,y,t) = 1 + 0.3*sin(x)*cos(y)*cos(t)
      {
        double mu_val = (*mag) * (1.0 + 0.3*sin(x)*cos(y)*cos(t));
        switch (what) {
          case VAL: return mu_val;
          case DDX: return (*mag) * 0.3*cos(x)*cos(y)*cos(t);
          case DDY: return (*mag) * (-0.3)*sin(x)*sin(y)*cos(t);
#ifdef P4_TO_P8
          case DDZ: return 0.;
#endif
        }
      ...
      case 13: // Decaying diffusion: μ(x,y,z,t) = 1 + 0.5*exp(-t)*cos(x+y)
      {
        double decay = exp(-t);
        double spatial = cos(x + y);
        switch (what) {
          case VAL: return (*mag) * (1.0 + 0.5*decay*spatial);
          case DDX: return (*mag) * (-0.5)*decay*sin(x + y);
          case DDY: return (*mag) * (-0.5)*decay*sin(x + y);
#ifdef P4_TO_P8
          case DDZ: return 0.;
#endif
        }
      ...
      case 14: // Oscillating anisotropic (2D/3D compatible)
      {
        XCODE( double X = (x-xmin.val)/(xmax.val-xmin.val) );
        YCODE( double Y = (y-ymin.val)/(ymax.val-ymin.val) );
        ZCODE( double Z = (z-zmin.val)/(zmax.val-zmin.val) );

        double osc = 1.0 + 0.3*sin(2.0*PI*t);
#ifdef P4_TO_P8
        double mu_val = (*mag) * osc * (1.0 + 0.2*sin(2*PI*X)*cos(2*PI*Y)*sin(2*PI*Z));
#else
        double mu_val = (*mag) * osc * (1.0 + 0.2*sin(2*PI*X)*cos(2*PI*Y));
#endif

        switch (what) {
          case VAL: return mu_val;
          case DDX: return (*mag) * osc * 0.2 * 2*PI/(xmax.val-xmin.val) *
#ifdef P4_TO_P8
                          cos(2*PI*X)*cos(2*PI*Y)*sin(2*PI*Z);
#else
                          cos(2*PI*X)*cos(2*PI*Y);
#endif
          case DDY: return (*mag) * osc * 0.2 * (-2*PI/(ymax.val-ymin.val)) *
#ifdef P4_TO_P8
...
#endif
#ifdef P4_TO_P8
          case DDZ: return (*mag) * osc * 0.2 * 2*PI/(zmax.val-zmin.val) *
...
// Update all diffusion coefficient instantiations to include time pointer
diff_coeff_cf_t mu_m_cf(VAL, diff_coeff_m.val, diff_coeff_m_mult.val, &current_time.val);
diff_coeff_cf_t mu_p_cf(VAL, diff_coeff_p.val, diff_coeff_p_mult.val, &current_time.val);

diff_coeff_cf_t DIM(mux_m_cf(DDX, diff_coeff_m.val, diff_coeff_m_mult.val, &current_time.val),
                    muy_m_cf(DDY, diff_coeff_m.val, diff_coeff_m_mult.val, &current_time.val),
                    muz_m_cf(DDZ, diff_coeff_m.val, diff_coeff_m_mult.val, &current_time.val));

diff_coeff_cf_t DIM(mux_p_cf(DDX, diff_coeff_p.val, diff_coeff_p_mult.val, &current_time.val),
                    muy_p_cf(DDY, diff_coeff_p.val, diff_coeff_p_mult.val, &current_time.val),
                    muz_p_cf(DDZ, diff_coeff_p.val, diff_coeff_p_mult.val, &current_time.val));
...
public:
  ...
  double *mag;
  cf_value_type_t what;
  double *t_ptr;  // ADD: time pointer for heat equation

  // Updated constructor with optional time pointer
  sol_cf_t(cf_value_type_t what, int &n, double &mag, double *t_ptr = nullptr)
    : what(what), n(&n), mag(&mag), t_ptr(t_ptr) {}

  double operator()(DIM(double x, double y, double z)) const
  {
    // Get current time (0 if Poisson or time not set)
    double t = (t_ptr && solve_heat_equation.val) ? *t_ptr : 0.0;

    switch (*n) {
      case 0: // sin(x)cos(y) - works for both Poisson and heat
      {
        if (solve_heat_equation.val && t_ptr) {
          // Heat equation: u(x,y,t) = sin(x)cos(y)e^{-2μt}
          double mu = diff_coeff_m_mult.val;  // Assumes constant μ for this MMS
          double decay = exp(-2.0 * mu * t);
          double base = sin(x) * cos(y);

          switch (what) {
            case VAL: return (*mag) * base * decay;
            case DDX: return (*mag) * cos(x) * cos(y) * decay;
            case DDY: return (*mag) * (-sin(x)) * sin(y) * decay;
#ifdef P4_TO_P8
            case DDZ: return 0.0;  // 2D solution extended to 3D
#endif
            case LAP: return (*mag) * (-2.0) * base * decay;
            case DDT: return (*mag) * base * (-2.0 * mu) * decay;  // Time derivative
          }
        } else {
          // Poisson equation: u(x,y) = sin(x)cos(y) (no time dependence)
          switch (what) {
            ...
            case DDZ: return 0.0;
#endif
            case LAP: return (*mag) * (-2.0) * sin(x) * cos(y);
            case DDT: return 0.0;  // No time derivative for Poisson
          }
        }
      }

      case 3: // For Poisson case 6 - keep unchanged
      {
        switch (what) {
#ifdef P4_TO_P8
          ...
          case LAP: return (*mag)*( 2.3/(x+0.5*y-0.3*z+3. + pow(x-0.7*y-0.9*z, 2.))
                -0.5*( pow(1.+2.*(x-0.7*y-0.9*z),2.) + pow(0.5-1.4*(x-0.7*y-0.9*z),2.)
                     + pow(-0.3-1.8*(x-0.7*y-0.9*z),2.) )/pow((x+0.5*y-0.3*z+3.
                     + pow(x-0.7*y-0.9*z, 2.)), 2.) );
#else
          ...
          case LAP: {
              ...
              return (*mag)*( 1.64/C - ( pow(2.0*(x+0.8*y)+1.0, 2.0)
                     + pow(1.6*(x+0.8*y)-0.7, 2.0) )/2.0/C/C );
            }
#endif
          case DDT: return 0.0;  // No time derivative for steady solution
        }
      }

      // ADD: More time-dependent solutions for heat equation testing
      case 10: // Polynomial in time: u = x*y*(1-t)
      {
        if (!solve_heat_equation.val) return 0.0;

        switch (what) {
          case VAL: return (*mag) * x * y * (1.0 - t);
          case DDX: return (*mag) * y * (1.0 - t);
          case DDY: return (*mag) * x * (1.0 - t);
#ifdef P4_TO_P8
          case DDZ: return 0.0;
#endif
          case LAP: return 0.0;  // Harmonic in space
          case DDT: return (*mag) * x * y * (-1.0);
        }
      }

      case 11: // Oscillating: u = cos(x+y)*sin(ωt)
      {
        if (!solve_heat_equation.val) return 0.0;

        double omega = PI;
        switch (what) {
          case VAL: return (*mag) * cos(x+y) * sin(omega*t);
          case DDX: return (*mag) * (-sin(x+y)) * sin(omega*t);
          case DDY: return (*mag) * (-sin(x+y)) * sin(omega*t);
#ifdef P4_TO_P8
          case DDZ: return 0.0;
#endif
          case LAP: return (*mag) * (-2.0*cos(x+y)) * sin(omega*t);
          case DDT: return (*mag) * cos(x+y) * omega * cos(omega*t);
        }
      }
...
// Keep existing but add time pointer
sol_cf_t u_m_cf(VAL, sol_m.val, sol_m_mult.val, &current_time.val);
sol_cf_t u_p_cf(VAL, sol_p.val, sol_p_mult.val, &current_time.val);

sol_cf_t ul_m_cf(LAP, sol_m.val, sol_m_mult.val, &current_time.val);
sol_cf_t ul_p_cf(LAP, sol_p.val, sol_p_mult.val, &current_time.val);

sol_cf_t DIM(ux_m_cf(DDX, sol_m.val, sol_m_mult.val, &current_time.val),
             uy_m_cf(DDY, sol_m.val, sol_m_mult.val, &current_time.val),
             uz_m_cf(DDZ, sol_m.val, sol_m_mult.val, &current_time.val));

sol_cf_t DIM(ux_p_cf(DDX, sol_p.val, sol_p_mult.val, &current_time.val),
             uy_p_cf(DDY, sol_p.val, sol_p_mult.val, &current_time.val),
             uz_p_cf(DDZ, sol_p.val, sol_p_mult.val, &current_time.val));

// ADD: Time derivative instantiations for heat equation
sol_cf_t ut_m_cf(DDT, sol_m.val, sol_m_mult.val, &current_time.val);
sol_cf_t ut_p_cf(DDT, sol_p.val, sol_p_mult.val, &current_time.val);

// DIAGONAL TERMS (Reaction term k)
...
public:
  // Updated constructor with optional time pointer
  linear_term_cf_t(int &n, double &mag, double *t_ptr = nullptr)
    : n(&n), mag(&mag), t_ptr(t_ptr) {}

  double operator()(DIM(double x, double y, double z)) const {
    double t = (t_ptr && solve_heat_equation.val) ? *t_ptr : 0.0;

    switch (*n) {
      case 0: return 0.;  // No reaction term
      case 1: return (*mag)*1.;  // Constant reaction
      case 2:  // Spatially varying (existing)
#ifdef P4_TO_P8
      ...
      // ADD: Time-dependent cases for heat equation
      case 3: // Time-decaying reaction: k(t) = e^{-t}
        return (*mag)*exp(-t);

      case 4: // Oscillating reaction: k(t) = 1 + 0.5*sin(2πt)
        return (*mag)*(1.0 + 0.5*sin(2.0*PI*t));

      case 5: // Space-time reaction: k(x,y,t) = sin(x)*cos(y)*cos(t)
#ifdef P4_TO_P8
        return (*mag)*sin(x)*cos(y)*cos(z)*cos(t);
#else
      return (*mag)*sin(x)*cos(y)*cos(t);
#endif
...
// Add time pointer to existing instantiations
linear_term_cf_t linear_term_m_coeff_cf(linear_term_m_coeff.val, linear_term_m_mult.val, &current_time.val);
linear_term_cf_t linear_term_p_coeff_cf(linear_term_p_coeff.val, linear_term_p_mult.val, &current_time.val);
// NONLINEAR TERMS
...
nonlinear_term_cf_t nonlinear_term_m_cf(VAL, nonlinear_term_m.val);
nonlinear_term_cf_t nonlinear_term_m_prime_cf(DDX, nonlinear_term_m.val);
nonlinear_term_cf_t nonlinear_term_p_cf(VAL, nonlinear_term_p.val);
nonlinear_term_cf_t nonlinear_term_p_prime_cf(DDX, nonlinear_term_p.val);

// Coefficient for nonlinear term - make time-aware
class nonlinear_term_coeff_cf_t: public CF_DIM
{
  int *n;
  double *mag;
  double *t_ptr;  // ADD: time pointer

public:
  // Updated constructor with optional time pointer
  nonlinear_term_coeff_cf_t(int &n, double &mag, double *t_ptr = nullptr)
    : n(&n), mag(&mag), t_ptr(t_ptr) {}
  ...
  double operator()(DIM(double x, double y, double z)) const {
    ...
    switch (*n) {
      case 0: return (*mag)*1.;  // Constant coefficient

      case 1:  // Spatially varying (existing)
#ifdef P4_TO_P8
        return (*mag)*cos(x+z)*exp(y);
#else
        return (*mag)*cos(x)*exp(y);
      ...
      // ADD: Time-dependent cases for heat equation
      case 2:  // Time-decaying coefficient: e^{-t}
        return (*mag)*exp(-t);

      case 3:  // Oscillating coefficient: 1 + 0.5*sin(2πt)
        return (*mag)*(1.0 + 0.5*sin(2.0*PI*t));

      case 4:  // Space-time varying
#ifdef P4_TO_P8
        return (*mag)*cos(x)*sin(y)*cos(z)*exp(-0.5*t);
#else
        return (*mag)*cos(x)*sin(y)*exp(-0.5*t);
#endif
...
// Update instantiations with time pointer
nonlinear_term_coeff_cf_t nonlinear_term_m_coeff_cf(nonlinear_term_m_coeff.val,
                                                     nonlinear_term_m_mult.val,
                                                     &current_time.val);
nonlinear_term_coeff_cf_t nonlinear_term_p_coeff_cf(nonlinear_term_p_coeff.val,
                                                     nonlinear_term_p_mult.val,
                                                     &current_time.val);

// MMS RHS for HEAT and POISSON
class rhs_m_cf_t: public CF_DIM
{
  double *t_ptr;  // ADD: time pointer

public:
  rhs_m_cf_t(double *t_ptr = nullptr) : t_ptr(t_ptr) {}  // ADD: constructor

  double operator()(DIM(double x, double y, double z)) const {
    // HEAT (time-dependent) branch
    if (solve_heat_equation.val) {
      // Get solution and its derivatives (all time-aware now)
      const double u   = u_m_cf(DIM(x,y,z));
      const double u_t = ut_m_cf(DIM(x,y,z));  // Use actual DDT from sol_cf_t!
      const double lap = ul_m_cf(DIM(x,y,z));
      const double ux  = ux_m_cf(DIM(x,y,z));
      const double uy  = uy_m_cf(DIM(x,y,z));
#ifdef P4_TO_P8
      const double uz  = uz_m_cf(DIM(x,y,z));
#endif

      // Get coefficients
      const double mu   = mu_m_cf(DIM(x,y,z));
      const double mux  = mux_m_cf(DIM(x,y,z));
      const double muy  = muy_m_cf(DIM(x,y,z));
#ifdef P4_TO_P8
      const double muz  = muz_m_cf(DIM(x,y,z));
#endif
      const double k    = linear_term_m_coeff_cf(DIM(x,y,z));

      // MMS: f = u_t - ∇·(μ∇u) + ku
      //     = u_t - μΔu - ∇μ·∇u + ku
      return u_t - mu * lap - SUMD(mux*ux, muy*uy, muz*uz) + k * u;
    }

    // POISSON (steady) branch - unchanged
    if (rhs_m_value.val == 1) return 0.0;
...
public:
  rhs_p_cf_t(double *t_ptr = nullptr) : t_ptr(t_ptr) {}  // ADD: constructor

  double operator()(DIM(double x, double y, double z)) const {
    // HEAT (time-dependent) branch
    if (solve_heat_equation.val) {
      const double u   = u_p_cf(DIM(x,y,z));
      const double u_t = ut_p_cf(DIM(x,y,z));  // Use actual DDT!
      const double lap = ul_p_cf(DIM(x,y,z));
      const double ux  = ux_p_cf(DIM(x,y,z));
      const double uy  = uy_p_cf(DIM(x,y,z));
#ifdef P4_TO_P8
      const double uz  = uz_p_cf(DIM(x,y,z));
#endif

      const double mu   = mu_p_cf(DIM(x,y,z));
      const double mux  = mux_p_cf(DIM(x,y,z));
      const double muy  = muy_p_cf(DIM(x,y,z));
#ifdef P4_TO_P8
      const double muz  = muz_p_cf(DIM(x,y,z));
#endif
      const double k    = linear_term_p_coeff_cf(DIM(x,y,z));
    ...
    // POISSON (steady) branch - unchanged
    if (rhs_p_value.val == 1) return 0.0;
...
rhs_m_cf_t rhs_m_cf(&current_time.val);
rhs_p_cf_t rhs_p_cf(&current_time.val);
...
class bc_coeff_cf_t: public CF_DIM
{
  int    *n;
  double *mag;
  double *t_ptr;  // ADD: time pointer for time-dependent BCs

public:
  // Updated constructor with optional time pointer
  bc_coeff_cf_t(int &n, double &mag, double *t_ptr = nullptr)
    : n(&n), mag(&mag), t_ptr(t_ptr) {}
  ...
  double operator()(DIM(double x, double y, double z)) const {
    ...
    switch (*n) {
      case 0: return (*mag);  // Constant
      case 1: return (*mag)*(x+y);  // Spatially varying
      ...
      // ADD: Time-dependent cases for heat equation
      case 8: // Oscillating Robin coefficient: α(t) = 1 + 0.5*sin(2πt)
        return (*mag)*(1.0 + 0.5*sin(2.0*PI*t));

      case 9: // Decaying coefficient: α(t) = e^{-t}
        return (*mag)*exp(-t);

      case 10: // Space-time varying: α(x,y,t) = (1 + sin(x)*cos(y))*cos(t)
#ifdef P4_TO_P8
        return (*mag)*(1.0 + sin(x)*cos(y)*sin(z))*cos(t);
#else
        return (*mag)*(1.0 + sin(x)*cos(y))*cos(t);
#endif
...
// Update instantiations with time pointer
bc_coeff_cf_t bc_coeff_cf_all[] = {
    bc_coeff_cf_t(bdry_00_coeff.val, bdry_00_coeff_mult.val, &current_time.val),
    bc_coeff_cf_t(bdry_01_coeff.val, bdry_01_coeff_mult.val, &current_time.val),
    bc_coeff_cf_t(bdry_02_coeff.val, bdry_02_coeff_mult.val, &current_time.val),
    bc_coeff_cf_t(bdry_03_coeff.val, bdry_03_coeff_mult.val, &current_time.val)
};
...
public:
  ...
  cf_value_type_t what;
//  MultiCircleShape* shape_generator;
  bdry_phi_cf_t(cf_value_type_t what, int &n) : what(what), n(&n) {}
//
//  // New constructor to accept shape generator
//  bdry_phi_cf_t(cf_value_type_t what, int &n, MultiCircleShape* generator)
//    : what(what), n(&n), shape_generator(generator) {}
...
class bc_value_robin_t : public CF_DIM
{
  ...
  CF_DIM DIM(*phi_x_cf_, *phi_y_cf_, *phi_z_cf_);
  CF_DIM *bc_coeff_cf_;

public:
  bc_value_robin_t(BoundaryConditionType *bc_type,
                   CF_DIM *bc_coeff_cf,
                   DIM(CF_DIM *phi_x_cf, CF_DIM *phi_y_cf, CF_DIM *phi_z_cf))
    : bc_type_(bc_type), bc_coeff_cf_(bc_coeff_cf),
      DIM(phi_x_cf_(phi_x_cf), phi_y_cf_(phi_y_cf), phi_z_cf_(phi_z_cf)) {}

  double operator()(DIM(double x, double y, double z)) const
  {
    switch (*bc_type_) {
      case DIRICHLET:
        // REMOVE the hardcoded hack - use exact solution for MMS
          return u_cf(DIM(x,y,z));  // Time-dependent exact solution value
      ...
      case ROBIN: {
        ...
        return mu_cf(DIM(x,y,z))*SUMD(nx*ux_cf(DIM(x,y,z)),
                                      ny*uy_cf(DIM(x,y,z)),
                                      nz*uz_cf(DIM(x,y,z)))
               + (*bc_coeff_cf_)(DIM(x,y,z))*u_cf(DIM(x,y,z));
      }
    }
  }
};
...
// Heat VTK snapshot with diagnostic fields
static void write_heat_vtk_snapshot(p4est_t* p4est,
                                    p4est_nodes_t* nodes,
                                    p4est_ghost_t* ghost,
                                    const std::string& out_dir,
                                    int file_idx,
                                    int timestep,        // ADD: timestep number
                                    double time_val,     // ADD: current time value
                                    Vec u,               // full heat solution
                                    Vec mask,            // signed mask (<0 inside)
                                    Vec bdry_phi_eff,    // boundary level set
                                    CF_DIM &u_cf)        // exact solution function
{
  PetscErrorCode ierr;

  // Sample exact solution at current time
  Vec u_exact;
  ierr = VecCreateGhostNodes(p4est, nodes, &u_exact); CHKERRXX(ierr);

  // ensure exact is sampled at 'time_val'
  double saved_t = current_time.val;
  current_time.val = time_val;
  sample_cf_on_nodes(p4est, nodes, u_cf, u_exact);
  current_time.val = saved_t;


  // Create split solutions and error
  Vec u_m, u_p, error;
  ierr = VecDuplicate(u, &u_m); CHKERRXX(ierr);
  ierr = VecDuplicate(u, &u_p); CHKERRXX(ierr);
  ierr = VecDuplicate(u, &error); CHKERRXX(ierr);

  // Split solution by domain and compute error
  double *u_arr, *um_arr, *up_arr, *m_arr, *exact_arr, *err_arr;
  PetscInt nloc;
  VecGetLocalSize(u, &nloc);

  VecGetArray(u, &u_arr);
  VecGetArray(u_m, &um_arr);
  VecGetArray(u_p, &up_arr);
  VecGetArray(mask, &m_arr);
  VecGetArray(u_exact, &exact_arr);
  VecGetArray(error, &err_arr);

  for (PetscInt i = 0; i < nloc; ++i) {
    if (m_arr[i] < 0.0) { // inside domain
      um_arr[i] = u_arr[i];
      up_arr[i] = 0.0;
      err_arr[i] = fabs(u_arr[i] - exact_arr[i]);
    } else {              // outside domain
      um_arr[i] = 0.0;
      up_arr[i] = u_arr[i];  // Should be 0, but keep for debugging
      err_arr[i] = 0.0;      // Don't compute error outside
    }
  }

  VecRestoreArray(u, &u_arr);
  VecRestoreArray(u_m, &um_arr);
  VecRestoreArray(u_p, &up_arr);
  VecRestoreArray(mask, &m_arr);
  VecRestoreArray(u_exact, &exact_arr);
  VecRestoreArray(error, &err_arr);

  // Get arrays for VTK output
  double *u_ptr, *u_m_ptr, *u_p_ptr, *mask_ptr, *phi_ptr, *u_exact_ptr, *error_ptr;
  VecGetArray(u,            &u_ptr);
  VecGetArray(u_m,          &u_m_ptr);
  VecGetArray(u_p,          &u_p_ptr);
  VecGetArray(mask,         &mask_ptr);
  VecGetArray(bdry_phi_eff, &phi_ptr);
  VecGetArray(u_exact,      &u_exact_ptr);
  VecGetArray(error,        &error_ptr);

  // Create filename with timestep
  std::ostringstream oss;
  oss << out_dir << "/vtu/heat_"
      << std::setw(4) << std::setfill('0') << file_idx
      << "_t" << std::setw(6) << std::setfill('0') << timestep;

  // Create a time array for visualization
  double *time_arr = new double[nloc];
  for (PetscInt i = 0; i < nloc; ++i) {
    time_arr[i] = time_val;
  }
  ...
  my_p4est_vtk_write_all(p4est, nodes, ghost,
                         ...
                         8, 0, oss.str().c_str(),
                         VTK_NODE_SCALAR, "u",         u_ptr,        // heat solution
                         VTK_NODE_SCALAR, "u_m",       u_m_ptr,      // inside only
                         VTK_NODE_SCALAR, "u_p",       u_p_ptr,      // outside only
                         VTK_NODE_SCALAR, "u_exact",   u_exact_ptr,  // exact solution
                         VTK_NODE_SCALAR, "error",     error_ptr,    // |u - exact|
                         VTK_NODE_SCALAR, "mask",      mask_ptr,     // domain mask
                         VTK_NODE_SCALAR, "bdry_phi",  phi_ptr,      // level set
                         VTK_NODE_SCALAR, "time",      time_arr);    // current time

  // Compute and print max error for diagnostics
  double max_error_inside = 0.0;
  double max_val_outside = 0.0;
  VecGetArray(mask, &m_arr);
  VecGetArray(error, &err_arr);
  VecGetArray(u, &u_arr);
  ...
  for (PetscInt i = 0; i < nloc; ++i) {
    if (m_arr[i] < 0.0) {
      max_error_inside = fmax(max_error_inside, err_arr[i]);
    } else {
      max_val_outside = fmax(max_val_outside, fabs(u_arr[i]));
    }
  }
  ...
  PetscPrintf(PETSC_COMM_WORLD,
              "Heat VTK t=%6.4f: %s (max err=%.3e, max outside=%.3e)\n",
              time_val, oss.str().c_str(), max_error_inside, max_val_outside);

  // Cleanup
  VecRestoreArray(u,            &u_ptr);
  VecRestoreArray(u_m,          &u_m_ptr);
  VecRestoreArray(u_p,          &u_p_ptr);
  VecRestoreArray(mask,         &mask_ptr);
  VecRestoreArray(bdry_phi_eff, &phi_ptr);
  VecRestoreArray(u_exact,      &u_exact_ptr);
  VecRestoreArray(error,        &error_ptr);

  delete[] time_arr;

  ierr = VecDestroy(u_m); CHKERRXX(ierr);
  ierr = VecDestroy(u_p); CHKERRXX(ierr);
  ierr = VecDestroy(u_exact); CHKERRXX(ierr);
  ierr = VecDestroy(error); CHKERRXX(ierr);
}


// Modified refresh callback that captures BC objects
static void bind_default_heat_refresh(
    my_p4est_heat_solver_mls_t& solver,
    p4est_t* p4est,
    p4est_nodes_t* nodes,
    Vec mu_m,  // Pass in pre-allocated
    Vec mu_p,  // Pass in pre-allocated
    double* time_ptr,
    bc_wall_type_t& bc_wall_type,  // ADD: Pass BC type reference
    u_cf_t& u_cf)                  // ADD: Pass BC value reference
{
  solver.refresh_coeffs_at_time =
      [=, &bc_wall_type, &u_cf](double t_op, double t_rhs,
          my_p4est_poisson_nodes_mls_t& ps,
          Vec k_m_orig, Vec k_p_orig,
          Vec f_m_orig, Vec f_p_orig)
      {
        // Save current time
        double saved_time = time_ptr ? *time_ptr : 0.0;

        // Sample μ at operator time
        if (time_ptr) *time_ptr = t_op;
        sample_cf_on_nodes(p4est, nodes, mu_m_cf, mu_m);
        sample_cf_on_nodes(p4est, nodes, mu_p_cf, mu_p);
        ps.set_mu(mu_m, mu_p);

        // Sample k at operator time
        sample_cf_on_nodes(p4est, nodes, linear_term_m_coeff_cf, k_m_orig);
        sample_cf_on_nodes(p4est, nodes, linear_term_p_coeff_cf, k_p_orig);

        // UPDATE: Refresh wall BCs at operator time
        ps.set_wc(bc_wall_type, u_cf);

        // Sample f at RHS time
        if (time_ptr) *time_ptr = t_rhs;
        sample_cf_on_nodes(p4est, nodes, rhs_m_cf, f_m_orig);
        sample_cf_on_nodes(p4est, nodes, rhs_p_cf, f_p_orig);

        // Restore original time
        if (time_ptr) *time_ptr = saved_time;
      };
}
...
int main (int argc, char* argv[])
{
  ...
  if (save_matrix_ascii.val || save_matrix_binary.val)
  {
    ...
    // After set_example(example.val);
    if (mpi.rank() == 0) {
      printf("Running example number: %d (%s)\n",
             example.val,
             solve_heat_equation.val ? "HEAT EQUATION" : "POISSON EQUATION");
    }
      ...
      for (int sub_iter = 0; sub_iter < num_sub_iter; ++sub_iter)
      {
        ...
        for (int k_shift = 0; k_shift < num_shifts_z_dir(); ++k_shift)
        {
          ...
          for (int j_shift = 0; j_shift < num_shifts_y_dir(); ++j_shift)
          {
            ...
            // Initialize vectors needed for problem setup
            Vec rhs_m, rhs_p, mu_m, mu_p, linear_term_m_coeff, linear_term_p_coeff;
            ...
            // Create the heat solver wrapper (handles both steady and time-dependent)
            my_p4est_heat_solver_mls_t solver(&ngbd_n, p4est, nodes, &current_time.val);
            ...
            // Sample diffusion coefficients at t=0 (for both Poisson and Heat)
            current_time.val = 0.0;
            sample_cf_on_nodes(p4est, nodes, mu_m_cf, mu_m);
            sample_cf_on_nodes(p4est, nodes, mu_p_cf, mu_p);
            solver.set_mu(mu_m, NULL, mu_p, NULL);

            // Set wall boundary conditions
            solver.set_wc(bc_wall_type, u_cf);

// Add boundaries - different handling for heat vs Poisson
for (int i = 0; i < num_bdry_max; ++i) {
  if (*bdry_present_all[i]) {
    if (solve_heat_equation.val) {
      // For heat equation: ALWAYS use continuous BCs (not pointwise)
      // This allows BCs to be evaluated dynamically at current_time.val
      solver.add_boundary((mls_opn_t)*bdry_opn_all[i], bdry_phi_vec_all[i],
                        NULL,  // phi_x array is NULL
                        (BoundaryConditionType)*bc_type_all[i],
                        bc_value_cf_all[i], bc_coeff_cf_all[i]);
    } else {
      // For Poisson equation: use pointwise or continuous as specified
...
// Add interfaces - similar logic
for (int i = 0; i < num_infc_max; ++i) {
  if (*infc_present_all[i]) {
    if (solve_heat_equation.val) {
      // For heat equation: use continuous interface conditions
      solver.add_interface((mls_opn_t)*infc_opn_all[i], infc_phi_vec_all[i],
                         NULL,
                         jc_value_cf_all[i], jc_flux_cf_all[i]);
    } else {
      // For Poisson equation: use pointwise or continuous as specified
      if (apply_bc_pointwise())
        solver.add_interface((mls_opn_t)*infc_opn_all[i], infc_phi_vec_all[i],
...
// ALWAYS preassemble (needed for solver initialization)
// But only set pointwise BCs for Poisson
if (apply_bc_pointwise()) {
  solver.preassemble_linear_system();

  // Only do pointwise BC sampling for Poisson (not heat)
  if (!solve_heat_equation.val) {
    ...
    // Allocate memory for interface conditions
    ...
    // Pass sampled values to solver
    for (int i = 0; i < num_bdry(); ++i) {
      solver.set_bc(i, (BoundaryConditionType) *bc_type_all[i],
                    pw_bc_values[i], pw_bc_values_robin[i], pw_bc_coeffs_robin[i]);
    }

    for (int i = 0; i < num_infc(); ++i) {
      solver.set_jc(i, pw_jc_sol_jump_taylor[i],
                    pw_jc_flx_jump_taylor[i], pw_jc_flx_jump_integr[i]);
    }
  }
}

// Set up refresh callback for heat equation
if (solve_heat_equation.val) {
    // Simplified callback - no BC updates needed since we're using continuous BCs
    solver.refresh_coeffs_at_time =
        [&, p4est, nodes](double t_op, double t_rhs,
            my_p4est_poisson_nodes_mls_t& ps,
            Vec k_m_orig, Vec k_p_orig,
            Vec f_m_orig, Vec f_p_orig)
        {
            // Save current time
            double saved_time = current_time.val;

            // Sample μ at operator time
            current_time.val = t_op;
            sample_cf_on_nodes(p4est, nodes, mu_m_cf, mu_m);
            sample_cf_on_nodes(p4est, nodes, mu_p_cf, mu_p);
            ps.set_mu(mu_m, mu_p);
            ...
            // Sample f at RHS time
            current_time.val = t_rhs;
            sample_cf_on_nodes(p4est, nodes, rhs_m_cf, f_m_orig);
            sample_cf_on_nodes(p4est, nodes, rhs_p_cf, f_p_orig);

            // Restore time
            current_time.val = saved_time;

            // Note: BCs are handled automatically via bc_value_cf_all[i] functions
            // which read current_time.val when the solver evaluates them
        };
}
            ...
            // =============================================================================
            // EQUATION-SPECIFIC SETUP AND SOLVE
            // =============================================================================

            if (solve_heat_equation.val) {
              // ===== HEAT EQUATION SETUP =====
              ierr = PetscPrintf(mpi.comm(), "\n=== HEAT EQUATION (Example %d) ===\n", example.val);
              ierr = PetscPrintf(mpi.comm(), "Time method: %s, dt=%f, t_final=%f\n",
                                time_method.val==0 ? "Backward Euler" :
                                time_method.val==1 ? "Crank-Nicolson" :
                                time_method.val==2 ? "BDF2" :
                                time_method.val==3 ? "Theta" : "Unknown",
                                dt.val, t_final.val); CHKERRXX(ierr);

              // Sample RHS and coefficients at t=0 (for MMS they may depend on time)
              current_time.val = 0.0;
              ...
              solver.set_linear_coefficients(linear_term_m_coeff, linear_term_p_coeff);
              solver.set_source_terms(rhs_m, rhs_p);

              // Set initial condition from exact solution at t=0
              Vec u0;
              ierr = VecCreateGhostNodes(p4est, nodes, &u0); CHKERRXX(ierr);
              sample_cf_on_nodes(p4est, nodes, u_cf, u0);
              solver.set_initial_condition(u0);

              // Verify initial condition
              double norm_u0;
              VecNorm(u0, NORM_2, &norm_u0);
              ierr = PetscPrintf(mpi.comm(), "Initial condition L2 norm: %e\n", norm_u0);

              VecDestroy(u0);

              // Configure time stepping method
              my_p4est_heat_solver_mls_t::TimeSteppingMethod method;
              switch (time_method.val) {
                case 0: method = my_p4est_heat_solver_mls_t::BACKWARD_EULER; break;
                case 1: method = my_p4est_heat_solver_mls_t::CRANK_NICOLSON; break;
                case 2: method = my_p4est_heat_solver_mls_t::BDF2; break;
                case 3: method = my_p4est_heat_solver_mls_t::THETA_METHOD; break;
                default: method = my_p4est_heat_solver_mls_t::BACKWARD_EULER; break;
              }
              solver.set_time_stepping_method(method, theta_value.val);
              solver.set_time_step(dt.val);

              // Handle nonlinear terms if present
              if (nonlinear_term_m.val != 0 || nonlinear_term_p.val != 0) {
                Vec nonlinear_term_m_coeff_sampled, nonlinear_term_p_coeff_sampled;
              ...
              if (save_vtk.val && timestep % save_timestep_every.val == 0) {
                write_heat_vtk_snapshot(p4est, nodes, ghost, out_dir, file_idx,
                                       timestep, current_time.val,
                                       solver.get_solution(),
                                       solver.get_mask(),
                                       solver.get_boundary_phi_eff(),
                                       u_cf);
              }

              // Main time-stepping loop
              while (current_time.val < t_final.val - 0.5*dt.val) {
                timestep++;

                // Advance one time step
                solver.advance_one_time_step();
                current_time.val = solver.get_current_time();

                // Print progress periodically
                if (timestep % 100 == 0) {
                  ierr = PetscPrintf(mpi.comm(), "Time step %d: t = %f / %f\n",
                                    timestep, current_time.val, t_final.val); CHKERRXX(ierr);
                }
              ...
              // Get final solution for error analysis
              VecCopy(solver.get_solution(), sol);

              ierr = PetscPrintf(mpi.comm(), "Heat equation evolution completed: final time = %f\n",
                                current_time.val); CHKERRXX(ierr);

            } else {
              // ===== POISSON EQUATION SETUP =====
              ierr = PetscPrintf(mpi.comm(), "\n=== POISSON EQUATION (Example %d) ===\n", example.val);

              // Sample RHS and coefficients (time=0 but doesn't matter for Poisson)
              current_time.val = 0.0;
              sample_cf_on_nodes(p4est, nodes, rhs_m_cf, rhs_m);
              sample_cf_on_nodes(p4est, nodes, rhs_p_cf, rhs_p);
              sample_cf_on_nodes(p4est, nodes, linear_term_m_coeff_cf, linear_term_m_coeff);
              ...
              // Set initial guess if requested
              if (use_nonzero_guess.val) {
              ...
              if (nonlinear_term_m.val == 0 && nonlinear_term_p.val == 0) {
                // Linear problem
                solver.solve_steady_state(sol, use_nonzero_guess.val);
              } else {
                // Nonlinear problem
                Vec nonlinear_term_m_coeff_sampled, nonlinear_term_p_coeff_sampled;
                ierr = VecDuplicate(linear_term_m_coeff, &nonlinear_term_m_coeff_sampled); CHKERRXX(ierr);
                ...
                solver.set_nonlinear_term(nonlinear_term_m_coeff_sampled, nonlinear_term_m_cf, nonlinear_term_m_prime_cf,
                                        nonlinear_term_p_coeff_sampled, nonlinear_term_p_cf, nonlinear_term_p_prime_cf);
                solver.set_solve_nonlinear_parameters(nonlinear_method.val, nonlinear_itmax.val, nonlinear_tol.val, 0);
                solver.solve_steady_state_nonlinear(sol, use_nonzero_guess.val);

                ierr = VecDestroy(nonlinear_term_m_coeff_sampled); CHKERRXX(ierr);
                ierr = VecDestroy(nonlinear_term_p_coeff_sampled); CHKERRXX(ierr);
              }

              ierr = PetscPrintf(mpi.comm(), "Poisson equation solved.\n"); CHKERRXX(ierr);
            }
            ...
            // Get solver data for error analysis
...
//            }
            //*/
              ...
              // error for heat equation solve
              if (solve_heat_equation.val) {
                // ===== HEAT EQUATION ERROR ANALYSIS =====
                ierr = PetscPrintf(mpi.comm(), "\n=== Heat Equation Error Analysis at t=%f ===\n", current_time.val);

                // 1. Solution error: ||u_h - u_exact||
                Vec u_exact_final;
                ierr = VecCreateGhostNodes(p4est, nodes, &u_exact_final); CHKERRXX(ierr);
                current_time.val = t_final.val;  // Ensure we're at final time
                sample_cf_on_nodes(p4est, nodes, u_cf, u_exact_final);

                VecAXPY(u_exact_final, -1.0, sol);  // u_exact - u_h
                double l2_error, linf_error;
                VecNorm(u_exact_final, NORM_2, &l2_error);
                VecNorm(u_exact_final, NORM_INFINITY, &linf_error);

                ierr = PetscPrintf(mpi.comm(), "Solution errors: L2=%e, Linf=%e\n", l2_error, linf_error);

                // 2. Check PDE residual: ||u_t - μΔu - ku - f||
                // This verifies our solution satisfies the PDE
                Vec residual;
                ierr = VecCreateGhostNodes(p4est, nodes, &residual); CHKERRXX(ierr);

                // Compute u_t numerically if you have previous timestep
                // Or use exact u_t for MMS verification

                // 3. Boundary error check
                double boundary_error = 0.0;
                // Sample solution at boundary points and compare with BC

                // 4. Conservation check (if applicable)
                double mass_initial = 0.0, mass_final = 0.0;
                // Compute integral of solution at t=0 and t=final

                VecDestroy(u_exact_final);
                VecDestroy(residual);
              }
