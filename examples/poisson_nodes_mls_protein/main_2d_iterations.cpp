...
param_t<int> NB_ITERATIONS(pl, 10, "NB_ITERATIONS", "Number of projections");
param_t<int> NB_RANDOM_SPLITS(pl, 30, "NB_RANDOM_SPLITS", "Number of random splits");
param_t<std::string> DIV_TYPE(pl, "maxime", "DIV_TYPE", "Type of derivative"); // "gibou" or "maxime
...
param_t<double> xmax(pl, PI, "xmax", "Box xmax");
param_t<double> ymax(pl, PI, "ymax", "Box ymax");
param_t<double> zmax(pl, PI, "zmax", "Box zmax");
...
param_t<bool> refine_rand(pl, true, "refine_rand", "Add randomness into adaptive grid"); //change these?
param_t<bool> enforce_graded_grid(pl, false, "enforce_graded_grid", "Enforce 1:2 ratio for adaptive grid"); //change these? was true before
// Refine random:
...
int main(int argc, char *argv[]) {
  ...
  int grid_refinement_iter = 0;     // the number of grid refinement is 0.
  ierr = PetscPrintf(mpi.comm(), "Level %2d / %2d.\n", lmin() + grid_refinement_iter, lmax() + grid_refinement_iter); CHKERRXX(ierr);

  double dxyz[3] = {(grid_xyz_max[0] - grid_xyz_min[0]) / pow(2., (double) lmax() + grid_refinement_iter),
                    (grid_xyz_max[1] - grid_xyz_min[1]) / pow(2., (double) lmax() + grid_refinement_iter),
                    (grid_xyz_max[2] - grid_xyz_min[2]) / pow(2., (double) lmax() + grid_refinement_iter)};
  ...
  lvl_arr.push_back(lmax() + grid_refinement_iter);

  ierr = PetscPrintf(mpi.comm(), "Level %2d / %2d. Sub split %2d (lvl %5.2f / %5.2f).\n", lmin() + grid_refinement_iter,
                     lmax() + grid_refinement_iter, 0, lmin() + grid_refinement_iter, lmax() + grid_refinement_iter);
  CHKERRXX(ierr);
  ...
  if (refine_strict()) {
     ...
     for (int i = 0; i < grid_refinement_iter; ++i) {
        my_p4est_refine(p4est, P4EST_FALSE, refine_every_cell, nullptr);
        my_p4est_partition(p4est, P4EST_FALSE, nullptr);
     }
  } else {
     splitting_criteria_cf_t data_tmp(lmin() + grid_refinement_iter, lmax() + grid_refinement_iter, &phi_eff_cf, lip(), band());
     p4est->user_pointer = (void *) (&data_tmp);

//     for (int i = 0; i < lmax() + grid_refinement_iter; ++i) {
...
//  splitting_criteria_cf_t data(lmin() + grid_refinement_iter, lmax() + grid_refinement_iter, &phi_eff_cf, lip(), band());
    splitting_criteria_random_t data(lmin(), lmax(), 100,10000);
  ...
  if (refine_rand()) {
     if (mpi.rank() == 0) cout << "Refining the grid randomly\n";
      ...
      if (mpi.rank() == 0) cout << "Grid refined randomly\n";
  }
  ...
  double diag = sqrt(SUMD(dxyz[0] * dxyz[0], dxyz[1] * dxyz[1], dxyz[2] * dxyz[2])); //should this be our diag scaling?
  ...
  for (int i = 0; i < num_bdry_max; ++i)
    ...
    if (mpi.rank() == 0) cout << "Grid initialized!\n";
...
for (int projection_iter = 0; projection_iter < NB_ITERATIONS(); projection_iter++) {

    if (mpi.rank() == 0) cout << "beginning projection " << projection_iter << "..." << endl;
/***********************************************************************************
*
...
***********************************************************************************/
    ...
    if (mpi.rank() == 0) cout << "before div calc" << endl;
    // Compute -div(U*)
    foreach_local_node(n, nodes) {
        ...
        if (phi_eff_cf(x, y) < 0) {
            ...
            ptr_minus_div_Ustar_m[n] = -(qnnn.dx_central_stability(ptr_ustar_m,DIV_TYPE()) + qnnn.dy_central_stability(ptr_vstar_m,DIV_TYPE()));
            ptr_minus_div_Ustar_p[n] = 20000000000000;
        } else {
            ptr_minus_div_Ustar_m[n] = 200000000000000;
            ptr_minus_div_Ustar_p[n] = -(qnnn.dx_central_stability(ptr_ustar_p,DIV_TYPE()) + qnnn.dy_central_stability(ptr_vstar_p,DIV_TYPE()));
        }
    }
    ...
    // To readout RHS
//    if (mpi.rank() == 0) cout << "RHS: " << endl;
//    for (int nout = 0; nout<255;nout++){
//        cout << ptr_minus_div_Ustar_m[nout] << endl;
//    }


 //   ierr = VecView(vec_minus_div_Ustar_m, 	PETSC_VIEWER_STDOUT_WORLD);
    ...
    my_p4est_vtk_write_all(p4est, nodes, ghost,
                           ...
                           0, 1, oss1.str().c_str(),
                           VTK_CELL_DATA, "minus_div_u_star_m",vec_minus_div_Ustar_m);

    solver.set_rhs(vec_minus_div_Ustar_m, vec_minus_div_Ustar_p);
    if (mpi.rank() == 0) cout << "rhs set!" << endl;
    ...
    // adding this to see if it rescales
    ...
    if (mpi.rank() == 0) cout << "hodge calculated!" << endl;
    ...
    ierr = VecGetArray(vec_hodge, &ptr_hodge);
    CHKERRXX(ierr);
    ierr = VecGhostUpdateBegin(vec_hodge, INSERT_VALUES, SCATTER_FORWARD);
    CHKERRXX(ierr);
    ierr = VecGhostUpdateEnd(vec_hodge, INSERT_VALUES, SCATTER_FORWARD);
    CHKERRXX(ierr);
...
//         if (!is_node_Wall(p4est, ni) && qnnn.is_stencil_in_negative_domain(mask_m_ptr)) {
    ...
    if (mpi.rank() == 0) cout << "projection " << projection_iter << " complete!" << endl;
    // For testing the stable projection algorithm, we set Ustar = Unp1:

    if(use_exact_solution = false) {
        ierr = VecGetArray(vec_ustar_m, &ptr_ustar_m); CHKERRXX(ierr);
        ierr = VecGetArray(vec_ustar_p, &ptr_ustar_p); CHKERRXX(ierr);
        ierr = VecGetArray(vec_vstar_m, &ptr_vstar_m); CHKERRXX(ierr);
        ierr = VecGetArray(vec_vstar_p, &ptr_vstar_p); CHKERRXX(ierr);
        foreach_local_node(n, nodes) {
            ptr_ustar_m[n] = ptr_unp1_m[n];
            ptr_vstar_m[n] = ptr_vnp1_m[n];
            ptr_ustar_p[n] = ptr_unp1_p[n];
            ptr_vstar_p[n] = ptr_vnp1_p[n];
        }
    }
    ...
    if (mpi.rank() == 0) cout << "Max unp1 error: "<< err_unp1_m_max << endl;


}

if (mpi.rank() == 0) cout << "All projections complete!" << endl;
    if (mpi.rank() == 0) cout << " " << endl;
    if (mpi.rank() == 0) cout << " " << endl;
...
Vec mask_m = solver.get_mask_m(); double *mask_m_ptr;
Vec mask_p = solver.get_mask_p(); double *mask_p_ptr;
ierr = VecCreateGhostNodes(p4est, nodes, &mask_m); CHKERRXX(ierr);
ierr = VecCreateGhostNodes(p4est, nodes, &mask_p); CHKERRXX(ierr);
ierr = VecGetArray(mask_m, &mask_m_ptr);
CHKERRXX(ierr);
ierr = VecGetArray(mask_p, &mask_p_ptr);
CHKERRXX(ierr);
  ...
  mpiret = MPI_Allreduce(MPI_IN_PLACE, &err_unp1_p_max, 1, MPI_DOUBLE, MPI_MAX, p4est->mpicomm); SC_CHECK_MPI(mpiret);
  mpiret = MPI_Allreduce(MPI_IN_PLACE, &err_vnp1_p_max, 1, MPI_DOUBLE, MPI_MAX, p4est->mpicomm); SC_CHECK_MPI(mpiret);
  if (mpi.rank() == 0) cout << "Velo errors caluclated" << endl;
  // Restoring vectors:
  ...
  ierr = VecDestroy(vec_vnp1_p); CHKERRXX(ierr);
    ...
    if (mpi.rank() == 0) cout << "Max unp1 error: "<< err_unp1_m_max << endl;
    if (mpi.rank() == 0) cout << "Max div(ustar) error: "<< err_dustar_m_max << endl;
  ...
  if (mpi.rank() == 0) cout << "Hodge errors caluclated" << endl;
...
ierr = VecGhostUpdateBegin(mask_m, INSERT_VALUES, SCATTER_FORWARD); CHKERRXX(ierr);
ierr = VecGhostUpdateBegin(mask_p, INSERT_VALUES, SCATTER_FORWARD); CHKERRXX(ierr);
  ierr = VecGhostUpdateEnd(mask_m, INSERT_VALUES, SCATTER_FORWARD); CHKERRXX(ierr);
  ierr = VecGhostUpdateEnd(mask_p, INSERT_VALUES, SCATTER_FORWARD); CHKERRXX(ierr);
  ...
  if (mpi.rank() == 0) cout << "Grad hodge max initiated" << endl;
  quad_neighbor_nodes_of_node_t qnnn{}; //NMB

ierr = VecGetArray(mask_m, &mask_m_ptr); CHKERRXX(ierr);
ierr = VecGetArray(mask_p, &mask_p_ptr); CHKERRXX(ierr);
  if (mpi.rank() == 0) cout << "Neighborhood called successfully" << endl;
  foreach_local_node(n, nodes) {
     double xyz[P4EST_DIM];
     node_xyz_fr_n(n, p4est, nodes, xyz);
    ...
    if (mpi.rank() == 0) cout << "Grad hodge errors caluclated" << endl;
  ...
  if (grid_refinement_iter > -1) {
     ierr = PetscPrintf(p4est->mpicomm, "Errors Neg: "); CHKERRXX(ierr);
     ierr = PetscPrintf(p4est->mpicomm, "hodge = %3.2e (%+3.2f), ", err_hodge_m_max, log(error_hodge_m_arr[grid_refinement_iter - 1] / error_hodge_m_arr[grid_refinement_iter]) / log(2)); CHKERRXX(ierr);
     ierr = PetscPrintf(p4est->mpicomm, "|grad(hodge)| = %3.2e (%+3.2f), ", err_grad_hodge_m_max, log(error_gradient_hodge_m_arr[grid_refinement_iter - 1] / error_gradient_hodge_m_arr[grid_refinement_iter]) / log(2)); CHKERRXX(ierr);
     ierr = PetscPrintf(p4est->mpicomm, "unp1 = %3.2e (%+3.2f), ", err_unp1_m_max, log(error_unp1_m_arr[grid_refinement_iter - 1] / error_unp1_m_arr[grid_refinement_iter]) / log(2)); CHKERRXX(ierr);
     ierr = PetscPrintf(p4est->mpicomm, "vnp1 = %3.2e (%+3.2f), ", err_vnp1_m_max, log(error_vnp1_m_arr[grid_refinement_iter - 1] / error_vnp1_m_arr[grid_refinement_iter]) / log(2)); CHKERRXX(ierr);
     ...
     ierr = PetscPrintf(p4est->mpicomm, "hodge = %3.2e (%+3.2f), ", err_hodge_p_max, log(error_hodge_p_arr[grid_refinement_iter - 1] / error_hodge_p_arr[grid_refinement_iter]) / log(2)); CHKERRXX(ierr);
     ierr = PetscPrintf(p4est->mpicomm, "|grad(hodge)| = %3.2e (%+3.2f), ", err_grad_hodge_p_max, log(error_gradient_hodge_p_arr[grid_refinement_iter - 1] / error_gradient_hodge_p_arr[grid_refinement_iter]) / log(2)); CHKERRXX(ierr);
     ierr = PetscPrintf(p4est->mpicomm, "unp1 = %3.2e (%+3.2f), ", err_unp1_p_max, log(error_unp1_p_arr[grid_refinement_iter - 1] / error_unp1_p_arr[grid_refinement_iter]) / log(2)); CHKERRXX(ierr);
     ierr = PetscPrintf(p4est->mpicomm, "vnp1 = %3.2e (%+3.2f), ", err_vnp1_p_max, log(error_vnp1_p_arr[grid_refinement_iter - 1] / error_vnp1_p_arr[grid_refinement_iter]) / log(2)); CHKERRXX(ierr);
  ...
  if (save_vtk()) {
     ...
     my_p4est_vtk_write_all(p4est, nodes, ghost,
                            ...
                            VTK_POINT_DATA, "minus_div_u_star_m",ptr_minus_div_Ustar_m,
                            VTK_POINT_DATA, "minus_div_u_star_p",ptr_minus_div_Ustar_p,
