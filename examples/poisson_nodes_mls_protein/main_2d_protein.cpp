...
param_t<int>    example (pl, 6, "example", "Predefined example:\n"
...
void set_example(int example)
{
  switch (example)
  {
    ...
    case 6: // union of spheres example from (Bochkov&Gibou, JCP, 2019)
      ...
      bdry_01_present.val = 1; bdry_01_geometry.val = 7; bdry_01_opn.val = MLS_INT; bdry_01_coeff.val = 3; bdry_01_coeff_mult.val = 1; bdry_01_type.val = ROBIN;
      bdry_02_present.val = 0; bdry_02_geometry.val = 0; bdry_02_opn.val = MLS_INT; bdry_02_coeff.val = 0; bdry_02_coeff_mult.val = 1; bdry_02_type.val = ROBIN;
      bdry_03_present.val = 0; bdry_03_geometry.val = 0; bdry_03_opn.val = MLS_INT; bdry_03_coeff.val = 0; bdry_03_coeff_mult.val = 1; bdry_03_type.val = ROBIN;
    ...
    case 31: // shperical interface
      ...
      // Added by Faranak based on Case 6 (union of spheres example from (Bochkov&Gibou, JCP, 2019))
      // Simplified Protein aggregation. Based on the https://doi.org/10.1016/j.cocis.2014.10.002:
      // Protein particle sizing: 1 nm–2 μm, add new compound domain
      case 32:
          /*
             * Equations:
             *
             ...
             *  int* n; (n = 3)
             *  double* mag; (mag = 1)
             *  (∣mag∣) · 0.5 · log( (x + 0.8 · y)² + (x − 0.7 · y) + 4.0 )
             *
             * Negative domain:
             *   -∇(μ⁻ ∇u⁻) + k⁻ * u⁻ = g⁻
             * Where:
             ...
             * Notes:
             * - μ⁺ and μ⁻ represent the coefficients for the respective domains.
             * - k⁺ and k⁻ are the reaction coefficients.
             * - g⁺ and g⁻ are the source terms for the positive and negative domains.
             * Robin BC on the two compound doamins
          */

          // using the exact solution as the one in union of two disks for outside the domain(n =3, mag = 1). for inside:
          //      case 0: switch (what) {
          //      #else // 2d
          //              case VAL: return  (*mag)*sin(x)*cos(y);
          // Setting the sol_m_mult.val = 0 --> mag = 0, u = 0 inside
          sol_m.val = 3; sol_m_mult.val = 1; diff_coeff_m.val = 0; diff_coeff_m_mult.val = 1; linear_term_m_coeff.val = 0; linear_term_m_mult.val = 1;
          sol_p.val = 0; sol_p_mult.val = 0; diff_coeff_p.val = 0; diff_coeff_p_mult.val = 1; linear_term_p_coeff.val = 0; linear_term_p_mult.val = 1;

          // No jump no interface
          num_infc.val = 0;
          num_bdry.val = 2;
          ...
          bdry_00_present.val = 1; bdry_00_geometry.val = 17; bdry_00_opn.val = MLS_INT; bdry_00_coeff.val = 0; bdry_00_coeff_mult.val = 2.5; bdry_00_type.val = ROBIN;
          bdry_01_present.val = 1; bdry_01_geometry.val = 18; bdry_01_opn.val = MLS_ADD; bdry_01_coeff.val = 0; bdry_01_coeff_mult.val = 5.0; bdry_01_type.val = ROBIN;
          bdry_02_present.val = 0; bdry_02_geometry.val = 0; bdry_02_opn.val = MLS_INT; bdry_02_coeff.val = 0; bdry_02_coeff_mult.val = 1; bdry_02_type.val = ROBIN;
          bdry_03_present.val = 0; bdry_03_geometry.val = 0; bdry_03_opn.val = MLS_INT; bdry_03_coeff.val = 0; bdry_03_coeff_mult.val = 1; bdry_03_type.val = ROBIN;
      ...
      case 33:
          ...
          infc_00_present.val = 1; infc_00_geometry.val = 19; infc_00_opn.val = MLS_INT;
          infc_01_present.val = 0; infc_01_geometry.val = 0; infc_01_opn.val = MLS_INT;
          infc_02_present.val = 0; infc_02_geometry.val = 0; infc_02_opn.val = MLS_INT;
          infc_03_present.val = 0; infc_03_geometry.val = 0; infc_03_opn.val = MLS_INT;
      ...
      case 34:
          // Solution and diffusion coefficients
          sol_m.val = 3; sol_m_mult.val = 1; diff_coeff_m.val = 0; diff_coeff_m_mult.val = 1; linear_term_m_coeff.val = 0; linear_term_m_mult.val = 1;
          sol_p.val = 0; sol_p_mult.val = 0; diff_coeff_p.val = 0; diff_coeff_p_mult.val = 1; linear_term_p_coeff.val = 0; linear_term_p_mult.val = 1;
          ...
          bdry_00_present.val = 1; bdry_00_geometry.val = 19; bdry_00_opn.val = MLS_INT; bdry_00_coeff.val = 0; bdry_00_coeff_mult.val = 1; bdry_00_type.val = ROBIN;
          bdry_01_present.val = 1; bdry_01_geometry.val = 20; bdry_01_opn.val = MLS_INT; bdry_01_coeff.val = 3; bdry_01_coeff_mult.val = 1; bdry_01_type.val = ROBIN;
          bdry_02_present.val = 0; bdry_02_geometry.val = 0; bdry_02_opn.val = MLS_INT; bdry_02_coeff.val = 0; bdry_02_coeff_mult.val = 1; bdry_02_type.val = ROBIN;
          bdry_03_present.val = 0; bdry_03_geometry.val = 0; bdry_03_opn.val = MLS_INT; bdry_03_coeff.val = 0; bdry_03_coeff_mult.val = 1; bdry_03_type.val = ROBIN;
          break;
...
int main (int argc, char* argv[])
{
  ...
  if (save_matrix_ascii.val || save_matrix_binary.val)
  {
    ...
    if (mpi.rank() == 0) {  // Only print on rank 0
        printf("Running example number: %d\n", example.val);
    }
