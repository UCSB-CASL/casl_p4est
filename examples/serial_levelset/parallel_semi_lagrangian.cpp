...
double SemiLagrangian::advect(const CF_2 &vx, const CF_2 &vy, std::vector<double>& phi){
  std::cout << "Init variables in advect" << endl;
  ...
  // Hold xy & rank & node index for non-local points to be sent via MPI_SEND/MPI_RECEIVE
  ...
  for (p4est_locidx_t ni = 0; ni < nodes_->num_owned_indeps; ++ni){ //Loop through all nodes of a single processor
    ...
    if (rank_found == 1) std::cout << "Rank = 1" << endl;
  ...
  std::cout << "About to send non-local points to other processors" << endl;
  ...
  std::cout << "About to do work on local and non local points" << endl;
  ...
  //for (int rank_it = 0; rank_it < xy_map.size(); ++rank_it){    //Need to change this to use a map iterator that points to map.begin().
  for (recv_it rank_it = xy_map.begin(); rank_it != xy_map.end(); ++rank_it){
    vector<double> phi_temp (rank_it->second.size());   //Used to hold one vector of phi values. Will copy to phi_send map after computing phi. phi_temp size is the number of elements in the vector
    for (int vec_it = 0; vec_it < rank_it->second.size(); ++vec_it){
      ...
      if (rank_found != p4est_->mpirank){
        int abc;
        std::cout << "ERROR, SEND DATA TO WRONG PROCESSOR" << endl;
        std::cin >> abc;
      }
      else
  ...
  std::cout << "About to send non-local points back to owners" << endl;
  ...
  //Clearing old vectors used for MPI
  receivers.clear();        //Processors that are receiving data from us. vector<int>
  senders.clear();          //Processors that are sending data to us.     vector<int>

  senders.resize(p4est_->mpisize);    //Init size to all processors
  num_senders = 0;          //Total number of processors we're sending data to.

  //Init the receivers buffer with the ranks of the processors from the send_buffer_map map.
  for (phi_send_it it = phi_send.begin(); it != phi_send.end(); ++it){
    receivers.push_back(it->first);    //it->first holds the first value of the pair (the rank).
  }

  //Populate senders and num_senders from the number of receivers.
  sc_notify(&receivers[0], receivers.size(), &senders[0], &num_senders, p4est_->mpicomm);
  senders.resize(num_senders);    //Resize to exact number of processors to be sent data

  //MPI_SEND to send data out to other processors from us.
  for (int it = 0; it < receivers.size(); ++it){
    std::vector<double> &phi_buf = phi_send[receivers[it]];    //Init send buffer from our map
    int phi_buf_size = phi_buf.size();    //Init the buffer size

    MPI_Send(&phi_buf_size, 1, MPI_INT,receivers[it], SIZE_TAG, p4est_->mpicomm);    //Send the buffer size
    MPI_Send(&phi_buf[0], phi_buf_size, MPI_DOUBLE, receivers[it], PHI_TAG, p4est_->mpicomm);    //Send the data to all designated processors
  }

  std::vector<double> phi_np2(0); //Used to hold temp values received from MPI. Will insert to phi after receiving data.

  //MPI_RECV to receive data from all other processors to us.
  for (int it = 0; it < senders.size(); ++it){
    int phi_buf_size;    //Buffer size that we will receive
    MPI_Recv(&phi_buf_size, 1, MPI_INT, senders[it], SIZE_TAG, p4est_->mpicomm, MPI_STATUS_IGNORE);   //Receive the buffer size

    std::vector<double> phi_temp_buf(phi_buf_size);      //Init a temp buffer that we will put received data in.
    MPI_Recv(&phi_temp_buf, phi_buf_size, MPI_DOUBLE, senders[it], PHI_TAG, p4est_->mpicomm, MPI_STATUS_IGNORE);  //Receive the data from processors

    phi_np2.insert(phi_np2.end(), phi_temp_buf.begin(), phi_temp_buf.end());  //Insert received buffers to temp phi_np2.
  }

  //AT THIS POINT
  // We now have two vectors<double> of phi: phi_np1 and phi_np2
  // phi_np1 has all the computed local phi values
  // phi_np2 has all the computed non-local values
  // Now we need to combine these two vectors into one, and then copy them into the old phi.

  phi_np1.insert(phi_np1.end(), phi_np2.begin(), phi_np2.end());    //Append calculated nonlocal phi to local phi values.
  //std::sort(phi_np1.begin(), phi_np1.end());
