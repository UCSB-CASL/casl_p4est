...
const std::string extra_info =	// NOLINT.
	std::string( "This program provides a general setup for superhydrophobic channel flow simulations.\n\n"
				 "It assumes no solid object and no passive scalar (i.e. no smoke) in the channel. If no dimension is set by the user, the \n"
				 "channel is set to be 6 x 2 (x 3) by default. If the number of trees in the streamwise (spanwise) direction, i.e. nx (nz),\n"
				 "is not provided by the user, it is set so that the aspect ratio of the computational cells is as close as possible to 1.\n"
				 "This tool considers periodic boundary conditions in the streamwise (and spanwise) direction(s). \n\n"
				 "Considering the following parameters,\n"
				 "\t delta := half the height of the channel,\n"
				 "\t f_x   := bulk force per unit mass driving the flow in the (positive) x direction,\n"
				 "\t nu    := kinematic viscosity of the fluid (nu = mu/rho),\n"
				 "\t U_b   := mean flow velocity across the channel (U_b = Q/(rho*2*delta(*width)) where Q is the mass flow rate in the channel),\n"
				 "the user can require the simulation to\n"
				 "1) either use a constant bulk force driving the flow by setting the value of Re_tau where Re_tau is defined as \n"
				 "                                    Re_tau = delta*sqrt(delta*f_x)/nu \n"
				 "\t In that case, the Navier-Stokes solver is invoked with the following inputs: \n"
				 "\t\t rho = 1.0;\n"
				 "\t\t f_x = 1.0/delta (so that u_tau = sqrt(f_x*tau) = 1.0 --> in fact, this nondimensionalizes velocities by u_tau);\n"
				 "\t\t mu  = rho*u_tau*delta/Re_tau, to match the desired Re_tau;\n"
				 "2) or enforce a constant mass flow by setting the value of Re_b where Re_b is defined as \n"
				 "                                    Re_b = delta*U_b/nu \n"
				 "\t In that case, the Navier-Stokes solver is invoked with the following inputs: \n"
				 "\t\t rho = 1.0;\n"
				 "\t\t f_x is calculated (and adapted) at every time-step to enforce Q = rho*2*delta(*width)\n"
				 "\t\t\t (so that U_b = 1.0 --> in fact, this nondimensionalizes velocities by U_b);\n"
				 "\t\t\t (f_x initialized to the laminar-case value);\n"
				 "\t\t mu  = rho*U_b*delta/Re_b = delta/Re_b, to match the desired Re_b.\n"
				 "The user can set either Re_tau or Re_b but not both!\n\n"
				 "When restarting from a saved state, the user can change the simulation setup (i.e. desired Re_b or Re_tau) but the fluid\n"
				 "parameters, the dimensions of the domain and macromesh will be identical to the ones loaded from the saved state. Therefore,\n"
				 "\t\t rho = as loaded from the saved state;\n"
				 "\t\t mu  = as loaded from the saved state;\n"
				 "1) when restarting with a value of Re_tau, \n"
				 "\t\t f_x = SQR(mu*Re_tau/(rho*delta))/delta = constant                                     [to match the desired Re_tau];\n"
				 "2) when restarting with a value of Re_b\n"
				 "\t\t f_x is calculated (and adapted) at every time-step to enforce Q = 2.0*mu*Re_b(*width) [to match the desired Re_b].\n"
				 "\t\t [If desired, the initial value of f_x can be set with the input parameter 'initial_Re_tau'.]\n"
				 "Developers: Raphael Egan (raphaelegan@ucsb.edu) with Fernando Temprano-Coleto's help for the analytical solutions.\n" );
...
public:
	...
	double operator()( DIM( double, double, double ) ) const override
	{ return forcing_term; }
	...
	void set_value( const double &new_forcing_term )
...
public:
	mass_flow_controller_t( const char &flow_direction, const double &section_ ) :
		flow_dir( flow_direction ), forcing_is_on( false ), desired_bulk_velocity( -1.0 ), latest_mass_flow( 0.0 ),
		section( section_ )
	{ P4EST_ASSERT( flow_dir < P4EST_DIM ); }
	...
	double update_forcing_and_get_mean_streamwise_velocity_correction( my_p4est_navier_stokes_t *ns,
																	   external_force_per_unit_mass_t *external_acceleration[P4EST_DIM] ) const
	{
		P4EST_ASSERT( forcing_is_on );
		double required_correction_to_hodge_derivative = ns->get_correction_in_hodge_derivative_for_enforcing_mass_flow(
			flow_dir, desired_bulk_velocity, &latest_mass_flow );
		external_acceleration[flow_dir]->update_term(
			-required_correction_to_hodge_derivative * ns->alpha() / ns->get_dt());
	...
	double read_latest_mass_flow() const
	{ return latest_mass_flow; }

	double targeted_bulk_velocity() const
	{
		P4EST_ASSERT( forcing_is_on );
...
{
	double tstart = NAN;	// Away with the warning that we haven't defined them.
	double dt = NAN;
	double tn = NAN;
	...
	int export_vtk, save_data_idx = -1;		// Away with the warning that we didn't initialize save_data_idx.
		...
		nexport_avg( cmd.get<unsigned int>( "nexport_avg", default_nexport_avg ) )
	{
		if( control_hodge == hodge_value )
			throw std::invalid_argument( "simulation_setup::simulation_setup(): this simulation setup does not allow for "
										 "control on hodge value (because of singularities at no-slip to free-slip "
										 "transition regions)." );
		...
		if( des_pc_cell == "hypre" )
			pc_cell = PCHYPRE;
		else if( des_pc_cell == "jacobi'" )
			pc_cell = PCJACOBI;
		else
		{
			if( des_pc_cell != "sor" && mpi.rank() == 0 )
				std::cerr << "The desired preconditioner for the cell-solver was either not allowed or not correctly understood. "
							 "Successive over-relaxation is used instead" << std::endl;
			pc_cell = PCSOR;
		}

		if( des_solver_cell == "cg" )
			cell_solver_type = KSPCG;
		else
		{
			if( des_solver_cell != "bicgstab" && mpi.rank() == 0 )
				std::cerr << "The desired Krylov solver for the cell-solver was either not allowed or not correctly understood. "
							 "BiCGStab is used instead" << std::endl;
			cell_solver_type = KSPBCGS;
		}

		if( des_pc_face == "hypre" )
			pc_face = PCHYPRE;
		else if( des_pc_face == "jacobi'" )
			pc_face = PCJACOBI;
		else
		{
			if( des_pc_face != "sor" && mpi.rank() == 0 )
				std::cerr << "The desired preconditioner for the face-solver was either not allowed or not correctly understood. "
							 "Successive over-relaxation is used instead" << std::endl;
			pc_face = PCSOR;
		}

		if( cmd.contains( "Re_b" ) && cmd.contains( "Re_tau" ) ) // safeguard...
			throw std::invalid_argument( "simulation_setup::simulation_setup(): forcing a constant bulk velocity AND a "
										 "constant pressure gradient cannot be done: you have to choose only one!" );
	...
	int running_save_data_idx() const
	{ return (int)floor( tn / dt_save_data ); }

	void update_save_data_idx()
	{ save_data_idx = running_save_data_idx(); }

	bool time_to_save_state() const
	{ return (save_state && running_save_data_idx() != save_data_idx); }
	...
	int running_export_vtk() const
	{ return ( int ) floor( tn / vtk_dt ); }

	void update_export_vtk()
	{ export_vtk = running_export_vtk(); }

	bool time_to_save_vtk() const
	{ return (save_vtk && running_export_vtk() != export_vtk); }

	double max_tolerated_velocity( const mass_flow_controller_t *controller,
								   external_force_per_unit_mass_t *external_acceleration[P4EST_DIM],
								   const my_p4est_shs_channel_t &channel ) const
	{
		return 5.0 * (flow_condition == constant_mass_flow?
										controller->targeted_bulk_velocity() :
										Reynolds * channel.canonical_u_tau( external_acceleration[0]->get_value() ) );
	}
	...
	void export_and_accumulate_timings( const my_p4est_navier_stokes_t *ns )
	{
		...
		P4EST_ASSERT( timings.find( projection_step ) != timings.end() &&
					  timings.find( viscous_step ) != timings.end() &&
					  timings.find( velocity_interpolation ) != timings.end()); // should *always* find those
	...
	void print_averaged_timings( const mpi_environment_t &mpi ) const
	{
		...
		CHKERRXX( ierr );
		ierr = PetscPrintf( mpi.comm(), " viscosity step: %.5e\n", global_computational_times.at( viscous_step ) / iter );
		CHKERRXX( ierr );
		ierr = PetscPrintf( mpi.comm(), " projection step: %.5e\n", global_computational_times.at( projection_step ) / iter );
		CHKERRXX( ierr );
		ierr = PetscPrintf( mpi.comm(), " computing velocities at nodes: %.5e\n", global_computational_times.at( velocity_interpolation ) / iter );
		CHKERRXX( ierr );
		ierr = PetscPrintf( mpi.comm(), " grid update: %.5e\n", global_computational_times.at( grid_update ) / iter );
		CHKERRXX( ierr );
		ierr = PetscPrintf( mpi.comm(), " full iteration (total): %.5e\n",
							(global_computational_times.at( viscous_step ) +
							 global_computational_times.at( projection_step ) +
							 global_computational_times.at( velocity_interpolation ) +
							 global_computational_times.at( grid_update )) / iter );
		CHKERRXX( ierr );
	}
};
	...
	if( ns->get_mpirank() == 0 )
	{
		if( !file_exists( filename ))
		{
			...
			for( int k = 0; k < ns->get_brick()->nxyztrees[1] * (1 << ns->get_lmax()); ++k )
				fprintf( fp_avg_profile, "  | %.12g", (-1.00 + (2.0 / (( double ) (ns->get_brick()->nxyztrees[1] *
																				   (1 << ns->get_lmax())))) *
															   (0.5 + k)));
...
class velocity_profiler_t
{
	...
	void initialize_averaged_velocity_profiles( const my_p4est_navier_stokes_t *ns, const double &tstart,
												const my_p4est_shs_channel_t &channel )
	{
		...
		ierr = PetscPrintf( ns->get_mpicomm(), "Saving slice-averaged velocity profile in %s\n",
							file_slice_avg_velocity_profile.c_str());
		CHKERRXX( ierr );
...
#else
		const double smallest_traverse_length_scale =
		...
		for( unsigned int bin_idx = 1; bin_idx < nbins_in_ridge; ++bin_idx )
		{
			if( nb_cells_in_ridge % 2 == 0 )
			{
				bin_index[(bin_offset + nb_cells_in_groove + nbins_in_ridge - 1 - bin_idx) % nb_cells_to_map] =
					nbins - 1 - bin_idx;
				bin_index[(bin_offset + nb_cells_in_groove + nbins_in_ridge + bin_idx) % nb_cells_to_map] =
					nbins - 1 - bin_idx;
			}
			else
			{
		...
		{
			file_line_avg_velocity_profile[bin_idx] =
				std::string( profile_path ) + "/line_averaged_velocity_profile_index_" + std::to_string( bin_idx ) +
				".dat";
			ierr = PetscPrintf( ns->get_mpicomm(), "Saving line-averaged velocity profile in %s\n",
								file_line_avg_velocity_profile[bin_idx].c_str());
			CHKERRXX( ierr );
			initialize_velocity_profile_file( file_line_avg_velocity_profile[bin_idx].c_str(), ns, tstart );
		}
		int mpiret = MPI_Barrier( ns->get_mpicomm());
		SC_CHECK_MPI( mpiret );
	}

public:
	velocity_profiler_t( const cmdParser cmd, const my_p4est_navier_stokes_t *ns, const simulation_setup &setup,
						 const my_p4est_shs_channel_t &channel )
	{
		profile_path = setup.export_dir + "/profiles";
		if( create_directory( profile_path, ns->get_mpirank(), ns->get_mpicomm()))
			throw std::runtime_error(
				"velocity_profiler_t::velocity_profiler_t(...): could not create exportation directory for velocity profiles " +
				profile_path );
		...
		if( cmd.contains( "restart" ))
		{
			...
			if( ns->get_mpirank() == 0 )
				all_binary_files_nm1_are_there =
			...
			for( unsigned int bin_idx = 0; bin_idx < nbins; ++bin_idx )
				if(( unsigned int ) ns->get_mpirank() == bin_idx % ns->get_mpisize())
					all_binary_files_nm1_are_there = all_binary_files_nm1_are_there && file_exists(
						profile_path + "/line_velocity_profile_nm1_index_" + std::to_string( bin_idx ) + ".bin" );

			int load_binary_files = (all_binary_files_nm1_are_there? 1 : 0);
			int mpiret = MPI_Allreduce( MPI_IN_PLACE, &load_binary_files, 1, MPI_INT, MPI_LAND, ns->get_mpicomm());
			SC_CHECK_MPI( mpiret );
			...
			if( load_binary_files )
			{
				if( ns->get_mpirank() == 0 )
				{
					read_vector_from_file( profile_path + "/slice_velocity_profile_nm1.bin",
										   slice_averaged_profile_nm1 );
				...
				for( unsigned int bin_idx = 0; bin_idx < nbins; ++bin_idx )
					if(( unsigned int ) ns->get_mpirank() == bin_idx % ns->get_mpisize())
					{
						read_vector_from_file(
							profile_path + "/line_velocity_profile_nm1_index_" + std::to_string( bin_idx ) + ".bin",
							line_averaged_profiles_nm1[bin_idx] );
						time_averaged_line_averaged_profiles[bin_idx].resize(
							line_averaged_profiles_nm1[bin_idx].size());
	...
	void gather_and_dump_profiles( const simulation_setup &setup, my_p4est_navier_stokes_t *ns, const double &u_scaling
								   ONLY3D( COMMA const bool &spanwise ))
	{
		...
		if( ns->get_mpirank() == 0 )
		{
			...
			else
			{
				...
				{
					time_averaged_slice_averaged_profile[idx] +=
						0.5 * setup.dt * (slice_averaged_profile_nm1[idx] + slice_averaged_profile[idx]);
					slice_averaged_profile_nm1[idx] = slice_averaged_profile[idx];
				}
			}

			if( iter_export_profile != 0 &&
				(iter_export_profile % setup.nexport_avg == 0 || setup.time_to_save_state()))
			{
				...
				{
					const std::string path_to_binary_slice_velocity_profile_nm1 =
						profile_path + "/slice_velocity_profile_nm1.bin";
					if( file_exists( path_to_binary_slice_velocity_profile_nm1 ))
						if( remove( path_to_binary_slice_velocity_profile_nm1.c_str()) != 0 )
							throw std::runtime_error(
								"main_shs_:: error when deleting file " + path_to_binary_slice_velocity_profile_nm1 );
					write_vector_to_binary_file( slice_averaged_profile_nm1,
												 path_to_binary_slice_velocity_profile_nm1 );
				...
				if( fp_velocity_profile == NULL )
					throw std::invalid_argument( "main_shs_" + std::to_string( P4EST_DIM ) +
												 "d: could not open file for slice-averaged velocity profile output." );
				for( int k = 0; k < ns->get_brick()->nxyztrees[1] * (1 << ns->get_lmax()); ++k )
				{
					fprintf( fp_velocity_profile, " %.12g",
							 time_averaged_slice_averaged_profile.at( k ) / (setup.tn - t_slice_average));
		...
		ns->get_line_averaged_vnp1_profiles( DIM( dir::x, dir::y, spanwise? dir::z : dir::x ), bin_index,
											 line_averaged_profiles, u_scaling );
		for( unsigned int bin_idx = 0; bin_idx < nbins; ++bin_idx )
		{
			if(( unsigned int ) ns->get_mpirank() == bin_idx % ns->get_mpisize())
			{
				...
				else
				{
					...
					{
						time_averaged_line_averaged_profiles[bin_idx][idx] += 0.5 * setup.dt *
																			  (line_averaged_profiles_nm1[bin_idx][idx] +
																			   line_averaged_profiles[bin_idx][idx]);
					...
					(iter_export_profile % setup.nexport_avg == 0 || (setup.time_to_save_state())))
				{
					...
					if( setup.time_to_save_state())
					{
						const std::string path_to_binary_line_velocity_profile_nm1 =
							profile_path + "/line_velocity_profile_nm1_index_" + std::to_string( bin_idx ) + ".bin";
						if( file_exists( path_to_binary_line_velocity_profile_nm1 ))
							if( remove( path_to_binary_line_velocity_profile_nm1.c_str()) != 0 )
								throw std::runtime_error( "main_shs_:: error when deleting file " +
														  std::string( path_to_binary_line_velocity_profile_nm1 ));
						write_vector_to_binary_file( line_averaged_profiles_nm1[bin_idx],
													 path_to_binary_line_velocity_profile_nm1 );
					...
					if( fp_velocity_profile == NULL )
						throw std::invalid_argument( "main_shs_" + std::to_string( P4EST_DIM ) +
													 "d: could not open file for line-averaged velocity profile output." );
					for( int k = 0; k < ns->get_brick()->nxyztrees[1] * (1 << ns->get_lmax()); ++k )
					{
						fprintf( fp_velocity_profile, " %.12g", time_averaged_line_averaged_profiles[bin_idx].at( k ) /
																(setup.tn - t_line_average[bin_idx]));
...
void initialize_drag_force_output( simulation_setup &setup, const my_p4est_navier_stokes_t *ns )
{
	...
	if( ns->get_mpirank() == 0 )
	{
		if( !file_exists( setup.file_drag ))
		{
			...
			fprintf( fp_drag, "%s",
					 (std::string( "%% tn | x-component | y-component" ) ONLY3D( +std::string( " | z-component" )) +
					  std::string( "\n" )).c_str());
			fclose( fp_drag );
		}
		else
...
void load_solver_from_state( const mpi_environment_t &mpi, const cmdParser &cmd,
							 ...
							 external_force_per_unit_mass_t *external_acceleration[P4EST_DIM],
							 splitting_criteria_cf_and_uniform_band_t *&data,
							 mass_flow_controller_t *&controller, simulation_setup& setup )
{
	...
	if( ORD( cmd.contains( "nx" ), cmd.contains( "ny" ), cmd.contains( "nz" ) ) ||
		ORD( cmd.contains( "length" ), cmd.contains( "height" ), cmd.contains( "width" ) ) )
		throw std::invalid_argument( "load_solver_from_state: the length, height and width as well as the numbers of "
									 "trees along x, y and z cannot be reset when restarting a simulation." );

	delete ns;		// Deleting a nullptr does nothing.
	...
	for( unsigned char dir = 0; dir < P4EST_DIM; ++dir )
		if( is_periodic( p4est_n, dir ) != periodicity[dir] || is_periodic( p4est_nm1, dir ) != periodicity[dir] )
			throw std::invalid_argument(
				"load_solver_from_state: the periodicity from the loaded state does not match the requirements." );
	...
	if( setup.flow_condition == constant_mass_flow )
	{
		...
		if( cmd.contains( "initial_Re_tau" ))
			external_acceleration[dir::x]->set_value( channel.acceleration_for_canonical_u_tau(
				ns->get_nu() * cmd.get<double>( "initial_Re_tau" ) / channel.delta()));
		else
			external_acceleration[dir::x]->set_value(
				channel.acceleration_for_constant_mass_flow( desired_U_b, setup.Reynolds, setup.nterms_in_series ));
		P4EST_ASSERT( fabs( channel.Re_b(
			ns->get_rho() * MULTD( controller->targeted_bulk_velocity(), channel.height(), channel.width()),
			ns->get_rho(), ns->get_nu()) - setup.Reynolds ) < setup.Reynolds * 10.0 * EPS );
	}
	else
	{
		...
		P4EST_ASSERT(
			fabs( channel.canonical_Re_tau( external_acceleration[0]->get_value(), ns->get_nu()) - setup.Reynolds ) <
			setup.Reynolds * 10.0 * EPS );
	}
	...
	data = new splitting_criteria_cf_and_uniform_band_t(
		cmd.get<int>( "lmin", (( splitting_criteria_t * ) p4est_n->user_pointer)->min_lvl ), channel.lmax(), &channel,
		uniform_band, lip );
	...
	ns->set_parameters( ns->get_mu(), ns->get_rho(), cmd.get<int>( "sl_order", ns->get_sl_order()), uniform_band,
						cmd.get<double>( "thresh", ns->get_split_threshold()), cmd.get<double>( "cfl", ns->get_cfl()));
	...
	PetscErrorCode ierr = PetscPrintf( ns->get_mpicomm(), "Simulation restarted from state saved in %s\n",
									   (cmd.get<std::string>( "restart" )).c_str());
	CHKERRXX( ierr );
}
...
p4est_connectivity_t *build_brick_and_get_connectivity( my_p4est_brick_t *&brick, const cmdParser& cmd )
{
	...
	n_tree_xyz[0] = cmd.get<int>( "nx", (int)(default_ntree_y * length / height) );
	xyz_min[0] = -0.5 * length;
	xyz_max[0] = 0.5 * length;
	n_tree_xyz[1] = cmd.get<int>( "ny", (int)height );
	xyz_min[1] = -0.5 * height;
	xyz_max[1] = 0.5 * height;
#ifdef P4_TO_P8
	n_tree_xyz[2] = cmd.get<int>( "nz", (int)(default_ntree_y*width/height) );
	xyz_min[2] = -0.5 * width;
	xyz_max[2] = 0.5 * width;
#endif
...
void create_solver_from_scratch( const mpi_environment_t& mpi, const cmdParser& cmd, my_p4est_navier_stokes_t *&ns,
								 my_p4est_brick_t *&brick, my_p4est_shs_channel_t& channel,
								 external_force_per_unit_mass_t *external_acceleration[P4EST_DIM],
								 splitting_criteria_cf_and_uniform_band_t *&data, mass_flow_controller_t *&controller,
								 simulation_setup& setup )
{
	...
	channel.configure( brick, DIM( cmd.get<double>( "pitch", default_pitch_to_height * (brick->xyz_max[1] - brick->xyz_min[1]) ),
								   cmd.get<double>( "GF", default_gas_fraction ),
								   cmd.contains( "spanwise" ) ), (signed char)cmd.get<int>( "lmax", default_lmax ) );
	...
	channel.create_p4est_ghost_and_nodes( p4est_nm1, ghost_nm1, nodes_nm1, data, connectivity, mpi,
										  cmd.get<int>( "lmin", default_lmin ),
										  cmd.get<unsigned int>( "wall_layer", default_wall_layer ),
										  cmd.get<double>( "lip", default_lip ) );
	...
	const double tree_dim[P4EST_DIM] = {
		DIM( channel.length() / brick->nxyztrees[0], channel.height() / brick->nxyztrees[1], channel.width() / brick->nxyztrees[2] )
	};
	...
	Vec phi;
	PetscErrorCode ierr = VecCreateGhostNodes( p4est_n, nodes_n, &phi );
	CHKERRXX( ierr );
	...
	delete controller;		// Nothing happens if it's nullptr.
	controller = new mass_flow_controller_t( dir::x, brick->xyz_min[dir::x] );
	for( unsigned char dir = 0; dir < P4EST_DIM; ++dir )
	{
	...
	if( setup.flow_condition == constant_pressure_gradient )
	{
		...
		external_acceleration[dir::x]->set_value( channel.acceleration_for_canonical_u_tau( desired_u_tau ) );
		ns->set_parameters( viscosity, mass_density, cmd.get<int>( "sl_order", default_sl_order ), data->uniform_band,
							cmd.get<double>( "thresh", default_thresh ), cmd.get<double>( "cfl", default_cfl ) );
	...
	else
	{
		...
		ns->set_parameters( viscosity, mass_density, cmd.get<int>( "sl_order", default_sl_order ), data->uniform_band,
							cmd.get<double>( "thresh", default_thresh ), cmd.get<double>( "cfl", default_cfl ) );
		P4EST_ASSERT( fabs( channel.Re_b( ns->get_rho() * MULTD( controller->targeted_bulk_velocity(), channel.height(), channel.width() ),
										  ns->get_rho(), ns->get_nu() ) - setup.Reynolds ) < setup.Reynolds * 10.0 * EPS );
	}
...
check_accuracy_of_solution( my_p4est_navier_stokes_t *ns, my_p4est_shs_channel_t &channel, simulation_setup &setup )
{
	...
	for( unsigned char dir = 0; dir < P4EST_DIM; ++dir )
	{
		ierr = VecGetArrayRead( v_faces[dir], &v_faces_ptr );
		CHKERRXX( ierr );
		...
		ierr = VecRestoreArrayRead( v_faces[dir], &v_faces_ptr );
		CHKERRXX( ierr );
	}
	...
	for( unsigned char dir = 0; dir < P4EST_DIM; ++dir )
	{
		ierr = VecGetArrayRead( v_nodes[dir], &v_nodes_p[dir] );
		CHKERRXX( ierr );
		if( setup.save_vtk )
		{
			ierr = VecCreateGhostNodes( ns->get_p4est(), ns->get_nodes(), &v_exact_nodes[dir] );
			CHKERRXX( ierr );
			ierr = VecCreateGhostNodes( ns->get_p4est(), ns->get_nodes(), &error_nodes[dir] );
			CHKERRXX( ierr );
			ierr = VecGetArray( error_nodes[dir], &error_nodes_p[dir] );
			CHKERRXX( ierr );
			ierr = VecGetArray( v_exact_nodes[dir], &v_exact_nodes_p[dir] );
			CHKERRXX( ierr );
		}
	}
	...
	for( unsigned char dir = 0; dir < P4EST_DIM; ++dir )
	{
		ierr = VecRestoreArrayRead( v_nodes[dir], &v_nodes_p[dir] );
		CHKERRXX( ierr );
		if( setup.save_vtk )
		{
			ierr = VecRestoreArray( error_nodes[dir], &error_nodes_p[dir] );
			CHKERRXX( ierr );
			ierr = VecRestoreArray( v_exact_nodes[dir], &v_exact_nodes_p[dir] );
			CHKERRXX( ierr );
		}
	}

	int mpiret = MPI_Allreduce( MPI_IN_PLACE, my_errors, 2 * P4EST_DIM, MPI_DOUBLE, MPI_MAX, ns->get_p4est()->mpicomm );
	SC_CHECK_MPI( mpiret );
	...
	if( setup.save_vtk )
	{
		if( create_directory( setup.vtk_path, ns->get_p4est()->mpirank, ns->get_p4est()->mpicomm ))
			throw std::runtime_error(
				"check_accuracy_of_solution: could not create exportation directory " + setup.vtk_path );
		...
		for( unsigned char dir = 0; dir < P4EST_DIM; ++dir )
		{
			ierr = VecGetArrayRead( error_nodes[dir], &error_vtk_p[dir] );
			CHKERRXX( ierr );
			ierr = VecGetArrayRead( v_exact_nodes[dir], &v_exact_vtk_p[dir] );
			CHKERRXX( ierr );
		}
		...
		my_p4est_vtk_write_all_general( ns->get_p4est(), ns->get_nodes(), ns->get_ghost(),
										...
										0, 0, 0,
										vtk_name.c_str(),
										VTK_NODE_VECTOR_BY_COMPONENTS, "velocity_exact",
										DIM( v_exact_vtk_p[0], v_exact_vtk_p[1], v_exact_vtk_p[2] ),
										VTK_NODE_VECTOR_BY_COMPONENTS, "err_v",
										DIM( error_vtk_p[0], error_vtk_p[1], error_vtk_p[2] ) );

		for( unsigned char dir = 0; dir < P4EST_DIM; ++dir )
		{
			ierr = VecRestoreArrayRead( error_nodes[dir], &error_vtk_p[dir] );
			CHKERRXX( ierr );
			ierr = VecRestoreArrayRead( v_exact_nodes[dir], &v_exact_vtk_p[dir] );
			CHKERRXX( ierr );
		}
	}
	for( unsigned char dir = 0; dir < P4EST_DIM; ++dir )
	{
		...
		{
			ierr = VecDestroy( error_nodes[dir] );
			CHKERRXX( ierr );
		}
		if( v_exact_nodes[dir] != NULL )
		{
			ierr = VecDestroy( v_exact_nodes[dir] );
			CHKERRXX( ierr );
		}
	}
...
void initialize_exportations_and_monitoring( const my_p4est_navier_stokes_t *ns, const cmdParser& cmd,
											 ...
											 velocity_profiler_t *&profiler )
{
	PetscErrorCode ierr;
	ierr = PetscPrintf( ns->get_mpicomm(), (std::string( "Parameters : " ) +
											(setup.flow_condition == constant_pressure_gradient? std::string( "Re_tau" ) : std::string( "Re_b" )) +
											std::string( " = %g, domain is %dx2" ) ONLY3D( +std::string("x%d") ) +
											std::string( " (delta units), P/delta = %g, GF = %g\n" )).c_str(),
						DIM( setup.Reynolds, (int)(channel.length() / channel.delta()), (int)(channel.width() / channel.delta()) ),
						channel.pitch_to_delta(), channel.GF() );
	CHKERRXX( ierr );

	ierr = PetscPrintf( ns->get_mpicomm(), "cfl = %g, wall layer = %u, rho = %g, mu = %g (1/mu = %g)\n", ns->get_cfl(),
						channel.ncells_layering_walls_from_ns_solver( ns->get_uniform_band() ), ns->get_rho(),
						ns->get_mu(), 1.0 / ns->get_mu() );
	CHKERRXX( ierr );
	...
	delete profiler;		// Nothing happens if deleting a nullptr.
	profiler = nullptr;
...
bool monitor_simulation( const simulation_setup &setup, const mass_flow_controller_t *controller,
						 my_p4est_navier_stokes_t *ns, external_force_per_unit_mass_t *external_acceleration[P4EST_DIM],
						 const my_p4est_shs_channel_t &channel )
{
	if( ns->get_mpirank() == 0 )
	{
															...
															const my_p4est_poisson_faces_t *face_solver )
{
	...
	if( ns->get_mpirank() == 0 && voro_global_volume[1] >= expected_volume )
		std::cerr << "The global volume of the Voronoi tesselation for faces of normal direction y is "
	...
	while( !setup.done() )
	{
		if( setup.iter > 0 )
		{
			if( flow_controller->read_latest_mass_flow() <= 0.0 )
				throw std::runtime_error( "main_shs_" + std::to_string( P4EST_DIM ) + "d: something went wrong, "
										  "the mass flow should be strictly positive and known to at this stage..." );

			bool solvers_can_be_reused = setup.set_dt_and_update_grid( ns );
			if( cell_solver && !solvers_can_be_reused )
			{
				delete cell_solver;
				cell_solver = nullptr;
			}
			if( face_solver && !solvers_can_be_reused )
			{
				delete face_solver;
				face_solver = nullptr;
			}
		...
		for( unsigned char dir = 0; dir < P4EST_DIM; ++dir )
		{
			ierr = VecCreateNoGhostFaces( ns->get_p4est(), ns->get_faces(), &dxyz_hodge_old[dir], dir );
			CHKERRXX( ierr );
		}
		unsigned int iter_hodge = 0;
		while( iter_hodge < setup.niter_hodge_max &&
			   (convergence_check_on_dxyz_hodge > setup.u_tol * channel.mean_u( flow_controller->read_latest_mass_flow(), ns->get_rho() )) )
		{
		...
		if( setup.save_profiles )
			profiler->gather_and_dump_profiles( setup, ns, (setup.flow_condition == constant_pressure_gradient
															? channel.canonical_u_tau(
														external_acceleration[0]->get_value()) : flow_controller->targeted_bulk_velocity())
