...
DEFINE_PARAMETER(pl,int,lmax,6,"Maximum level of refinement (default: 6)");
...
DEFINE_PARAMETER(pl,double,tfinal,0.0,"Simulation end time (default: 1.0)");
...
void set_geometry(){
  switch(example_){
  case FRANK_SPHERE:
         xmin = 0.0; ymin = 0.0; CODE2D(zmin = 0.0); CODE3D(zmin = 0.0);
         xmax = 1.0; ymax = 1.0; CODE2D(zmin = 0.0); CODE3D(zmax = 1.0);
...
// --------------------------------------------------------------------------------------------------------------
void save_stefan_fields(p4est_t *p4est, p4est_nodes_t *nodes, p4est_ghost_t *ghost,vec_and_ptr_t phi,vec_and_ptr_t Tl,vec_and_ptr_t Ts,vec_and_ptr_dim_t v_int, char* filename ){
...
int main(int argc, char** argv) {
 ...
 // -----------------------------------------------
  ...
  PetscPrintf(mpi.comm(),"Geometry is: \n xmin = %f, xmax = %f, \n ymin = %f, ymax = %f, \n zmin = %f, zmax = %f",xmin,xmax,ymin,ymax,zmin,zmax);
  ...
  sample_cf_on_nodes(p4est,nodes,IC_temp,T_l_n.vec);
  ...
  sample_cf_on_nodes(p4est,nodes,IC_temp,T_s_n.vec);
...
//  ierr = PetscFClose(mpi.comm(),fich_mem); CHKERRXX(ierr);
  ...
  for (tn;tn<tfinal; tn+=dt, tstep++){
      ...
      if(save_vtk && (tstep%save_every_iter ==0)){
        ...
        save_stefan_fields(p4est,nodes,ghost,phi,T_l_n,T_s_n,v_interface,output);
        }
...
//  // stopwatch
//  parStopWatch w;
//  w.start("Running example: stefan_mls");
