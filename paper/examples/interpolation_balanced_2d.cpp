...
int main (int argc, char* argv[]){
	...
	try{
		...
		const double prefactor      = cmd.get<double>("prefactor", 1.0);
		const int repeat            = cmd.get("repeat",1);
		const int test              = cmd.get<int>("test");
		const double alpha          = cmd.get("alpha", 0.05);
		...
		PetscSynchronizedPrintf(p4est->mpicomm, "[%04d] interpolation points = %d \n", p4est->mpirank, points.size());
		PetscSynchronizedFlush(p4est->mpicomm, stdout);
