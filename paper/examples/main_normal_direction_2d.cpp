...
struct circles:CF_2{
  double operator()(double x, double y) const {
    double r = 0.15;
    double f1 = r - sqrt(SQR(x-0.75) + SQR(y-1));
    double f2 = r - sqrt(SQR(x-1.25) + SQR(y-1));
    return MAX(f1, f2);
  }
};
...
int main (int argc, char* argv[]){
  ...
  circles sq;
#endif
  splitting_criteria_cf_t data(cmd.get("lmin", 6), cmd.get("lmax", 9), &sq, 1.3);
  ...
  for (double t=0; t<tf; t+=dt, tc++){
    ...
    level_set.reinitialize_1st_order_time_2nd_order_space(phi, 100);
