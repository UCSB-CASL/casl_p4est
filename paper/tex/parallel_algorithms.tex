...
In \texttt{p4est} the adaptive grid is represented as a non-overlapping collection of trees that are rooted in individual cells of a common coarse grid (cf.\ Figure \ref{fig:p4est_zcurve}).
...
% (The \pforest code actually supports both 2D and 3D meshes whose trees are
...
\begin{algorithmic}[1]
\State $H \gets G.\texttt{macromesh()}$ \Comment{start with the coarsest  grid}
\For {$\mathit{tr} \ein G.\texttt{local\_trees()}$} \Comment{update the hierarchy to match local cells}
	\For {$c \ein \mathit{tr}.\texttt{local\_cells()}$}
		\State $H.\texttt{update\_tree(} \mathit{tr}, c \texttt{)}$
	\EndFor
\EndFor

\For {$c \ein G.\texttt{ghost\_cells()}$} \Comment{update the hierarchy to mach local cells}
...
\Function{$H.\texttt{update\_tree}$}{$\mathit{tr}, c$}   \Comment{recursive tree reconstruction}
	...
	\While{$c_l.\texttt{level()} \not= c.\texttt{level()} $} \Comment{recursively search for a cell of the same size as \textit{c}}
		\If {$c_l.$\texttt{is\_leaf()}} $c_l$\texttt{.split()} \Comment{if the current cell is leaf, split the cell and continue search}
		\EndIf
		\State $h \gets c_l.\texttt{length()}$ \Comment{select the search path based on cell coordinate.}
...
\subsection{Interpolation and semi-Lagrangian methods} \label{sec:interpolation_semilagrangian}
...
\begin{algorithm}[htbp]
\caption{$values \gets \texttt{Interpolate (}H, F, \underline{\mathbf{X}}\texttt{)}$: interpolate the value of $F$, defined on the local tree hierarchy $H$, at coordinates $\underline{\mathbf{X}}$. This is achieved in three phases as detailed in the text. I) First, interpolation points are buffered in two arrays, those than can be interpolated locally and those that should be sent to other processes. II) In the second phase, each process initiates the remote buffer exchange and computes the number of messages it should expect to receive using the communication matrix idea described in section \ref{sec:interpolation_semilagrangian}. III) Finally, each process alternates between computing local interpolation and receiving and performing remote interpolation requests from other processes.}
\begin{algorithmic}[1]
\State $col \gets 0, \mathit{local\_buff} \gets \mathit{null}, \mathit{remote\_buff} \gets \mathit{null}$ \Comment{Phase I -- buffering}
\For {$\underline{\mathbf{p}} \ein \underline{\mathbf{X}}$} 
	\State $[owners\_rank, \mathit{cell}] \gets H.\texttt{search(}\underline{\mathbf{p}}\texttt{)}$ \Comment{search for the owner's rank and cell}
	\If {$owners\_rank = \mathit{mpirank}$} \Comment{can interpolate locally}
        \State $\mathit{local\_buff}[owners\_rank].\texttt{push\_back(}\underline{\mathbf{p}}, cell\texttt{)}$ 
	\Else \Comment{requires remote interpolation}
        \State $\mathit{remote\_buff}[owners\_rank].\texttt{push\_back(}\underline{\mathbf{p}}\texttt{)}$ 
        \State $\mathit{col}[owners\_rank] \gets 1$
	\EndIf
\EndFor
\\
\For {$r \ein \mathit{[0,mpisize)}$} \Comment{Phase II -- initiate communication and compute number of messages}
	\If {$\mathit{col}[r] \deq 1$} 
        \State $req \gets \texttt{MP\_Isend(}r, \texttt{QUERY\_TAG}, \mathit{remote\_buff}[r]\texttt{)}$ \Comment{initiate a non-blocking send to process \textit{r}}
        	\State $\textit{query\_requests}.\texttt{push\_back(}req\texttt{)}$
	\EndIf
\EndFor
\State  $S \gets \texttt{sum(}col\texttt{)}$ \Comment{compute number of messages to send}
\State  $R \gets \texttt{MPI\_Reduce\_scatter(}\mathit{col},\texttt{MPI\_SUM)}$ \Comment{compute number of messages to receive}
\\
\State $\mathit{done} \gets \mathit{false}$ \Comment{Phase III -- interpolation}
\State $it \gets \mathit{local\_buff}[mpirank].\texttt{begin()}$ 
\While {$\mathit{!done}$} 
	\If {$\mathit{it} \not= \mathit{local\_buff[mpirank]}.\texttt{end()}$}
		\State $\mathit{values} \gets \texttt{process\_local\_interpolation(}it\texttt{)}$ \Comment{process local interpolations}
		\State $\texttt{++}\mathit{it}$
	\EndIf
	\If {$R > 0$} \Comment{search for interpolation query in the message queue}
		\State $\mathit{message} \gets \texttt{MPI\_Iprobe(\texttt{MPI\_ANY\_SOURCE}, \texttt{QUERY\_TAG})}$
		\If {$\mathit{message}.\texttt{is\_pending()}$}
			\State $\mathit{values} \gets \texttt{process\_remote\_queries(}\mathit{message}\texttt{)}$ \Comment{receive, search, and interpolate values}
			\State $req \gets \texttt{MPI\_Isend(}\mathit{message}.\texttt{MPI\_SOURCE}, \texttt{REPLY\_TAG}, \mathit{values}\texttt{)}$ \Comment{send back interpolated values}
			\State $\textit{reply\_requests}.\texttt{push\_back(}req\texttt{)}$
			\State $R\texttt{--}$
		\EndIf
	\EndIf
	\If {$S > 0$} \Comment{search for interpolation reply in the message queue}
		\State $\mathit{message} \gets \texttt{MPI\_Iprobe(\texttt{MPI\_ANY\_SOURCE}, \texttt{REPLY\_TAG})}$
		\If {$\mathit{message}.\texttt{is\_pending()}$}
			\State $\mathit{values} \gets \texttt{process\_replies(}\mathit{message}\texttt{)}$ \Comment{receive remotely interpolated values}
			\State $S\texttt{--}$
		\EndIf
	\EndIf
	\State $\mathit{done} \gets S \deq 0 \And R \deq 0 \And \mathit{it \deq local\_buff[mpirank]}.\texttt{end()}$
\EndWhile
\State $\texttt{MPI\_Waitall(}query\_requests, reply\_requests\texttt{)}$ \Comment{make sure all messages have been received}
...
\State $\Delta t   \gets \texttt{MPI\_Allreduce(}\Delta t_l,\texttt{MPI\_MIN)}$ \Comment{compute $\Delta t$ based on CFL condition across all processes}
\State $H^n \gets \texttt{Reconstruct(} G^n\texttt{)}$  \Comment{using algorithm \ref{alg:reconstruction}}
...
\While {$\mathit{true}$}
	...
	\State $\phi^{n+1} \gets \texttt{Interpolate(}H^n, \phi^n, \underline{\textbf{X}}_d\texttt{)}$ \Comment{using algorithm \ref{alg:interpolation}}
