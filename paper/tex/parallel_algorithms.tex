...
\For {$tr : G.\texttt{local\_trees()}$} \Comment{build hierarchy for local cells}
	\For {$c : tr.\texttt{cells()}$}
...
\State \Return $H$
\\
\Function{$H.\texttt{update\_tree}$}{$tr, c$}   \Comment{recursive tree reconstruction}
...
\begin{algorithmic}[1]
\State $col \gets 0, \mathit{buff} \gets null$ \Comment{Phase I -- buffering}
\For {$\underline{\mathbf{p}} : \underline{\mathbf{X}}$} 
	\State $[r, cell] \gets H.\texttt{search(}\underline{\mathbf{p}}\texttt{)}$ \Comment{search for the owner's rank and cell}
	\If {$r = mpirank$}
...
\EndFor
\State $send\_req \gets \texttt{MPI\_Isend(}y_B\texttt{)}$ \Comment{II -- begin updating ghost values}
\State $recv\_req \gets \texttt{MPI\_Irecv(}y_G\texttt{)}$
