...
public:
  ...
  inline void initialize(double xm_ = 0., double xp_ = 1., double ym_ = 0., double yp_ = 1., int nx_ = 1, int ny_ = 1)
  {
    xm = xm_; xp = xp_;
    ym = ym_; yp = yp_;
    nx = nx_; ny = ny_;
  ...
  inline int total_nodes() {return (nx+1)*(ny+1);}

  inline void set_eps(double eps_in) {eps = eps_in;}
};
