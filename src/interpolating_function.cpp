...
double BilinearInterpolatingFunction::operator ()(double x, double y) const
{
  ...
  double xy [] = {x, y};
  quad_mpirank = my_p4est_brick_point_lookup(p4est, xy, &quad_tree, &quad_locidx, &quad);
  ...
  double val = bilinear_interpolation(p4est, quad_tree, quad, F_inter, x, y);
    ...
    for (p4est_locidx_t qu = 0; qu < tree->quadrants.elem_count; ++qu)
    {
      ...
      for (unsigned short i = 0; i<P4EST_CHILDREN; ++i)
      {
        ...
        if (!is_processed(petsc_node_locidx))
        {
          ...
          if (p4est_new->mpirank != my_p4est_brick_point_lookup(p4est, xy, &tr_it_old, &qu_locidx_old, &quad_old))
