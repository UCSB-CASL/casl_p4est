...
void matrix_t::tranpose_matvec(const vector<double> x[], vector<double> b[], const size_t &n_vectors) const
{
#ifdef CASL_THROWS
...
double matrix_t::scale_by_maxabs(vector<double> x[], const size_t &n_vectors)
{
#ifdef CASL_THROWS
...
//    cblas_dscal(x[k].size(), (1.0/abs_max), x[k].data(), 1);
  ...
  for(int i = 0; i < m; i++){
    for(size_t k = 0; k < MAX(n_vectors, (size_t) n); k++)
    {
      if(k < n_vectors)
        x[k][i] /= abs_max;
      if(k < (size_t) n)
