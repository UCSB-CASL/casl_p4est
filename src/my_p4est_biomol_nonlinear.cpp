#include "my_p4est_biomol_nonlinear.h"
...
p4est_connectivity_t* my_p4est_biomol_nonlinear_t::molecule::domain_connectivity = NULL;
mpi_environment_t* my_p4est_biomol_nonlinear_t::molecule::mpi = NULL;
const int my_p4est_biomol_nonlinear_t::nangle_per_mol =
    #ifdef P4_TO_P8
    3
    #else
    1
    #endif
    ;
FILE* my_p4est_biomol_nonlinear_t::log_file     = NULL;
FILE* my_p4est_biomol_nonlinear_t::timing_file  = NULL;
FILE* my_p4est_biomol_nonlinear_t::error_file   = NULL;
...
void my_p4est_biomol_nonlinear_t::molecule::read(const string& pqr, const int overlap)
{
#ifdef CASL_THROWS
  int                 my_local_error;
  string              err_msg;
 ...
 *
void my_p4est_biomol_nonlinear_t::molecule::read_serial(const string& pqr)
{
  if (atoms.size() != 0)
  {
    PetscPrintf(mpi->comm(), "------------------------------------------------ \n");
...
*/
void my_p4est_biomol_nonlinear_t::molecule::calculate_center_of_domain(double* domain_center) const
{
...
int my_p4est_biomol_nonlinear_t::molecule::update_connectivity(p4est_connectivity_t* conn_)
{
  if(domain_connectivity == NULL)
    domain_connectivity = conn_;
  return domain_connectivity != conn_;
}
int my_p4est_biomol_nonlinear_t::molecule::update_mpi(mpi_environment_t* mpi_)
{
  if(mpi == NULL)
    mpi = mpi_;
  return mpi != mpi_;
}
my_p4est_biomol_nonlinear_t::molecule::molecule(const string& pqr_, const double* xyz_c, double* angles, const double angstrom_to_domain_, const int overlap)
  :
    #ifdef CASL_THROWS
    mol_err_manager(mpi->rank(), mpi->size(), mpi->comm(), my_p4est_biomol_nonlinear_t::error_file),
    #endif
...
double my_p4est_biomol_nonlinear_t::molecule::calculate_scaling_factor(const double cube_side_length_to_min_domain_size) const
{
#ifdef CASL_THROWS
  int local_error = (cube_side_length_to_min_domain_size < EPS || cube_side_length_to_min_domain_size >= 1.);
  string err_msg = "my_p4est_biomol_nonlinear_t::molecule::calculate_scaling_factor(const double) requires a value in )0, 1( as an input.";
...
void my_p4est_biomol_nonlinear_t::molecule::scale_rotate_and_translate(const double* angstrom_to_domain_, const double* xyz_c, double* angles)
{
#ifdef CASL_THROWS
  int local_error = (angstrom_to_domain_!= NULL && *angstrom_to_domain_ <= 0);
  string err_msg = "my_p4est_biomol_nonlinear_t::molecule::scale_rotate_and_translate(const double*, ...): the first argument must point to a strictly positive value, if non NULL.";
  mol_err_manager.check_my_local_error(local_error, err_msg);
#endif
  ...
  err_msg = "my_p4est_biomol_nonlinear_t::molecule::scale_rotate_and_translate(...): the bounding box of the molecule ends up out of the computational domain.";
  mol_err_manager.check_my_local_error(local_error, err_msg);
#endif
}
void my_p4est_biomol_nonlinear_t::molecule::translate()
{
  double domain_center[P4EST_DIM];
  calculate_center_of_domain(&domain_center[0]);
  translate(domain_center);
}
void my_p4est_biomol_nonlinear_t::molecule::translate(const double *xyz_c)
{
  scale_rotate_and_translate(NULL, xyz_c, NULL);
}
void my_p4est_biomol_nonlinear_t::molecule::rotate(double* angles, const double *xyz_c)
{
  scale_rotate_and_translate(NULL, xyz_c, angles);
}
void my_p4est_biomol_nonlinear_t::molecule::scale_and_translate(const double* angstrom_to_domain_, const double* xyz_c)
{
  scale_rotate_and_translate(angstrom_to_domain_, xyz_c, NULL);
}
void my_p4est_biomol_nonlinear_t::molecule::reduce_to_single_atom()
{
...
bool my_p4est_biomol_nonlinear_t::molecule::is_bounding_box_in_domain(const double* box_c) const
{
  double *vertices_to_coordinates = domain_connectivity->vertices;
  p4est_topidx_t *tree_to_vertex  = domain_connectivity->tree_to_vertex;
...
double my_p4est_biomol_nonlinear_t::molecule::operator ()(const double x, const double y
                                                      #ifdef P4_TO_P8
                                                      , const double z
                                                      #endif
                                                      )const {
...
p4est_bool_t my_p4est_biomol_nonlinear_t::SAS_creator::refine_for_exact_calculation_fn(p4est_t *park, p4est_topidx_t which_tree, p4est_quadrant_t *quad)
{
  const my_p4est_biomol_nonlinear_t* biomol = (my_p4est_biomol_nonlinear_t*) park->user_pointer;
...
p4est_bool_t my_p4est_biomol_nonlinear_t::SAS_creator::refine_for_reinitialization_fn(p4est_t *park, p4est_topidx_t which_tree, p4est_quadrant_t *quad)
{
  const my_p4est_biomol_nonlinear_t* biomol = (my_p4est_biomol_nonlinear_t*) park->user_pointer;
  // Let's not enforce the min level when we build the SAS grid because that might lead to irrelevant
  // handling and/or communications of kinda big reduced lists of atoms for the sole purpose of refining
...
void my_p4est_biomol_nonlinear_t::SAS_creator::scatter_locally(p4est_t*& park)
{
...
#ifdef CASL_THROWS
    string sas_err_msg = "my_p4est_biomol_nonlinear_t::SAS_creator_brute_force::scatter_locally(const my_p4est_biomol_nonlinear_t*&): something weird occured, you might have memory leak here...";
  ...
  string sas_err_msg = "\n my_p4est_biomol_nonlinear_t::SAS_creator_brute_force::scatter_locally(...) killed because of logic error \n \n";
...
void my_p4est_biomol_nonlinear_t::SAS_creator::scatter_to_new_layout(p4est_t*& park, const bool ghost_flag)
{
  my_p4est_biomol_nonlinear_t* biomol            = (my_p4est_biomol_nonlinear_t*) park->user_pointer;
  Vec& phi_sas                              = biomol->phi;
  p4est_nodes_t* & nodes                    = biomol->nodes;
  ...
  string sas_err_msg = "my_p4est_biomol_nonlinear_t::SAS_creator::scatter_to_new_layout(), nodes are not NULL, you have memory leak...";
  sas_err_manager->check_my_local_error(sas_local_error, sas_err_msg);
#endif
  ...
  sas_err_msg = "my_p4est_biomol_nonlinear_t::SAS_creator::scatter_to_new_layout(), vectors do not have the same size...";
  sas_err_manager->check_my_local_error(sas_local_error, sas_err_msg);
#endif
...
void my_p4est_biomol_nonlinear_t::SAS_creator::partition_forest_and_update_sas(p4est_t*& park)
{
  if(sub_timer != NULL)
...
int  my_p4est_biomol_nonlinear_t::SAS_creator::reinitialization_weight_fn(p4est_t * park, p4est_topidx_t which_tree, p4est_quadrant_t * quadrant)
{
  (void) which_tree;
  my_p4est_biomol_nonlinear_t* biomol = (my_p4est_biomol_nonlinear_t*) park->user_pointer;
...
void my_p4est_biomol_nonlinear_t::SAS_creator::ghost_creation_and_final_partitioning(p4est_t *&park)
{
  my_p4est_biomol_nonlinear_t* biomol = (my_p4est_biomol_nonlinear_t*) park->user_pointer;
  PetscInt my_index_offset = 0;
  for (int proc_rank = 0; proc_rank < mpi_rank; ++proc_rank)
...
void my_p4est_biomol_nonlinear_t::SAS_creator::refine_and_partition(p4est_t* & park, const int& step_idx)
{
  if(sas_timer != NULL)
...
void my_p4est_biomol_nonlinear_t::SAS_creator::refine_the_p4est(p4est_t* & park)
{
  my_p4est_biomol_nonlinear_t* biomol = (my_p4est_biomol_nonlinear_t*) park->user_pointer;
  P4EST_ASSERT(biomol->phi_read_only_p == NULL);
  ierr = VecGetArrayRead(biomol->phi, &biomol->phi_read_only_p); CHKERRXX(ierr);
...
my_p4est_biomol_nonlinear_t::SAS_creator::SAS_creator(p4est_t*& park, const bool timing_flag, const bool subtiming_flag)
  : mpi_rank(park->mpirank),
    mpi_size(park->mpisize),
    mpi_comm(park->mpicomm),
    phi_sas_lower_bound(-1.5*((my_p4est_biomol_nonlinear_t*) (park->user_pointer))->parameters.root_diag()),
    sas_timer((my_p4est_biomol_nonlinear_t::timing_file != NULL && timing_flag)?new parStopWatch(parStopWatch::all_timings, my_p4est_biomol_nonlinear_t::timing_file, park->mpicomm):NULL),
    sub_timer((my_p4est_biomol_nonlinear_t::timing_file != NULL && timing_flag && subtiming_flag)?new parStopWatch(parStopWatch::all_timings, my_p4est_biomol_nonlinear_t::timing_file, park->mpicomm):NULL)
{}

void my_p4est_biomol_nonlinear_t::SAS_creator::construct_SAS(p4est_t *&park)
{
  my_p4est_biomol_nonlinear_t* biomol = (my_p4est_biomol_nonlinear_t*) park->user_pointer;
  int step_idx=1;
...
my_p4est_biomol_nonlinear_t::SAS_creator::~SAS_creator()
{
  if(sas_timer != NULL)
  {
    sas_timer->stop();sas_timer->read_duration();
...
void my_p4est_biomol_nonlinear_t::SAS_creator_brute_force::initialization_routine(p4est_t *&park)
{
  my_p4est_biomol_nonlinear_t* biomol = (my_p4est_biomol_nonlinear_t*) park->user_pointer;
...
int  my_p4est_biomol_nonlinear_t::SAS_creator_brute_force::weight_fn(p4est_t * park, p4est_topidx_t which_tree, p4est_quadrant_t * quadrant)
{
  (void) which_tree;
  my_p4est_biomol_nonlinear_t* biomol = (my_p4est_biomol_nonlinear_t*) park->user_pointer;
...
void my_p4est_biomol_nonlinear_t::SAS_creator_brute_force::weighted_partition(p4est_t*& park)
{
  my_p4est_partition(park, P4EST_FALSE, weight_fn);
}

void my_p4est_biomol_nonlinear_t::SAS_creator_brute_force::specific_refinement(p4est_t* & park)
{
  my_p4est_refine(park, P4EST_FALSE, refine_for_reinitialization_fn, NULL);
}

void my_p4est_biomol_nonlinear_t::SAS_creator_brute_force::update_phi_sas_and_quadrant_data(p4est_t*& park)
{
  my_p4est_biomol_nonlinear_t* biomol            = (my_p4est_biomol_nonlinear_t*) park->user_pointer;
  Vec& phi_sas                              = biomol->phi;
  double* & phi_sas_p                       = biomol->phi_p;
  ...
  string sas_err_msg = "\n my_p4est_biomol_nonlinear_t::SAS_creator_brute_force::update_phi_sas_and_quadrant_data(...): something went wrong when balancing the calculations.\n \n";
  sas_err_manager->check_my_local_error(sas_local_error, sas_err_msg);
#endif
  // coordinates buffer
  ...
  sas_err_msg = "\n my_p4est_biomol_nonlinear_t::SAS_creator_brute_force::update_phi_sas_and_quadrant_data(...): a proc wants to send messages but expects queries too... \n \n";
  sas_err_manager->check_my_local_error(sas_local_error, sas_err_msg);

  sas_local_error = (num_remaining_queries > 0) && (my_nb_to_send_recv >= 0);
  sas_err_msg = "\n my_p4est_biomol_nonlinear_t::SAS_creator_brute_force::update_phi_sas_and_quadrant_data(...): a proc expects queries but has to send messages too \n \n";
  sas_err_manager->check_my_local_error(sas_local_error, sas_err_msg);

  sas_local_error = (my_nb_to_send_recv == 0) && (num_remaining_queries != 0);
  sas_err_msg = "\n my_p4est_biomol_nonlinear_t::SAS_creator_brute_force::update_phi_sas_and_quadrant_data(...): a proc expects queries but has just the right number of values to compute... \n \n";
  sas_err_manager->check_my_local_error(sas_local_error, sas_err_msg);

  sas_local_error = (num_remaining_queries == 0) && (my_nb_to_send_recv < 0);
  sas_err_msg = "\n my_p4est_biomol_nonlinear_t::SAS_creator_brute_force::update_phi_sas_and_quadrant_data(...): a proc does not expect queries but has too few values to compute...\n \n";
  sas_err_manager->check_my_local_error(sas_local_error, sas_err_msg);
#endif
  ...
  {
#ifdef CASL_THROWS
    sas_err_msg = "\n my_p4est_biomol_nonlinear_t::SAS_creator_brute_force::update_phi_sas_and_quadrant_data(...): the maximum number of local quadrants has been reached, the method needs to be redesigned with real quadrant data...\n \n";
...
void my_p4est_biomol_nonlinear_t::SAS_creator_list_reduction::initialization_routine(p4est_t *&park)
{
...
//    {
//        // print a warning
//      string warning_message = "my_p4est_biomol_nonlinear_t::SAS_creator_list_reduction::initialization_routine(): THIS PART DOES NOT SCALE AND SHOULD NOT BE USED IF POSSIBLE...\n";
...
int my_p4est_biomol_nonlinear_t::SAS_creator_list_reduction::weight_fn(p4est_t * park, p4est_topidx_t which_tree, p4est_quadrant_t * quadrant)
{
  (void) which_tree;
  my_p4est_biomol_nonlinear_t* biomol = (my_p4est_biomol_nonlinear_t*) park->user_pointer;
...
void my_p4est_biomol_nonlinear_t::SAS_creator_list_reduction::weighted_partition(p4est_t *&park)
{
  my_p4est_partition(park, P4EST_FALSE, weight_fn);
}

void my_p4est_biomol_nonlinear_t::SAS_creator_list_reduction::update_phi_sas_and_quadrant_data(p4est_t*& park)
{
  my_p4est_biomol_nonlinear_t* biomol            = (my_p4est_biomol_nonlinear_t*) park->user_pointer;
  Vec& phi_sas                              = biomol->phi;
  double* & phi_sas_p                       = biomol->phi_p;
  ...
  string sas_err_msg = "\n my_p4est_biomol_nonlinear_t::SAS_creator_list_reduction::update_phi_sas_and_quadrant_data(...): something went wrong when figuring out the off-proc queries.\n \n";
  sas_err_manager->check_my_local_error(sas_local_error, sas_err_msg);
#endif
  ...
  sas_err_msg = "\n my_p4est_biomol_nonlinear_t::SAS_creator_list_reduction::update_phi_sas_and_quadrant_data(...): the total numbers of expected queries and replies do not match. \n \n";
  sas_err_manager->check_my_local_error(sas_local_error, sas_err_msg);
#endif
...
void my_p4est_biomol_nonlinear_t::SAS_creator_list_reduction::replace_fn(p4est_t *park, p4est_topidx_t which_tree, int num_outgoing, p4est_quadrant_t *outgoing[], int num_incoming, p4est_quadrant_t *incoming[])
{
  (void) num_incoming;
  if (num_outgoing > 1) {
...
#ifdef CASL_THROWS
    my_p4est_biomol_nonlinear_t* biomol = (my_p4est_biomol_nonlinear_t*) park->user_pointer;
    string err_msg = "my_p4est_biomol_nonlinear_t::SAS_creator_list_reduction::replace_fn(...): invoked coarsening...";
...
void my_p4est_biomol_nonlinear_t::SAS_creator_list_reduction::specific_refinement(p4est_t*& park)
{
  my_p4est_biomol_nonlinear_t* biomol = (my_p4est_biomol_nonlinear_t*) park->user_pointer;
  ...
  string sas_error_msg = "my_p4est_biomol_nonlinear_t::SAS_creator_list_reduction::specific_refinement(): something went wrong when associating reduced lists to new quadrants...";
  biomol->err_manager.check_my_local_error(sas_local_error, sas_error_msg);
#endif
}

void my_p4est_biomol_nonlinear_t::check_if_directory(const string& folder_path) const
{
  ...
  string err_msg = "my_p4est_biomol_nonlinear_t::check_if_directory(const string*): invalid directory path: " + folder_path + " does not exist or is not a directory.";
  no_debug_error_manager.check_my_local_error(local_error, err_msg);
}
bool my_p4est_biomol_nonlinear_t::molecule::check_if_file(const string& file_path) const
{
  struct stat st;
  int err_st = stat(file_path.c_str(), &st);
  return (0 == err_st && S_ISREG(st.st_mode));
}
void my_p4est_biomol_nonlinear_t::check_validity_of_vector_of_mol() const
{
#ifdef CASL_THROWS
  int local_error;
  string err_msg;
  ...
  local_error = nmol() <= 0;
  err_msg = "my_p4est_biomol_nonlinear_t:: check_validity_of_vector_of_mol(): empty list of molecules.";
  err_manager.check_my_local_error(local_error, err_msg);

  // are offset consistent?
  local_error = (atom_index_offset.size() != (size_t) nmol());
  err_msg = "my_p4est_biomol_nonlinear_t:: check_validity_of_vector_of_mol(): the size of the vector of offset(s) of atom indices is wrong.";
  err_manager.check_my_local_error(local_error, err_msg);

  local_error = (atom_index_offset.at(0) != 0);
  err_msg = "my_p4est_biomol_nonlinear_t:: check_validity_of_vector_of_mol(): the vector of offset(s) of atom indices is invalid.";
  err_manager.check_my_local_error(local_error, err_msg);
  ...
  err_msg = "my_p4est_biomol_nonlinear_t:: check_validity_of_vector_of_mol(): the biggest molecule is misidentified.";
  err_manager.check_my_local_error(local_error, err_msg);

  local_error = !are_all_molecules_scaled_consistently();
  err_msg = "my_p4est_biomol_nonlinear_t:: check_validity_of_vector_of_mol(): scaling is not consistent.";
  err_manager.check_my_local_error(local_error, err_msg);
  ...
  for (int k = 0; k < nmol(); ++k) {
    ...
    err_msg = "my_p4est_biomol_nonlinear_t:: check_validity_of_vector_of_mol(): a bounding box is not entirely in the domain";
    err_manager.check_my_local_error(local_error, err_msg);
...
bool my_p4est_biomol_nonlinear_t::are_all_molecules_scaled_consistently() const
{
  bool result = nmol() > 0;
  for (int k = 0; k < nmol(); ++k) {
    const molecule& mol_k = bio_molecules.at(k);
...
bool my_p4est_biomol_nonlinear_t::is_no_molecule_scaled() const
{
  bool result = true;
  for (int k = 0; k < nmol(); ++k) {
    const molecule& mol_k = bio_molecules.at(k);
...
void my_p4est_biomol_nonlinear_t::get_vector_of_current_centroids(vector<double>& current_centroids)
{
...
void my_p4est_biomol_nonlinear_t::rescale_all_molecules()
{
...
void my_p4est_biomol_nonlinear_t::rescale_all_molecules(const double* new_centroids)
{
...
void my_p4est_biomol_nonlinear_t::add_single_molecule(const string& file_path, const double* centroid, double* angles, const double* angstrom_to_domain_)
{
  ...
  if(angstrom_to_domain_ == NULL) // no specific scaling factor given for the new molecule
  {
    ...
    {
      string err_message = "my_p4est_biomol_nonlinear_t::add_single_molecule(...): the current vector of molecules has inconsistent scaling factors, this shouldn't happen...";
#ifdef CASL_THROWS
      int local_error = 1;
      err_manager.check_my_local_error(local_error, err_message);
#endif
      cerr <<  err_message << endl;
      cerr << "my_p4est_biomol_nonlinear_t::add_single_molecule(...): ... rescaling the molecules (fixing a logic error)..." << endl;
...
int my_p4est_biomol_nonlinear_t::find_mol_index(const int& global_atom_index, const int& guess) const
{
#ifdef CASL_THROWS
  ...
  {
    string err_msg = "my_p4est_biomol_nonlinear_t::find_mol_index(const int&): invalid global atom index, out of range: global_atom_index = " + to_string(global_atom_index) + " and total_nb_atoms = " + to_string(total_nb_atoms) +", called from proc " + to_string(p4est->mpirank) + " ...";
...
const Atom* my_p4est_biomol_nonlinear_t::get_atom(const int& global_atom_index, int& guess) const
{
...
my_p4est_biomol_nonlinear_t::my_p4est_biomol_nonlinear_t(p4est_t* p4est_, mpi_environment_t* mpi_,
  ...
  parameters(norm(calculate_dimensions_of_root_cells(p4est_)))
{
  ...
  string err_msg = "my_p4est_biomol_nonlinear_t::my_p4est_biomol_nonlinear_t(p4est_t*, mpi_environment_t*, ...): a valid pointer to an mpi environment is required for the second argument (none provided).";
  err_manager.check_my_local_error(local_error, err_msg);

  local_error = p4est_->mpicomm != mpi_->comm();
  err_msg = "my_p4est_biomol_nonlinear_t::my_p4est_biomol_nonlinear_t(p4est_t*, mpi_environment_t*, ...): the mpi communicators of the p4est and the mpi environment do not match.";
  err_manager.check_my_local_error(local_error, err_msg);

  if(pqr_names == NULL && (angles != NULL || centroids != NULL || rel_side_length_biggest_box != NULL))
    cerr << "my_p4est_biomol_nonlinear_t::my_p4est_biomol_nonlinear_t(...): angle(s), centroid(s) and/or desired scaling set, but pqr file(s) undefined "
  ...
  if(local_error)
    err_msg = "my_p4est_biomol_nonlinear_t::my_p4est_biomol_nonlinear_t(...): invalid number of rotation angles (fifth pointer argument). It should contain "+ to_string(nangle_per_mol*nmolecules) + " or " + to_string(nangle_per_mol) + " values (the same rotation for all molecules), but is of size " + to_string(angles->size()) + ".";
  err_manager.check_my_local_error(local_error, err_msg);

  local_error = (centroids != NULL && centroids->size() != (size_t) P4EST_DIM*nmolecules);
  err_msg = "my_p4est_biomol_nonlinear_t::my_p4est_biomol_nonlinear_t(...): the number of centroid coordinates is invalid (last pointer argument). It should contain " + to_string(P4EST_DIM*nmolecules) + " values, but is of size " + to_string(centroids->size()) + ".";
  err_manager.check_my_local_error(local_error, err_msg);
#endif
  if(timing_file != NULL)
...
#ifdef CASL_THROWS
  err_msg = "my_p4est_biomol_nonlinear_t::constructor(...), the molecule pointer to the domain connectivity (static variable) cannot be changed once it is set.";
...
#ifdef CASL_THROWS
  err_msg = "my_p4est_biomol_nonlinear_t::constructor(...), the pointer to the mpi environment (static variable) cannot be changed once set.";
  err_manager.check_my_local_error(update_failure, err_msg);
#endif

  if(timer != NULL)
    timer->start("Construct the my_p4est_biomol_nonlinear_t object (reading the molecules, scaling them, ...)");
  ...
  err_msg = "my_p4est_biomol_nonlinear_t::constructor(...), couldn't delete all reduced lists of global atom indices, there might be memory leak(s)...";
  err_manager.check_my_local_error(local_error, err_msg);
#endif
...
Vec my_p4est_biomol_nonlinear_t::return_phi_vector()
{
  if(phi_read_only_p != NULL)
  {
#ifdef CASL_THROWS
    string message = "my_p4est_biomol_nonlinear_t::return_phi_vector(...): the pointer to the read-only local array of phi was not NULL...\n";
  ...
  {
#ifdef CASL_THROWS
    string message = "my_p4est_biomol_nonlinear_t::return_phi_vector(...): the pointer to the local array of phi was not NULL...\n";
    PetscFPrintf(p4est->mpicomm, error_file, message.c_str());
#endif
  ...
  string message = "my_p4est_biomol_nonlinear_t::return_phi_vector(): the phi vector is NULL, it can't be returned...";
  err_manager.check_my_local_error(local_error, message);
#endif
  Vec phi_to_return = phi; phi = NULL;
  return phi_to_return;
}
p4est_nodes_t* my_p4est_biomol_nonlinear_t::return_nodes()
{
#ifdef CASL_THROWS
  int local_error = nodes == NULL;
  string message = "my_p4est_biomol_nonlinear_t::return_nodes(): the nodes are not valid they can't be returned...";
  err_manager.check_my_local_error(local_error, message);
#endif
  p4est_nodes_t* nodes_to_return = nodes; nodes = NULL;
  return nodes_to_return;
}
p4est_ghost_t* my_p4est_biomol_nonlinear_t::return_ghost()
{
#ifdef CASL_THROWS
  int local_error = ghost == NULL;
  string message = "my_p4est_biomol_nonlinear_t::return_ghost(): the ghosts are not valid, they can't be returned...";
  err_manager.check_my_local_error(local_error, message);
#endif
  p4est_ghost_t* ghost_to_return = ghost; ghost = NULL;
  return ghost_to_return;
}
void my_p4est_biomol_nonlinear_t::return_phi_vector_nodes_and_ghost(Vec& phi_out, p4est_nodes_t*& nodes_out, p4est_ghost_t*& ghost_out)
{
  phi_out           = return_phi_vector();
  nodes_out         = return_nodes();
  ghost_out         = return_ghost();
}
my_p4est_biomol_nonlinear_t::~my_p4est_biomol_nonlinear_t()
{
  if(nodes != NULL)
  {
...
void my_p4est_biomol_nonlinear_t::add_single_molecule(const string& file_path, const vector<double> *centroid, vector<double> *angles, const double *angstrom_to_domain)
{
#ifdef CASL_THROWS
  int local_error = (centroid != NULL && centroid->size() != P4EST_DIM);
  string err_msg = "my_p4est_biomol_nonlinear_t::add_single_molecule(const string *, const vector<double> *, vector<double> *, const double *): vector of centroid components has invalid size.";
  err_manager.check_my_local_error(local_error, err_msg);
  local_error = (angles != NULL && angles->size() != nangle_per_mol);
  err_msg = "my_p4est_biomol_nonlinear_t::add_single_molecule(const string *, const vector<double> *, vector<double> *, const double *): vector of angles has invalid size.";
  err_manager.check_my_local_error(local_error, err_msg);
#endif
  add_single_molecule(file_path,
...
void my_p4est_biomol_nonlinear_t::rescale_all_molecules(const double& new_scaling_factor)
{
  vector<double> current_centroids;
  get_vector_of_current_centroids(current_centroids);
  rescale_all_molecules(new_scaling_factor, &current_centroids);
}
void my_p4est_biomol_nonlinear_t::rescale_all_molecules(const double& new_scaling_factor, const vector<double>* centroids)
{
#ifdef CASL_THROWS
  int local_error = (new_scaling_factor < 0);
  string err_msg = "my_p4est_biomol_nonlinear_t::rescale_all_molecules(const double&, const vector<double>*): the scaling factor must be strictly positive.";
  err_manager.check_my_local_error(local_error, err_msg);
  local_error = (centroids != NULL && centroids->size() != (size_t) P4EST_DIM*nmol());
  err_msg = "my_p4est_biomol_nonlinear_t::rescale_all_molecules(const double&, const vector<double>*): the vector of centroid coordinates has invalid size.";
  err_manager.check_my_local_error(local_error, err_msg);
#endif
  angstrom_to_domain = new_scaling_factor;
  rescale_all_molecules((centroids != NULL)?(&centroids->at(0)):NULL);
}
void my_p4est_biomol_nonlinear_t::set_biggest_bounding_box(const double& biggest_cube_side_length_to_min_domain_size)
{
#ifdef CASL_THROWS
  int local_error = (nmol()==0);
  string err_msg = "my_p4est_biomol_nonlinear_t::set_biggest_bounding_box(const double&): empty vector of molecule, impossible action.";
  err_manager.check_my_local_error(local_error, err_msg);
#endif
...
void my_p4est_biomol_nonlinear_t::print_summary() const
{
  if(log_file == NULL)
    return;
  for (int k = 0; k < nmol(); ++k) {
...
void my_p4est_biomol_nonlinear_t::set_grid_and_surface_parameters(const int &lmin, const int &lmax, const double &lip_, const double &rp_, const int ooa_)
{
  // splitting criterion first
  int need_to_reset_p4est = (int) parameters.set_splitting_criterion(lmin, lmax, lip_);
#ifdef CASL_THROWS
  string err_msg = "my_p4est_biomol_nonlinear_t::set_grid_and_surface_parameters(...): the splitting criterion cannot be reset.";
  err_manager.check_my_local_error(need_to_reset_p4est, err_msg);
#endif
  //probe_radius
#ifdef CASL_THROWS
  int local_error = (!are_all_molecules_scaled_consistently());
  err_msg = "my_p4est_biomol_nonlinear_t::set_grid_and_surface_parameters(...): the probe radius cannot be (re)set if molecules are not scaled consistently.";
  err_manager.check_my_local_error(local_error, err_msg);
#endif
...
#ifdef CASL_THROWS
  err_msg = "my_p4est_biomol_nonlinear_t::set_grid_and_surface_parameters(...): the order of accuracy cannot be (re)set.";
  err_manager.check_my_local_error(need_to_reset_p4est, err_msg);
#endif
...
void my_p4est_biomol_nonlinear_t::set_splitting_criterion(const int& lmin, const int& lmax, const double& lip_)
{
  set_grid_and_surface_parameters(lmin, lmax, lip_, parameters.probe_radius(), parameters.order_of_accuracy());
}
void my_p4est_biomol_nonlinear_t::set_probe_radius(const double& rp)
{
...
void my_p4est_biomol_nonlinear_t::set_order_of_accuracy(const int& ooa)
{
  set_grid_and_surface_parameters(parameters.min_level(), parameters.max_level(), parameters.lip_cst(), parameters.probe_radius(), ooa);
}
vector<double> my_p4est_biomol_nonlinear_t::calculate_dimensions_of_root_cells(p4est_t* p4est_)
{
...
vector<double> my_p4est_biomol_nonlinear_t::calculate_domain_dimensions(p4est_t* p4est_)
{
...
double my_p4est_biomol_nonlinear_t::get_largest_radius_of_all() const
{
  check_validity_of_vector_of_mol();
...
p4est_t* my_p4est_biomol_nonlinear_t::reset_p4est()
{
  p4est_t* new_p4est = my_p4est_new(p4est->mpicomm, p4est->connectivity, 0, NULL, NULL);
  p4est_destroy(p4est);
  return new_p4est;
}
void my_p4est_biomol_nonlinear_t::update_max_level()
{
  global_max_level = 0;
...
void my_p4est_biomol_nonlinear_t::add_reduced_list(p4est_topidx_t which_tree, p4est_quadrant_t *quad, reduced_list_ptr parent_list_ptr, const bool& need_exact_phi)
{
...
double my_p4est_biomol_nonlinear_t::operator ()(const double x, const double y
                                            #ifdef P4_TO_P8
                                            , const double z
                                            #endif
                                            ) const
...
double my_p4est_biomol_nonlinear_t::reduced_operator(const double *xyz, const int& reduced_list_idx, const bool need_exact_value, const bool last_stage) const
{
...
double my_p4est_biomol_nonlinear_t::better_distance(const double *xyz, const int& reduced_list_idx, double* kink_point) const
{
  double phi  = -DBL_MAX, tmp;
  int mol_idx = 0;
...
void my_p4est_biomol_nonlinear_t::build_brick()
{
...
void my_p4est_biomol_nonlinear_t::partition_uniformly(const bool export_cavities, const bool build_ghost)
{
#ifdef CASL_THROWS
  int local_error = (p4est == NULL);
  string err_msg = "my_p4est_biomol_nonlinear_t::partition_uniformly(): this function requires a valid p4est object.";
  err_manager.check_my_local_error(local_error, err_msg);

  local_error = (phi != NULL && nodes == NULL);
  err_msg = "my_p4est_biomol_nonlinear_t::partition_uniformly(): this function requires valid nodes if the levelset function is defined.";
  err_manager.check_my_local_error(local_error, err_msg);
  ...
  if(phi != NULL)
  {
    ...
    err_msg = "my_p4est_biomol_nonlinear_t::partition_uniformly(): the size of the levelset vector is not equal to the total number of nodes.";
    err_manager.check_my_local_error(local_error, err_msg);
  }
#endif
...
int my_p4est_biomol_nonlinear_t::partition_weight_for_enforcing_min_level(p4est_t *park, p4est_topidx_t which_tree, p4est_quadrant_t *quadrant)
{
  (void) which_tree;
  my_p4est_biomol_nonlinear_t* biomol = (my_p4est_biomol_nonlinear_t*) park->user_pointer;
...
void my_p4est_biomol_nonlinear_t::enforce_min_level(bool export_cavities)
{
#ifdef CASL_THROWS
  int local_error = (p4est == NULL);
  string err_msg = "my_p4est_biomol_nonlinear_t::enforce_min_level_and_partition_uniformly(): this function requires a valid p4est object.";
  err_manager.check_my_local_error(local_error, err_msg);

  local_error = (phi == NULL || nodes == NULL);
  err_msg = "my_p4est_biomol_nonlinear_t::enforce_min_level_and_partition_uniformly(): this function requires valid nodes and a valid node-sampled levelset function.";
  err_manager.check_my_local_error(local_error, err_msg);
  ...
  if(phi != NULL)
  {
    ...
    PetscErrorCode ierr = VecGetSize(phi, &size); CHKERRXX(ierr);
    local_error = size != total_nb_nodes;
    err_msg = "my_p4est_biomol_nonlinear_t::enforce_min_level_and_partition_uniformly(): the size of the node-sampled levelset function is not equal to the total number of nodes.";
    err_manager.check_my_local_error(local_error, err_msg);
  }
#endif
  ...
  my_p4est_partition(p4est, P4EST_FALSE, my_p4est_biomol_nonlinear_t::partition_weight_for_enforcing_min_level);
  ...
  err_msg = "my_p4est_biomol_nonlinear_t::enforce_min_level(): logic error, inconsistent number of points after partitioning...";
  err_manager.check_my_local_error(local_error, err_msg);
#endif
  VecScatter ctx, ctx_inner_domain;
  IS is;
  ...
  p4est_refine_ext(p4est, P4EST_TRUE, -1, my_p4est_biomol_nonlinear_t::refine_fn_min_level, NULL, my_p4est_biomol_nonlinear_t::replace_fn_min_level);
  ...
  err_msg = "my_p4est_biomol_nonlinear_t::enforce_min_level_and_partition_uniformly(): some nodes were not updated when enforcing the min level.";
  err_manager.check_my_local_error(local_error, err_msg);
#endif
...
void my_p4est_biomol_nonlinear_t::replace_fn_min_level(p4est_t *park, p4est_topidx_t which_tree, int num_outgoing, p4est_quadrant_t *outgoing[], int num_incoming, p4est_quadrant_t *incoming[])
{
  (void) num_incoming;
  (void) which_tree;
  if (num_outgoing > 1) {
...
#ifdef CASL_THROWS
    my_p4est_biomol_nonlinear_t* biomol = (my_p4est_biomol_nonlinear_t*) park->user_pointer;
    string err_msg = "my_p4est_biomol_nonlinear_t::replace_fn_min_level(...): invoked coarsening...";
    biomol->err_manager.print_message_and_abort(err_msg, 22446688);
#else
    MPI_Abort(park->mpicomm, 22446688);
#endif
...
p4est_bool_t my_p4est_biomol_nonlinear_t::refine_fn_min_level(p4est_t *park, p4est_topidx_t which_tree, p4est_quadrant_t *quad)
{
  const my_p4est_biomol_nonlinear_t* biomol = (my_p4est_biomol_nonlinear_t*) park->user_pointer;
  (void) which_tree;
...
void my_p4est_biomol_nonlinear_t::remove_internal_cavities_poisson(const bool export_cavities)
{
  PetscErrorCode ierr;
#ifdef CASL_THROWS
  int local_error = phi == NULL;
  string err_msg = "my_p4est_biomol_nonlinear_t::remove_internal_cavities_poisson(): this method requires a valid levelset.";
  err_manager.check_my_local_error(local_error, err_msg);

  local_error = nodes == NULL;
  err_msg = "my_p4est_biomol_nonlinear_t::remove_internal_cavities_poisson(): this method requires valid nodes.";
  err_manager.check_my_local_error(local_error, err_msg);
  ...
  err_msg = "my_p4est_biomol_nonlinear_t::remove_internal_cavities_poisson(): the size of the level set vector is not equal to the total number of nodes.";
  err_manager.check_my_local_error(local_error, err_msg);

  local_error = !neighbors->is_initialized;
  err_msg = "my_p4est_biomol_nonlinear_t::remove_internal_cavities_poisson(): the neighbors mus be initialized.";
  err_manager.check_my_local_error(local_error, err_msg);
#endif
...
double my_p4est_biomol_nonlinear_t::inner_box_identifier::operator ()(double x, double y
                                                                  #ifdef P4_TO_P8
                                                                  , double z
                                                                  #endif
                                                                  )const
...
bool my_p4est_biomol_nonlinear_t::is_point_in_outer_domain_and_updated(p4est_locidx_t k, quad_neighbor_nodes_of_node_t& qnnn, const my_p4est_node_neighbors_t* ngbd, double*& inner_domain_p) const
{
...
void my_p4est_biomol_nonlinear_t::remove_internal_cavities_region_growing(const bool export_cavities)
{
  PetscErrorCode ierr;
#ifdef CASL_THROWS
  int local_error = phi == NULL;
  string err_msg = "my_p4est_biomol_nonlinear_t::remove_internal_cavities_region_growing(): this method requires a valid levelset.";
  err_manager.check_my_local_error(local_error, err_msg);

  local_error = nodes == NULL;
  err_msg = "my_p4est_biomol_nonlinear_t::remove_internal_cavities_region_growing(): this method requires valid nodes.";
  err_manager.check_my_local_error(local_error, err_msg);
  ...
  ierr = VecGetSize(phi, &size); CHKERRXX(ierr);
  local_error = size != nb_total_nodes;
  err_msg = "my_p4est_biomol_nonlinear_t::remove_internal_cavities_region_growing(): the size of the level set vector is not equal to the total number of nodes.";
  err_manager.check_my_local_error(local_error, err_msg);

  local_error = !neighbors->is_initialized;
  err_msg = "my_p4est_biomol_nonlinear_t::remove_internal_cavities_region_growing(): the neighbors must be initialized.";
  err_manager.check_my_local_error(local_error, err_msg);
#endif
...
p4est_t* my_p4est_biomol_nonlinear_t::construct_SES(const sas_generation_method& method_to_use, const bool SAS_timing_flag, const bool SAS_subtiming_flag, string vtk_folder)
{
  // sanity checks
  check_validity_of_vector_of_mol();
  int local_error = (nodes != NULL);
#ifdef CASL_THROWS
  string err_msg = "my_p4est_biomol_nonlinear_t::construct_SES(...): nodes should be NULL when invoked.";
  err_manager.check_my_local_error(local_error, err_msg);
#else
  if(local_error)
  {
...
#ifdef CASL_THROWS
  err_msg = "my_p4est_biomol_nonlinear_t::construct_SES(...): ghost should be NULL when invoked.";
  err_manager.check_my_local_error(local_error, err_msg);
#else
  if(local_error)
  {
...
#ifdef CASL_THROWS
  err_msg = "my_p4est_biomol_nonlinear_t::construct_SES(...): phi should be NULL when invoked.";
  err_manager.check_my_local_error(local_error, err_msg);
#else
  if(local_error)
  {
...
#ifdef CASL_THROWS
  err_msg = "my_p4est_biomol_nonlinear_t::construct_SES(...): innder_domain should be NULL when invoked.";
  err_manager.check_my_local_error(local_error, err_msg);
#else
  if(local_error)
  {
...
#ifdef CASL_THROWS
  err_msg = "my_p4est_biomol_nonlinear_t::construct_SES(...): the p4est is already refined, the method assumes a coarse p4est when invoked.";
  err_manager.check_my_local_error(local_error, err_msg);
#else
  if(local_error)
    p4est = reset_p4est();
  ...
  {
#ifdef CASL_THROWS
    err_msg = "my_p4est_biomol_nonlinear_t::construct_SES(...): we assume no user-defined data, internal management of user-defined data is not handled (yet)...";
    err_manager.check_my_local_error(local_error, err_msg);
#else
  ...
  if(sas_creator != NULL)
  {
    ...
    default:
#ifdef CASL_THROWS
      err_msg = "my_p4est_biomol_nonlinear_t::construct_SES(const sas_generation_method& ): unknown sas generation method...";
  ...
  // the p4est must point to a my_p4est_biomol_nonlinear_t object for the construction of the SAS
  void* user_pointer_saved = p4est->user_pointer;
...
#ifdef CASL_THROWS
  local_error = reduced_list::get_nb_reduced_lists() != 0;
  err_msg = "my_p4est_biomol_nonlinear_t::construct_SES(): some reduced lists have not been deleted after the creation of the SAS surface, memory leak has happened...";
  err_manager.check_my_local_error(local_error, err_msg);
#endif
  delete sas_creator; sas_creator = NULL;
  ...
  if(method_to_use != list_reduction_with_exact_phi)
  {
    ...
    default:
  #ifdef CASL_THROWS
      err_msg = "my_p4est_biomol_nonlinear_t::construct_SES(): the order of accuracy should be either 1 or 2!!!";
...
void my_p4est_biomol_nonlinear_t::expand_ghost()
{
...
void my_p4est_biomol_nonlinear_t::set_quad_weight(p4est_quadrant_t* &quad, const p4est_nodes_t* & nodes, const double* const& phi_fct, const double& lower_bound)
{
...
p4est_bool_t my_p4est_biomol_nonlinear_t::coarsen_fn(p4est_t *park, p4est_topidx_t which_tree, p4est_quadrant_t *quad[])
{
  const my_p4est_biomol_nonlinear_t* biomol = (my_p4est_biomol_nonlinear_t*) park->user_pointer;
...
int my_p4est_biomol_nonlinear_t::weight_for_coarsening(p4est_t * park, p4est_topidx_t which_tree, p4est_quadrant_t * quadrant)
{
  (void) which_tree;
  (void) park;
...
bool my_p4est_biomol_nonlinear_t::coarsening_step(int& step_idx, bool export_acceleration)
{
  P4EST_ASSERT(nodes != NULL);
  P4EST_ASSERT(ghost != NULL);
  P4EST_ASSERT(phi_p == NULL);
  ...
  ierr  = VecGetArrayRead(phi, &phi_read_only_p); CHKERRXX(ierr);
  my_p4est_coarsen(p4est, P4EST_FALSE, my_p4est_biomol_nonlinear_t::coarsen_fn, NULL);
  ...
  string err_msg = "\n my_p4est_biomol_nonlinear_t::coarsening_step(...) killed because of logic error \n \n";
  err_manager.check_my_local_error(local_error, err_msg);
#endif

  ierr  = VecGetArrayRead(phi, &phi_read_only_p); CHKERRXX(ierr);
  my_p4est_partition(p4est, P4EST_TRUE, my_p4est_biomol_nonlinear_t::weight_for_coarsening);
  ierr  = VecRestoreArrayRead(phi, &phi_read_only_p); CHKERRXX(ierr); phi_read_only_p = NULL;
...
my_p4est_biomol_nonlinear_solver_t::my_p4est_biomol_nonlinear_solver_t(const my_p4est_biomol_nonlinear_t *biomolecules_)
  : biomolecules(biomolecules_)
{
  ...
  // create the solvers
  jump_solver     = new my_p4est_poisson_nodes_mls_t(biomolecules->neighbors);
  node_solver     = new my_p4est_poisson_nodes_t(biomolecules->neighbors);
}
void    my_p4est_biomol_nonlinear_solver_t::set_molecular_relative_permittivity(double epsilon_molecule)
{
#ifdef CASL_THROWS
  int local_error = (epsilon_molecule < 1.0);
  string error_message = "my_p4est_biomol_nonlinear_solver_t::set_molecular_relative_permittivity(...): the value of the molecular relative permittivity must be greater than or equal to 1.0.";
  ...
  {
    // print a warning
    string message = "my_p4est_biomol_nonlinear_solver_t::set_molecular_relative_permittivity(...): the molecular permittivity was already set, it will be reset...\n";
...
void    my_p4est_biomol_nonlinear_solver_t::set_electrolyte_relative_permittivity(double epsilon_electrolyte)
{
#ifdef CASL_THROWS
  int local_error = (epsilon_electrolyte < 1.0);
  string error_message = "my_p4est_biomol_nonlinear_solver_t::set_electrolyte_relative_permittivity(...): the value of the electrolyte relative permittivity must be greater than or equal to 1.0.";
  biomolecules->err_manager.check_my_local_error(local_error, error_message);
  if(elec_rel_permittivity > 1.0-EPS && fabs(elec_rel_permittivity - epsilon_electrolyte) > EPS*elec_rel_permittivity)
  {
    // print a warning
    string message = "my_p4est_biomol_nonlinear_solver_t::set_electrolyte_relative_permittivity(...): the electrolyte permittivity was already set, it will be reset...\n";
    ierr = PetscFPrintf(biomolecules->p4est->mpicomm, biomolecules->error_file, message.c_str()); CHKERRXX(ierr);
  }
#endif
...
void    my_p4est_biomol_nonlinear_solver_t::set_relative_permittivities(double epsilon_molecule, double epsilon_electrolyte)
{
  set_molecular_relative_permittivity(epsilon_molecule);
  set_electrolyte_relative_permittivity(epsilon_electrolyte);
}
void    my_p4est_biomol_nonlinear_solver_t::set_temperature_in_kelvin(double temperature_in_K)
{
#ifdef CASL_THROWS
  int local_error = (temperature_in_K < EPS);
  string error_message = "my_p4est_biomol_nonlinear_solver_t::set_temperature_in_K(...): the value of the temperature must be strictly positive.";
  biomolecules->err_manager.check_my_local_error(local_error, error_message);
  if(is_temperature_set() && temperature_in_K > EPS && fabs(temperature - temperature_in_K) > EPS*temperature)
  {
    // print a warning
    string message = "my_p4est_biomol_nonlinear_solver_t::set_temperature_in_kelvin(...): the temperature was already set, it will be reset...\n";
    ierr = PetscFPrintf(biomolecules->p4est->mpicomm, biomolecules->error_file, message.c_str()); CHKERRXX(ierr);
  }
#endif
  // if the value is changed, psi_star, psi_bar and psi are no longer valid
...
void    my_p4est_biomol_nonlinear_solver_t::set_far_field_ion_density(double n_0)
{
#ifdef CASL_THROWS
  int local_error = (n_0 < EPS);
  string error_message = "my_p4est_biomol_nonlinear_solver_t::set_far_field_ion_density(...): the value of the far-field ion density must be strictly positive.";
  biomolecules->err_manager.check_my_local_error(local_error, error_message);
  if(is_far_field_ion_density_set() && n_0 > EPS && fabs(far_field_ion_density - n_0) > EPS*far_field_ion_density)
  {
    // print a warning
    string message = "my_p4est_biomol_nonlinear_solver_t::set_far_field_ion_density(...): the far-field ion density was already set, it will be reset...\n";
    ierr = PetscFPrintf(biomolecules->p4est->mpicomm, biomolecules->error_file, message.c_str()); CHKERRXX(ierr);
  }
#endif
  // if the value is changed, psi is no longer valid
...
void    my_p4est_biomol_nonlinear_solver_t::set_ion_charge(int z)
{
#ifdef CASL_THROWS
  int local_error = (z <= 0);
  string error_message = "my_p4est_biomol_nonlinear_solver_t::set_ion_charge(...): the ion charge must be a strictly positive integer.";
  biomolecules->err_manager.check_my_local_error(local_error, error_message);
  if(is_ion_charge_set() && z > 0 && ion_charge != z)
  {
    // print a warning
    string message = "my_p4est_biomol_nonlinear_solver_t::set_ion_charge(...): the ion charge was already set, it will be reset...\n";
    ierr = PetscFPrintf(biomolecules->p4est->mpicomm, biomolecules->error_file, message.c_str()); CHKERRXX(ierr);
  }
#endif
  // if the value is changed, psi_star, psi_bar and psi are no longer valid
...
void    my_p4est_biomol_nonlinear_solver_t::set_debye_length_in_meters(double debye_length_in_m)
{
#ifdef CASL_THROWS
  int local_error = (debye_length_in_m < EPS);
  string error_message = "my_p4est_biomol_nonlinear_solver_t::set_debye_length_in_meters(...): the value of the debye length must be strictly positive.";
  biomolecules->err_manager.check_my_local_error(local_error, error_message);
#endif
...
#ifdef CASL_THROWS
    string err_msg = "my_p4est_biomol_nonlinear_solver_t::set_debye_length_in_meters(): the debye length is already set by the relevant parameters, it can't be set to the new value: abort...";
  ...
  if(!is_ion_charge_set() && is_temperature_set() && is_far_field_ion_density_set())
  {
    ...
    {
#ifdef CASL_THROWS
      string err_msg = "my_p4est_biomol_nonlinear_solver_t::set_debye_length_in_meters(): the calculated ion charge is not an integer, it can't be fixed: abort...";
...
double  my_p4est_biomol_nonlinear_solver_t::get_debye_length_in_meters() const
{
  if(are_all_debye_parameters_set())
  ...
  {
#ifdef CASL_THROWS
    string err_msg = "my_p4est_biomol_nonlinear_solver_t::get_debye_length_in_meters(): not all debye parameters are set, the debye length cannot be calculated: abort...";
...
void    my_p4est_biomol_nonlinear_solver_t::return_psi_star_psi_naught_and_psi_bar(Vec &psi_star_out, Vec &psi_naught_out, Vec &psi_bar_out)
{
#ifdef CASL_THROWS
  int local_error = ((psi_bar == NULL) || (psi_star == NULL));
  string message = "my_p4est_biomol_nonlinear_solver_t::return_psi_bar_and_psi_star(): (one of) the psi_bar and psi_star vectors is (are) NULL, (it) they can't be returned...";
...
Vec     my_p4est_biomol_nonlinear_solver_t::return_psi_hat()
{
#ifdef CASL_THROWS
  int local_error = psi_hat == NULL;
  string message = "my_p4est_biomol_nonlinear_solver_t::return_psi_hat(): the psi_hat vector is NULL, it can't be returned...";
  biomolecules->err_manager.check_my_local_error(local_error, message);
#endif
...
Vec my_p4est_biomol_nonlinear_solver_t::return_residual()
{
#ifdef CASL_THROWS
  int local_error = jump_solver->rhs == NULL;
  string message = "my_p4est_biomol_nonlinear_solver_t::return_residual(): the rhs of the jump solver (aka as the residual) vector is NULL \nIt can't be interpolated and returned...";
  biomolecules->err_manager.check_my_local_error(local_error, message);
#endif
...
void    my_p4est_biomol_nonlinear_solver_t::return_all_psi_vectors(Vec &psi_star_out, Vec &psi_naught_out, Vec &psi_bar_out, Vec &psi_hat_out, bool validation_flag)
{
  if(!validation_flag)
...
void    my_p4est_biomol_nonlinear_solver_t::make_sure_is_node_sampled(Vec &vector)
{
  if(vector != NULL)
  {
...
void    my_p4est_biomol_nonlinear_solver_t::solve_singular_part()
{
  ...
  string err_msg  = "my_p4est_biomol_nonlinear_solver_t::solve_singular_part(): some parameters are not set yet, the singular problem can't be solved...";
  ...
  default:
#ifdef CASL_THROWS
    err_msg = "my_p4est_biomol_nonlinear_solver_t::solve_singular_part(), the order of accuracy should be either 1 or 2!!!";
...
void my_p4est_biomol_nonlinear_solver_t::calculate_jumps_in_normal_gradient(Vec &eps_grad_n_psi_hat_jump, bool validation_flag)
{
...
int     my_p4est_biomol_nonlinear_solver_t::solve_nonlinear(double upper_bound_residual, int it_max, bool validation_flag)
{
  int iter = 0;
  ...
  string err_msg  = "my_p4est_biomol_nonlinear_solver_t::solve_nonlinear(...): some parameters are not set yet, the nonlinear problem can't be solved...";
  biomolecules->err_manager.check_my_local_error(local_error, err_msg);
  local_error     = it_max < 1;
  err_msg         = "my_p4est_biomol_nonlinear_solver_t::solve_nonlinear(...): the maximum number of iterations must be one at least!...";
  biomolecules->err_manager.check_my_local_error(local_error, err_msg);
  local_error     = ( (it_max > 1) && (upper_bound_residual <= 0.0));
  err_msg         = "my_p4est_biomol_nonlinear_solver_t::solve_nonlinear(...): the upper bound for the residual must be strictly positive!...";
  biomolecules->err_manager.check_my_local_error(local_error, err_msg);
#endif
...
void my_p4est_biomol_nonlinear_solver_t::get_solvation_free_energy(bool validation_flag)
{
  if(validation_flag)
...
#ifdef CASL_THROWS
  string my_msg = "my_p4est_biomol_nonlinear_solver_t::get_solvation_free_energy(): the solvation free energy is not properly defined in 2D, forget it! \n    Returning... \n";
...
#ifdef CASL_THROWS
  int local_error = !are_all_parameters_set();
  string err_msg  = "my_p4est_biomol_nonlinear_solver_t::get_solvation_free_energy(): some parameters are not set yet, the nonlinear problem can't be solved, the solvation free energy can't be calculated...";
  biomolecules->err_manager.check_my_local_error(local_error, err_msg);
#endif
  if(!psi_star_psi_naught_and_psi_bar_are_set)
  {
    psi_hat_is_set = false;
#ifdef CASL_THROWS
    err_msg  = "my_p4est_biomol_nonlinear_solver_t::get_solvation_free_energy(): the solution psi_bar for the singular parts is not known yet, it will be calculated...\n";
  ...
  {
#ifdef CASL_THROWS
    err_msg  = "my_p4est_biomol_nonlinear_solver_t::get_solvation_free_energy(): the solution of the general nonlinear Poisson-Boltzmann equation is not known, it will be calculated...\n";
    ierr = PetscFPrintf(biomolecules->p4est->mpicomm, biomolecules->error_file, err_msg.c_str()); CHKERRXX(ierr);
#endif
...
void my_p4est_biomol_nonlinear_solver_t::get_residual_at_voronoi_points_and_set_as_rhs(const Vec& psi_hat_on_voronoi)
{
...
void my_p4est_biomol_nonlinear_solver_t::get_linear_diagonal_terms(Vec& pristine_diagonal_terms)
{
  if(pristine_diagonal_terms == NULL)
  {
...
void my_p4est_biomol_nonlinear_solver_t::clean_matrix_diagonal(const Vec& pristine_diagonal)
{
...
Vec my_p4est_biomol_nonlinear_solver_t::get_psi(double max_absolute_psi, bool validation_flag)
{
  if(validation_flag)
  ...
  {
#ifdef CASL_THROWS
    string err_msg = "my_p4est_biomol_nonlinear_solver_t::get_psi(), the phi vector is not set, psi cannot be constructed... \n";
...
#ifdef CASL_THROWS
    // print a warning
    string message  = "my_p4est_biomol_nonlinear_solver_t::get_psi(), psi_bar and/or psi_star not set yet, they will be computed and created... \n";
    ierr = PetscFPrintf(biomolecules->p4est->mpicomm, biomolecules->error_file, message.c_str()); CHKERRXX(ierr);
#endif
...
#ifdef CASL_THROWS
    // print a warning
    string message  = "my_p4est_biomol_nonlinear_solver_t::get_psi(), psi_hat is not set yet, it will be computed using the linear equation ... \n";
    ierr = PetscFPrintf(biomolecules->p4est->mpicomm, biomolecules->error_file, message.c_str()); CHKERRXX(ierr);
#endif
    solve_linear();
  }
...
my_p4est_biomol_nonlinear_solver_t::~my_p4est_biomol_nonlinear_solver_t()
{
  if(node_solver != NULL){
