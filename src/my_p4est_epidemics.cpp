...
void my_p4est_epidemics_t::solve(int iter)
{
    ...
    if(iter==0)
    {
        ...
        for(size_t n=0; n<nodes->indep_nodes.elem_count ; ++n)
        {
            ...
            //            rhs_u_p[n] = U_n_p[n] + dt_n*(R_A*s*U_n_p[n] + Xi_B*R_A*(V_n_p[n] - W_n_p[n])*U_n_p[n] - U_n_p[n]);
            //            rhs_v_p[n] = V_n_p[n] + dt_n*(R_B*s*V_n_p[n] + Xi_A*R_B*(U_n_p[n] - W_n_p[n])*V_n_p[n] - V_n_p[n]);
            //            rhs_w_p[n] = W_n_p[n] + dt_n*(Xi_A*R_B*(U_n_p[n] - W_n_p[n])*V_n_p[n] + Xi_B*R_A*(V_n_p[n] - W_n_p[n])*U_n_p[n] - 2*W_n_p[n]);
        }
...
{
    double xc, yc;
    double r = MAX(2*MIN(dxyz[0],dxyz[1]), 0.05);
...
void my_p4est_epidemics_t::save_vtk(int iter)
{
    ...
    for(size_t n=0; n<nodes_vis->indep_nodes.elem_count; ++n)
    {
        double xyz[P4EST_DIM];
        node_xyz_fr_n(n, p4est_vis, nodes_vis, xyz);
        den_vis_p[n] = interp_density(xyz[0],xyz[1]);
    }
