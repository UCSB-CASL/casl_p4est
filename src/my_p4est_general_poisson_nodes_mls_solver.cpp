...
my_p4est_general_poisson_nodes_mls_solver_t::my_p4est_general_poisson_nodes_mls_solver_t(const my_p4est_node_neighbors_t *ngbd):my_p4est_poisson_nodes_mls_t(ngbd)
{
  ...
  rhs_original_copy       = NULL;
  diag_m_original_copy    = NULL;
  diag_p_original_copy    = NULL;
  diag_m_copy             = NULL;
  diag_p_copy             = NULL;
  sinh_soln               = NULL;
  soln_ghost              = NULL;
...
{
  if (rhs_original_copy != NULL)    { ierr = VecDestroy(rhs_original_copy);    CHKERRXX(ierr); }
  if (diag_m_original_copy != NULL) { ierr = VecDestroy(diag_m_original_copy); CHKERRXX(ierr); }
  if (diag_p_original_copy != NULL) { ierr = VecDestroy(diag_p_original_copy); CHKERRXX(ierr); }
  if (diag_m_copy != NULL)          { ierr = VecDestroy(diag_m_copy);          CHKERRXX(ierr); }
  if (diag_p_copy != NULL)          { ierr = VecDestroy(diag_p_copy);          CHKERRXX(ierr); }
  if (A_0_soln != NULL)             { ierr = VecDestroy(A_0_soln);             CHKERRXX(ierr); }
  if (A_0_sinh_soln != NULL)        { ierr = VecDestroy(A_0_sinh_soln);        CHKERRXX(ierr); }
  if (A_k_sinh_soln != NULL)        { ierr = VecDestroy(A_k_sinh_soln);        CHKERRXX(ierr); }
  if (sinh_soln != NULL)            { ierr = VecDestroy(sinh_soln);        CHKERRXX(ierr); }
  if (soln_ghost != NULL)           { ierr = VecDestroy(soln_ghost);        CHKERRXX(ierr); }
}
...
int     my_p4est_general_poisson_nodes_mls_solver_t::solve_nonlinear(Vec soln,double upper_bound_residual, int it_max, bool validation_flag)
{
    ...
    while(iter<it_max && residual_of_del_soln_norm>upper_bound_residual )
    {
      ...
      ierr= MatMultAdd(submat_jump_ghost_,soln,soln,soln_ghost);CHKERRXX(ierr);


      ierr = VecAXPY(soln_ghost,1,rhs_jump_); CHKERRXX(ierr);
      ...
      ierr= MatMult(A_k, sinh_soln, A_k_sinh_soln); CHKERRXX(ierr);
      ...
      ierr = PetscPrintf(p4est_->mpicomm, "residual of del_soln->%3.2e for iter->%d \n", residual_of_del_soln_norm,iter); CHKERRXX(ierr);
    ...
    ierr= VecDestroy(del_soln);CHKERRXX(ierr);
