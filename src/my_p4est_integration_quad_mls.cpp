...
#include "my_p4est_integration_quad_mls.h"
#endif
...
void my_p4est_integration_quad_mls_t::initialize()
{
  initialized = true;
  ...
  std::vector<double> p(P4EST_CHILDREN*n_phis, -1);
  std::vector<double> px(P4EST_CHILDREN*n_phis, 0);
  std::vector<double> py(P4EST_CHILDREN*n_phis, 0);
  std::vector<double> pxx(P4EST_CHILDREN*n_phis, 0);
  std::vector<double> pxy(P4EST_CHILDREN*n_phis, 0);
  std::vector<double> pyy(P4EST_CHILDREN*n_phis, 0);

  std::vector<double *> P(n_phis, NULL); for (int i = 0; i < n_phis; i++) {ierr = VecGetArray(phi->at(i), &P[i]); CHKERRXX(ierr);}
  std::vector<double *> Px(n_phis, NULL); for (int i = 0; i < n_phis; i++) {ierr = VecGetArray(phix->at(i), &Px[i]); CHKERRXX(ierr);}
  std::vector<double *> Py(n_phis, NULL); for (int i = 0; i < n_phis; i++) {ierr = VecGetArray(phiy->at(i), &Py[i]); CHKERRXX(ierr);}
  ...
  for(p4est_topidx_t tree_idx = p4est->first_local_tree; tree_idx <= p4est->last_local_tree; ++tree_idx)
  {
    ...
    for(size_t quad_idx = 0; quad_idx < tree->quadrants.elem_count; ++quad_idx)
    {
      ...
      for (int i = 0; i < n_phis; i++)
        ...
        {
          p[i*P4EST_CHILDREN + j] = P[i][ q2n[ s + j ] ];
          px[i*P4EST_CHILDREN + j] = Px[i][ q2n[ s + j ] ];
          py[i*P4EST_CHILDREN + j] = Py[i][ q2n[ s + j ] ];
          pxx[i*P4EST_CHILDREN + j] = Pxx[i][ q2n[ s + j ] ];
          pxy[i*P4EST_CHILDREN + j] = Pxy[i][ q2n[ s + j ] ];
          pyy[i*P4EST_CHILDREN + j] = Pyy[i][ q2n[ s + j ] ];
        }

#ifdef P4_TO_P8
      cubes.push_back(cube3_mls_t(x0, x1, y0, y1, z0, z1));
#else
      cubes.push_back(cube2_quad_mls_t(x0, x1, y0, y1));
#endif

//      cubes.back().construct_domain(p.data(), pxx.data(), pxy.data(), pyy.data(), *action, *color);
      cubes.back().construct_domain(p.data(), pxx.data(), pxy.data(), pyy.data(), px.data(), py.data(), *action, *color);
    }
  }

  for (int i = 0; i < n_phis; i++) {ierr = VecRestoreArray(phi->at(i), &P[i]); CHKERRXX(ierr);}
  for (int i = 0; i < n_phis; i++) {ierr = VecRestoreArray(phix->at(i), &Px[i]); CHKERRXX(ierr);}
  for (int i = 0; i < n_phis; i++) {ierr = VecRestoreArray(phiy->at(i), &Py[i]); CHKERRXX(ierr);}
...
double my_p4est_integration_quad_mls_t::perform(int_type_t int_type, Vec f, int n0, int n1, int n2)
{
  PetscErrorCode ierr;
  double sum = 0.;
...
#else
        cube2_quad_mls_t cube(x0, x1, y0, y1);
#endif

//        cube.construct_domain(p.data(), pxx.data(), pxy.data(), pyy.data(), *action, *color);
        cube.construct_domain(p.data(), pxx.data(), pxy.data(), pyy.data(), px.data(), py.data(), *action, *color);
