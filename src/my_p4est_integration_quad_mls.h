#ifndef MY_P4EST_INTEGRATION_QUAD_MLS_H
#define MY_P4EST_INTEGRATION_QUAD_MLS_H
...
class my_p4est_integration_quad_mls_t
{
public:
  ...
  std::vector<Vec>      *phi;
  std::vector<Vec>      *phixx, *phixy, *phiyy;
  std::vector<Vec>      *phix, *phiy;
...
#else
  std::vector<cube2_quad_mls_t> cubes;
#endif


  my_p4est_integration_quad_mls_t()
  ...
  void set_phi    (std::vector<Vec> &phi_,
                   std::vector<Vec> &phixx_, std::vector<Vec> &phixy_, std::vector<Vec> &phiyy_,
                   std::vector<Vec> &phix_, std::vector<Vec> &phiy_,
                   std::vector<action_t> &action_, std::vector<int> &color_)
  {
    phi = &phi_;
    phixx = &phixx_; phixy = &phixy_; phiyy = &phiyy_;
    phix = &phix_; phiy = &phiy_;
...
#endif // MY_P4EST_INTEGRATION_QUAD_MLS_H
