...
const FD_interface_data& my_p4est_interface_manager_t::get_cell_interface_data_for(const p4est_locidx_t& quad_idx, const p4est_locidx_t& neighbor_quad_idx, const u_char& face_dir) const
{
  ...
  if(cell_FD_interface_data != NULL) // check if stored, first
  {
    map_of_interface_neighbors_t::const_iterator it = cell_FD_interface_data->find({quad_idx, neighbor_quad_idx});
    if(it != cell_FD_interface_data->end())
      return it->second;
  }
...
FD_interface_data my_p4est_interface_manager_t::get_interface_neighbor(const p4est_locidx_t& quad_idx, const p4est_locidx_t& neighbor_quad_idx, const u_char& face_dir) const
{
  return get_cell_interface_data_for(quad_idx, neighbor_quad_idx, face_dir);
}
...
int my_p4est_interface_manager_t::is_map_consistent()
{
  ...
  for (map_of_interface_neighbors_t::const_iterator it = cell_FD_interface_data->begin(); it != cell_FD_interface_data->end(); ++it)
  {
    ...
    if(this_one.neighbor_dof_idx < p4est->local_num_quadrants)
    {
      which_interface_neighbor_t other_one              = {this_one.neighbor_dof_idx, this_one.local_dof_idx};
      const FD_interface_data& other_interface_neighbor = cell_FD_interface_data->at(other_one);
      it_is_alright = it_is_alright && this_interface_neighbor.is_consistent_with(other_interface_neighbor);
    ...
    else
    {
      ...
      other_one.local_dof_idx = ghost_nb_quad->p.piggy3.local_num;
      other_one.neighbor_dof_idx = this_one.local_dof_idx;
      map_of_query_interface_neighbors[rank_owner].push_back(other_one);
      map_of_mirrors[rank_owner].push_back(this_one);
    }
  }
