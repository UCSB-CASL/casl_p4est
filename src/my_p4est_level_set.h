...
class my_p4est_level_set_t {
  ...
  void compute_derivatives( Vec phi_petsc, DIM(Vec dxx_petsc, Vec dyy_petsc, Vec dzz_petsc)) const;
  void compute_derivatives_above_threshold( Vec phi_petsc, double threshold, DIM(Vec dxx_petsc, Vec dyy_petsc, Vec dzz_petsc)) const;
...
public:
   ...
   * \return dt
   */
  double advect_in_normal_direction(const CF_DIM& vn, Vec phi, DIM(Vec phi_xx = NULL, Vec phi_yy = NULL, Vec phi_zz = NULL));
   ...
   * \return dt
   */
  double advect_in_normal_direction(const Vec vn, Vec phi, double dt, DIM(Vec phi_xx = NULL, Vec phi_yy = NULL, Vec phi_zz = NULL));
   ...
   * \return dt
   */
  double advect_in_normal_direction(const Vec vn, const Vec vn_np1, Vec phi, double dt, DIM(Vec phi_xx = NULL, Vec phi_yy = NULL, Vec phi_zz = NULL));

  /* extrapolate using geometrical extrapolation */
  void extend_Over_Interface( Vec phi_petsc, Vec q_petsc, BoundaryConditionsDIM &bc, int order=2, int band_to_extend=INT_MAX ) const;
