...
{
    std::cout<<"epsilon for level set perturbation "<<epsilon<<std::endl;
    ...
    for(int  n=0; n<nodes->indep_nodes.elem_count; n++)
    {
...
int my_p4est_level_set::reinitialize_2nd_order_with_tolerance(Vec phi_petsc, double l1_tolerance,double &l1_error,double &l2_error)
{
    int total_number_of_iterations=500;
    int number_of_iterations=1;
    int ierr=0;

    double l1_error_previous=0; double l2_error_previous=0;

    this->compute_level_set_error(phi_petsc,l1_error,l2_error);
    std::cout<<" l1_error "<<l1_error<<" l2_error "<<l2_error<<std::endl;
    int i=0;
    while(i<total_number_of_iterations &&
          (  (ABS(l1_error-l1_error_previous)/l1_error) >l1_tolerance )&&
          (  (ABS(l2_error-l2_error_previous)/l2_error) >l1_tolerance)  )
    {
        this->reinitialize_2nd_order(phi_petsc,number_of_iterations);
        ierr=VecGhostUpdateBegin(phi_petsc,INSERT_VALUES,SCATTER_FORWARD); CHKERRXX(ierr);
        ierr=VecGhostUpdateEnd(phi_petsc,INSERT_VALUES,SCATTER_FORWARD); CHKERRXX(ierr);
        l1_error_previous=l1_error; l2_error_previous=l2_error;
        this->compute_level_set_error(phi_petsc,l1_error,l2_error);
        std::cout<<" l1_error "<<l1_error<<" l2_error "<<l2_error<<std::endl;
...
int my_p4est_level_set::compute_level_set_error(Vec phi_petsc, double &l1_error, double &l2_error)
{
    int ierr=0;
    double l1_error_local=0.00;
    double l2_error_local=0.00;
    Vec phi_x,phi_y
        #ifdef P4_TO_P8
            ,phi_z
        #endif
    ...
    ierr=VecDuplicate(phi_petsc,&phi_x); CHKERRXX(ierr);// memory increase local to the function 1
    ierr=VecDuplicate(phi_petsc,&phi_y); CHKERRXX(ierr);// memory increase local to the function 2

#ifdef P4_TO_P8
    ierr=VecDuplicate(phi_petsc,&phi_z); CHKERRXX(ierr);// memory increase local to the function 3
#endif

    Vec grad_phi;
    ierr=VecDuplicate(phi_petsc,&grad_phi); CHKERRXX(ierr);// memory increase local to the function 4


    PetscScalar *phi_local,*phi_x_local,*phi_y_local
        #ifdef P4_TO_P8
            ,*phi_z_local
        #endif
            , *grad_phi_local;
    ...
    ierr=VecGetArray(phi_petsc,&phi_local); CHKERRXX(ierr);
    ierr=VecGetArray(phi_x,&phi_x_local); CHKERRXX(ierr);
    ierr=VecGetArray(phi_y,&phi_y_local); CHKERRXX(ierr);

#ifdef P4_TO_P8
    ierr=VecGetArray(phi_z,&phi_z_local); CHKERRXX(ierr);
#endif

    ierr=VecGetArray(grad_phi,&grad_phi_local); CHKERRXX(ierr);

    for(p4est_locidx_t ix=0;ix<this->nodes->num_owned_indeps;ix++)
    {
        phi_x_local[ix]=this->ngbd->neighbors[ix].dx_central(phi_local);
        phi_y_local[ix]=this->ngbd->neighbors[ix].dy_central(phi_local);
#ifdef P4_TO_P8
        phi_z_local[ix]=this->ngbd->neighbors[ix].dz_central(phi_local);
#endif

        phi_x_local[ix]=phi_x_local[ix]*phi_x_local[ix];
        phi_y_local[ix]=phi_y_local[ix]*phi_y_local[ix];
#ifdef P4_TO_P8
        phi_z_local[ix]=phi_z_local[ix]*phi_z_local[ix];
#endif

        grad_phi_local[ix]=phi_x_local[ix]+phi_y_local[ix];

#ifdef P4_TO_P8
        grad_phi_local[ix]+=phi_z_local[ix];
#endif


        grad_phi_local[ix]=pow(grad_phi_local[ix],0.5);

        grad_phi_local[ix]=grad_phi_local[ix]-1.00;
        l1_error_local+=ABS(grad_phi_local[ix]);
        l2_error_local+=grad_phi_local[ix]*grad_phi_local[ix];
    }

    l1_error_local=l1_error_local/this->nodes->num_owned_indeps;
    l2_error_local=l2_error_local/this->nodes->num_owned_indeps;



    int n_data_to_reduce=1;

    ierr=MPI_Allreduce(&l1_error_local,&l1_error,n_data_to_reduce,MPI_DOUBLE,MPI_SUM,MPI_COMM_WORLD);   CHKERRXX(ierr);
    ierr=MPI_Allreduce(&l2_error_local,&l2_error,n_data_to_reduce,MPI_DOUBLE,MPI_SUM,MPI_COMM_WORLD);   CHKERRXX(ierr);

    l2_error=pow(l2_error,0.5);

    ierr=VecRestoreArray(phi_petsc,&phi_local); CHKERRXX(ierr);

    ierr=VecRestoreArray(phi_x,&phi_x_local);CHKERRXX(ierr);
    ierr=VecRestoreArray(phi_y,&phi_y_local); CHKERRXX(ierr);

#ifdef P4_TO_P8
    ierr=VecRestoreArray(phi_z,&phi_z_local); CHKERRXX(ierr);
#endif

    ierr=VecRestoreArray(grad_phi,&grad_phi_local); CHKERRXX(ierr);
    ...
    //    ierr=VecGhostUpdateBegin(grad_phi,INSERT_VALUES, SCATTER_FORWARD); CHKERRXX(ierr);
    //    ierr=VecGhostUpdateEnd(grad_phi,INSERT_VALUES,SCATTER_FORWARD); CHKERRXX(ierr);




    ierr=VecDestroy(phi_x); CHKERRXX(ierr);// memory decrease local to the function 1
    ierr=VecDestroy(phi_y); CHKERRXX(ierr);// memory decrease local to the function 2

#ifdef P4_TO_P8
    ierr=VecDestroy(phi_z); CHKERRXX(ierr);// memory decrease local to the function 3
#endif

    ierr=VecDestroy(grad_phi); CHKERRXX(ierr);// memory decrease local to the function 4
}
...
#ifdef P4_TO_P8
double my_p4est_level_set::advect_in_normal_direction(const Vec vn, Vec phi,double alpha_cfl, Vec phi_xx, Vec phi_yy, Vec phi_zz)
#else
double my_p4est_level_set::advect_in_normal_direction(const Vec vn, Vec phi, double alpha_cfl, Vec phi_xx, Vec phi_yy)
#endif
{
    PetscErrorCode ierr;
    ...
    for (p4est_locidx_t n = 0; n<nodes->num_owned_indeps; ++n){
        ...
        /* choose CFL = 0.8 ... just for fun! */
        dt_local = MIN(dt_local, alpha_cfl*ABS(s_min/vn_p[n]));
    }
    MPI_Allreduce(&dt_local, &dt, 1, MPI_DOUBLE, MPI_MIN, p4est->mpicomm);
...
#ifdef P4_TO_P8
    double diag = this->mult_diag*sqrt(dx*dx + dy*dy + dz*dz);
#else
    double diag = this->mult_diag*sqrt(dx*dx + dy*dy);
#endif
...
void my_p4est_level_set::extend_Over_Interface_With_Stride( Vec phi_petsc, Vec q_petsc, BoundaryConditionType bc_type, Vec bc_vec, int order, int band_to_extend ) const
{
#ifdef CASL_THROWS
    if(bc_type==NOINTERFACE) throw std::invalid_argument("[CASL_ERROR]: extend_over_interface: no interface defined in the boundary condition ... needs to be dirichlet or neumann.");
    if(order!=0 && order!=1 && order!=2) throw std::invalid_argument("[CASL_ERROR]: extend_over_interface: invalid order. Choose 0, 1 or 2");
    ...
    /* NOTE: I don't understand why the this->mult_diag coefficient is needed ... 1 should work, but it doesn't */
#ifdef P4_TO_P8
    double diag = 1.00*sqrt(dx*dx + dy*dy + dz*dz);
#else
    double diag = 1.00*sqrt(dx*dx + dy*dy);
#endif

   diag=diag*this->mult_diag;
    std::vector<double> q0;
    std::vector<double> q1;
    std::vector<double> q2;
    ...
    if(order >= 2)                                   q2.resize(nodes->num_owned_indeps);
    double *bc;
     ierr = VecGetArray(bc_vec, &bc); CHKERRXX(ierr);
    /* now buffer the interpolation points */
    for(p4est_locidx_t n=0; n<nodes->num_owned_indeps; ++n)
    {
#ifdef P4_TO_P8
        ...
        if(phi[n]>0 && bc[n]>0.5&&phi[n]<band_to_extend*diag && grad_phi.norm_L2()>EPS)
        {
            grad_phi /= grad_phi.norm_L2();
            p4est_indep_t *node = (p4est_indep_t*)sc_array_index(&nodes->indep_nodes, n);
            p4est_topidx_t tree_id = node->p.piggy3.which_tree;
    ...
    ierr = VecRestoreArray(q_petsc, &q); CHKERRXX(ierr);
     ierr = VecRestoreArray(bc_vec, &bc); CHKERRXX(ierr);
    ierr = VecRestoreArray(phi_petsc, &phi); CHKERRXX(ierr);
