...
public:
  ...
  int reinitialize_2nd_order_with_tolerance(Vec phi_petsc,double l1_tolerance,double &l1_error,double &l2_error);

  int compute_level_set_error(Vec phi_petsc,double &l1_error,double &l2_error);
   ...
   */
#ifdef P4_TO_P8
  double advect_in_normal_direction(const Vec vn, Vec phi,double alpha_cfl, Vec phi_xx = NULL, Vec phi_yy = NULL, Vec phi_zz = NULL);
#else
  double advect_in_normal_direction(const Vec vn, Vec phi,double alpha_cfl, Vec phi_xx = NULL, Vec phi_yy = NULL);
#endif
   ...
   */
#ifdef P4_TO_P8
  double advect_in_normal_direction_with_max_dt(const Vec vn, Vec phi,double alpha_cfl,double max_dt, Vec phi_xx = NULL, Vec phi_yy = NULL, Vec phi_zz = NULL);
#else
  double advect_in_normal_direction_with_max_dt(const Vec vn, Vec phi,double alpha_cfl, double max_dt,Vec phi_xx = NULL, Vec phi_yy = NULL);
#endif
  ...
  /* extrapolate using geometrical extrapolation */
  void extend_Over_Interface_With_Stride( Vec phi_petsc, Vec q_petsc, BoundaryConditionType bc_type, Vec bc_vec, int order=2, int band_to_extend=INT_MAX ) const;
