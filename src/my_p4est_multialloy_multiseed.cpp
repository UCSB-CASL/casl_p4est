#include "my_p4est_multialloy_multiseed.h"
...
//  dxyz_close_interface = 4*dxyz_min;
  ...
  dxyz_close_interface_ = 4.*dxyz_max_;
...
//  ls.extend_from_interface_to_whole_domain_TVD(phi_, c0n_n_, c0n_interface);
  ...
  std::vector<Vec> v_interface_[P4EST_DIM];

  std::vector<Vec> vx_interface(num_seeds_, v_interface_np1_[0]);
  std::vector<Vec> vy_interface(num_seeds_, v_interface_np1_[1]);

  v_interface_[0] = vx_interface;
  v_interface_[1] = vy_interface;

  /* bousouf update this for second order in time */
  sl.update_p4est(v_interface_np1_, dt_n_, phi_seed_, phi_);
//  sl.update_p4est(v_interface_, dt_n_, phi_seed_);

  /* construct the effective phi */
  ierr = VecDestroy(phi_); CHKERRXX(ierr);
  ierr = VecCreateGhostNodes(p4est_np1, nodes_np1, &phi_); CHKERRXX(ierr);
...
//  ierr = VecDestroy(c0n_interface); CHKERRXX(ierr);
  ...
  ierr = VecDuplicate(phi_, &normal_velocity_n); CHKERRXX(ierr);
  ...
  normal_velocity_np1_ = normal_velocity_n;
...
//  double kappa_thresh = 1.0/dxyz_min_/100.;
...
//  for(size_t n=0; n<nodes_->indep_nodes.elem_count; ++n)
//  {
//    if (phi_p[n] < 0. && phi_p[n] > -phi_thresh_*dxyz_min_ && kappa_p[n] > kappa_thresh)
//      phi_p[n] *= -1.;
//  }
...
//  double fraction = 1.e-6;
//  double *normal_velocity_np1_p;

//  ierr = VecGetArray(normal_velocity_np1_, &normal_velocity_np1_p); CHKERRXX(ierr);

//  for (int i = 0; i < phi_seed_.size(); ++i)
//  {
//    double *phi_seed_ptr;
//    ierr = VecGetArray(phi_seed_[i], &phi_seed_ptr); CHKERRXX(ierr);
//    for(size_t n=0; n<nodes_->indep_nodes.elem_count; ++n)
//    {
//      if (fabs(phi_seed_ptr[n]) < fraction*dxyz_min_)
//      {
//        if (normal_velocity_np1_p[n] > 0. && phi_seed_ptr[n] > 0) phi_seed_ptr[n] *= -1.;
//        if (normal_velocity_np1_p[n] < 0. && phi_seed_ptr[n] < 0) phi_seed_ptr[n] *= -1.;
//      }
//    }
//    ierr = VecRestoreArray(phi_seed_[i], &phi_seed_ptr); CHKERRXX(ierr);
//  }
...
//  ierr = VecRestoreArray(normal_velocity_np1_, &normal_velocity_np1_p); CHKERRXX(ierr);
  ...
  double phi_limit = 10.*dxyz_close_interface_;
  for (int i = 0; i < num_seeds_; ++i)
  {
    ls_new.reinitialize_1st_order_time_2nd_order_space(phi_seed_[i], 50, 2.*phi_limit);
//        ls_new.reinitialize_1st_order(phi_seed_[i]);
//    ls_new.reinitialize_2nd_order(phi_seed_[i]);
    ls_new.perturb_level_set_function(phi_seed_[i], EPS);

    double *phi_ptr;
    ierr = VecGetArray(phi_seed_[i], &phi_ptr); CHKERRXX(ierr);
    for (size_t n = 0; n < nodes_np1->indep_nodes.elem_count; ++n)
    {
      if      (phi_ptr[n] < -phi_limit) phi_ptr[n] = -phi_limit;
      else if (phi_ptr[n] >  phi_limit) phi_ptr[n] =  phi_limit;
    }
    ierr = VecRestoreArray(phi_seed_[i], &phi_ptr); CHKERRXX(ierr);
  }

  copy_ghosted_vec(phi_seed_[0], phi_); CHKERRXX(ierr);

  double *phi_ptr;
  ierr = VecGetArray(phi_, &phi_ptr); CHKERRXX(ierr);

  for (int i = 1; i < phi_seed_.size(); ++i)
  {
    double *phi_seed_ptr;
    ierr = VecGetArray(phi_seed_[i], &phi_seed_ptr); CHKERRXX(ierr);

    for (size_t n = 0; n < nodes_np1->indep_nodes.elem_count; ++n)
      phi_ptr[n] = MAX(phi_ptr[n], phi_seed_ptr[n]);
  ...
  PetscPrintf(p4est_->mpicomm, "Here? \n");
...
//  int max_iterations_ = 40;
  ...
  solver_all_in_one.set_seeds(num_seeds_, phi_seed_, theta_seed_);
...
//    throw std::invalid_argument("my_p4est_multialloy_t::save_vtk could not create directory");
  ...
  const double *v_gamma_p[P4EST_DIM];;
  ierr = VecGetArrayRead(v_interface_np1_[0], &v_gamma_p[0]); CHKERRXX(ierr);
  ierr = VecGetArrayRead(v_interface_np1_[1], &v_gamma_p[1]); CHKERRXX(ierr);
  ...
  std::vector<double *> phi_seed_p(num_seeds_, NULL);

  ierr = VecGetArray(phi_seed_[0], &phi_seed_p[0]); CHKERRXX(ierr);
  ierr = VecGetArray(phi_seed_[1], &phi_seed_p[1]); CHKERRXX(ierr);
  ierr = VecGetArray(phi_seed_[2], &phi_seed_p[2]); CHKERRXX(ierr);
  ...
  my_p4est_vtk_write_all(  p4est_, nodes_, ghost_,
                           ...
                           VTK_POINT_DATA, "vx", v_gamma_p[0],
                           VTK_POINT_DATA, "vy", v_gamma_p[1],
                           VTK_POINT_DATA, "phi_0", phi_seed_p[0],
                           VTK_POINT_DATA, "phi_1", phi_seed_p[1],
                           VTK_POINT_DATA, "phi_2", phi_seed_p[2],
  ...
  ierr = VecRestoreArrayRead(v_interface_np1_[0], &v_gamma_p[0]); CHKERRXX(ierr);
  ierr = VecRestoreArrayRead(v_interface_np1_[1], &v_gamma_p[1]); CHKERRXX(ierr);


  ierr = VecRestoreArray(phi_seed_[0], &phi_seed_p[0]); CHKERRXX(ierr);
  ierr = VecRestoreArray(phi_seed_[1], &phi_seed_p[1]); CHKERRXX(ierr);
  ierr = VecRestoreArray(phi_seed_[2], &phi_seed_p[2]); CHKERRXX(ierr);
