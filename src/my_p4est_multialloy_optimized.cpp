...
{
//  ierr = PetscPrintf(p4est->mpicomm, "Solving temperature\n"); CHKERRXX(ierr);
...
//  solver_t.set_is_matrix_computed(matrices_are_constructed);
  ...
  solver_t.solve(temperature_np1, 1, KSPBCGS, PCHYPRE);
...
//  solver_t.set_is_matrix_computed(matrices_are_constructed);
  ...
  solver_t.solve(temperature_multiplier, 0, KSPBCGS, PCHYPRE);
...
{
//  ierr = PetscPrintf(p4est->mpicomm, "Solving concentration\n"); CHKERRXX(ierr);
...
//  solver_c.set_is_matrix_computed(matrices_are_constructed);
  ...
  solver_c.solve(cl_np1,1, KSPBCGS, PCSOR);

  my_p4est_level_set_t ls(ngbd);
  ls.extend_Over_Interface_TVD(phi, cl_np1, 20, 2);
...
//  solver_c.set_is_matrix_computed(matrices_are_constructed);
  ...
  solver_c.solve(concentration_multiplier, 0, KSPBCGS, PCHYPRE);

  ls.extend_Over_Interface_TVD(phi, concentration_multiplier, 20, 2);
...
{
//  ierr = PetscPrintf(p4est->mpicomm, "Solving concentration_sec\n"); CHKERRXX(ierr);
  double *phi_p;
  ierr = VecGetArray(phi, &phi_p); CHKERRXX(ierr);
  for(size_t n=0; n<nodes->indep_nodes.elem_count; ++n)
    phi_p[n] *= -1;
...
//    robin_coef_p[n] = -cooling_velocity*(1-kp)/solute_diffusivity_l;
  ...
  solver_c.solve(cl_sec_np1, 1, KSPBCGS, PCHYPRE);
...
//    robin_coef_p[n] = -cooling_velocity*(1-kp)/solute_diffusivity_l;
  ...
  solver_c.solve(c_mult_tmp, 0, KSPBCGS, PCHYPRE);
...
void my_p4est_multialloy_t::update_grid()
{
  ...
  {
    interp.set_input(temperature_np1, linear);
  ...
  copy_ghosted_vec(temperature_n, temperature_np1);
...
//  interp.interpolate(cs_sec_n);
  ...
  for(int dir=0; dir<P4EST_DIM; ++dir)
  {
    ...
    interp.set_input(v_interface_np1[dir], linear);
...
//  ierr = VecDuplicate(phi, &normal_velocity_np1_tmp); CHKERRXX(ierr);
  ...
  for(size_t n=0; n<nodes->indep_nodes.elem_count; ++n)
  {
    ...
    c_interface_tmp_p[n] = (t_interface_p[n]-Tm-ml_sec*c_sec_interface_p[n])/ml
            + eps_c(theta_p[n])*kappa_p[n]/ml
            + eps_v(theta_p[n])*normal_velocity_np1_p[n]/ml;
#endif
  }
  ierr = VecRestoreArray(c_interface_tmp, &c_interface_tmp_p); CHKERRXX(ierr);
  ierr = VecRestoreArray(kappa, &kappa_p); CHKERRXX(ierr);
  ...
  int num_of_iterations_per_step = 50;

//  ierr = PetscPrintf(p4est->mpicomm, "Temperature_multiplier\n"); CHKERRXX(ierr);
...
//    ierr = PetscPrintf(p4est->mpicomm, "Concentration_sec\n"); CHKERRXX(ierr);
    ...
    error_bc_l2 = sqrt(integrate_over_interface(p4est, nodes, phi, integrand));
...
void my_p4est_multialloy_t::adjust_interface_concentration(bool use_simple)
{
  if (use_simple)
  {
    ...
    double *c_interface_p, *c_sec_interface_p, *t_interface_p, *kappa_p, *theta_p, *normal_velocity_np1_p, *phi_p;
    ierr = VecGetArray(normal_velocity_np1, &normal_velocity_np1_p); CHKERRXX(ierr);
    ierr = VecGetArray(c_interface, &c_interface_p); CHKERRXX(ierr);
    ierr = VecGetArray(c_sec_interface, &c_sec_interface_p); CHKERRXX(ierr);
    ierr = VecGetArray(t_interface, &t_interface_p); CHKERRXX(ierr);
    ...
    {
      if (fabs(phi_p[n]) < 3*diag)
        c_interface_p[n] = (t_interface_p[n] - Tm)/ml - ml_sec/ml * c_sec_interface_p[n]
            + eps_c(theta_p[n])*kappa_p[n]/ml
            + eps_v(theta_p[n])*normal_velocity_np1_p[n]/ml;
    }
    ...
    for(size_t n=0; n<nodes->indep_nodes.elem_count; ++n)
    {
      if (fabs(phi_p[n]) < 3*diag)
          c_interface_p[n] -= (c_interface_p[n] + ml_sec/ml * c_sec_interface_p[n] - (t_interface_p[n] - Tm)/ml
                               - eps_c(theta_p[n])*kappa_p[n]/ml - eps_v(theta_p[n])*normal_velocity_np1_p[n]/ml)
              / denominator_p[n];
    }
