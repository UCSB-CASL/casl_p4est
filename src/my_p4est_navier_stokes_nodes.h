...
protected:
  ...
  class splitting_criteria_vorticity_nodes_t : public splitting_criteria_tag_t
  {
  private:
  ...
  public:
    ...
    double uniform_band;
    double smoke_thresh;
    splitting_criteria_vorticity_nodes_t(int min_lvl, int max_lvl, double lip, double uniform_band, double threshold_vorticity, double max_L2_norm_u, double smoke_thresh, double threshold_norm_grad_u);
  ...
  {
  private:
    my_p4est_navier_stokes_nodes_t* _prnt;
  public:
    wall_bc_value_hodge_t(my_p4est_navier_stokes_nodes_t* obj) : _prnt(obj) {}
  ...
  private:
    my_p4est_navier_stokes_nodes_t* _prnt;
  public:
    interface_bc_value_hodge_t(my_p4est_navier_stokes_nodes_t* obj) : _prnt(obj) {}
    double operator()(DIM(double x, double y, double z)) const;
  };
  ...
  std::vector<double> ext_force_term;
  ...
  inline void get_Q_and_lambda_2_values(const quad_neighbor_nodes_of_node_t& qnnn, const double *vnp1_p[P4EST_DIM], const double& x_scaling, const double& U_scaling, double& Qvalue, double& lambda_2_value) const
  {
...
#ifdef CASL_THROWS
    if(pp > 0.0)
      throw std::runtime_error("my_p4est_navier_stokes_nodes_t::get_Q_and_lambda_2_values(): obtained a positive pp");
#endif
    pp = MIN(pp, -EPS);
    ...
    for (unsigned char kk = 0; kk < P4EST_DIM; ++kk)
    {
      ...
      if(fabs(3.0*qq*sqrt(-3.0/pp)/(2.0*pp)) > 1.0 + EPS)
        throw std::runtime_error("my_p4est_navier_stokes_nodes_t::get_Q_and_lambda_2_values(): argument of acos > 1 in absolute value");
#endif
...
#ifdef CASL_THROWS
      if(measuring)
        throw std::runtime_error("my_p4est_navier_stokes_nodes_t::ns_timer_t::start_watch(): the watch needs to be stopped before being restarted.");
#endif
      measuring  = true;
      task       = task_;
...
#ifdef CASL_THROWS
      if(!measuring)
        throw std::runtime_error("my_p4est_navier_stokes_nodes_t::ns_timer_t::stop_watch(): the watch can't be stopped if it wasn't started first.");
#endif
...
public:
  my_p4est_navier_stokes_nodes_t(my_p4est_node_neighbors_t *ngbd_nm1, my_p4est_node_neighbors_t *ngbd_n);
  my_p4est_navier_stokes_nodes_t(const mpi_environment_t& mpi, const char* path_to_saved_state, double &simulation_time);
  ...
  inline double get_vorticity_split_threshold() const{ return 0.0;} // Rochi to be decided
  ...
  {
    my_p4est_poisson_nodes_mls_t* viscosity_step_solver = NULL;
    solve_viscosity(viscosity_step_solver);
    delete viscosity_step_solver;
  }
  void solve_viscosity(my_p4est_poisson_nodes_mls_t* &viscosity_step_solver, const bool& use_initial_guess = false, const KSPType& ksp = KSPBCGS, const PCType& pc = PCSOR);

  void solve_projection()
  {
    my_p4est_poisson_nodes_mls_t* projection_solver = NULL;
    solve_projection(projection_solver);
    delete projection_solver;
  }
  double solve_projection(my_p4est_poisson_nodes_mls_t* &projection_solver, const bool& use_initial_guess = false, const KSPType& ksp = KSPBCGS, const PCType& pc = PCSOR,
