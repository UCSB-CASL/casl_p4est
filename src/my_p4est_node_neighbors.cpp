...
void my_p4est_node_neighbors_t::first_derivatives_central(const Vec f[], DIM(Vec fx[], Vec fy[], Vec fz[]), const unsigned int& n_vecs, const unsigned int& bs) const
{
  ...
  if( n_vecs > CASL_NUM_SIMULTANEOUS_FIELD_COMPUT )
    throw std::runtime_error( "[CASL_ERROR] You're requesting more simultaneous fields than specified in macro CASL_NUM_SIMULTANEOUS_FIELD_COMPUT (for first_derivatives_central) " );
  if( bs > CASL_NUM_SIMULTANEOUS_BLOCK_COMPUT )
  throw std::runtime_error( "[CASL_ERROR] You're requesting more simultaneous blocks than specified in macro CASL_NUM_SIMULTANEOUS_BLOCK_COMPUT (for first_derivatives_central) " );
