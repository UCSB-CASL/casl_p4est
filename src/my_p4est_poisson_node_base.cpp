...
{
    if (this->A             != NULL) this->ierr = MatDestroy(this->A);                      CHKERRXX(this->ierr);
    if (this->A_null_space  != NULL) this->ierr = MatNullSpaceDestroy (A_null_space); CHKERRXX(this->ierr);
    if (this->ksp           != NULL) ierr = KSPDestroy(ksp);                    CHKERRXX(this->ierr);
    if(this->add_!=NULL)       this->ierr=VecDestroy(this->add_);         CHKERRXX(this->ierr);
    if(this->phi_is_all_positive!=NULL)
    {
        //this->ierr=VecDestroy(this->phi_is_all_positive); CHKERRXX(this->ierr);
    }
    if(this->phi_!=NULL) this->ierr=VecDestroy(this->phi_); CHKERRXX(this->ierr);
...
//        // worst case for debugging only
//        #ifdef P4_TO_P8

//        d_nnz[n]=25; o_nnz[n]=25;
...
void PoissonSolverNodeBase::preallocate_periodic_matrix()
{
    PetscBool save_memory=PETSC_TRUE;
...
//            continue;
    ...
    {
        // worst case when time go back to the previous option
        #ifdef P4_TO_P8

    ierr = MatSeqAIJSetPreallocation(A, 25, NULL); CHKERRXX(ierr);
    ierr = MatMPIAIJSetPreallocation(A, 25, NULL, 25, NULL); CHKERRXX(ierr);
...
//    VecGetArray(rhs_,&rhs_value);
//    std::cout<<" rhs value "<<rhs_value[0]<<std::endl;
...
void PoissonSolverNodeBase::setupM2()
{
//    ierr = PetscLogEventBegin(log_PoissonSolverNodeBase_solve, A, rhs_, ksp, 0); CHKERRXX(ierr);
...
//    {
//        PetscInt sol_size;
//        ierr = VecGetLocalSize(this->rhs_, &sol_size); CHKERRXX(ierr);
//        if (sol_size != nodes->num_owned_indeps){
//            std::ostringstream oss;
//            oss << "[CASL_ERROR]: solution vector must be preallocated and locally have the same size as num_owned_indeps"
//                << "solution.local_size = " << sol_size << " nodes->num_owned_indeps = " << nodes->num_owned_indeps << std::endl;
...
void PoissonSolverNodeBase::setupM2_Neuman()
{
...
void PoissonSolverNodeBase::setupM2_Dirichlet()
{
...
//                if(n==0)
//                    std::cout<<"rhs_p[n]"<<n<<" "<<x_C<<" "<<y_C<<" "<<rhs_p[n]<<std::endl;
#endif
                continue;
            }
#ifdef P4_TO_P8
            ...
            //    std::cout<<"rhs_p[n] "<<n<<" "<<rhs_p[n]<<std::endl;
...
// std::cout<<"rhs_p[0]"<<0<<rhs_p[0]<<std::endl;
...
//    {
//        local_add = true;
//        ierr = VecCreateMPI(MPI_COMM_WORLD, nodes->num_owned_indeps,PETSC_DETERMINE, &add_); CHKERRXX(ierr);
//        ierr = VecSet(add_, diag_add_); CHKERRXX(ierr);
...
//    {
//        local_phi = true;
//        ierr = VecCreateMPI(MPI_COMM_WORLD, nodes->num_owned_indeps,PETSC_DETERMINE ,&phi_); CHKERRXX(ierr);
//        ierr = VecSet(phi_, -1.); CHKERRXX(ierr);
...
//    ierr = VecGetArray(add_,    &add_p   ); CHKERRXX(ierr);
     ...
     */
...

        {
                ...
                //---------------------------------------------------------------------
//                double w_000 = add_p[n] - ( w_m00 + w_p00 + w_0m0 + w_0p0 + w_00m + w_00p );
                double w_000 =  - ( w_m00 + w_p00 + w_0m0 + w_0p0 + w_00m + w_00p );
...
//                w_00m /= w_000; w_00p /= w_000;
                ...
                // add coefficients in the matrix
                //---------------------------------------------------------------------
                ierr = MatSetValue(A, node_000_g, node_000_g, w_000, ADD_VALUES); CHKERRXX(ierr);
                    ...
                    if (w_m00_mm != 0)
                    {
                        ierr = MatSetValue(A, node_000_g, petsc_gloidx[node_m00_mm], w_m00_mm, ADD_VALUES);
                        CHKERRXX(ierr);
                    }
                    if (w_m00_mp != 0) {ierr = MatSetValue(A, node_000_g, petsc_gloidx[node_m00_mp], w_m00_mp, ADD_VALUES); CHKERRXX(ierr);}
                    if (w_m00_pm != 0) {ierr = MatSetValue(A, node_000_g, petsc_gloidx[node_m00_pm], w_m00_pm, ADD_VALUES); CHKERRXX(ierr);}
...
//                double diag = add_p[n]-(w_m00+w_p00+w_0m0+w_0p0);
                   double diag = -(w_m00+w_p00+w_0m0+w_0p0);
//                w_m00 /= diag;
//                w_p00 /= diag;
//                w_0m0 /= diag;
//                w_0p0 /= diag;
                ...
                // addition to diagonal elements
                //---------------------------------------------------------------------
                ierr = MatSetValue(A, node_000_g, node_000_g, diag, ADD_VALUES); CHKERRXX(ierr);
...
void PoissonSolverNodeBase::setup_dirichlet_matrix()
{
    this->preallocate_matrix();

    this->ierr=VecDuplicate(this->phi_,&this->phi_is_all_positive);  CHKERRXX(this->ierr);
    this->ierr=VecSet(this->phi_is_all_positive,0.00);   CHKERRXX(this->ierr);

    this->ierr=VecDuplicate(this->phi_,&this->is_crossed_neumann); CHKERRXX(this->ierr);
    this->ierr=VecSet(this->is_crossed_neumann,0.00); CHKERRXX(this->ierr);

    this->ierr=VecDuplicate(this->phi_,&this->phi_is_below_epsilon);  CHKERRXX(this->ierr);
    this->ierr=VecSet(this->phi_is_below_epsilon,0.00);   CHKERRXX(this->ierr);



    PetscScalar *phi_is_all_positive_array;
    this->ierr = VecGetArray(this->phi_is_all_positive,    &phi_is_all_positive_array   ); CHKERRXX(ierr);

    PetscScalar *is_crossed_neumann_array;
    this->ierr = VecGetArray(this->is_crossed_neumann,    &is_crossed_neumann_array   ); CHKERRXX(ierr);

    PetscScalar *phi_is_below_epsilon_array;
    this->ierr = VecGetArray(this->phi_is_below_epsilon,    &phi_is_below_epsilon_array   ); CHKERRXX(ierr);
     ...
     */
        ...
        if(is_node_Wall(p4est, ni))
        {
            ...
            phi_is_all_positive_array[n]=1.00;
            this->n_phi_is_all_positive++;
        ...
        if(!is_node_Wall(p4est, ni))
        {
            ...
            {
                ierr = MatSetValue(A, node_000_g, node_000_g, bc_strength, ADD_VALUES); CHKERRXX(ierr);
                phi_is_below_epsilon_array[n]=1;
                matrix_has_nullspace=false;
                continue;
            }
            ...
            bool is_all_positive=(P_mmm > 0 && P_pmm > 0 && P_mpm > 0 && P_ppm > 0 &&
                                  P_mmp > 0 && P_pmp > 0 && P_mpp > 0 && P_ppp > 0);
    ...
    this->ierr = VecRestoreArray(this->phi_is_all_positive,    &phi_is_all_positive_array   ); CHKERRXX(this->ierr);
    this->ierr = VecRestoreArray(this->is_crossed_neumann,    &is_crossed_neumann_array   ); CHKERRXX(this->ierr);
    this->ierr=VecRestoreArray(this->phi_is_below_epsilon,&phi_is_below_epsilon_array); CHKERRXX(this->ierr);

    this->ierr=VecGhostUpdateBegin(this->phi_is_all_positive,INSERT_VALUES,SCATTER_FORWARD); CHKERRXX(this->ierr);
    this->ierr=VecGhostUpdateEnd(this->phi_is_all_positive,INSERT_VALUES,SCATTER_FORWARD); CHKERRXX(this->ierr);

    this->ierr=VecGhostUpdateBegin(this->is_crossed_neumann,INSERT_VALUES,SCATTER_FORWARD); CHKERRXX(this->ierr);
    this->ierr=VecGhostUpdateEnd(this->is_crossed_neumann,INSERT_VALUES,SCATTER_FORWARD); CHKERRXX(this->ierr);
...
void PoissonSolverNodeBase::setup_neuman_matrix()
{
    preallocate_matrix();
       ...
       if(!is_node_Wall(p4est, ni))
        {
            ...
            bool is_all_positive=(P_mmm > 0 && P_pmm > 0 && P_mpm > 0 && P_ppm > 0 &&
                                  P_mmp > 0 && P_pmp > 0 && P_mpp > 0 && P_ppp > 0);
//            &&
//                                  phi_000>0.00  &&  phi_m00>0.00 && phi_p00>0.00 &&
//                                  phi_0m0>0.00  &&  phi_0p0>0.00 && phi_00m>0.00 &&
//                                  phi_00p>0.00);
...
//                if(phi_000<=0.00  ||  phi_m00<=0.00 || phi_p00<=0.00 || phi_0m0<=0.00 || phi_0p0<=0.00 || phi_00m<=0.00 || phi_00p<=0.00)
//                {
////                    std::cout<<" To check how come the level set of a function is smaller than zero"<<std::endl;
////                    std::cout<<" while the level set values of all the neighbors is all bigger than zero "<<std::endl;
...
//                }
            ...
            if (is_ngbd_crossed_neumann && bc_->interfaceType() == NEUMANN)
            {
                ...
                {
                    is_crossed_neumann_array[n]=1.00;
                    this->n_is_crossed_neuman++;
...
void PoissonSolverNodeBase::setup_volume_matrix()
{
    // register for logging purpose
    ierr = PetscLogEventBegin(log_PoissonSolverNodeBase_rhsvec_setup, 0, 0, 0, 0); CHKERRXX(ierr);
        ...
        const quad_neighbor_nodes_of_node_t& qnnn = (*node_neighbors_)[n];
            ...
            bool is_ngbd_crossed_neumann = ( P_mmm*P_mpm<0 || P_mpm*P_ppm<0 || P_ppm*P_pmm<0 || P_pmm*P_mmm<0 );
                ...
                double volume_cut_cell = cube.area_In_Negative_Domain(phi_cube);
                   ...
                   add_p[n] = add_p[n]*volume_cut_cell ;
...
void PoissonSolverNodeBase::setup_volume_matrix2()
{
    // register for logging purpose
    ierr = PetscLogEventBegin(log_PoissonSolverNodeBase_rhsvec_setup, 0, 0, 0, 0); CHKERRXX(ierr);
...
void PoissonSolverNodeBase::setup_interface_matrix(Vec *v_interface)
{
    // register for logging purpose
    ierr = PetscLogEventBegin(log_PoissonSolverNodeBase_rhsvec_setup, 0, 0, 0, 0); CHKERRXX(ierr);
    ...
    double *phi_p, *phi_xx_p, *phi_yy_p, *v_interface_p;
    ierr = VecGetArray(phi_,    &phi_p   ); CHKERRXX(ierr);
    ierr = VecGetArray(phi_xx_, &phi_xx_p); CHKERRXX(ierr);
    ierr = VecGetArray(phi_yy_, &phi_yy_p); CHKERRXX(ierr);
#ifdef P4_TO_P8
    ...
    ierr = VecGetArray(*v_interface,    &v_interface_p   ); CHKERRXX(ierr);

    for(p4est_locidx_t n=0; n<nodes->num_owned_indeps; n++) // loop over nodes
    {
        v_interface_p[n]=0.00;
        ...
        double z_C  = node_z_fr_k(ni) + tree_zmin;
            ...
            if (is_ngbd_crossed_neumann && bc_->interfaceType() == NEUMANN)
            {
                ...
                {
#ifdef P4_TO_P8
                    double one_point_00=1.00;
                    OctValue bc_value( one_point_00,
                                       one_point_00,
                                       one_point_00,
                                       one_point_00,
                                       one_point_00,
...
#else
                      double one_point_00=1.00;
                    QuadValue bc_value( one_point_00,
                                        one_point_00,
                                        one_point_00,
                                        one_point_00);
                    double integral_bc = cube.integrate_Over_Interface(bc_value, phi_cube);
#endif
                    v_interface_p[n]=integral_bc;
                }
            }
        }
    ...
    ierr = VecRestoreArray(*v_interface,    &v_interface_p  ); CHKERRXX(ierr);

    this->ierr=VecGhostUpdateBegin(*v_interface,INSERT_VALUES,SCATTER_FORWARD); CHKERRXX(this->ierr);
    this->ierr=VecGhostUpdateEnd(*v_interface,INSERT_VALUES,SCATTER_FORWARD); CHKERRXX(this->ierr);
    ierr = PetscLogEventEnd(log_PoissonSolverNodeBase_rhsvec_setup, rhs_, 0, 0, 0); CHKERRXX(ierr);
}
...
#endif
{
    this->ierr=VecDuplicate(phi,&this->phi_); CHKERRXX(this->ierr);
    this->ierr=VecCopy(phi,this->phi_); CHKERRXX(this->ierr);

    this->ierr=VecGhostUpdateBegin(this->phi_,INSERT_VALUES,SCATTER_FORWARD); CHKERRXX(this->ierr);
    this->ierr=VecGhostUpdateEnd(this->phi_,INSERT_VALUES,SCATTER_FORWARD);   CHKERRXX(this->ierr);
...
void PoissonSolverNodeBase::printLaplaceMatrix()
{

}

void PoissonSolverNodeBase::print_quad_neighbor_nodes_of_node_t()
{
    ...
    oss2Debug << this->convert2FullPath("nodes_neighbors information")<<"_"<<myRank<<".txt";
    ...
    {
        const quad_neighbor_nodes_of_node_t& qnnn = (*node_neighbors_)[n];
        qnnn.print_debug_global(outFile,this->petsc_gloidx);
    }
    ...
    int myRank2;
    MPI_Comm_rank(MPI_COMM_WORLD, &myRank2);

    std::stringstream oss2Debug2;
    std::string mystr2Debug2;
    oss2Debug2 << this->convert2FullPath("nodes_neighbors_information_local")<<"_"<<myRank2<<".txt";
