...
#include <src/mls_cube3.h>
#include <src/mls_cube2.h>
#else
...
void my_p4est_poisson_nodes_mls_t::setup_negative_laplace_matrix_neumann_wall_1st_order()
{
  preallocate_matrix();
...
void my_p4est_poisson_nodes_mls_t::setup_negative_laplace_rhsvec_neumann_wall_1st_order()
{
  // register for logging purpose
...
void my_p4est_poisson_nodes_mls_t::setup_negative_variable_coeff_laplace_matrix()
{
  preallocate_matrix();
...
#ifdef P4_TO_P8
void my_p4est_poisson_nodes_mls_t::set_phi(std::vector<Vec>& phi, std::vector<Vec>& phi_xx = NULL, std::vector<Vec>& phi_yy = NULL, std::vector<Vec>& phi_zz = NULL)
#else
void my_p4est_poisson_nodes_mls_t::set_phi(std::vector<Vec>& phi, std::vector<Vec>& phi_xx = NULL, std::vector<Vec>& phi_yy = NULL)
#endif
{
  phi_ = &phi;
...
#endif
  {
    ...
    {
      phi_xx_->push_back(Vec());
      ierr = VecCreateGhostNodes(p4est, nodes, &phi_xx_[i]); CHKERRXX(ierr);
      phi_yy_->push_back(Vec());
      ierr = VecCreateGhostNodes(p4est, nodes, &phi_yy_[i]); CHKERRXX(ierr);
#ifdef P4_TO_P8
      phi_zz_->push_back(Vec());
      ierr = VecCreateGhostNodes(p4est, nodes, &phi_zz_[i]); CHKERRXX(ierr);
#endif

#ifdef P4_TO_P8
      node_neighbors_->second_derivatives_central(phi_[i], phi_xx_[i], phi_yy_[i], phi_zz_[i]);
#else
      node_neighbors_->second_derivatives_central(phi_[i], phi_xx_[i], phi_yy_[i]);
#endif
    }
  ...
  for (unsigned int i = 0; i < phi_->size(); i++)
  {
    phi_interp.push_back(my_p4est_interpolation_nodes_t(node_neighbors_));
#ifdef P4_TO_P8
    phi_interp[i].set_input(phi_[i], phi_xx_[i], phi_yy_[i], phi_zz_[i], linear);
#else
    phi_interp[i].set_input(phi_[i], phi_xx_[i], phi_yy_[i], linear);
#endif
  }
}
...
void my_p4est_poisson_nodes_mls_t::construct_domain(){
  ...
  for (int i = 0; i < bc_->size(); i++){
    all_neumann   = all_neuman    && (bc_->at(i).interfaceType() == ROBIN || bc_->at(i).interfaceType() == NEUMANN);
    all_dirichlet = all_dirichlet && (bc_->at(i).interfaceType() == DIRICHLET);
  }

  mixed_bc = !all_dirichlet && !all_neumann;
