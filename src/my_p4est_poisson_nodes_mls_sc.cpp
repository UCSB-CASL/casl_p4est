...
//        if(node_type_p[) {fv_size_x += cube_refinement_; fv_xmin = x_C-0.5*dx_min_;} else {fv_xmin = x_C;}
//        if(!is_node_xpWall(p4est_, ni)) {fv_size_x += cube_refinement_; fv_xmax = x_C+0.5*dx_min_;} else {fv_xmax = x_C;}
...
//        // get coordinates of auxiliary cube nodes
//        int fv_aux_nx = 3;
//        std::vector<double> fv_aux_x(fv_aux_nx);
//        double fv_aux_dx = (fv_xmax-fv_xmin) / (double)(fv_aux_nx - 1);
//        fv_aux_x[0] = fv_xmin;
//        for (short i = 1; i < fv_aux_nx; ++i)
//          fv_aux_x[i] = fv_aux_x[i-1] + fv_aux_dx;

//        int fv_aux_ny = 3;
//        std::vector<double> fv_aux_y(fv_aux_ny);
//        double fv_aux_dy = (fv_ymax-fv_ymin) / (double)(fv_aux_ny - 1);
//        fv_aux_y[0] = fv_ymin;
//        for (short i = 1; i < fv_aux_ny; ++i)
//          fv_aux_y[i] = fv_aux_y[i-1] + fv_aux_dy;
//#ifdef P4_TO_P8
//        int fv_aux_nz = 3;
//        std::vector<double> fv_aux_z(fv_aux_nz);
//        double fv_aux_dz = (fv_zmax-fv_zmin) / (double)(fv_aux_nz - 1);
//        fv_aux_z[0] = fv_zmin;
//        for (short i = 1; i < fv_aux_nz; ++i)
//          fv_aux_z[i] = fv_aux_z[i-1] + fv_aux_dz;
//#endif
//        // sample level-set function at cube nodes and check if crossed
//        for (short phi_idx = 0; phi_idx < num_interfaces_; ++phi_idx)
//        {
...
//#ifdef P4_TO_P8
//          for (short k = 0; k < fv_aux_nz; ++k)
//#endif
//            for (short j = 0; j < fv_aux_ny; ++j)
//              for (short i = 0; i < fv_aux_nx; ++i)
//              {
//#ifdef P4_TO_P8
//                int idx = k*fv_aux_nx*fv_aux_ny + j*fv_aux_nx + i;
//                double phi_val = phi_interp_local[phi_idx]->interpolate(fv_aux_x[i], fv_aux_y[j], fv_aux_z[k]);
//#else
//                int idx = j*fv_aux_nx + i;
//                double phi_val = phi_interp_local[phi_idx]->interpolate(fv_aux_x[i], fv_aux_y[j]);
//#endif
//                is_one_positive = is_one_positive || phi_val > 0;
//                is_one_negative = is_one_negative || phi_val < 0;
//              }
        ...
        if (volume_cut_cell/full_cell_volume > domain_rel_thresh)//domain_rel_thresh)
        {
...
#ifdef P4_TO_P8
              z_m00 += cubes[idx_m]->integrate_in_dir(delta_z_cf_, 0);
              z_p00 += cubes[idx_p]->integrate_in_dir(delta_z_cf_, 1);
#endif
            }
...
#ifdef P4_TO_P8
              z_0m0 += cubes[idx_m]->integrate_in_dir(delta_z_cf_, 2);
              z_0p0 += cubes[idx_p]->integrate_in_dir(delta_z_cf_, 3);
#endif
            }
          ...
          for (short j = 0; j < fv_size_y; ++j)
            for (short i = 0; i < fv_size_x; ++i)
            {
              ...
              x_00m += cubes[idx_m]->integrate_in_dir(delta_x_cf_, 4);
              x_00p += cubes[idx_p]->integrate_in_dir(delta_x_cf_, 5);

              y_00m += cubes[idx_m]->integrate_in_dir(delta_y_cf_, 4);
...
//          if (setup_matrix && (sqrt(SQR(x_C+0.75)+SQR(y_C)+SQR(z_C))<0.2))
//          {
//            std::vector<simplex3_mls_quadratic_t *> simplices;
//            int n_sps = NTETS;
...
//            simplex3_mls_quadratic_vtk::write_simplex_geometry(simplices, to_string("/home/dbochkov/Outputs/nodes_mls/nodes/"), to_string(n));
//          }
...
//            if (xy_derivative_found)
//              for (int i = 0; i < num_neighbors_max_; ++i)
//              {
//                if (map_xy_derivative[i]) w[i] += 2.*weights_xy_derivative[i]*xy_coeff/dx_min_/dy_min_;
//              }
          }
          else
          {
...
#ifdef P4_TO_P8
        for (short k = 0; k < fv_nz; k+=fv_nz-1)
#endif
          for (short j = 0; j < fv_ny; j+=fv_ny-1)
            for (short i = 0; i < fv_nx; i+=fv_nx-1)
            {
#ifdef P4_TO_P8
                int idx = k*fv_nx*fv_ny + j*fv_nx + i;
#else
...
void my_p4est_poisson_nodes_mls_sc_t::get_all_neighbors_(const p4est_locidx_t n, p4est_locidx_t *neighbors, bool *neighbor_exists)
{
  p4est_indep_t *ni = (p4est_indep_t*)sc_array_index(&nodes_->indep_nodes, n);
