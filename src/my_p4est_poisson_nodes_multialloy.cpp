...
my_p4est_poisson_nodes_multialloy_t::my_p4est_poisson_nodes_multialloy_t(const my_p4est_node_neighbors_t *node_neighbors)
...
{
  if (A_t   != NULL) {ierr = MatDestroy(A_t);   CHKERRXX(ierr);}
  if (ksp_t != NULL) {ierr = KSPDestroy(ksp_t); CHKERRXX(ierr);}

  for (short i = 0; i < A_c.size(); ++i)
    if (A_c[i] != NULL) {ierr = MatDestroy(A_c[i]); CHKERRXX(ierr);}

  for (short i = 0; i < ksp_c.size(); ++i)
    if (ksp_c[i] != NULL) {ierr = MatDestroy(ksp_c[i]); CHKERRXX(ierr);}
...
void my_p4est_poisson_nodes_multialloy_t::preallocate_matrix()
{  
  // enable logging for the preallocation
  ierr = PetscLogEventBegin(log_my_p4est_poisson_nodes_matrix_preallocation, A_t, 0, 0, 0); CHKERRXX(ierr);
  ...
  // set up the matrix
  ierr = MatCreate(p4est->mpicomm, &A_t); CHKERRXX(ierr);
  ierr = MatSetType(A_t, MATAIJ); CHKERRXX(ierr);
  ierr = MatSetSizes(A_t, num_owned_local , num_owned_local,
                     num_owned_global, num_owned_global); CHKERRXX(ierr);
  ierr = MatSetFromOptions(A_t); CHKERRXX(ierr);

  A_c.resize(num_comps, Mat());
  for (short i = 0; i < num_comps; ++i)
  {
    ierr = MatCreate(p4est->mpicomm, &A_c[i]); CHKERRXX(ierr);
    ierr = MatSetType(A_c[i], MATAIJ); CHKERRXX(ierr);
    ierr = MatSetSizes(A_c[i], num_owned_local , num_owned_local,
                       num_owned_global, num_owned_global); CHKERRXX(ierr);
    ierr = MatSetFromOptions(A_c[i]); CHKERRXX(ierr);
  }
   ...
   */
  std::vector<PetscInt> d_nnz_c(num_owned_local, 1), o_nnz_c(num_owned_local, 0);
  std::vector<PetscInt> d_nnz_t(num_owned_local, 1), o_nnz_t(num_owned_local, 0);
  ...
  for (p4est_locidx_t n=0; n<num_owned_local; n++)
  {
    ...
    if (qnnn.d_00m_p0*qnnn.d_00m_0p != 0) // node_00m_mm will enter discretization
      qnnn.node_00m_mm < num_owned_local ? d_nnz_t[n]++ : o_nnz_t[n]++;
    if (qnnn.d_00m_m0*qnnn.d_00m_0p != 0) // node_00m_pm will enter discretization
      qnnn.node_00m_pm < num_owned_local ? d_nnz_t[n]++ : o_nnz_t[n]++;
    if (qnnn.d_00m_p0*qnnn.d_00m_0m != 0) // node_00m_mp will enter discretization
      qnnn.node_00m_mp < num_owned_local ? d_nnz_t[n]++ : o_nnz_t[n]++;
    if (qnnn.d_00m_m0*qnnn.d_00m_0m != 0) // node_00m_pp will enter discretization
      qnnn.node_00m_pp < num_owned_local ? d_nnz_t[n]++ : o_nnz_t[n]++;

    if (qnnn.d_00p_p0*qnnn.d_00p_0p != 0) // node_00p_mm will enter discretization
      qnnn.node_00p_mm < num_owned_local ? d_nnz_t[n]++ : o_nnz_t[n]++;
    if (qnnn.d_00p_m0*qnnn.d_00p_0p != 0) // node_00p_pm will enter discretization
      qnnn.node_00p_pm < num_owned_local ? d_nnz_t[n]++ : o_nnz_t[n]++;
    if (qnnn.d_00p_p0*qnnn.d_00p_0m != 0) // node_00p_mp will enter discretization
      qnnn.node_00p_mp < num_owned_local ? d_nnz_t[n]++ : o_nnz_t[n]++;
    if (qnnn.d_00p_m0*qnnn.d_00p_0m != 0) // node_00p_pp will enter discretization
      qnnn.node_00p_pp < num_owned_local ? d_nnz_t[n]++ : o_nnz_t[n]++;
#endif

    // note that concentrations are defined for phi > 0
    if (phi_p[n] > -2.*diag_min)
    {
#ifdef P4_TO_P8
    if (qnnn.d_m00_p0*qnnn.d_m00_0p != 0) // node_m00_mm will enter discretization
      qnnn.node_m00_mm < num_owned_local ? d_nnz_c[n]++ : o_nnz_c[n]++;
    if (qnnn.d_m00_m0*qnnn.d_m00_0p != 0) // node_m00_pm will enter discretization
      qnnn.node_m00_pm < num_owned_local ? d_nnz_c[n]++ : o_nnz_c[n]++;
    if (qnnn.d_m00_p0*qnnn.d_m00_0m != 0) // node_m00_mp will enter discretization
      qnnn.node_m00_mp < num_owned_local ? d_nnz_c[n]++ : o_nnz_c[n]++;
    if (qnnn.d_m00_m0*qnnn.d_m00_0m != 0) // node_m00_pp will enter discretization
      qnnn.node_m00_pp < num_owned_local ? d_nnz_c[n]++ : o_nnz_c[n]++;
#else
    if (qnnn.d_m00_p0 != 0) // node_m00_mm will enter discretization
      qnnn.node_m00_mm < num_owned_local ? d_nnz_c[n]++ : o_nnz_c[n]++;
    if (qnnn.d_m00_m0 != 0) // node_m00_pm will enter discretization
...
#ifdef P4_TO_P8
    if (qnnn.d_p00_p0*qnnn.d_p00_0p != 0) // node_p00_mm will enter discretization
      qnnn.node_p00_mm < num_owned_local ? d_nnz_c[n]++ : o_nnz_c[n]++;
    if (qnnn.d_p00_m0*qnnn.d_p00_0p != 0) // node_p00_pm will enter discretization
      qnnn.node_p00_pm < num_owned_local ? d_nnz_c[n]++ : o_nnz_c[n]++;
    if (qnnn.d_p00_p0*qnnn.d_p00_0m != 0) // node_p00_mp will enter discretization
      qnnn.node_p00_mp < num_owned_local ? d_nnz_c[n]++ : o_nnz_c[n]++;
    if (qnnn.d_p00_m0*qnnn.d_p00_0m != 0) // node_p00_pp will enter discretization
      qnnn.node_p00_pp < num_owned_local ? d_nnz_c[n]++ : o_nnz_c[n]++;
#else
    if (qnnn.d_p00_p0 != 0) // node_p0_m will enter discretization
      qnnn.node_p00_mm < num_owned_local ? d_nnz_c[n]++ : o_nnz_c[n]++;
    if (qnnn.d_p00_m0 != 0) // node_p0_p will enter discretization
...
#ifdef P4_TO_P8
    if (qnnn.d_0m0_p0*qnnn.d_0m0_0p != 0) // node_0m0_mm will enter discretization
      qnnn.node_0m0_mm < num_owned_local ? d_nnz_c[n]++ : o_nnz_c[n]++;
    if (qnnn.d_0m0_m0*qnnn.d_0m0_0p != 0) // node_0m0_pm will enter discretization
      qnnn.node_0m0_pm < num_owned_local ? d_nnz_c[n]++ : o_nnz_c[n]++;
    if (qnnn.d_0m0_p0*qnnn.d_0m0_0m != 0) // node_0m0_mp will enter discretization
      qnnn.node_0m0_mp < num_owned_local ? d_nnz_c[n]++ : o_nnz_c[n]++;
    if (qnnn.d_0m0_m0*qnnn.d_0m0_0m != 0) // node_0m0_pp will enter discretization
      qnnn.node_0m0_pp < num_owned_local ? d_nnz_c[n]++ : o_nnz_c[n]++;
#else
    if (qnnn.d_0m0_p0 != 0) // node_0m_m will enter discretization
      qnnn.node_0m0_mm < num_owned_local ? d_nnz_c[n]++ : o_nnz_c[n]++;
    if (qnnn.d_0m0_m0 != 0) // node_0m_p will enter discretization
...
#ifdef P4_TO_P8
    if (qnnn.d_0p0_p0*qnnn.d_0p0_0p != 0) // node_0p0_mm will enter discretization
      qnnn.node_0p0_mm < num_owned_local ? d_nnz_c[n]++ : o_nnz_c[n]++;
    if (qnnn.d_0p0_m0*qnnn.d_0p0_0p != 0) // node_0p0_pm will enter discretization
      qnnn.node_0p0_pm < num_owned_local ? d_nnz_c[n]++ : o_nnz_c[n]++;
    if (qnnn.d_0p0_p0*qnnn.d_0p0_0m != 0) // node_0p0_mp will enter discretization
      qnnn.node_0p0_mp < num_owned_local ? d_nnz_c[n]++ : o_nnz_c[n]++;
    if (qnnn.d_0p0_m0*qnnn.d_0p0_0m != 0) // node_0p0_pp will enter discretization
      qnnn.node_0p0_pp < num_owned_local ? d_nnz_c[n]++ : o_nnz_c[n]++;
#else
    if (qnnn.d_0p0_p0 != 0) // node_0p_m will enter discretization
      qnnn.node_0p0_mm < num_owned_local ? d_nnz_c[n]++ : o_nnz_c[n]++;
    if (qnnn.d_0p0_m0 != 0) // node_0p_p will enter discretization
...
#ifdef P4_TO_P8
    if (qnnn.d_00m_p0*qnnn.d_00m_0p != 0) // node_00m_mm will enter discretization
      qnnn.node_00m_mm < num_owned_local ? d_nnz_c[n]++ : o_nnz_c[n]++;
    if (qnnn.d_00m_m0*qnnn.d_00m_0p != 0) // node_00m_pm will enter discretization
      qnnn.node_00m_pm < num_owned_local ? d_nnz_c[n]++ : o_nnz_c[n]++;
    if (qnnn.d_00m_p0*qnnn.d_00m_0m != 0) // node_00m_mp will enter discretization
      qnnn.node_00m_mp < num_owned_local ? d_nnz_c[n]++ : o_nnz_c[n]++;
    if (qnnn.d_00m_m0*qnnn.d_00m_0m != 0) // node_00m_pp will enter discretization
      qnnn.node_00m_pp < num_owned_local ? d_nnz_c[n]++ : o_nnz_c[n]++;

    if (qnnn.d_00p_p0*qnnn.d_00p_0p != 0) // node_00p_mm will enter discretization
      qnnn.node_00p_mm < num_owned_local ? d_nnz_c[n]++ : o_nnz_c[n]++;
    if (qnnn.d_00p_m0*qnnn.d_00p_0p != 0) // node_00p_pm will enter discretization
      qnnn.node_00p_pm < num_owned_local ? d_nnz_c[n]++ : o_nnz_c[n]++;
    if (qnnn.d_00p_p0*qnnn.d_00p_0m != 0) // node_00p_mp will enter discretization
      qnnn.node_00p_mp < num_owned_local ? d_nnz_c[n]++ : o_nnz_c[n]++;
    if (qnnn.d_00p_m0*qnnn.d_00p_0m != 0) // node_00p_pp will enter discretization
      qnnn.node_00p_pp < num_owned_local ? d_nnz_c[n]++ : o_nnz_c[n]++;
#endif
    }
  ...
  ierr = MatSeqAIJSetPreallocation(A_t, 0, (const PetscInt*)&d_nnz_t[0]); CHKERRXX(ierr);
  ierr = MatMPIAIJSetPreallocation(A_t, 0, (const PetscInt*)&d_nnz_t[0], 0, (const PetscInt*)&o_nnz_t[0]); CHKERRXX(ierr);

  for (short i = 0; i < num_comps; ++i)
  {
    ierr = MatSeqAIJSetPreallocation(A_c[i], 0, (const PetscInt*)&d_nnz_c[0]); CHKERRXX(ierr);
    ierr = MatMPIAIJSetPreallocation(A_c[i], 0, (const PetscInt*)&d_nnz_c[0], 0, (const PetscInt*)&o_nnz_c[0]); CHKERRXX(ierr);
  }

  ierr = PetscLogEventEnd(log_my_p4est_poisson_nodes_matrix_preallocation, A_t, 0, 0, 0); CHKERRXX(ierr);
}

void my_p4est_poisson_nodes_multialloy_t::preallocate_matrix_t()
{
  // enable logging for the preallocation
  ierr = PetscLogEventBegin(log_my_p4est_poisson_nodes_matrix_preallocation, A_t, 0, 0, 0); CHKERRXX(ierr);
...
// Solving
//-------------------------------------------------------------------------------------------------------------------------------
void my_p4est_poisson_nodes_multialloy_t::solve_t(Vec solution, bool use_nonzero_initial_guess, KSPType ksp_type, PCType pc_type)
{
  ierr = PetscLogEventBegin(log_my_p4est_poisson_nodes_solve, A_t, RHS_t, ksp_t, 0); CHKERRXX(ierr);

  // set ksp type
  ierr = KSPSetType(ksp_t, ksp_type); CHKERRXX(ierr);
  if (use_nonzero_initial_guess)
    ierr = KSPSetInitialGuessNonzero(ksp_t, PETSC_TRUE); CHKERRXX(ierr);
   ...
   */
  if (solve_t_first_time)
  {
    ierr = KSPSetOperators(ksp_t, A_t, A_t, SAME_NONZERO_PATTERN); CHKERRXX(ierr);
  } else {
    ierr = KSPSetOperators(ksp_t, A_t, A_t, SAME_PRECONDITIONER);  CHKERRXX(ierr);
  }

  // set pc type
  PC pc;
  ierr = KSPGetPC(ksp_t, &pc); CHKERRXX(ierr);
  ...
  // Solve the system
  ierr = KSPSetTolerances(ksp_t, 1e-16, PETSC_DEFAULT, PETSC_DEFAULT, PETSC_DEFAULT); CHKERRXX(ierr);
  ierr = KSPSetFromOptions(ksp_t); CHKERRXX(ierr);

  ierr = PetscLogEventBegin(log_my_p4est_poisson_nodes_KSPSolve, ksp_t, RHS_t, solution, 0); CHKERRXX(ierr);

  ierr = KSPSolve(ksp_t, RHS_t, solution); CHKERRXX(ierr);

  ierr = PetscLogEventEnd  (log_my_p4est_poisson_nodes_KSPSolve, ksp_t, RHS_t, solution, 0); CHKERRXX(ierr);
  ...
  ierr = PetscLogEventEnd(log_my_p4est_poisson_nodes_solve, A_t, RHS_t, ksp_t, 0); CHKERRXX(ierr);
}

void my_p4est_poisson_nodes_multialloy_t::solve_c(int comp_idx, Vec solution, bool use_nonzero_initial_guess, KSPType ksp_type, PCType pc_type)
{
  ierr = PetscLogEventBegin(log_my_p4est_poisson_nodes_solve, A_c[comp_idx], RHS_c[comp_idx], ksp_c[comp_idx], 0); CHKERRXX(ierr);

  // set ksp type
  ierr = KSPSetType(ksp_c[comp_idx], ksp_type); CHKERRXX(ierr);
  if (use_nonzero_initial_guess)
    ierr = KSPSetInitialGuessNonzero(ksp_c[comp_idx], PETSC_TRUE); CHKERRXX(ierr);
   ...
   * have to recompute the preconditioner
   */
  if (solve_t_first_time || comp_idx != 0)
  {
    ierr = KSPSetOperators(ksp_c[comp_idx], A_c[comp_idx], A_c[comp_idx], SAME_NONZERO_PATTERN); CHKERRXX(ierr);
  } else {
    ierr = KSPSetOperators(ksp_c[comp_idx], A_c[comp_idx], A_c[comp_idx], SAME_PRECONDITIONER);  CHKERRXX(ierr);
  }

  // set pc type
  PC pc;
  ierr = KSPGetPC(ksp_c[comp_idx], &pc); CHKERRXX(ierr);
  ierr = PCSetType(pc, pc_type); CHKERRXX(ierr);
  ...
  // Solve the system
  ierr = KSPSetTolerances(ksp_c[comp_idx], 1e-16, PETSC_DEFAULT, PETSC_DEFAULT, PETSC_DEFAULT); CHKERRXX(ierr);
  ierr = KSPSetFromOptions(ksp_c[comp_idx]); CHKERRXX(ierr);

  ierr = PetscLogEventBegin(log_my_p4est_poisson_nodes_KSPSolve, ksp_c[comp_idx], RHS_c[comp_idx], solution, 0); CHKERRXX(ierr);

  ierr = KSPSolve(ksp_c[comp_idx], RHS_c[comp_idx], solution); CHKERRXX(ierr);

  ierr = PetscLogEventEnd  (log_my_p4est_poisson_nodes_KSPSolve, ksp_c[comp_idx], RHS_c[comp_idx], solution, 0); CHKERRXX(ierr);
...
//-------------------------------------------------------------------------------------------------------------------------------
void my_p4est_poisson_nodes_multialloy_t::pre_set_up_linear_system()
{
  preallocate_matrices();

  // register for logging purpose
  ierr = PetscLogEventBegin(log_my_p4est_poisson_nodes_matrix_setup, A_t, 0, 0, 0); CHKERRXX(ierr);
  ...
  {    
    ...
    // check if node is wall and Dirichlet BC
    if (is_node_Wall(p4est, ni))
    {
#ifdef P4_TO_P8
      bool is_dirichlet_bc = (bc_t->wallType(x_C,y_C,z_C) == DIRICHLET);
#else
      bool is_dirichlet_bc = (bc_t->wallType(x_C,y_C) == DIRICHLET);
#endif

      if (is_dirichlet_bc) {
        ierr = MatSetValue(A_t, node_000_g, node_000_g, bc_strength, INSERT_VALUES); CHKERRXX(ierr);
      }

      for (short i = 0; i < num_comps; ++i)
      {
#ifdef P4_TO_P8
        bool is_dirichlet_bc = (bc_c->at(i).wallType(x_C,y_C,z_C) == DIRICHLET);
#else
        bool is_dirichlet_bc = (bc_c->at(i).wallType(x_C,y_C) == DIRICHLET);
#endif
        if (is_dirichlet_bc) {
          ierr = MatSetValue(A_c[i], node_000_g, node_000_g, bc_strength, INSERT_VALUES); CHKERRXX(ierr);
        }
      }
    }
    ...
    // check if too close to the interface
    if((ABS(phi_000)<eps && bc_->interfaceType() == DIRICHLET) ){
      ierr = MatSetValue(A_c[0], node_000_g, node_000_g, bc_strength, ADD_VALUES); CHKERRXX(ierr);
      pointwise_c0_gamma[n].push_back(0, 0, 1);
    }
    ...
    if (fabs(phi_000) < 2.*diag_min)
    {
      ...
      if(!is_node_xmWall(p4est, ni)) {fv_size_x++; fv_xmin = x_C-0.5*dx_min;} else {fv_xmin = x_C;}
      if(!is_node_xpWall(p4est, ni)) {fv_size_x++; fv_xmax = x_C+0.5*dx_min;} else {fv_xmax = x_C;}

      if(!is_node_ymWall(p4est, ni)) {fv_size_y++; fv_ymin = y_C-0.5*dy_min;} else {fv_ymin = y_C;}
      if(!is_node_ypWall(p4est, ni)) {fv_size_y++; fv_ymax = y_C+0.5*dy_min;} else {fv_ymax = y_C;}
#ifdef P4_TO_P8
      if(!is_node_zmWall(p4est, ni)) {fv_size_z++; fv_zmin = z_C-0.5*dz_min;} else {fv_zmin = z_C;}
      if(!is_node_zpWall(p4est, ni)) {fv_size_z++; fv_zmax = z_C+0.5*dz_min;} else {fv_zmax = z_C;}
#endif

      // get coordinates of cube nodes
      fv_x.resize(fv_size_x, fv_xmin);
      for (short i = 1; i < fv_size_x; ++i)
        fv_x[i] = fv_x[i-1] + 0.5*dx_min;

      fv_y.resize(fv_size_y, fv_ymin);
      for (short i = 1; i < fv_size_y; ++i)
        fv_y[i] = fv_y[i-1] + 0.5*dy_min;
#ifdef P4_TO_P8
      fv_z.resize(fv_size_z, fv_zmin);
      for (short i = 1; i < fv_size_z; ++i)
        fv_z[i] = fv_z[i-1] + 0.5*dz_min;
#endif

      // sample level-set function at cube nodes and check if crossed
#ifdef P4_TO_P8
      phi_cube.resize(fv_size_x*fv_size_y*fv_size_z, -1);
#else
      phi_cube.resize(fv_size_x*fv_size_y, -1);
#endif
      ...
      for (short k = 0; k < fv_size_z; ++k)
        ...
        for (short j = 0; j < fv_size_y; ++j)
          ...
          {
#ifdef P4_TO_P8
            int idx = k*fv_size_z*fv_size_y + j*fv_size_y + i;
            phi_cube[idx] = phi_interp(fv_x[i], fv_y[j], fv_z[k]);
#else
            int idx = j*fv_size_y + i;
            phi_cube[idx] = phi_interp(fv_x[i], fv_y[j]);
#endif
            is_one_positive = is_one_positive || phi_cube[idx] > 0;
            is_one_negative = is_one_negative || phi_cube[idx] < 0;
          }
    }
    ...
    if (phi_000 < 0.) {
      ierr = MatSetValue(A_c[0], node_000_g, node_000_g, bc_strength, INSERT_VALUES); CHKERRXX(ierr);
      if (!is_ngbd_crossed_neumann)
        for (short i = 1; i < num_comps; ++i) {
          ierr = MatSetValue(A_c[i], node_000_g, node_000_g, bc_strength, INSERT_VALUES); CHKERRXX(ierr);
        }
    }

    node_near_interface[n] = is_ngbd_crossed_neumann;

    // Compute matrix coefficients away from interface
#ifdef P4_TO_P8
    //------------------------------------
    ...
    double w_m00_m0 = w_m00*d_m00_m0/(d_m00_m0+d_m00_p0);
    double w_m00_p0 = w_m00*d_m00_p0/(d_m00_m0+d_m00_p0);

    double w_p00_m0 = w_p00*d_p00_m0/(d_p00_m0+d_p00_p0);
    double w_p00_p0 = w_p00*d_p00_p0/(d_p00_m0+d_p00_p0);

    double w_0m0_m0 = w_0m0*d_0m0_m0/(d_0m0_m0+d_0m0_p0);
    double w_0m0_p0 = w_0m0*d_0m0_p0/(d_0m0_m0+d_0m0_p0);

    double w_0p0_m0 = w_0p0*d_0p0_m0/(d_0p0_m0+d_0p0_p0);
    double w_0p0_p0 = w_0p0*d_0p0_p0/(d_0p0_m0+d_0p0_p0);
#endif


    // Add coefficients into A_t
    ierr = MatSetValue(A_t, node_000_g, node_000_g, 1.0, ADD_VALUES); CHKERRXX(ierr);
#ifdef P4_TO_P8
    double diag_term = 1.0 - dt*lambda*(w_m00+w_p00+w_0m0+w_0p0+w_00m+w_00p);
    double total_scalling = dt*lambda/diag_term;
    if (fabs(w_m00_mm) > EPS) {ierr = MatSetValue(A_t, node_000_g, petsc_gloidx[node_m00_mm], total_scalling*w_m00_mm, ADD_VALUES); CHKERRXX(ierr);}
    if (fabs(w_m00_mp) > EPS) {ierr = MatSetValue(A_t, node_000_g, petsc_gloidx[node_m00_mp], total_scalling*w_m00_mp, ADD_VALUES); CHKERRXX(ierr);}
    if (fabs(w_m00_pm) > EPS) {ierr = MatSetValue(A_t, node_000_g, petsc_gloidx[node_m00_pm], total_scalling*w_m00_pm, ADD_VALUES); CHKERRXX(ierr);}
    if (fabs(w_m00_pp) > EPS) {ierr = MatSetValue(A_t, node_000_g, petsc_gloidx[node_m00_pp], total_scalling*w_m00_pp, ADD_VALUES); CHKERRXX(ierr);}

    if (fabs(w_p00_mm) > EPS) {ierr = MatSetValue(A_t, node_000_g, petsc_gloidx[node_p00_mm], total_scalling*w_p00_mm, ADD_VALUES); CHKERRXX(ierr);}
    if (fabs(w_p00_mp) > EPS) {ierr = MatSetValue(A_t, node_000_g, petsc_gloidx[node_p00_mp], total_scalling*w_p00_mp, ADD_VALUES); CHKERRXX(ierr);}
    if (fabs(w_p00_pm) > EPS) {ierr = MatSetValue(A_t, node_000_g, petsc_gloidx[node_p00_pm], total_scalling*w_p00_pm, ADD_VALUES); CHKERRXX(ierr);}
    if (fabs(w_p00_pp) > EPS) {ierr = MatSetValue(A_t, node_000_g, petsc_gloidx[node_p00_pp], total_scalling*w_p00_pp, ADD_VALUES); CHKERRXX(ierr);}

    if (fabs(w_0m0_mm) > EPS) {ierr = MatSetValue(A_t, node_000_g, petsc_gloidx[node_0m0_mm], total_scalling*w_0m0_mm, ADD_VALUES); CHKERRXX(ierr);}
    if (fabs(w_0m0_mp) > EPS) {ierr = MatSetValue(A_t, node_000_g, petsc_gloidx[node_0m0_mp], total_scalling*w_0m0_mp, ADD_VALUES); CHKERRXX(ierr);}
    if (fabs(w_0m0_pm) > EPS) {ierr = MatSetValue(A_t, node_000_g, petsc_gloidx[node_0m0_pm], total_scalling*w_0m0_pm, ADD_VALUES); CHKERRXX(ierr);}
    if (fabs(w_0m0_pp) > EPS) {ierr = MatSetValue(A_t, node_000_g, petsc_gloidx[node_0m0_pp], total_scalling*w_0m0_pp, ADD_VALUES); CHKERRXX(ierr);}

    if (fabs(w_0p0_mm) > EPS) {ierr = MatSetValue(A_t, node_000_g, petsc_gloidx[node_0p0_mm], total_scalling*w_0p0_mm, ADD_VALUES); CHKERRXX(ierr);}
    if (fabs(w_0p0_mp) > EPS) {ierr = MatSetValue(A_t, node_000_g, petsc_gloidx[node_0p0_mp], total_scalling*w_0p0_mp, ADD_VALUES); CHKERRXX(ierr);}
    if (fabs(w_0p0_pm) > EPS) {ierr = MatSetValue(A_t, node_000_g, petsc_gloidx[node_0p0_pm], total_scalling*w_0p0_pm, ADD_VALUES); CHKERRXX(ierr);}
    if (fabs(w_0p0_pp) > EPS) {ierr = MatSetValue(A_t, node_000_g, petsc_gloidx[node_0p0_pp], total_scalling*w_0p0_pp, ADD_VALUES); CHKERRXX(ierr);}

    if (fabs(w_00m_mm) > EPS) {ierr = MatSetValue(A_t, node_000_g, petsc_gloidx[node_00m_mm], total_scalling*w_00m_mm, ADD_VALUES); CHKERRXX(ierr);}
    if (fabs(w_00m_mp) > EPS) {ierr = MatSetValue(A_t, node_000_g, petsc_gloidx[node_00m_mp], total_scalling*w_00m_mp, ADD_VALUES); CHKERRXX(ierr);}
    if (fabs(w_00m_pm) > EPS) {ierr = MatSetValue(A_t, node_000_g, petsc_gloidx[node_00m_pm], total_scalling*w_00m_pm, ADD_VALUES); CHKERRXX(ierr);}
    if (fabs(w_00m_pp) > EPS) {ierr = MatSetValue(A_t, node_000_g, petsc_gloidx[node_00m_pp], total_scalling*w_00m_pp, ADD_VALUES); CHKERRXX(ierr);}

    if (fabs(w_00p_mm) > EPS) {ierr = MatSetValue(A_t, node_000_g, petsc_gloidx[node_00p_mm], total_scalling*w_00p_mm, ADD_VALUES); CHKERRXX(ierr);}
    if (fabs(w_00p_mp) > EPS) {ierr = MatSetValue(A_t, node_000_g, petsc_gloidx[node_00p_mp], total_scalling*w_00p_mp, ADD_VALUES); CHKERRXX(ierr);}
    if (fabs(w_00p_pm) > EPS) {ierr = MatSetValue(A_t, node_000_g, petsc_gloidx[node_00p_pm], total_scalling*w_00p_pm, ADD_VALUES); CHKERRXX(ierr);}
    if (fabs(w_00p_pp) > EPS) {ierr = MatSetValue(A_t, node_000_g, petsc_gloidx[node_00p_pp], total_scalling*w_00p_pp, ADD_VALUES); CHKERRXX(ierr);}
#else
    double diag_term = 1.0 - dt*lambda*(w_m00+w_p00+w_0m0+w_0p0);
    double total_scalling = dt*lambda/diag_term;
    if (fabs(w_m00_m0) > EPS) ierr = MatSetValue(A_t, node_000_g, node_m00_pm_g, total_scalling*w_m00_m0, ADD_VALUES); CHKERRXX(ierr);
    if (fabs(w_m00_p0) > EPS) ierr = MatSetValue(A_t, node_000_g, node_m00_mm_g, total_scalling*w_m00_p0, ADD_VALUES); CHKERRXX(ierr);

    if (fabs(w_p00_m0) > EPS) ierr = MatSetValue(A_t, node_000_g, node_p00_pm_g, total_scalling*w_p00_m0, ADD_VALUES); CHKERRXX(ierr);
    if (fabs(w_p00_p0) > EPS) ierr = MatSetValue(A_t, node_000_g, node_p00_mm_g, total_scalling*w_p00_p0, ADD_VALUES); CHKERRXX(ierr);

    if (fabs(w_0m0_m0) > EPS) ierr = MatSetValue(A_t, node_000_g, node_0m0_pm_g, total_scalling*w_0m0_m0, ADD_VALUES); CHKERRXX(ierr);
    if (fabs(w_0m0_p0) > EPS) ierr = MatSetValue(A_t, node_000_g, node_0m0_mm_g, total_scalling*w_0m0_p0, ADD_VALUES); CHKERRXX(ierr);

    if (fabs(w_0p0_m0) > EPS) ierr = MatSetValue(A_t, node_000_g, node_0p0_pm_g, total_scalling*w_0p0_m0, ADD_VALUES); CHKERRXX(ierr);
    if (fabs(w_0p0_p0) > EPS) ierr = MatSetValue(A_t, node_000_g, node_0p0_mm_g, total_scalling*w_0p0_p0, ADD_VALUES); CHKERRXX(ierr);
#endif
    scalling_t[n] = diag_term;

    // Add coefficients into A_c
    if (phi_000 > 0.)
      for (short i = 0; i < num_comps; ++i)
      {
        if (i == 0 && !is_ngbd_crossed_dirichlet || i > 0 && !is_ngbd_crossed_neumann)
        {
          ierr = MatSetValue(A_t, node_000_g, node_000_g, 1.0, ADD_VALUES); CHKERRXX(ierr);
#ifdef P4_TO_P8
          double diag_term = 1.0 - dt*Dl[i]*(w_m00+w_p00+w_0m0+w_0p0+w_00m+w_00p);
          double total_scalling = dt*Dl[i]/diag_term;
          if (fabs(w_m00_mm) > EPS) {ierr = MatSetValue(A_t, node_000_g, petsc_gloidx[node_m00_mm], total_scalling*w_m00_mm, ADD_VALUES); CHKERRXX(ierr);}
          if (fabs(w_m00_mp) > EPS) {ierr = MatSetValue(A_t, node_000_g, petsc_gloidx[node_m00_mp], total_scalling*w_m00_mp, ADD_VALUES); CHKERRXX(ierr);}
          if (fabs(w_m00_pm) > EPS) {ierr = MatSetValue(A_t, node_000_g, petsc_gloidx[node_m00_pm], total_scalling*w_m00_pm, ADD_VALUES); CHKERRXX(ierr);}
          if (fabs(w_m00_pp) > EPS) {ierr = MatSetValue(A_t, node_000_g, petsc_gloidx[node_m00_pp], total_scalling*w_m00_pp, ADD_VALUES); CHKERRXX(ierr);}
          ...
          if (fabs(w_00p_pp) > EPS) {ierr = MatSetValue(A_t, node_000_g, petsc_gloidx[node_00p_pp], total_scalling*w_00p_pp, ADD_VALUES); CHKERRXX(ierr);}
#else
          double diag_term = 1.0 - dt*Dl[i]*(w_m00+w_p00+w_0m0+w_0p0);
          double total_scalling = dt*Dl[i]/diag_term;
          if (fabs(w_m00_m0) > EPS) ierr = MatSetValue(A_t, node_000_g, node_m00_pm_g, total_scalling*w_m00_m0, ADD_VALUES); CHKERRXX(ierr);
          if (fabs(w_m00_p0) > EPS) ierr = MatSetValue(A_t, node_000_g, node_m00_mm_g, total_scalling*w_m00_p0, ADD_VALUES); CHKERRXX(ierr);
    ...
    // Compute matrix coefficients for c0 near the interface
    if (is_ngbd_crossed_dirichlet && phi_000 > 0.)
    {
      ...
      if(!is_interface_m00)
      {
        ...
        if (w_m00_mm != 0) {ierr = MatSetValue(A, node_000_g, petsc_gloidx[node_m00_mm], total_scalling*w_m00_mm, ADD_VALUES); CHKERRXX(ierr);}
        if (w_m00_mp != 0) {ierr = MatSetValue(A, node_000_g, petsc_gloidx[node_m00_mp], total_scalling*w_m00_mp, ADD_VALUES); CHKERRXX(ierr);}
        if (w_m00_pm != 0) {ierr = MatSetValue(A, node_000_g, petsc_gloidx[node_m00_pm], total_scalling*w_m00_pm, ADD_VALUES); CHKERRXX(ierr);}
        if (w_m00_pp != 0) {ierr = MatSetValue(A, node_000_g, petsc_gloidx[node_m00_pp], total_scalling*w_m00_pp, ADD_VALUES); CHKERRXX(ierr);}
      }
      ...
      if(!is_interface_p00)
      {
        ...
        if (w_p00_mm != 0) {ierr = MatSetValue(A, node_000_g, petsc_gloidx[node_p00_mm], total_scalling*w_p00_mm, ADD_VALUES); CHKERRXX(ierr);}
        if (w_p00_mp != 0) {ierr = MatSetValue(A, node_000_g, petsc_gloidx[node_p00_mp], total_scalling*w_p00_mp, ADD_VALUES); CHKERRXX(ierr);}
        if (w_p00_pm != 0) {ierr = MatSetValue(A, node_000_g, petsc_gloidx[node_p00_pm], total_scalling*w_p00_pm, ADD_VALUES); CHKERRXX(ierr);}
        if (w_p00_pp != 0) {ierr = MatSetValue(A, node_000_g, petsc_gloidx[node_p00_pp], total_scalling*w_p00_pp, ADD_VALUES); CHKERRXX(ierr);}
      }
      ...
      if(!is_interface_0m0)
      {
        ...
        if (w_0m0_mm != 0) {ierr = MatSetValue(A, node_000_g, petsc_gloidx[node_0m0_mm], total_scalling*w_0m0_mm, ADD_VALUES); CHKERRXX(ierr);}
        if (w_0m0_mp != 0) {ierr = MatSetValue(A, node_000_g, petsc_gloidx[node_0m0_mp], total_scalling*w_0m0_mp, ADD_VALUES); CHKERRXX(ierr);}
        if (w_0m0_pm != 0) {ierr = MatSetValue(A, node_000_g, petsc_gloidx[node_0m0_pm], total_scalling*w_0m0_pm, ADD_VALUES); CHKERRXX(ierr);}
        if (w_0m0_pp != 0) {ierr = MatSetValue(A, node_000_g, petsc_gloidx[node_0m0_pp], total_scalling*w_0m0_pp, ADD_VALUES); CHKERRXX(ierr);}
      }
      ...
      if(!is_interface_0p0)
      {
        ...
        if (w_0p0_mm != 0) {ierr = MatSetValue(A, node_000_g, petsc_gloidx[node_0p0_mm], total_scalling*w_0p0_mm, ADD_VALUES); CHKERRXX(ierr);}
        if (w_0p0_mp != 0) {ierr = MatSetValue(A, node_000_g, petsc_gloidx[node_0p0_mp], total_scalling*w_0p0_mp, ADD_VALUES); CHKERRXX(ierr);}
        if (w_0p0_pm != 0) {ierr = MatSetValue(A, node_000_g, petsc_gloidx[node_0p0_pm], total_scalling*w_0p0_pm, ADD_VALUES); CHKERRXX(ierr);}
        if (w_0p0_pp != 0) {ierr = MatSetValue(A, node_000_g, petsc_gloidx[node_0p0_pp], total_scalling*w_0p0_pp, ADD_VALUES); CHKERRXX(ierr);}
      }
      ...
      if(!is_interface_00m)
      {
        ...
        if (w_00m_mm != 0) {ierr = MatSetValue(A, node_000_g, petsc_gloidx[node_00m_mm], total_scalling*w_00m_mm, ADD_VALUES); CHKERRXX(ierr);}
        if (w_00m_mp != 0) {ierr = MatSetValue(A, node_000_g, petsc_gloidx[node_00m_mp], total_scalling*w_00m_mp, ADD_VALUES); CHKERRXX(ierr);}
        if (w_00m_pm != 0) {ierr = MatSetValue(A, node_000_g, petsc_gloidx[node_00m_pm], total_scalling*w_00m_pm, ADD_VALUES); CHKERRXX(ierr);}
        if (w_00m_pp != 0) {ierr = MatSetValue(A, node_000_g, petsc_gloidx[node_00m_pp], total_scalling*w_00m_pp, ADD_VALUES); CHKERRXX(ierr);}
      }
      ...
      if(!is_interface_00p)
      {
        ...
        if (w_00p_mm != 0) {ierr = MatSetValue(A, node_000_g, petsc_gloidx[node_00p_mm], total_scalling*w_00p_mm, ADD_VALUES); CHKERRXX(ierr);}
        if (w_00p_mp != 0) {ierr = MatSetValue(A, node_000_g, petsc_gloidx[node_00p_mp], total_scalling*w_00p_mp, ADD_VALUES); CHKERRXX(ierr);}
        if (w_00p_pm != 0) {ierr = MatSetValue(A, node_000_g, petsc_gloidx[node_00p_pm], total_scalling*w_00p_pm, ADD_VALUES); CHKERRXX(ierr);}
        if (w_00p_pp != 0) {ierr = MatSetValue(A, node_000_g, petsc_gloidx[node_00p_pp], total_scalling*w_00p_pp, ADD_VALUES); CHKERRXX(ierr);}
      }
#else
      //---------------------------------------------------------------------
      // Shortley-Weller method, dimension by dimension
      ...
      double diag_term = 1.0 - dt*Dl[0]*(w_m00+w_p00+w_0m0+w_0p0);
      double total_scalling = dt*Dl[0]/diag_term;

      scalling_c[0][n] = diag_term;
      ...
      if(!is_interface_m00 && !is_node_xmWall(p4est, ni)) {
        ...
        if (d_m00_m0 != 0) ierr = MatSetValue(A, node_000_g, node_m00_pm_g, total_scalling*w_m00*d_m00_m0/(d_m00_m0+d_m00_p0), ADD_VALUES); CHKERRXX(ierr);
        if (d_m00_p0 != 0) ierr = MatSetValue(A, node_000_g, node_m00_mm_g, total_scalling*w_m00*d_m00_p0/(d_m00_m0+d_m00_p0), ADD_VALUES); CHKERRXX(ierr);
      }
      if(!is_interface_p00 && !is_node_xpWall(p4est, ni)) {
        ...
        if (d_p00_m0 != 0) ierr = MatSetValue(A, node_000_g, node_p00_pm_g, total_scalling*w_p00*d_p00_m0/(d_p00_m0+d_p00_p0), ADD_VALUES); CHKERRXX(ierr);
        if (d_p00_p0 != 0) ierr = MatSetValue(A, node_000_g, node_p00_mm_g, total_scalling*w_p00*d_p00_p0/(d_p00_m0+d_p00_p0), ADD_VALUES); CHKERRXX(ierr);
      }
      if(!is_interface_0m0 && !is_node_ymWall(p4est, ni)) {
        ...
        if (d_0m0_m0 != 0) ierr = MatSetValue(A, node_000_g, node_0m0_pm_g, total_scalling*w_0m0*d_0m0_m0/(d_0m0_m0+d_0m0_p0), ADD_VALUES); CHKERRXX(ierr);
        if (d_0m0_p0 != 0) ierr = MatSetValue(A, node_000_g, node_0m0_mm_g, total_scalling*w_0m0*d_0m0_p0/(d_0m0_m0+d_0m0_p0), ADD_VALUES); CHKERRXX(ierr);
      }
      if(!is_interface_0p0 && !is_node_ypWall(p4est, ni)) {
        ...
        if (d_0p0_m0 != 0) ierr = MatSetValue(A, node_000_g, node_0p0_pm_g, total_scalling*w_0p0*d_0p0_m0/(d_0p0_m0+d_0p0_p0), ADD_VALUES); CHKERRXX(ierr);
        if (d_0p0_p0 != 0) ierr = MatSetValue(A, node_000_g, node_0p0_mm_g, total_scalling*w_0p0*d_0p0_p0/(d_0p0_m0+d_0p0_p0), ADD_VALUES); CHKERRXX(ierr);
      }
#endif
      // save info about interface points
      ...
      // far away from the interface
      if(phi_000>0. &&  ((!is_ngbd_crossed_neumann && is_one_positive) || bc_->interfaceType() == DIRICHLET )){
      ...
      if (is_ngbd_crossed_neumann && (bc_->interfaceType() == NEUMANN || bc_->interfaceType() == ROBIN) )
      {
...
#else
        /* using four cubes instead of one:
         * mm0 m00 0m0 000
         * mp0 0p0 m00 000
         * pm0 0m0 p00 000
         * pp0 p00 0p0 000
        ...
        double x0 = x_C, xm = cube.x0, xp = cube.x1;
        double y0 = y_C, ym = cube.y0, yp = cube.y1;
...
//          double s_0p0 = dx * fraction_Interval_Covered_By_Irregular_Domain(P_mpm, P_ppm, dx, dy);
          ...
          double s_m00 = 0.5 * dy * (fraction_Interval_Covered_By_Irregular_Domain(P_m00, P_mm0, dx, dy) +
                                     fraction_Interval_Covered_By_Irregular_Domain(P_m00, P_mp0, dx, dy));

          double s_p00 = 0.5 * dy * (fraction_Interval_Covered_By_Irregular_Domain(P_p00, P_pm0, dx, dy) +
                                     fraction_Interval_Covered_By_Irregular_Domain(P_p00, P_pp0, dx, dy));

          double s_0m0 = 0.5 * dx * (fraction_Interval_Covered_By_Irregular_Domain(P_0m0, P_mm0, dx, dy) +
                                     fraction_Interval_Covered_By_Irregular_Domain(P_0m0, P_pm0, dx, dy));

          double s_0p0 = 0.5 * dx * (fraction_Interval_Covered_By_Irregular_Domain(P_0p0, P_mp0, dx, dy) +
...
//            else                           w_000 += mu_*robin_coef_p[n]*interface_area;
      ...
      if (!is_ngbd_crossed_neumann && (bc_->interfaceType() == NEUMANN || bc_->interfaceType() == ROBIN) && phi_000<0. && is_one_positive)
      {
        ierr = MatSetValue(A, node_000_g, node_000_g, 1.0, ADD_VALUES); CHKERRXX(ierr);
...
void my_p4est_poisson_nodes_multialloy_t::setup_negative_laplace_rhsvec()
{
  // register for logging purpose
...
//            }
          ...
          for (short i = 0; i < P4EST_CHILDREN; ++i)
          {
            ...
            switch (i) {
              case 0: bc_value.val00 = bc_->interfaceValue(xm,ym); bc_value.val01 = bc_->interfaceValue(xm,y0); bc_value.val10 = bc_->interfaceValue(x0,ym); bc_value.val11 = bc_->interfaceValue(x0,y0); break;
              case 1: bc_value.val00 = bc_->interfaceValue(xm,yp); bc_value.val01 = bc_->interfaceValue(x0,yp); bc_value.val10 = bc_->interfaceValue(xm,y0); bc_value.val11 = bc_->interfaceValue(x0,y0); break;
              case 2: bc_value.val00 = bc_->interfaceValue(xp,ym); bc_value.val01 = bc_->interfaceValue(x0,ym); bc_value.val10 = bc_->interfaceValue(xp,y0); bc_value.val11 = bc_->interfaceValue(x0,y0); break;
              case 3: bc_value.val00 = bc_->interfaceValue(xp,yp); bc_value.val01 = bc_->interfaceValue(xp,y0); bc_value.val10 = bc_->interfaceValue(x0,yp); bc_value.val11 = bc_->interfaceValue(x0,y0); break;
            }
...
void my_p4est_poisson_nodes_multialloy_t::setup_negative_variable_coeff_laplace_matrix()
{
  preallocate_matrix();
...
void my_p4est_poisson_nodes_multialloy_t::setup_negative_variable_coeff_laplace_rhsvec()
{
  // register for logging purpose
  ierr = PetscLogEventBegin(log_my_p4est_poisson_nodes_rhsvec_setup, 0, 0, 0, 0); CHKERRXX(ierr);
...
#ifdef P4_TO_P8
void my_p4est_poisson_nodes_multialloy_t::set_phi(Vec phi, Vec phi_xx, Vec phi_yy, Vec phi_zz)
#else
void my_p4est_poisson_nodes_multialloy_t::set_phi(Vec phi, Vec phi_xx, Vec phi_yy)
#endif
{
  phi_ = phi;
...
#ifdef P4_TO_P8
void my_p4est_poisson_nodes_multialloy_t::set_mu(Vec mue, Vec mue_xx, Vec mue_yy, Vec mue_zz)
#else
void my_p4est_poisson_nodes_multialloy_t::set_mu(Vec mue, Vec mue_xx, Vec mue_yy)
#endif
{
  mue_ = mue;
  is_matrix_computed = false;
...
void my_p4est_poisson_nodes_multialloy_t::shift_to_exact_solution(Vec sol, Vec uex){
#ifdef CASL_THROWS
  if (!matrix_has_nullspace)
