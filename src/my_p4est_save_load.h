 ...
 * \param dtype             [in]    data sampling dype: either NODE_DATA, CELL_DATA or FACE_DATA
 ...
 * [NOTE 2:]  the ghost structure is required for all types of data (i.e. dtype == NODE_DATA, CELL_DATA OR FACE_DATA)
 * [NOTE 3:]  the nodes structure is required only for the NODE_DATA type (the pointer can be NULL otherwise)
 ...
 * [NOTE:]  nodes can be NULL if no NODE_DATA is loaded;
 * [NOTE:]  modified from the function taken from src/dm/examples/tutorials/ex15.c in the Petsc distribution
 * Developer: Raphael Egan (raphaelegan@ucsb.edu)
 */
 ...
 *                                      2) unsigned int nvecs: the number of PETSc vector(s) within the (grouped) exportation (greater than or equal to 1)
 ...
 *                                        2) int dtype: type of data-sampling, either NODE_DATA, CELL_DATA or FACE_DATA
 *                                        3) unsigned int nvecs: the number of PETSc vector(s) within the (grouped) load (greater than or equal to 1)
 ...
 *                                        for each load of (grouped) PETSc vector(s), the function expects
 *                                        1) const char* filename: the name of the file from which the (grouped) PETSc vector(s) will be loaded (within the folder)
 *                                        2) int dtype: type of data-sampling, either NODE_DATA, CELL_DATA (CANNOT be FACE_DATA in this case)
 *                                        3) unsigned int nvecs: the number of PETSc vector(s) within the (grouped) load (greater than or equal to 1)
 *                                        4) Vec* x: pointer to the Vec object (if nvecs = 1) or the array of Vec objects (if nvecs > 1) to be filled with the loaded vector(s)
 * Developer: Raphael Egan (raphaelegan@ucsb.edu)
 */
