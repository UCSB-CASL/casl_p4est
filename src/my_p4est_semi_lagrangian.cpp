...
#endif
{
  ...
  my_p4est_node_neighbors_t qnnn(&hierarchy, nodes_,periodic_xyz);
...
#ifdef P4_TO_P8
void SemiLagrangian::update_p4est_second_order_uniform_inside_domain(Vec vx, Vec vy, Vec vz, double dt, Vec &phi, Vec phi_xx, Vec phi_yy, Vec phi_zz)
#else
void SemiLagrangian::update_p4est_second_order_uniform_inside_domain(Vec vx, Vec vy, double dt, Vec &phi, Vec phi_xx, Vec phi_yy)
#endif
{
  ...
  for( int iter = 0; iter < nb_iter; ++iter )
  {
    ...
    // here the modification from the original implementation
    my_p4est_refine (p4est_np1, P4EST_FALSE, refine_criteria_sl_uniform_inside_domain , NULL);

    // since we still use one level set here so splitting_criteria_t has not been changed
    // if more than one level set is to be used,
    // in addition to the third argument of the function my_p4est_refine
    // a suitable overriden data structure must be defined instead of splitting_criteria_t
    // this data structure should carry more than one level set
    // and all the data such that refine_fn could work correctly
    // in the sense that p4est_np1->user_pointer points to the right data structure
    // coupled with the refine function
...
//#ifdef P4_TO_P8
//void SemiLagrangian::update_p4est_second_order_uniform_inside_domain_with_internal_interface(Vec vx, Vec vy, Vec vz, double dt, Vec &phi_domain_shape,Vec &phi_internal_interface,   Vec phi_xx, Vec phi_yy, Vec phi_zz)
//#else
//void SemiLagrangian::update_p4est_second_order_uniform_inside_domain_with_internal_interface(Vec vx, Vec vy, double dt, Vec &phi_,Vec &phi_domain_shape,Vec &phi_internal_interface,  Vec phi_xx, Vec phi_yy)
//#endif
//{
