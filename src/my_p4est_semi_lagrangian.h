...
class SemiLagrangian
{
  ...
  static p4est_bool_t refine_criteria_sl_uniform_inside_domain(p4est_t *p4est, p4est_topidx_t which_tree, p4est_quadrant_t *quad)
  {
    splitting_criteria_update_t *data = (splitting_criteria_update_t*) p4est->user_pointer;
    ...
    else
    {
     ...
     data->hierarchy->find_smallest_quadrant_containing_point(xyz, quad_tmp, remote_matches);
      ...
      bool IsAllNegative=true;
       ...
       for(short j=0; j<P4EST_CHILDREN; ++j)
       {
         IsAllNegative =IsAllNegative && (phi_tmp[ q2n[ quad_tmp_idx*P4EST_CHILDREN + j ] ] <=0);
       }
...
public:
  ...
  void update_p4est_second_order_uniform_inside_domain(Vec vx, Vec vy, Vec vz, double dt, Vec &phi, Vec phi_xx = NULL, Vec phi_yy = NULL, Vec phi_zz = NULL);
  ...
  void update_p4est_second_order_uniform_inside_domain(Vec vx, Vec vy,double dt, Vec &phi, Vec phi_xx = NULL, Vec phi_yy = NULL);
  void update_p4est_second_order(const CF_2& vx, const CF_2& vy, double dt, Vec &phi, Vec phi_xx = NULL, Vec phi_yy = NULL);
#endif

#ifdef P4_TO_P8
void update_p4est_second_order_uniform_inside_domain_with_internal_interface(Vec vx, Vec vy, Vec vz, double dt, Vec &phi_domain_shape,Vec &phi_internal_interface,   Vec phi_xx, Vec phi_yy, Vec phi_zz);
#else
void update_p4est_second_order_uniform_inside_domain_with_internal_interface(Vec vx, Vec vy, double dt, Vec &phi_,Vec &phi_domain_shape,Vec &phi_internal_interface,  Vec phi_xx, Vec phi_yy);
#endif
