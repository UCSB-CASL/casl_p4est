  ...
  double operator()(double x, double y, double z) const
  {
    ...
    switch (what)
    {
      case VAL:
      {
        ...
        for (int i = 0; i < N; ++i)
        {
          perturb_0 +=  beta[i]*сos(n[i]*(t - theta[i]));
        }
      ...
      case CUR:
      {
        ...
        for (int i = 0; i < N; ++i)
        {
          perturb_0 +=  beta[i]*сos(n[i]*(t - theta[i]));
          perturb_1 += -beta[i]*sin(n[i]*(t - theta[i]))*n[i];
          perturb_2 += -beta[i]*сos(n[i]*(t - theta[i]))*n[i]*n[i];
        }
        ...
        double phi_x = inside*(r_x - r_0*e_x);
        double phi_y = inside*(r_y - r_0*e_y);
        double phi_z = inside*(r_z - r_0*e_z);

        double phi_xx = inside*(r_xx - r_0*(e_xx*psi + e_x*psi_x + e_x*psi_x + e*psi_xx));
        double phi_yy = inside*(r_yy - r_0*(e_yy*psi + e_y*psi_y + e_y*psi_y + e*psi_yy));
        double phi_zz = inside*(r_zz - r_0*(e_zz*psi + e_z*psi_z + e_z*psi_z + e*psi_zz));

        double phi_xy = inside*(r_xy - r_0*(e_xy*psi + e_x*psi_y + e_x*psi_y + e*psi_xy));
        double phi_yz = inside*(r_yz - r_0*(e_yz*psi + e_y*psi_z + e_y*psi_z + e*psi_yz));
        double phi_zx = inside*(r_zx - r_0*(e_zx*psi + e_z*psi_x + e_z*psi_x + e*psi_zx));
        ...
        return (phi_x*phi_y*phi_xy - 2.*phi_x*phi_y*phi_xy + phi_x*phi_y*phi_xy +
                ...
                phi_z*phi_x*phi_zx - 2.*phi_z*phi_x*phi_zx + phi_z*phi_x*phi_zx)/pow(phi_x*phi_x + phi_y*phi*y + phi_z*phi*z + EPS, 1.5);
      }
    }
  }
};
