...
/*
 * + SOURCE FILE REVISED, DEBUGGED (existing bug in 3D) AND CLEANED UP on (FEBRUARY 6, 2020) [Raphael Egan]
 ...
 * + adapted to the "new" my_p4est_hierarchy_t class that no longer creates and stores a copy of the local
 * grid partition, on December 07, 2020 [Raphael Egan]
 */
...
void my_p4est_cell_neighbors_t::find_neighbor_cells_of_cell(set_of_neighboring_quadrants& ngbd, const p4est_locidx_t& quad_idx, const p4est_topidx_t& tree_idx, const char dir_xyz[P4EST_DIM], p4est_qcoord_t *smallest_quad_size) const
{
  ...
  node.x = quad->x + size/2 + dir_xyz[0]*size + (dir_xyz[0] == -1 ? -my_p4est_hierarchy_t::smallest_logical_quad_size : 0); // "-my_p4est_hierarchy_t::smallest_logical_quad_size" for consistency with the following recursive logic (irrelevant if dir == 0, correct by construction if dir == 1)
  node.y = quad->y + size/2 + dir_xyz[1]*size + (dir_xyz[1] == -1 ? -my_p4est_hierarchy_t::smallest_logical_quad_size : 0); // "-my_p4est_hierarchy_t::smallest_logical_quad_size" for consistency with the following recursive logic (irrelevant if dir == 0, correct by construction if dir == 1)
#ifdef P4_TO_P8
  node.z = quad->z + size/2 + dir_xyz[2]*size + (dir_xyz[2] == -1 ? -my_p4est_hierarchy_t::smallest_logical_quad_size : 0); // "-my_p4est_hierarchy_t::smallest_logical_quad_size" for consistency with the following recursive logic (irrelevant if dir == 0, correct by construction if dir == 1)
#endif
  if(!is_node_in_domain(node, myb, p4est->connectivity))
    return; // no neighbor to find there (past the boundary of the domain) --> return

  find_neighbor_cells_of_cell_recursive(ngbd, node.p.which_tree, node, quad->level, dir_xyz, smallest_quad_size);
  return;
}

void my_p4est_cell_neighbors_t::find_neighbor_cells_of_cell_recursive(set_of_neighboring_quadrants& ngbd, const p4est_topidx_t& tr, const p4est_indep_t& node, int8_t current_level,
                                                                      const char dir_xyz[P4EST_DIM], p4est_qcoord_t *smallest_quad_size) const
{
  p4est_locidx_t pos;
  bool found_locally;
  hierarchy->find_quad_owning_pixel_quad((p4est_quadrant_t&) node, tr, pos, found_locally);
  /* now find the children of this constructed cell in the desired direction and add them to the list */
  const p4est_tree_t* tree = (found_locally ? p4est_tree_array_index(p4est->trees, tr) : NULL);
  const p4est_quadrant_t* neighbor_quad = (pos < 0 ? NULL : p4est_const_quadrant_array_index(&(found_locally ? tree->quadrants : ghost->ghosts), pos));

  if(neighbor_quad != NULL && neighbor_quad->level <= current_level)
  {
    p4est_quadrant_t to_insert = *neighbor_quad;
    to_insert.p.piggy3.local_num = (found_locally ? pos + tree->quadrants_offset : pos + p4est->local_num_quadrants);
    to_insert.p.piggy3.which_tree = tr;
    ngbd.insert(to_insert);
    if(smallest_quad_size != NULL)
      *smallest_quad_size = MIN(*smallest_quad_size, P4EST_QUADRANT_LEN(neighbor_quad->level));
    return;
  }
  if(neighbor_quad == NULL && current_level == ((splitting_criteria_t*) p4est->user_pointer)->max_lvl)
    return; // didn't find anything but won't find anything more anyways since max level reached
  P4EST_ASSERT((neighbor_quad == NULL && current_level < ((splitting_criteria_t*) p4est->user_pointer)->max_lvl) || neighbor_quad->level > current_level);
  int8_t next_level = current_level + 1;
  const p4est_qcoord_t mirror_shift     = P4EST_QUADRANT_LEN(MIN(next_level + 1, P4EST_QMAXLEVEL));
  const p4est_qcoord_t tranverse_shift  = P4EST_QUADRANT_LEN(next_level + 1);
  p4est_indep_t shift_node = node;
  if(dir_xyz[0] != 0) shift_node.x -= dir_xyz[0]*mirror_shift;  // shift the node in the mirror direction
  if(dir_xyz[1] != 0) shift_node.y -= dir_xyz[1]*mirror_shift;  // shift the node in the mirror direction
#ifdef P4_TO_P8
  if(dir_xyz[2] != 0) shift_node.z -= dir_xyz[2]*mirror_shift;  // shift the node in the mirror direction
#endif

  for(char sx = (dir_xyz[0] == 0 ? -1 : 0); sx <= (dir_xyz[0] == 0 ? +1 : 0); sx += 2)      // sweep {-1, 1} if zero
    for(char sy = (dir_xyz[1] == 0 ? -1 : 0); sy <= (dir_xyz[1] == 0 ? +1 : 0); sy += 2)    // sweep {-1, 1} if zero
#ifdef P4_TO_P8
      for(char sz = (dir_xyz[1] == 0 ? -1 : 0); sz <= (dir_xyz[1] == 0 ? +1 : 0); sz += 2)  // sweep {-1, 1} if zero
#endif
      {
        p4est_indep_t other_node = shift_node;
        other_node.x += sx*tranverse_shift; if(next_level == P4EST_QMAXLEVEL) other_node.x = other_node.x & ~(my_p4est_hierarchy_t::smallest_logical_quad_size - 1);
        other_node.y += sy*tranverse_shift; if(next_level == P4EST_QMAXLEVEL) other_node.y = other_node.y & ~(my_p4est_hierarchy_t::smallest_logical_quad_size - 1);
#ifdef P4_TO_P8
        other_node.z += sz*tranverse_shift; if(next_level == P4EST_QMAXLEVEL) other_node.z = other_node.z & ~(my_p4est_hierarchy_t::smallest_logical_quad_size - 1);
#endif
        find_neighbor_cells_of_cell_recursive(ngbd, tr, other_node, next_level, dir_xyz, smallest_quad_size);
      }
  return;
}
