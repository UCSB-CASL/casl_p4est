...
private:
   ...
   * \param [in] node:      bottom, front, left corner node of a (one of the 4 in 2D, 8 in 3D) hypothetical quadrant of level P4EST_QMAXLEVEL
   *                        surrounding the center of Q'
   * \param [in] current_level: the current refinement level being considered for search termination (work completed if one finds an actual
   *                        quadrant of level current_level)
   ...
   */
  void find_neighbor_cells_of_cell_recursive(set_of_neighboring_quadrants& ngbd, const p4est_topidx_t& tr, const p4est_indep_t& node, int8_t current_level,
...
public:
   ...
   * GENERAL PROCEDURE:
   * Let Q be the quadrant whose neighbors are searched for. The method first searches for the center of a (hypothetical) quadrant Q' of the same
   * size as Q but located relatively to Q as if it was the requested neighbor. Based on that search result, the hierarchy is enquired:
