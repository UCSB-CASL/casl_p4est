...
void Voronoi3D::construct_partition(const double *xyz_min, const double *xyz_max, const bool *periodic)
{
  ...
  // get the seed coordinates (clipped in case of non-periodic walls)
  ...
  const double xyz_min_tmp[3] = {(xyz_min[0] - x_center)/min_dist, (xyz_min[1] - y_center)/min_dist, (xyz_min[2] - z_center)/min_dist};
  const double xyz_max_tmp[3] = {(xyz_max[0] - x_center)/min_dist, (xyz_max[1] - y_center)/min_dist, (xyz_max[2] - z_center)/min_dist};
...
//  voronoi.put(po, idx_center_seed, x_center, y_center, z_center);
  ...
  if(cl.start() && voronoi.compute_cell(voro_cell,cl))
  {
    ...
    for(unsigned int n=0; n<neigh.size(); n++)
    {
      ...
      if(neigh[n]<0)
      {
        ...
        {
        case WALL_m00:
          new_voro_nb.p.x = xyz_min[0]-(center_seed.x-xyz_min[0]); new_voro_nb.p.y = center_seed.y; new_voro_nb.p.z = center_seed.z;
          break;
        case WALL_p00:
          new_voro_nb.p.x = xyz_max[0]+(xyz_max[0]-center_seed.x); new_voro_nb.p.y = center_seed.y; new_voro_nb.p.z = center_seed.z;
          break;
        case WALL_0m0:
          new_voro_nb.p.x = center_seed.x; new_voro_nb.p.y = xyz_min[1]-(center_seed.y-xyz_min[1]); new_voro_nb.p.z = center_seed.z;
          break;
        case WALL_0p0:
          new_voro_nb.p.x = center_seed.x; new_voro_nb.p.y = xyz_max[1]+(xyz_max[1]-center_seed.y); new_voro_nb.p.z = center_seed.z;
          break;
        case WALL_00m:
          new_voro_nb.p.x = center_seed.x; new_voro_nb.p.y = center_seed.y; new_voro_nb.p.z = xyz_min[2]-(center_seed.z-xyz_min[2]);
          break;
        case WALL_00p:
          new_voro_nb.p.x = center_seed.x; new_voro_nb.p.y = center_seed.y; new_voro_nb.p.z = xyz_max[2]+(xyz_max[2]-center_seed.z);
          break;
        default:
...
//  /* add the center point */
//  double x_tmp = ((fabs(center_seed.x-xyz_min[0])<(xyz_max[0] - xyz_min[0])*EPS) ? (xyz_min[0]+(xyz_max[0] - xyz_min[0])*EPS) : ((fabs(center_seed.x-xyz_max[0])<(xyz_max[0] - xyz_min[0])*EPS) ? (xyz_max[0]-(xyz_max[0] - xyz_min[0])*EPS) : center_seed.x));
//  double y_tmp = ((fabs(center_seed.y-xyz_min[1])<(xyz_max[1] - xyz_min[1])*EPS) ? (xyz_min[1]+(xyz_max[1] - xyz_min[1])*EPS) : ((fabs(center_seed.y-xyz_max[1])<(xyz_max[1] - xyz_min[1])*EPS) ? (xyz_max[1]-(xyz_max[1] - xyz_min[1])*EPS) : center_seed.y));
//  double z_tmp = ((fabs(center_seed.z-xyz_min[2])<(xyz_max[2] - xyz_min[2])*EPS) ? (xyz_min[2]+(xyz_max[2] - xyz_min[2])*EPS) : ((fabs(center_seed.z-xyz_max[2])<(xyz_max[2] - xyz_min[2])*EPS) ? (xyz_max[2]-(xyz_max[2] - xyz_min[2])*EPS) : center_seed.z));
//  voronoi.put(po, idx_center_seed, x_tmp, y_tmp, z_tmp);

//  vector<double> point_distances(nb_seeds.size());
//  vector<unsigned int> index(nb_seeds.size(), 0);
//  for(unsigned int m=0; m<nb_seeds.size(); ++m)
//  {
...
//    double x_tmp = ((fabs(nb_seeds[idx].p.x-xyz_min[0])<(xyz_max[0] - xyz_min[0])*EPS) ? (xyz_min[0]+(xyz_max[0] - xyz_min[0])*EPS) : ((fabs(nb_seeds[idx].p.x-xyz_max[0])<(xyz_max[0] - xyz_min[0])*EPS) ? (xyz_max[0]-(xyz_max[0] - xyz_min[0])*EPS) : nb_seeds[idx].p.x));
//    double y_tmp = ((fabs(nb_seeds[idx].p.y-xyz_min[1])<(xyz_max[1] - xyz_min[1])*EPS) ? (xyz_min[1]+(xyz_max[1] - xyz_min[1])*EPS) : ((fabs(nb_seeds[idx].p.y-xyz_max[1])<(xyz_max[1] - xyz_min[1])*EPS) ? (xyz_max[1]-(xyz_max[1] - xyz_min[1])*EPS) : nb_seeds[idx].p.y));
//    double z_tmp = ((fabs(nb_seeds[idx].p.z-xyz_min[2])<(xyz_max[2] - xyz_min[2])*EPS) ? (xyz_min[2]+(xyz_max[2] - xyz_min[2])*EPS) : ((fabs(nb_seeds[idx].p.z-xyz_max[2])<(xyz_max[2] - xyz_min[2])*EPS) ? (xyz_max[2]-(xyz_max[2] - xyz_min[2])*EPS) : nb_seeds[idx].p.z));
//    voronoi.put(po, nb_seeds[idx].n, x_tmp, y_tmp, z_tmp);
////    double x_tmp = ((fabs(nb_seeds[m].p.x-xyz_min[0])<(xyz_max[0] - xyz_min[0])*EPS) ? (xyz_min[0]+(xyz_max[0] - xyz_min[0])*EPS) : ((fabs(nb_seeds[m].p.x-xyz_max[0])<(xyz_max[0] - xyz_min[0])*EPS) ? (xyz_max[0]-(xyz_max[0] - xyz_min[0])*EPS) : nb_seeds[m].p.x));
////    double y_tmp = ((fabs(nb_seeds[m].p.y-xyz_min[1])<(xyz_max[1] - xyz_min[1])*EPS) ? (xyz_min[1]+(xyz_max[1] - xyz_min[1])*EPS) : ((fabs(nb_seeds[m].p.y-xyz_max[1])<(xyz_max[1] - xyz_min[1])*EPS) ? (xyz_max[1]-(xyz_max[1] - xyz_min[1])*EPS) : nb_seeds[m].p.y));
////    double z_tmp = ((fabs(nb_seeds[m].p.z-xyz_min[2])<(xyz_max[2] - xyz_min[2])*EPS) ? (xyz_min[2]+(xyz_max[2] - xyz_min[2])*EPS) : ((fabs(nb_seeds[m].p.z-xyz_max[2])<(xyz_max[2] - xyz_min[2])*EPS) ? (xyz_max[2]-(xyz_max[2] - xyz_min[2])*EPS) : nb_seeds[m].p.z));
////    voronoi.put(po, nb_seeds[m].n, x_tmp, y_tmp, z_tmp);
//  }
