...
//    double xyz_max[3] = {0.0001, 0.0001,0.0001};
//    double xyz_min[3] = {-0.0001, -0.0001, -0.0001};

    double eps = (xyz_max[0]-xyz_min[0])*EPS;
...
void Voronoi3D::print_VTK_Format( const std::vector<Voronoi3D>& voro, const char* file_name,
                                  const double *xyz_min, const double *xyz_max, const bool *periodic, double *vn_p, double *sol_voro_p, int num_local_voro)
{
    FILE* f;
    f = fopen(file_name, "w");
#ifdef CASL_THROWS
    ...
    /* now append field data values */
    fprintf(f, "\nPOINT_DATA %d\n", nb_vertices);
    fprintf(f, "\nSCALARS Sol double 1\n");
    fprintf(f, "\nLOOKUP_TABLE default\n");
    for(unsigned int n=0; n<voro_global.size(); ++n)
        if(voro_global[n].voronoi!=NULL && voro[n].points.size()>0)
            for(unsigned int m=0; m<voro[n].points.size(); ++m)
                    fprintf(f, "%e\n", sol_voro_p[voro[n].points[m].n]);
